...
void LocalPlayer::aiStep()
{
    ...
    if (!isSleeping() && (abilities.mayfly || isAllowedToFly()))
    {
        ...
        else if (jumpTriggerTime > 0 && twoJumpsRegistered) // the 4J team checked if the player was NOT jumping after the two jumps, aka had let go of the jump button
        {
            ...
            if (abilities.flying)
            {
                input->sneaking = false; // 4J added - would we ever intentially want to go into flying mode whilst sneaking?
            }
        }
    }
    ...
    if (abilities.flying)
    {
        ...
        {
            yd -= (0.15 + 0.42); // 4J - for flying mode, MINECRAFT_ACTION_SNEAK_TOGGLE isn't a toggle but just indicates that this button is down
        }
        if (input->jumping)
        {
            noJumpDelay = 0;
...
void LocalPlayer::awardStat(Stat *stat, byteArray param)
{
    ...
    if (!ProfileManager.IsGuest(GetXboxPad()) && app.CanRecordStatsAndAchievements() && ProfileManager.IsFullVersion())
    {
    ...
    if (!app.CanRecordStatsAndAchievements())
    {
        return;
    }
    if (stat == nullptr)
    ...
    else
    {
        ...
        // AWARD : The Haggler, Acquire 30 emeralds.
        {
            ...
            if (stat == emeraldMined || stat == emeraldBought)
            {
                ...
                if (totalSum >= 30)
                {
                    awardStat(GenericStats::theHaggler(), GenericStats::param_theHaggler());
                }
            }
        }
        ...
        // AWARD : Adventuring Time, visit at least 17 biomes
        {
            ...
            if (justEnteredBiome)
            {
                ...
                if (biomeCount >= 17)
                {
                    awardStat(GenericStats::adventuringTime(), GenericStats::param_adventuringTime());
                }
            }
        }
#endif
...
bool LocalPlayer::creativeModeHandleMouseClick(int button, bool buttonPressed)
{
    if (buttonPressed)
    {
        ...
        if (lastClickState != lastClick_invalid)
        {
            ...
            if (lastClickState == lastClick_disabled)
            {
                return false;
            }
...
bool LocalPlayer::handleMouseClick(int button)
{
    ...
    {
        if (button == 0 && minecraft->localgameModes[GetXboxPad()]->hasMissTime())
        {
            missTime = 10;
        }
    }
