...
{
    if (assigned && (x == this->x && y == this->y && z == this->z))
    {
        return;
    }
    ...
    for (int yy = y0; yy < y1; yy++)
    {
        for (int zz = 0; zz < 16; zz++)
        {
            for (int xx = 0; xx < 16; xx++)
            {
                ...
                if (!((tileId == Tile::stone_Id) || (tileId == Tile::dirt_Id) || (tileId == Tile::unbreakable_Id) || (tileId == 255)))
                {
                    continue;
                }
...
bool Chunk::isEmpty()
{
    if (!levelRenderer->getGlobalChunkFlag(x, y, z, level, LevelRenderer::CHUNK_FLAG_COMPILED))
    {
        return false;
    }
