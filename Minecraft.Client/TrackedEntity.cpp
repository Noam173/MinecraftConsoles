...
void TrackedEntity::updatePlayer(EntityTracker *tracker, shared_ptr<ServerPlayer> sp)
{
    ...
    if (visibility == eVisibility_SeenAndVisible && (seenBy.find(sp) == seenBy.end() || e->forcedLoading))
    {
        seenBy.insert(sp);
        ...
        if (e->instanceof(eTYPE_LIVINGENTITY))
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                if (item != nullptr)
                {
                    sp->connection->send(std::make_shared<SetEquippedItemPacket>(e->entityId, i, item));
                }
            }
        }
