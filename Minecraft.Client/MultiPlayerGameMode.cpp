...
{
    if (!minecraft->player->isAllowedToMine())
    {
        return;
    }
...
bool MultiPlayerGameMode::useItemOn(shared_ptr<Player> player, Level *level, shared_ptr<ItemInstance> item, int x, int y, int z, int face, Vec3 *hit, bool bTestUseOnly, bool *pbUsedItem)
{
    if (pbUsedItem)
    {
        *pbUsedItem = false; // Did we actually use the held item?
    }
    ...
    if (!didSomething && item != nullptr && dynamic_cast<TileItem *>(item->getItem()))
    {
        ...
        if (!tile->mayPlace(level, x, y, z, face, player, item))
        {
            return false;
        }
    }
