...
void Screen::setClipboard(const wstring &str)
{
    ...
    if (h)
    {
        memcpy(GlobalLock(h), str.c_str(), len);
        GlobalUnlock(h);
...
void Screen::updateEvents()
{
    ...
    for (int vk = 0; vk < 256; vk++)
    {
        ...
        if (vk == VK_ESCAPE)
        {
            mappedKey = Keyboard::KEY_ESCAPE;
        }
        else if (vk == VK_RETURN)
        {
            mappedKey = Keyboard::KEY_RETURN;
        }
        else if (vk == VK_BACK)
        {
            mappedKey = Keyboard::KEY_BACK;
        }
        else if (vk == VK_UP)
        {
            mappedKey = Keyboard::KEY_UP;
        }
        else if (vk == VK_DOWN)
        {
            mappedKey = Keyboard::KEY_DOWN;
        }
        else if (vk == VK_LEFT)
        {
            mappedKey = Keyboard::KEY_LEFT;
        }
        else if (vk == VK_RIGHT)
        {
            mappedKey = Keyboard::KEY_RIGHT;
        }
        else if (vk == VK_LSHIFT || vk == VK_RSHIFT)
        {
            mappedKey = Keyboard::KEY_LSHIFT;
        }
        else if (vk == VK_TAB)
        {
            mappedKey = Keyboard::KEY_TAB;
        }
        else if (vk >= 'A' && vk <= 'Z')
        {
            ch = static_cast<wchar_t>(vk - 'A' + L'a');
            if (g_KBMInput.IsKeyDown(VK_LSHIFT) || g_KBMInput.IsKeyDown(VK_RSHIFT))
            {
