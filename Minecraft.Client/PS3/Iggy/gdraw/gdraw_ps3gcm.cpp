...
static void RADLINK gdraw_DrawIndexedTriangles(GDrawRenderState *r, GDrawPrimitive *p, GDrawVertexBuffer *buf, GDrawStats *RADRESTRICT stats)
{
    ...
    else if (p->indices)
    {
        ...
        if (ring_data)
        { // normal case: fits inside ring buffer
...
static void create_all_shaders()
{
    ...
    {
        create_fragment_program(&gdraw->fprog[0][i], pshader_basic_arr + i);
    }
    for (i = 0; i < GDRAW_BLENDSPECIAL__count; ++i)
    {
        create_fragment_program(&gdraw->exceptional_blend[i], pshader_exceptional_blend_arr + i);
    }
    for (i = 0; i < 32; ++i)
    {
        create_fragment_program(&gdraw->filter_prog[0][i], pshader_filter_arr + i);
    }
    for (i = 0; i < MAX_TAPS + 1; ++i)
    {
        create_fragment_program(&gdraw->blur_prog[i], pshader_blur_arr + i);
    }
    create_fragment_program(&gdraw->colormatrix, pshader_color_matrix_arr);

    for (i = 0; i < GDRAW_TEXTURE__count; ++i)
    {
        gdraw->basic_fprog[i] = &gdraw->fprog[i][0];
    }
