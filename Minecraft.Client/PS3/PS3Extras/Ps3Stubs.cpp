...
DWORD TlsAlloc(VOID)
{
    return TLSStoragePS3::Instance()->Alloc();
}
BOOL TlsFree(DWORD dwTlsIndex)
{
    return TLSStoragePS3::Instance()->Free(dwTlsIndex);
}
LPVOID TlsGetValue(DWORD dwTlsIndex)
{
    return TLSStoragePS3::Instance()->GetValue(dwTlsIndex);
}
BOOL TlsSetValue(DWORD dwTlsIndex, LPVOID lpTlsValue)
{
    return TLSStoragePS3::Instance()->SetValue(dwTlsIndex, lpTlsValue);
}
...
BOOL SetFilePointer(HANDLE hFile, LONG lDistanceToMove, PLONG lpDistanceToMoveHigh, DWORD dwMoveMethod)
{
    ...
    {
    case FILE_BEGIN:
        whence = CELL_FS_SEEK_SET;
        break;
    case FILE_CURRENT:
        whence = CELL_FS_SEEK_CUR;
        break;
    case FILE_END:
        whence = CELL_FS_SEEK_END;
        break;
    };
