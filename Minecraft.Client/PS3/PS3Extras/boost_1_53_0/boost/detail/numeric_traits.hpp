...
struct digit_traits_select<false>
{
    ...
    struct traits
    {
        BOOST_STATIC_CONSTANT(int, digits = (sizeof(T) * std::numeric_limits<unsigned char>::digits - (is_signed<T>::value ? 1 : 0)));
    };
};
...
#endif
  public:
    typedef typename if_true<(int(x::is_signed) && (!int(x::is_bounded)
                                                    // digits is the number of no-sign bits
                                                    || (int(x::digits) + 1 >= digit_traits<boost::intmax_t>::digits)))>::template then<Integer,
                                                                                                                                       ...
                                                                                                                                       typename if_true<(int(x::digits) + 1 < digit_traits<signed int>::digits)>::template then<
                                                                                                                                           ...
                                                                                                                                           typename if_true<(int(x::digits) + 1 < digit_traits<signed long>::digits)>::template then<
                                                                                                                                               ...
                                                                                                                                               // else
                                                                                                                                               intmax_t>::type>::type>::type difference_type;
#else
    BOOST_STATIC_ASSERT(boost::is_integral<Integer>::value);

    typedef typename if_true<(sizeof(Integer) >= sizeof(intmax_t))>::template then<
        ...
        typename if_true<(sizeof(Integer) < sizeof(std::ptrdiff_t))>::template then<
            std::ptrdiff_t,
            intmax_t>::type>::type difference_type;
#endif
};
