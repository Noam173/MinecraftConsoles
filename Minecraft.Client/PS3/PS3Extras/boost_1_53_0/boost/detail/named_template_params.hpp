...
class resolve_default
{
    ...
    enum
    {
        is_def = is_default<Arg>::value
    };
    typedef typename choose_arg_or_default<is_def>::type Selector;
#endif
  public:
    typedef typename Selector ::template select<Arg, DefaultGen, Base, Traits>::type type;
};
...
template <bool Named>
struct choose_default_dispatch_;
template <>
...
struct choose_default_argument
{
    enum
    {
        is_named = is_named_param_list<PreviousArg>::value
    };
...
#define BOOST_NAMED_TEMPLATE_PARAM(TYPE)                                                    \
    ...
    {                                                                                       \
        template <class Base, class Arg, class Traits>                                      \
        struct select                                                                       \
        {                                                                                   \
    ...
    template <>                                                                             \
    struct get_##TYPE##_dispatch<1>                                                         \
    {                                                                                       \
        typedef get_##TYPE##_from_named type;                                               \
    };                                                                                      \
    template <>                                                                             \
    struct get_##TYPE##_dispatch<0>                                                         \
    {                                                                                       \
    ...
    {                                                                                       \
        enum                                                                                \
        {                                                                                   \
            is_named = is_named_param_list<X>::value                                        \
        };                                                                                  \
    ...
    template <>                                                                             \
    struct default_generator<default_##TYPE>                                                \
    {                                                                                       \
