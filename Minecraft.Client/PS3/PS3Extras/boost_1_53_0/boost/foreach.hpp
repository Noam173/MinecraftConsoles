...
#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES) || BOOST_WORKAROUND(BOOST_MSVC, >= 1310) && !defined(_PREFAST_) || (BOOST_WORKAROUND(__GNUC__, == 4) && (__GNUC_MINOR__ <= 5) && !defined(BOOST_INTEL) && !defined(BOOST_CLANG)) || (BOOST_WORKAROUND(__GNUC__, == 3) && (__GNUC_MINOR__ >= 4) && !defined(BOOST_INTEL) && !defined(BOOST_CLANG))
...
#if BOOST_WORKAROUND(BOOST_MSVC, <= 1300) || BOOST_WORKAROUND(__BORLANDC__, < 0x593) || (BOOST_WORKAROUND(BOOST_INTEL_CXX_VERSION, <= 700) && defined(_MSC_VER)) || BOOST_WORKAROUND(__SUNPRO_CC, < 0x5100) || BOOST_WORKAROUND(__DECCXX_VER, <= 60590042)
#define BOOST_FOREACH_NO_RVALUE_DETECTION
#endif
// Some compilers do not correctly implement the lvalue/rvalue conversion
// rules of the ternary conditional operator.
#if defined(BOOST_FOREACH_NO_RVALUE_DETECTION) || defined(BOOST_NO_SFINAE) || BOOST_WORKAROUND(BOOST_MSVC, BOOST_TESTED_AT(1400)) || BOOST_WORKAROUND(BOOST_INTEL_WIN, BOOST_TESTED_AT(1400)) || BOOST_WORKAROUND(__GNUC__, < 3) || (BOOST_WORKAROUND(__GNUC__, == 3) && (__GNUC_MINOR__ <= 2)) || (BOOST_WORKAROUND(__GNUC__, == 3) && (__GNUC_MINOR__ <= 3) && defined(__APPLE_CC__)) || BOOST_WORKAROUND(__IBMCPP__, BOOST_TESTED_AT(600)) || BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3206)) || BOOST_WORKAROUND(__SUNPRO_CC, >= 0x5100) || BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x590))
...
struct is_noncopyable
    ...
    : boost::mpl::or_<
          boost::is_abstract<T>, boost::is_base_and_derived<boost::noncopyable, T>>
...
template <typename T>
inline boost::mpl::true_ *
boost_foreach_is_lightweight_proxy(boost::iterator_range<T> *&, boost::foreach::tag)
{
    return 0;
}
...
template <typename T>
inline boost::mpl::true_ *
boost_foreach_is_lightweight_proxy(boost::sub_range<T> *&, boost::foreach::tag)
{
    return 0;
}
...
struct is_char_array
    : mpl::and_<
          is_array<T>, mpl::or_<
                           is_convertible<T, char const *>, is_convertible<T, wchar_t const *>>>
{
};
...
// as in BOOST_FOREACH( char ch, boost::as_array("hello") ) ...
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::eval_if<
        C, range_const_iterator<container>, range_mutable_iterator<container>>::type type;
};
...
// as in BOOST_FOREACH( char ch, boost::as_array("hello") ) ...
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::eval_if<
        C, range_reverse_iterator<container const>, range_reverse_iterator<container>>::type type;
};
...
struct rvalue_probe
{
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_<
        boost::mpl::or_<boost::is_abstract<T>, boost::is_array<T>>, private_type_, T>::type value_type;
    operator value_type()
    {
...
#define BOOST_FOREACH_IS_RVALUE(COL) \
    boost::foreach_detail_::and_(    \
        boost::foreach_detail_::not_(boost::foreach_detail_::is_array_(COL)), (true ? 0 : boost::foreach_detail_::is_rvalue_((true ? boost::foreach_detail_::make_probe(COL) : (COL)), 0)))
...
// can't ever return an array or an abstract type by value
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_<
        boost::is_array<T>, private_type_, T>::type value_type;
#else
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_<
        boost::mpl::or_<boost::is_abstract<T>, boost::is_array<T>>, private_type_, T>::type value_type;
#endif
...
#define BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL)                                                              \
    boost::foreach_detail_::and_(                                                                            \
        boost::foreach_detail_::not_(BOOST_FOREACH_IS_NONCOPYABLE(COL)), boost_foreach_is_lightweight_proxy( \
                                                                             boost::foreach_detail_::to_ptr(COL), boost_foreach_argument_dependent_lookup_hack_value))
...
#define BOOST_FOREACH_SHOULD_COPY(COL) \
    (true ? 0 : boost::foreach_detail_::or_(BOOST_FOREACH_IS_RVALUE(COL), BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL)))
...
#define BOOST_FOREACH_SHOULD_COPY(COL)         \
    (boost::foreach_detail_::should_copy_impl( \
        true ? 0 : boost::foreach_detail_::or_(boost::foreach_detail_::is_array_(COL), BOOST_FOREACH_IS_NONCOPYABLE(COL), boost::foreach_detail_::not_(boost::foreach_detail_::is_const_(COL))), true ? 0 : BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL), &BOOST_FOREACH_ID(_foreach_is_rvalue)))
...
#define BOOST_FOREACH_SHOULD_COPY(COL) \
    (true ? 0 : boost::foreach_detail_::or_(boost::foreach_detail_::is_rvalue_((COL), 0), BOOST_FOREACH_IS_LIGHTWEIGHT_PROXY(COL)))
...
#define BOOST_FOREACH_CONTAIN(COL)   \
    boost::foreach_detail_::contain( \
        BOOST_FOREACH_EVALUATE(COL), BOOST_FOREACH_SHOULD_COPY(COL))

#define BOOST_FOREACH_BEGIN(COL)   \
    boost::foreach_detail_::begin( \
        BOOST_FOREACH_ID(_foreach_col), BOOST_FOREACH_TYPEOF(COL), BOOST_FOREACH_SHOULD_COPY(COL))
...
#define BOOST_FOREACH_DONE(COL)   \
    boost::foreach_detail_::done( \
        BOOST_FOREACH_ID(_foreach_cur), BOOST_FOREACH_ID(_foreach_end), BOOST_FOREACH_TYPEOF(COL))

#define BOOST_FOREACH_NEXT(COL)   \
    boost::foreach_detail_::next( \
        BOOST_FOREACH_ID(_foreach_cur), BOOST_FOREACH_TYPEOF(COL))
