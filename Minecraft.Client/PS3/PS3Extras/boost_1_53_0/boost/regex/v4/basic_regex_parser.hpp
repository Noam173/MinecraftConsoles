...
void basic_regex_parser<charT, traits>::parse(const charT *p1, const charT *p2, unsigned l_flags)
{
    ...
    if ((p1 == p2) &&
        (((l_flags & regbase::main_option_type) != regbase::perl_syntax_group) || (l_flags & regbase::no_empty_expressions)))
    {
...
bool basic_regex_parser<charT, traits>::parse_literal()
{
    ...
    if (
        ((this->flags() & (regbase::main_option_type | regbase::mod_x | regbase::no_perl_ex)) != regbase::mod_x) || !this->m_traits.isctype(*m_position, this->m_mask_space))
    {
...
bool basic_regex_parser<charT, traits>::parse_open_paren()
{
    ...
    //
    if (
        ((this->flags() & (regbase::main_option_type | regbase::no_perl_ex)) == 0) || ((this->flags() & (regbase::main_option_type | regbase::emacs_ex)) == (regbase::basic_syntax_group | regbase::emacs_ex)))
    {
...
escape_type_class_jump:
    ...
    case regex_constants::escape_type_extended_backref:
        if (0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex)))
        {
            ...
            if ((syn == regex_constants::syntax_open_brace) || (syn == regex_constants::escape_type_left_word) || (syn == regex_constants::escape_type_end_buffer))
            {
                if (++m_position == m_end)
                {
                    fail(regex_constants::error_escape, m_position - m_base, incomplete_message);
...
bool basic_regex_parser<charT, traits>::parse_match_any()
{
    ...
    static_cast<re_dot *>(
        ...
        ->mask = static_cast<unsigned char>(this->flags() & regbase::no_mod_s
                                            ...
                                            : this->flags() & regbase::mod_s ? re_detail::force_newline
                                                                             : re_detail::dont_care);
    return true;
}
...
bool basic_regex_parser<charT, traits>::parse_repeat(std::size_t low, std::size_t high)
{
    ...
    //
    if ((m_position != m_end) && ((0 == (this->flags() & (regbase::main_option_type | regbase::no_perl_ex))) || ((regbase::basic_syntax_group | regbase::emacs_ex) == (this->flags() & (regbase::main_option_type | regbase::emacs_ex)))))
    {
        ...
        if ((m_position != m_end) && (0 == (this->flags() & regbase::main_option_type)) && (this->m_traits.syntax_type(*m_position) == regex_constants::syntax_plus))
        {
            pocessive = true;
            ++m_position;
        }
...
#pragma warning(disable : 4127)
    ...
    case regex_constants::escape_type_hex:
        ...
        if (this->m_traits.syntax_type(*m_position) == regex_constants::syntax_open_brace)
        {
            ...
            if ((m_position == m_end) || (i < 0) || ((std::numeric_limits<charT>::is_specialized) && (i > (int)(std::numeric_limits<charT>::max)())) || (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_close_brace))
            {
                // Rewind to start of escape:
                --m_position;
                while (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_escape)
...
#pragma warning(disable : 4127)
    ...
    do
    {
        while ((m_position != m_end) && (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_escape))
        {
            ++m_position;
        }
        if (m_position == m_end)
    ...
    case regex_constants::syntax_open_mark:
        {
            ...
            else
            {
                ...
                if (this->m_traits.syntax_type(*m_position) == regex_constants::escape_type_left_word)
                {
                    ...
                    if ((this->m_traits.syntax_type(*m_position) != regex_constants::syntax_equal) && (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_not))
                    {
                        // Rewind to start of (? sequence:
                        --m_position;
                        while (this->m_traits.syntax_type(*m_position) != regex_constants::syntax_open_mark)
...
bool basic_regex_parser<charT, traits>::unwind_alts(std::ptrdiff_t last_paren_start)
{
    ...
    //
    if ((this->m_alt_insert_point == static_cast<std::ptrdiff_t>(this->m_pdata->m_data.size())) && m_alt_jumps.size() && (m_alt_jumps.back() > last_paren_start) &&
        !(
            ((this->flags() & regbase::main_option_type) == regbase::perl_syntax_group) &&
            ((this->flags() & regbase::no_empty_expressions) == 0)))
    {
