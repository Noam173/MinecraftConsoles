...
#if (BOOST_WORKAROUND(__BORLANDC__, >= 0x560) && BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x570))) || BOOST_WORKAROUND(BOOST_MSVC, < 1300) || BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3003))
//
...
class u32regex_token_iterator_implementation
{
  ...
  public:
...
#elif (BOOST_WORKAROUND(__BORLANDC__, >= 0x560) && BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x570))) || BOOST_WORKAROUND(BOOST_MSVC, < 1300) || BOOST_WORKAROUND(__MWERKS__, BOOST_TESTED_AT(0x3003)) || BOOST_WORKAROUND(__HP_aCC, < 60700)
    template <class T>
    ...
    bool compare(const u32regex_token_iterator_implementation &that)
    {
        ...
        return (&re.get_data() == &that.re.get_data()) && (end == that.end) && (flags == that.flags) && (N == that.N) && (what[0].first == that.what[0].first) && (what[0].second == that.what[0].second);
    }
