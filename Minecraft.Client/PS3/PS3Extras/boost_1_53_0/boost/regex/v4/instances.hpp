...
#endif
#if !(defined(BOOST_DINKUMWARE_STDLIB) && (BOOST_DINKUMWARE_STDLIB <= 1)) && !(defined(BOOST_INTEL_CXX_VERSION) && (BOOST_INTEL_CXX_VERSION <= 800)) && !(defined(__SGI_STL_PORT) || defined(_STLPORT_VERSION)) && !defined(BOOST_REGEX_ICU_INSTANCES)
#if !BOOST_WORKAROUND(BOOST_MSVC, < 1300)
...
template BOOST_REGEX_DECL
    std::locale
    cpp_regex_traits_base<BOOST_REGEX_CHAR_T>::imbue(const std::locale &l);
...
template BOOST_REGEX_DECL void cpp_regex_traits_implementation<BOOST_REGEX_CHAR_T>::init();
template BOOST_REGEX_DECL
...
template BOOST_REGEX_DECL bool cpp_regex_traits_implementation<BOOST_REGEX_CHAR_T>::isctype(const BOOST_REGEX_CHAR_T c, char_class_type mask) const;
#endif
} // namespace re_detail
template BOOST_REGEX_DECL int cpp_regex_traits<BOOST_REGEX_CHAR_T>::toi(const BOOST_REGEX_CHAR_T *&first, const BOOST_REGEX_CHAR_T *last, int radix) const;
template BOOST_REGEX_DECL
    std::string
    cpp_regex_traits<BOOST_REGEX_CHAR_T>::catalog_name(const std::string &name);
template BOOST_REGEX_DECL
    std::string &
    cpp_regex_traits<BOOST_REGEX_CHAR_T>::get_catalog_name_inst();
template BOOST_REGEX_DECL
    std::string
    cpp_regex_traits<BOOST_REGEX_CHAR_T>::get_catalog_name();
#ifdef BOOST_HAS_THREADS
template BOOST_REGEX_DECL static_mutex &cpp_regex_traits<BOOST_REGEX_CHAR_T>::get_mutex_inst();
#endif
#endif
...
#if (defined(__GLIBCPP__) || defined(__GLIBCXX__)) && !defined(BOOST_REGEX_ICU_INSTANCES) && !defined(__SGI_STL_PORT) && !defined(_STLPORT_VERSION)
