    ...
    const basic_char_set<charT, traits> &char_set, mpl::true_ *)
{
    ...
    while (first != last)
    {
        ...
        {
            if (this->m_traits.translate(static_cast<charT>(i), this->m_icase) == this->m_traits.translate(first->first, this->m_icase))
            {
                result->_map[i] = true;
            }
        }
        ...
        case syntax_element_backstep:
            {
                ...
                // adjust the type of the state to allow for faster matching:
                state->type = this->get_repeat_type(state);
                if ((state->type == syntax_element_dot_rep) || (state->type == syntax_element_char_rep) || (state->type == syntax_element_short_set_rep))
                {
                    if (rep->max != rep->min)
                    {
                        return -1;
    ...
    while (state)
    {
        ...
        case syntax_element_set:
            if (l_map)
            {
                ...
                for (unsigned int i = 0; i < (1u << CHAR_BIT); ++i)
                {
                    if (static_cast<re_set *>(state)->_map[static_cast<unsigned char>(m_traits.translate(static_cast<charT>(i), l_icase))])
                    {
                        l_map[i] |= mask;
                    }
                }
        ...
        case syntax_element_long_set_rep:
            {
                ...
                else
                {
                    ...
                    create_startmap(state->next.p, l_map, pnull, mask);
                    if ((state->type == syntax_element_alt) || (static_cast<re_repeat *>(state)->min == 0) || (not_last_jump == 0))
                    {
