          ...
          char>
{
    ...
    bool equal(const mb_from_wchar<Base> &rhs) const
    {
        ...
        return 0 == m_bend && 0 == m_bnext && this->base_reference() == rhs.base_reference();
    }
  ...
  public:
    ...
    template <class T>
    mb_from_wchar(BOOST_PFTO_WRAPPER(T) start) : super_t(Base(BOOST_MAKE_PFTO_WRAPPER(static_cast<T>(start)))),
                                                 m_bend(0),
                                                 m_bnext(0),
                                                 m_full(false)
    {
    ...
    mb_from_wchar(const mb_from_wchar &rhs) : super_t(rhs.base_reference()),
                                              m_bend(rhs.m_bend),
                                              m_bnext(rhs.m_bnext),
                                              m_full(rhs.m_full)
    {
