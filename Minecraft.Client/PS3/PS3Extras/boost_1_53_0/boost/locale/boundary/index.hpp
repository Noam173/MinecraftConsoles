...
template <typename BaseIterator>
class segment_index_iterator : public boost::iterator_facade<
                                   ...
                                   segment<BaseIterator> const &>
{
  public:
    ...
    segment_index_iterator(base_iterator p, mapping_type const *map, rule_type mask, bool full_select) : map_(map),
                                                                                                         mask_(mask),
                                                                                                         full_select_(full_select)
    {
        set(p);
    }
    segment_index_iterator(bool is_begin, mapping_type const *map, rule_type mask, bool full_select) : map_(map),
                                                                                                       mask_(mask),
                                                                                                       full_select_(full_select)
    {
        if (is_begin)
...
template <typename BaseIterator>
class boundary_point_index_iterator : public boost::iterator_facade<
                                          ...
                                          boundary_point<BaseIterator> const &>
{
  public:
    ...
    boundary_point_index_iterator(bool is_begin, mapping_type const *map, rule_type mask) : map_(map),
                                                                                            mask_(mask)
    {
        if (is_begin)
        {
    ...
    boundary_point_index_iterator(base_iterator p, mapping_type const *map, rule_type mask) : map_(map),
                                                                                              mask_(mask)
    {
        set(p);
    }
...
/// \cond INTERNAL
template <typename BaseIterator>
segment_index<BaseIterator>::segment_index(boundary_point_index<BaseIterator> const &other) : map_(other.map_),
                                                                                              mask_(0xFFFFFFFFu),
                                                                                              full_select_(false)
{
}
...
template <typename BaseIterator>
boundary_point_index<BaseIterator>::boundary_point_index(segment_index<BaseIterator> const &other) : map_(other.map_),
                                                                                                     mask_(0xFFFFFFFFu)
{
}
