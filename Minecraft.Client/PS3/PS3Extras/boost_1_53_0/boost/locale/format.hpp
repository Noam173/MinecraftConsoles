...
struct formattible
{
    ...
    formattible() : pointer_(0),
                    writer_(&formattible::void_write)
    {
    }

    formattible(formattible const &other) : pointer_(other.pointer_),
...
class basic_format
{
  public:
    ...
    ///
    basic_format(string_type format_string) : format_(format_string),
                                              translate_(false),
                                              parameters_count_(0)
    {
    }
    ...
    ///
    basic_format(message_type const &trans) : message_(trans),
                                              translate_(true),
                                              parameters_count_(0)
    {
    }
  ...
  private:
    ...
    {
      public:
        format_guard(details::format_parser &fmt) : fmt_(&fmt),
                                                    restored_(false)
        {
        }
        void restore()
