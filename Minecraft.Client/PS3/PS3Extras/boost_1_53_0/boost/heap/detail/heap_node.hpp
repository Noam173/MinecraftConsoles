...
struct heap_node_base : bi::list_base_hook<typename mpl::if_c<auto_unlink,
                                                              bi::link_mode<bi::auto_unlink>,
                                                              bi::link_mode<bi::safe_link>>::type>
{
};
...
struct node_cloner
{
    node_cloner(Alloc &allocator) : allocator(allocator)
    {
    }
...
struct node_disposer
{
    ...
    node_disposer(Alloc &alloc) : alloc_(alloc)
    {
    }
...
struct heap_node : heap_node_base<!constant_time_child_size>
{
  ...
  public:
    ...
    typedef bi::list<node_base,
                     bi::constant_time_size<constant_time_child_size>>
        child_list;
    ...
    heap_node(ValueType const &v) : value(v)
    {
    }

#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES) && !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
    template <class... Args>
    heap_node(Args &&...args) : value(std::forward<Args>(args)...)
    {
    }
#endif

    /* protected:                      */
    heap_node(heap_node const &rhs) : value(rhs.value)
    {
  ...
  public:
    template <typename Alloc>
    heap_node(heap_node const &rhs, Alloc &allocator) : value(rhs.value)
    {
...
struct parent_pointing_heap_node : heap_node<value_type>
{
    ...
    parent_pointing_heap_node(value_type const &v) : super_t(v), parent(NULL)
    {
    }

#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES) && !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
    template <class... Args>
    parent_pointing_heap_node(Args &&...args) : super_t(std::forward<Args>(args)...), parent(NULL)
    {
    }
#endif
    ...
    struct node_cloner
    {
        node_cloner(Alloc &allocator, parent_pointing_heap_node *parent) : allocator(allocator), parent_(parent)
        {
        }
    ...
    template <typename Alloc>
    parent_pointing_heap_node(parent_pointing_heap_node const &rhs, Alloc &allocator, parent_pointing_heap_node *parent) : super_t(static_cast<super_t const &>(rhs)), parent(parent)
    {
...
#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES) && !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
    template <class... Args>
    marked_heap_node(Args &&...args) : super_t(std::forward<Args>(args)...), mark(false)
    {
    }
#endif
