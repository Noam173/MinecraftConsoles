...
#if !defined(BOOST_OPTIONAL_NO_INPLACE_FACTORY_SUPPORT) && BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x581))
...
#if defined(__GNUC__) && (__GNUC__ * 100 + __GNUC_MINOR__) > 302 && !defined(__INTEL_COMPILER)
...
class optional_base : public optional_tag
{
  ...
  private:
    ...
    {
        get_ptr_impl()->internal_type::~internal_type();
        m_initialized = false;
    }
#else
    void destroy_impl(is_not_reference_tag)
...
template <class T>
inline bool operator==(none_t, optional<T> const &y)
{
