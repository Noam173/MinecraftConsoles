...
inline object make_iterator(
    Accessor1 get_start, Accessor2 get_finish, NextPolicies next_policies, Target &(*)())
{
    return objects::make_iterator_function<Target>(
        boost::protect(boost::bind(get_start, _1)), boost::protect(boost::bind(get_finish, _1)), next_policies);
}
...
struct iterators_impl
{
    ...
    struct apply
    {
        ...
        static iterator begin(T &x)
        {
            return x.begin();
        }
        static iterator end(T &x)
        {
            return x.end();
        }
    };
...
struct iterators
    : detail::iterators_impl<
          boost::is_const<T>::value>::template apply<T>
{
};
...
object range(Accessor1 start, Accessor2 finish)
{
    return detail::make_iterator(
        start, finish, objects::default_iterator_call_policies(), detail::target(start));
}
