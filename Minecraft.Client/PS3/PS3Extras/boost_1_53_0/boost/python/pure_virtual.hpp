    ...
    : def_visitor<pure_virtual_visitor<PointerToMemberFunction>>
{
  ...
  private:
    ...
    void visit(C_ &c, char const *name, Options &options) const
    {
        ...
        c.def(
            name, m_pmf, options.doc(), options.keywords(), options.policies());
        ...
        c.def(
            name, make_function(
                      detail::nullary_function_adaptor<void (*)()>(pure_virtual_called), default_call_policies(), detail::error_signature<held_type>(detail::get_signature(m_pmf))));
    }
