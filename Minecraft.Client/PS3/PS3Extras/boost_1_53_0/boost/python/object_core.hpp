...
#include BOOST_PP_ITERATE()
    ...
    {
        return this->slice(
            slice_bound<T>::type(start), slice_bound<V>::type(end));
    }
#endif
  ...
  private: // def visitation for adding callable objects as class methods
    ...
    void visit(ClassT &cl, char const *name, python::detail::def_helper<DocStringT> const &helper) const
    {
        ...
        BOOST_STATIC_ASSERT(
            (is_same<char const *, DocStringT>::value || detail::is_string_literal<DocStringT const>::value));
...
struct is_derived
    : is_convertible<
          typename remove_reference<T>::type *, U const *>
{
};
#endif
...
PyObject *object_base_initializer(T const &x)
{
    typedef typename is_derived<
        BOOST_DEDUCED_TYPENAME objects::unforward_cref<T>::type, object>::type is_obj;
...
struct object_initializer : object_initializer_impl<
                                is_proxy<T>::value, converter::is_object_manager<T>::value>
{
};
