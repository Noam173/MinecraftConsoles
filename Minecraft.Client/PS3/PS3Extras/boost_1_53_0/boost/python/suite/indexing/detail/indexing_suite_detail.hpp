  ...
  public:
        ...
        typename std::vector<PyObject *>::size_type len)
    {
        ...
        while (right != proxies.end())
        {
            ...
            p().set_index(
                extract<Proxy &>(*right)().get_index() - (difference_type(to) - from - len));
...
class container_element
{
  public:
    ...
    container_element(container_element const &ce)
        : ptr(ce.ptr.get() == 0 ? 0 : new element_type(*ce.ptr.get())), container(ce.container), index(ce.index)
    {
    }
...
template <
    class Container, class DerivedPolicies, class ContainerElement, class Index>
...
template <
    class Container, class DerivedPolicies, class ProxyHandler, class Data, class Index>
struct slice_helper
{
    static object
