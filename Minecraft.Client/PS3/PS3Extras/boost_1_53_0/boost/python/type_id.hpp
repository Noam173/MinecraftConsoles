...
#if defined(__GNUC__) && ((__GNUC__ > 3) || ((__GNUC__ == 3) && (__GNUC_MINOR__ >= 1))) && !defined(__EDG_VERSION__)
...
#if (defined(__GNUC__) && __GNUC__ >= 3) || defined(_AIX) || (defined(__sgi) && defined(__host_mips)) || (defined(__hpux) && defined(__HP_aCC)) || (defined(linux) && defined(__INTEL_COMPILER) && defined(__ICC))
...
{
    return type_info(
#if !defined(_MSC_VER) || (!BOOST_WORKAROUND(BOOST_MSVC, <= 1300) && !BOOST_WORKAROUND(BOOST_INTEL_CXX_VERSION, <= 700))
        typeid(T)
...
#if (defined(__EDG_VERSION__) && __EDG_VERSION__ < 245) || (defined(__sgi) && defined(_COMPILER_VERSION) && _COMPILER_VERSION <= 744)
