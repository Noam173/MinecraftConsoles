...
struct regex_search_result : public iterator_range<IteratorT>
{
    ...
    regex_search_result(const match_results_type &MatchResults) : base_type(MatchResults[0].first, MatchResults[0].second),
    ...
    regex_search_result(const regex_search_result &Other) : base_type(Other.begin(), Other.end()),
...
struct find_regexF
{
    ...
    // Construction
    find_regexF(regex_reference_type Rx, match_flag_type MatchFlags = match_default) : m_Rx(Rx), m_MatchFlags(MatchFlags)
    {
    }
