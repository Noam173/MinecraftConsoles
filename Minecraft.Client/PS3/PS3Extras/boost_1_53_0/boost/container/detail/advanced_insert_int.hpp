...
struct insert_range_proxy
{
    ...
    {
        this->first_ = ::boost::container::uninitialized_copy_or_move_alloc_n_source(this->a_, this->first_, n, pos);
    }
...
struct insert_copy_proxy
{
    ...
    void uninitialized_copy_n_and_update(Iterator p, size_type n)
    {
        BOOST_ASSERT(n == 1);
        (void)n;
        alloc_traits::construct(this->a_, container_detail::to_raw_pointer(&*p), v_);
    }
...
struct insert_move_proxy
{
    ...
    void uninitialized_copy_n_and_update(Iterator p, size_type n)
    {
        BOOST_ASSERT(n == 1);
        (void)n;
        alloc_traits::construct(this->a_, container_detail::to_raw_pointer(&*p), ::boost::move(v_));
    }
...
struct insert_non_movable_emplace_proxy
{
  ...
  private:
    ...
    void priv_uninitialized_copy_some_and_update(const index_tuple<IdxPack...> &, Iterator p, size_type n)
    {
        BOOST_ASSERT(n == 1);
        (void)n;
        alloc_traits::construct(this->a_, container_detail::to_raw_pointer(&*p), ::boost::forward<Args>(get<IdxPack>(this->args_))...);
    }
...
#define BOOST_PP_LOCAL_MACRO(N)                                                                                                \
    ...
    struct BOOST_PP_CAT(insert_emplace_proxy_arg, N)                                                                           \
        : BOOST_PP_CAT(insert_non_movable_emplace_proxy_arg, N)<A, Iterator BOOST_PP_ENUM_TRAILING_PARAMS(N, P)>               \
    {                                                                                                                          \
        typedef BOOST_PP_CAT(insert_non_movable_emplace_proxy_arg, N)<A, Iterator BOOST_PP_ENUM_TRAILING_PARAMS(N, P)> base_t; \
        typedef typename base_t::value_type value_type;                                                                        \
        typedef typename base_t::size_type size_type;                                                                          \
        typedef boost::container::allocator_traits<A> alloc_traits;                                                            \
                                                                                                                               \
