...
struct node_alloc_holder
{
    ...
    typedef typename allocator_traits_type::template portable_rebind_alloc<Node>::type NodeAlloc;
  ...
  public:
    ...
    void copy_assign_alloc(const node_alloc_holder &x)
    {
        ...
        container_detail::assign_alloc(static_cast<NodeAlloc &>(this->members_), static_cast<const NodeAlloc &>(x.members_), flag);
    }
    ...
    void move_assign_alloc(node_alloc_holder &x)
    {
        ...
        container_detail::move_alloc(static_cast<NodeAlloc &>(this->members_), static_cast<NodeAlloc &>(x.members_), flag);
    }
    ...
    NodePtr create_node(Args &&...args)
    {
        ...
        allocator_traits<NodeAlloc>::construct(this->node_alloc(), container_detail::addressof(p->m_data), boost::forward<Args>(args)...);
...
#define BOOST_PP_LOCAL_MACRO(n)                                                                                                         \
    ...
    BOOST_PP_EXPR_IF(n, template <)                                                                                                     \
                        BOOST_PP_ENUM_PARAMS(n, class P) BOOST_PP_EXPR_IF(n, >)                                                         \
                            NodePtr                                                                                                     \
                            create_node(BOOST_PP_ENUM(n, BOOST_CONTAINER_PP_PARAM_LIST, _))                                             \
    {                                                                                                                                   \
        NodePtr p = this->allocate_one();                                                                                               \
        Deallocator node_deallocator(p, this->node_alloc());                                                                            \
        allocator_traits<NodeAlloc>::construct(this->node_alloc(), container_detail::addressof(p->m_data)                               \
...
#include BOOST_PP_LOCAL_ITERATE()
    ...
    void allocate_many_and_construct(FwdIterator beg, difference_type n, Inserter inserter)
    {
        /*
        NodePtr p = this->allocate_one();
        Deallocator node_deallocator(p, this->node_alloc());
        ...
        : public NodeAlloc
    {
      ...
      public:
        ...
        explicit members_holder(BOOST_FWD_REF(ConvertibleToAlloc) c2alloc)
            : NodeAlloc(boost::forward<ConvertibleToAlloc>(c2alloc)), m_icont()
        {
        }

        template <class ConvertibleToAlloc>
        members_holder(BOOST_FWD_REF(ConvertibleToAlloc) c2alloc, const Pred &c)
            : NodeAlloc(boost::forward<ConvertibleToAlloc>(c2alloc)), m_icont(typename ICont::value_compare(c))
        {
        }

        explicit members_holder(const Pred &c)
            : NodeAlloc(), m_icont(typename ICont::value_compare(c))
        {
        }
