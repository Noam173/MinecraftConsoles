...
struct rbtree_hook
{
    typedef typename container_detail::bi::make_set_base_hook<container_detail::bi::void_pointer<VoidPointer>, container_detail::bi::link_mode<container_detail::bi::normal_link>, container_detail::bi::optimize_size<true>>::type type;
};
...
struct intrusive_rbtree_type
{
    ...
    typedef typename container_detail::rbtree_node<value_type, void_pointer> node_type;
    typedef node_compare<ValueCompare, node_type> node_compare_type;
    typedef typename container_detail::bi::make_rbtree<node_type, container_detail::bi::compare<node_compare_type>, container_detail::bi::base_hook<typename rbtree_hook<void_pointer>::type>, container_detail::bi::constant_time_size<true>, container_detail::bi::size_type<size_type>>::type container_type;
...
class rbtree
    : protected container_detail::node_alloc_holder<A, typename container_detail::intrusive_rbtree_type<A, tree_value_compare<Key, Value, KeyCompare, KeyOfValue>>::type, KeyCompare>
{
    typedef typename container_detail::intrusive_rbtree_type<A, tree_value_compare<Key, Value, KeyCompare, KeyOfValue>>::type Icont;
  ...
  public:
    ...
    class const_iterator
        : public std::iterator<std::bidirectional_iterator_tag, value_type, rbtree_difference_type, rbtree_const_pointer, rbtree_const_reference>
    {
      protected:
        typedef typename Icont::iterator iiterator;
        iiterator m_it;
    ...
    rbtree(bool unique_insertion, InputIterator first, InputIterator last, const key_compare &comp,
           ...
           ,
           typename container_detail::enable_if_c<container_detail::is_input_iterator<InputIterator>::value || container_detail::is_same<alloc_version, allocator_v1>::value>::type * = 0
#endif
           )
        : AllocHolder(a, comp)
    {
        ...
        : AllocHolder(a, comp)
    {
        ...
        else
        {
            ...
            this->allocate_many_and_construct(first, std::distance(first, last), insert_equal_end_hint_functor(this->icont()));
        }
    }

    template <class InputIterator>
    rbtree(ordered_range_t, InputIterator first, InputIterator last, const key_compare &comp = key_compare(), const allocator_type &a = allocator_type()
#if !defined(BOOST_CONTAINER_DOXYGEN_INVOKED)
                                                                                                                  ,
           typename container_detail::enable_if_c<container_detail::is_input_iterator<InputIterator>::value || container_detail::is_same<alloc_version, allocator_v1>::value>::type * = 0
#endif
    ...
    rbtree &operator=(BOOST_COPY_ASSIGN_REF(rbtree) x)
    {
        if (&x != this)
        {
            ...
            this->icont().clone_from(x.icont(), RecyclingCloner(*this, other_tree), Destroyer(this->node_alloc()));
    ...
    rbtree &operator=(BOOST_RV_REF(rbtree) x)
    {
        if (&x != this)
        {
            ...
            else
            {
                ...
                // Now recreate the source tree reusing nodes stored by other_tree
                this->icont().clone_from(x.icont(), RecyclingMoveCloner(*this, other_tree), Destroyer(this->node_alloc()));
  ...
  public:
    ...
    // insert/erase
    std::pair<iterator, bool> insert_unique_check(const key_type &key, insert_commit_data &data)
    {
    ...
    std::pair<iterator, bool> insert_unique_check(const_iterator hint, const key_type &key, insert_commit_data &data)
    {
        std::pair<iiterator, bool> ret =
    ...
    iterator insert_unique_commit(BOOST_FWD_REF(MovableConvertible) mv, insert_commit_data &data)
    {
    ...
    std::pair<iterator, bool> insert_unique(const value_type &v)
    {
        ...
        return std::pair<iterator, bool>(this->insert_unique_commit(v, data), true);
    }
    ...
    std::pair<iterator, bool> insert_unique(BOOST_FWD_REF(MovableConvertible) mv)
    {
        ...
        return std::pair<iterator, bool>(this->insert_unique_commit(boost::forward<MovableConvertible>(mv), data), true);
    }
  ...
  private:
    std::pair<iterator, bool> emplace_unique_impl(NodePtr p)
    {
        ...
        return std::pair<iterator, bool>(iterator(iiterator(this->icont().insert_unique_commit(*p, data))), true);
    }
...
#define BOOST_PP_LOCAL_MACRO(n)                                                                                                       \
    ...
    {                                                                                                                                 \
        return this->emplace_unique_impl(AllocHolder::create_node(BOOST_PP_ENUM(n, BOOST_CONTAINER_PP_PARAM_FORWARD, _)));            \
    }                                                                                                                                 \
                                                                                                                                      \
    BOOST_PP_EXPR_IF(n, template <)                                                                                                   \
                        BOOST_PP_ENUM_PARAMS(n, class P) BOOST_PP_EXPR_IF(n, >)                                                       \
    ...
    {                                                                                                                                 \
        return this->emplace_unique_hint_impl(hint, AllocHolder::create_node(BOOST_PP_ENUM(n, BOOST_CONTAINER_PP_PARAM_FORWARD, _))); \
    }                                                                                                                                 \
                                                                                                                                      \
    BOOST_PP_EXPR_IF(n, template <)                                                                                                   \
                        BOOST_PP_ENUM_PARAMS(n, class P) BOOST_PP_EXPR_IF(n, >)                                                       \
                            iterator                                                                                                  \
                            emplace_equal(BOOST_PP_ENUM(n, BOOST_CONTAINER_PP_PARAM_LIST, _))                                         \
    {                                                                                                                                 \
