...
struct slist_hook
{
    typedef typename container_detail::bi::make_slist_base_hook<container_detail::bi::void_pointer<VoidPointer>, container_detail::bi::link_mode<container_detail::bi::normal_link>>::type type;
};
...
struct intrusive_slist_type
{
    ...
    typedef typename boost::intrusive::pointer_traits<typename allocator_traits_type::pointer>::template rebind_pointer<void>::type
        void_pointer;
    typedef typename container_detail::slist_node<value_type, void_pointer> node_type;

    typedef typename container_detail::bi::make_slist<node_type, container_detail::bi::base_hook<typename slist_hook<void_pointer>::type>, container_detail::bi::constant_time_size<true>, container_detail::bi::size_type<typename allocator_traits_type::size_type>>::type container_type;
    typedef container_type type;
};

template <class T, class IIterator>
class slist_const_iterator
    : public std::iterator<std::forward_iterator_tag, T, typename iiterator_types<T, IIterator>::difference_type, typename iiterator_types<T, IIterator>::const_pointer, typename iiterator_types<T, IIterator>::const_reference>
{
  protected:
    IIterator m_it;
  ...
  public:
    ...
    slist_const_iterator operator++(int)
    {
        ...
        ++*this;
        return slist_const_iterator(tmp);
    }
...
#endif
class slist
    : protected container_detail::node_alloc_holder<Allocator, typename container_detail::intrusive_slist_type<Allocator>::type>
{
    /// @cond
    typedef typename container_detail::intrusive_slist_type<Allocator>::type Icont;
...
#define BOOST_PP_LOCAL_MACRO(n)                                                                                                                        \
    ...
    {                                                                                                                                                  \
        NodePtr pnode(AllocHolder::create_node(BOOST_PP_ENUM(n, BOOST_CONTAINER_PP_PARAM_FORWARD, _)));                                                \
        return iterator(this->icont().insert_after(prev.get(), *pnode));                                                                               \
    }                                                                                                                                                  \
//!
...
#include BOOST_PP_LOCAL_ITERATE()
    ...
    {
        BOOST_ASSERT(this->node_alloc() == x.node_alloc());
        this->icont().splice_after(prev_pos.get(), x.icont(), before_first.get(), before_last.get());
    }
    ...
    {
        BOOST_ASSERT(this->node_alloc() == x.node_alloc());
        this->icont().splice_after(prev_pos.get(), x.icont(), before_first.get(), before_last.get(), n);
    }
...
{
    return std::lexicographical_compare(sL1.begin(), sL1.end(), sL2.begin(), sL2.end());
}
