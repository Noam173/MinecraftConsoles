...
class basic_string_base
{
  ...
  protected:
    ...
    void is_short(bool yes)
    {
        ...
        if (yes && !was_short)
        {
            allocator_traits_type::destroy(this->alloc(), static_cast<long_t *>(static_cast<void *>(&this->members_.m_repr.r)));
            this->members_.m_repr.s.h.is_short = true;
        }
        else if (!yes && was_short)
        {
            allocator_traits_type::construct(this->alloc(), static_cast<long_t *>(static_cast<void *>(&this->members_.m_repr.r)));
  ...
  protected:
                       ...
                       size_type &received_size, pointer reuse = 0)
    {
        ...
        return container_detail::allocator_version_traits<Allocator>::allocation_command(this->alloc(), command, limit_size, preferred_size, received_size, reuse);
    }
    ...
    {
        allocator_traits_type::construct(this->alloc(), container_detail::to_raw_pointer(p), value);
    }
    ...
    : private container_detail::basic_string_base<Allocator>
{
  ...
  public: // Constructor, destructor, assignment.
    ...
    basic_string(reserve_t, size_type n,
        ...
        : base_t(allocator_traits_type::select_on_container_copy_construction(a), n + 1)
    {
        ...
        : base_t(a)
    {
        ...
        {
            this->assign(s.begin() + pos, s.begin() + pos + container_detail::min_value(n, s.size() - pos));
        }
    }
    ...
    void reserve(size_type res_arg)
    {
        ...
        if (this->capacity() < res_arg)
        {
            ...
            size_type new_cap = this->next_capacity(n);
            pointer new_start = this->allocation_command(allocate_new, n, new_cap, new_cap).first;
            size_type new_length = 0;

            const pointer addr = this->priv_addr();
            new_length += priv_uninitialized_copy(addr, addr + this->priv_size(), new_start);
    ...
    void shrink_to_fit()
    {
        ...
        if (this->priv_storage() > InternalBufferChars)
        {
            ...
            if (this->priv_size() < InternalBufferChars)
            {
                ...
                Traits::copy(container_detail::to_raw_pointer(this->priv_short_addr()), container_detail::to_raw_pointer(long_addr), long_size + 1);
                this->is_short(true);
    ...
    iterator insert(const_iterator p, ForwardIter first, ForwardIter last, typename container_detail::enable_if_c<!container_detail::is_convertible<ForwardIter, size_type>::value && !container_detail::is_input_iterator<ForwardIter>::value>::type * = 0)
    {
        ...
        if (first != last)
        {
            ...
            else
            {
                ...
                allocation_ret = this->allocation_command(allocate_new | expand_fwd | expand_bwd, old_size + n + 1,
            ...
            if (enough_capacity)
            {
                ...
                else
                {
                    ...
                    priv_uninitialized_copy(p, const_iterator(old_start + old_length + 1),
            ...
            else
            {
                ...
                if (!allocation_ret.second)
                {
                    ...
                    new_length += priv_uninitialized_copy(const_iterator(old_start), p, new_start);
                    new_length += priv_uninitialized_copy(first, last, new_start + new_length);
                    new_length += priv_uninitialized_copy(p, const_iterator(old_start + old_size),
    ...
    basic_string &replace(size_type pos1, size_type n1, const basic_string &str)
    {
        ...
        const pointer addr = this->priv_addr();
        return this->replace(const_iterator(addr + pos1), const_iterator(addr + pos1 + len), str.begin(), str.end());
    }
    ...
    size_type find_first_of(const CharT *s, size_type pos, size_type n) const
    {
        ...
        else
        {
            const pointer addr = this->priv_addr();
            pointer finish = addr + sz;
            const_iterator result = std::find_first_of(addr + pos, finish, s, s + n, Eq_traits<Traits>());
    ...
    size_type find_first_not_of(const CharT *s, size_type pos, size_type n) const
    {
        ...
        else
        {
            ...
            const const_iterator result = std::find_if(addr + pos, finish, Not_within_traits<Traits>(s, s + n));
    ...
    size_type find_first_not_of(CharT c, size_type pos = 0) const
    {
        ...
        else
        {
            const pointer addr = this->priv_addr();
            const pointer finish = addr + this->priv_size();
            const const_iterator result = std::find_if(addr + pos, finish,
  ...
  private:
    ...
    void priv_shrink_to_fit_dynamic_buffer(AllocVersion, typename container_detail::enable_if<container_detail::is_same<AllocVersion, allocator_v1>>::type * = 0)
    {
        ...
        BOOST_TRY
        {
            const std::pair<pointer, bool> ret = this->allocation_command(allocate_new, long_size + 1, long_size + 1, real_cap, long_addr);
            // Copy and update
            Traits::copy(container_detail::to_raw_pointer(ret.first), container_detail::to_raw_pointer(this->priv_long_addr()), long_size + 1);
    ...
    template <class AllocVersion>
    void priv_shrink_to_fit_dynamic_buffer(AllocVersion, typename container_detail::enable_if<container_detail::is_same<AllocVersion, allocator_v2>>::type * = 0)
    {
        size_type received_size;
        if (this->alloc().allocation_command(shrink_in_place | nothrow_allocation, this->priv_long_storage(), this->priv_long_size() + 1, received_size, this->priv_long_addr()).first)
        {
    ...
    template <class FwdIt, class Count>
    inline void priv_uninitialized_fill_n(FwdIt first, Count count, const CharT val)
    {
    ...
    inline size_type priv_uninitialized_copy(InpIt first, InpIt last, FwdIt dest)
    {
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(BOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> BOOST_RV_REF_END mx, BOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> BOOST_RV_REF_END my)
{
    mx += my;
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(BOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> BOOST_RV_REF_END mx, const basic_string<CharT, Traits, Allocator> &y)
{
    mx += y;
    return boost::move(mx);
}
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(const basic_string<CharT, Traits, Allocator> &x, BOOST_RV_REF_BEG basic_string<CharT, Traits, Allocator> BOOST_RV_REF_END my)
{
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(const CharT *s, basic_string<CharT, Traits, Allocator> y)
{
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(basic_string<CharT, Traits, Allocator> x, const CharT *s)
{
    x += s;
    return x;
}
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(CharT c, basic_string<CharT, Traits, Allocator> y)
{
    y.insert(y.begin(), c);
    return y;
}
...
template <class CharT, class Traits, class Allocator>
inline basic_string<CharT, Traits, Allocator> operator+(basic_string<CharT, Traits, Allocator> x, const CharT c)
{
    x += c;
    return x;
}
