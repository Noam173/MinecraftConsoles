...
#define M0(Z, N, D)                                          \
    ...
    struct initialize_locals<                                \
        ...
        Context >                                            \
    {                                                        \
        typedef BOOST_PP_CAT(vector, N)<                     \
            BOOST_PP_ENUM(N, M1, _)> type;                   \
    };                                                       \
    /**/
...
#define M0(Z, N, D)                                                                        \
    ...
    initialize_locals(                                                                     \
        BOOST_PP_CAT(vector, N) < BOOST_PHOENIX_A(N) > const &locals, Context const &ctx)  \
    {                                                                                      \
        BOOST_PP_CAT(vector, N)<BOOST_PP_ENUM(N, M1, _)> vars = {BOOST_PP_ENUM(N, M2, _)}; \
        return vars;                                                                       \
    }                                                                                      \
    /**/
BOOST_PP_REPEAT_FROM_TO(1, BOOST_PHOENIX_LIMIT, M0, _)
...
template <
    BOOST_PP_ENUM(
        BOOST_PHOENIX_LOCAL_LIMIT, BOOST_PHOENIX_MAP_LOCAL_TEMPLATE_PARAM, _)>
...
struct get_index
{
    BOOST_STATIC_CONSTANT(int,
                          value = (static_cast<int>((sizeof(Map::get(generate_pointer<Tag>()))) / sizeof(char)) - 2));
...
struct outer_local
{
    typedef typename apply_local<Local, typename Env::outer_env_type>::type
        type;
};
...
struct get_local_or_void
{
    typedef typename mpl::eval_if_c <
        Index<Locals::size_value, fusion::result_of::at_c<Locals, Index>, mpl::identity<fusion::void_>>::type
            type;
};
...
{
    typedef typename mpl::eval_if_c<
        Index == -1, outer_local<Local, Env>, get_local_or_void<typename Env::locals_type, Index>>::type
        type;
};
...
struct get_local
{
    ...
    typedef typename get_local_from_index<Local, Env, index_value>::type
        type;
};
...
struct eval_local
{
    ...
    get(Env const &env, mpl::true_)
    {
        ...
        return get<RT, index_value>(
            env.outer_env, mpl::bool_<index_value == -1>());
    }
