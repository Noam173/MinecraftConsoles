...
struct scoped_environment
    : fusion::sequence_facade<
          scoped_environment<Env, OuterEnv, Locals, Map>, fusion::random_access_traversal_tag>
{
    ...
    scoped_environment(
        Env const &env, OuterEnv const &outer_env, Locals const &locals)
        : env(env), outer_env(outer_env), locals(locals)
    {
    }

    scoped_environment(scoped_environment const &o)
        : env(o.env), outer_env(o.outer_env), locals(o.locals) {};

    Env const &env;
    OuterEnv const &outer_env;
    Locals const &locals;

    typedef typename fusion::result_of::pop_front<
        typename add_const<
            typename proto::detail::uncvref<Env>::type>::type>::type
        args_type;
                ...
                typename mpl::eval_if_c<                           \
                    is_const<                                      \
                        typename remove_reference<                 \
                            typename Seq::env_type>::type>::value, \
                    add_const<                                     \
                        typename proto::detail::uncvref<           \
                            typename Seq::env_type>::type>,        \
                    proto::detail::uncvref<                        \
                        typename Seq::env_type>>::type>::type      \
                type;                                              \
                                                                   \
