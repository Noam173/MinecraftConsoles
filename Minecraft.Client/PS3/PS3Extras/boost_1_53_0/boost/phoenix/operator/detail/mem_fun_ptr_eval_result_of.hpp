...
#define BOOST_PHOENIX_MEM_FUN_PTR_EVAL_RESULT_OF_CHILD(Z, N, D)         \
    typedef                                                             \
        typename evaluator::impl<                                       \
            BOOST_PP_CAT(A, N), Context, proto::empty_env>::result_type \
...
{
    BOOST_PP_REPEAT(
        BOOST_PHOENIX_ITERATION, BOOST_PHOENIX_MEM_FUN_PTR_EVAL_RESULT_OF_CHILD, _)

    typedef
        typename boost::result_of<
            child1(
                BOOST_PP_ENUM_SHIFTED_PARAMS(
                    BOOST_PHOENIX_ITERATION, child))>::type
            type;
};
