...
#define BOOST_TT_ALIGNMENT_BASE_TYPES BOOST_PP_TUPLE_TO_LIST( \
    12, (                                                     \
            char, short, int, long, ::boost::long_long_type, float, double, long double, void *, function_ptr, member_ptr, member_function_ptr))
#else
#define BOOST_TT_ALIGNMENT_BASE_TYPES BOOST_PP_TUPLE_TO_LIST( \
    11, (                                                     \
            char, short, int, long, float, double, long double, void *, function_ptr, member_ptr, member_function_ptr))
#endif
...
#define BOOST_TT_CHOOSE_MIN_ALIGNMENT(R, P, I, T)                                               \
    typename lower_alignment_helper<                                                            \
        BOOST_PP_CAT(found, I), target, T>::type BOOST_PP_CAT(t, I);                            \
    enum                                                                                        \
    {                                                                                           \
        BOOST_PP_CAT(found, BOOST_PP_INC(I)) = lower_alignment_helper < BOOST_PP_CAT(found, I), \
        target,                                                                                 \
        T > ::value                                                                             \
    };
...
union lower_alignment {
    ...
    BOOST_PP_LIST_FOR_EACH_I(
        BOOST_TT_CHOOSE_MIN_ALIGNMENT, ignored, BOOST_TT_ALIGNMENT_TYPES)
};

union max_align {
    BOOST_PP_LIST_FOR_EACH_I(
        BOOST_TT_CHOOSE_T, ignored, BOOST_TT_ALIGNMENT_TYPES)
};
...
class type_with_alignment_imp
{
    ...
    typedef typename mpl::if_c<
        ::boost::detail::is_aligned<::boost::alignment_of<t1>::value, Align>::value, t1, ::boost::detail::max_align>::type align_t;
...
{
  public:
    typedef align::a64 type;
};
template <>
class type_with_alignment<128>
{
  public:
    typedef align::a128 type;
};
