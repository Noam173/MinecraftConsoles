...
class multi_array : public multi_array_ref<T, NumDims>
{
  ...
  public:
    ...
    explicit multi_array(ExtentList const &extents,
                         const general_storage_order<NumDims> &so) : super_type((T *)initial_base_, extents, so)
    {
        boost::function_requires<
            detail::multi_array::CollectionConcept<ExtentList>>();
        allocate_space();
    ...
    explicit multi_array(ExtentList const &extents,
                         ...
                         Allocator const &alloc) : super_type((T *)initial_base_, extents, so), allocator_(alloc)
    {
        boost::function_requires<
            detail::multi_array::CollectionConcept<ExtentList>>();
        allocate_space();
    ...
    explicit multi_array(const detail::multi_array ::extent_gen<NumDims> &ranges) : super_type((T *)initial_base_, ranges)
    {

        allocate_space();
    }

    explicit multi_array(const detail::multi_array ::extent_gen<NumDims> &ranges,
                         const general_storage_order<NumDims> &so) : super_type((T *)initial_base_, ranges, so)
    {
    ...
    explicit multi_array(const detail::multi_array ::extent_gen<NumDims> &ranges,
                         const general_storage_order<NumDims> &so,
                         Allocator const &alloc) : super_type((T *)initial_base_, ranges, so), allocator_(alloc)
    {

        allocate_space();
    }

    multi_array(const multi_array &rhs) : super_type(rhs), allocator_(rhs.allocator_)
    {
        allocate_space();
