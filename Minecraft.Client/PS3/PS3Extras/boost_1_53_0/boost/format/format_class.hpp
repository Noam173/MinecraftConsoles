...
class basic_format
{
  ...
  public:
...
#if !defined(BOOST_NO_MEMBER_TEMPLATE_FRIENDS) && !BOOST_WORKAROUND(__BORLANDC__, <= 0x570) && !BOOST_WORKAROUND(_CRAYC, != 0) && !BOOST_WORKAROUND(__DECCXX_VER, BOOST_TESTED_AT(60590042))
    ...
    template <class Ch2, class Tr2, class Alloc2, class T>
    friend void io::detail::distribute(basic_format<Ch2, Tr2, Alloc2> &, T);
  ...
  private:
    ...
    enum style_values
    {
        ordered = 1, // set only if all directives are  positional
        special_needs = 4
    };
