...
#include <boost/flyweight/detail/perfect_fwd.hpp>
        ...
        rep_type(const rep_type &x) : value_ptr(x.value_ptr)
        {
            if (!x.value_ptr)
            {
                new (key_ptr()) key_type(*x.key_ptr());
            }
        }
      ...
      private:
        ...
        bool value_cted() const
        {
            return value_ptr == spc_ptr();
        }
        ...
        void copy_value() const
        {
            if (!value_cted())
            {
                value_ptr = new (spc_ptr()) value_type(*value_ptr);
            }
        }

        mutable typename boost::aligned_storage<
            (sizeof(key_type) > sizeof(value_type)) ? sizeof(key_type) : sizeof(value_type),
            (boost::alignment_of<key_type>::value >
             boost::alignment_of<value_type>::value)
                ? boost::alignment_of<key_type>::value
                : boost::alignment_of<value_type>::value>::type spc;
...
#include <boost/flyweight/detail/perfect_fwd.hpp>
      ...
      private:
        ...
        void construct_value() const
        {
            if (!value_ptr)
            {
                value_ptr = new (spc_ptr()) value_type(key);
            }
        }
...
template <typename Key, typename Value, typename KeyFromValue>
struct key_value : mpl::if_<
                       is_same<KeyFromValue, no_key_from_value>,
                       detail::regular_key_value<Key, Value>,
                       detail::optimized_key_value<Key, Value, KeyFromValue>>::type
{
};
