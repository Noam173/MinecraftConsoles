...
class const_strict_lock_ptr
{
  ...
  public:
    ...
    const_strict_lock_ptr(T const &value, Lockable &mtx) : lk_(mtx), value_(value)
    {
    }
...
class strict_lock_ptr : public const_strict_lock_ptr<T, Lockable>
{
  ...
  public:
    ...
    strict_lock_ptr(T &value, Lockable &mtx) : base_type(value, mtx)
    {
    }
  ...
  private:
    class deref_value
    {
      private:
        ...
        explicit deref_value(synchronized_value &outer) : lk_(outer.mtx_), value_(outer.value_)
        {
        }
      ...
      public:
        ...
        deref_value(BOOST_THREAD_RV_REF(deref_value) other) : lk_(boost::move(BOOST_THREAD_RV(other).lk_)), value_(BOOST_THREAD_RV(other).value_)
        {
        }
        operator T()
        {
    ...
    class const_deref_value
    {
      private:
        ...
        explicit const_deref_value(synchronized_value const &outer) : lk_(outer.mtx_), value_(outer.value_)
        {
        }
      ...
      public:
        ...
        const_deref_value(BOOST_THREAD_RV_REF(const_deref_value) other) : lk_(boost::move(BOOST_THREAD_RV(other).lk_)), value_(BOOST_THREAD_RV(other).value_)
        {
        }
