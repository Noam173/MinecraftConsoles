...
struct BOOST_THREAD_DECL thread_data_base
{
    ...
    thread_data_base() : count(0), thread_handle(detail::win32::invalid_handle_value),
                         ...
                         // #if defined BOOST_THREAD_PROVIDES_INTERRUPTIONS
                         ,
                         interruption_handle(create_anonymous_event(detail::win32::manual_reset_event, detail::win32::event_initially_reset)), interruption_enabled(true)
    // #endif
    {
    }
...
struct BOOST_SYMBOL_VISIBLE timeout
{
    ...
    timeout(uintmax_t milliseconds_) : start(win32::GetTickCount()),
    ...
    timeout(boost::system_time const &abs_time_) : start(win32::GetTickCount()),
                                                   milliseconds(0),
                                                   relative(false),
                                                   abs_time(abs_time_)
    {
    ...
    struct BOOST_SYMBOL_VISIBLE remaining_time
    {
        ...
        remaining_time(uintmax_t remaining) : more(remaining > max_non_infinite_wait),
  ...
  private:
    ...
    explicit timeout(sentinel_type) : start(0), milliseconds(~uintmax_t(0)), relative(true)
    {
    }
};
