...
class strict_lock
{
  ...
  public:
     ...
     */
    explicit strict_lock(mutex_type &mtx) : mtx_(mtx)
    {
        mtx.lock();
    } /*< locks on construction >*/

#if !defined BOOST_THREAD_NO_CXX11_HDR_INITIALIZER_LIST
    strict_lock(std::initializer_list<thread_detail::lockable_wrapper<Lockable>> l_) : mtx_(*(const_cast<thread_detail::lockable_wrapper<Lockable> *>(l_.begin())->m))
    {
        mtx_.lock();
    }
#endif
...
class nested_strict_lock
{
  ...
  public:
     ...
     *
     */
    explicit nested_strict_lock(Lock &lk) : lk_(lk) /*< Store reference to lk >*/
    {
...
#if !defined BOOST_THREAD_NO_CXX11_HDR_INITIALIZER_LIST
    nested_strict_lock(std::initializer_list<thread_detail::lockable_wrapper<Lock>> l_) : lk_(*(const_cast<thread_detail::lockable_wrapper<Lock> *>(l_.begin())->m))
    {
        /*< Define BOOST_THREAD_DONT_CHECK_PRECONDITIONS if you don't want to check lk ownership >*/
