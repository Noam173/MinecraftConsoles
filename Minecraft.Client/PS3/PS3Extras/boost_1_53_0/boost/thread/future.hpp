...
BOOST_SCOPED_ENUM_DECLARE_BEGIN(launch){
    ...
    BOOST_SCOPED_ENUM_DECLARE_BEGIN(future_status){
        ready,
        timeout,
        deferred} BOOST_SCOPED_ENUM_DECLARE_END(future_status)
...
{
  public:
    future_uninitialized() : future_error(system::make_error_code(future_errc::no_state))
    {
    }
};
...
{
  public:
    broken_promise() : future_error(system::make_error_code(future_errc::broken_promise))
    {
    }
};
...
{
  public:
    future_already_retrieved() : future_error(system::make_error_code(future_errc::future_already_retrieved))
    {
    }
};
...
{
  public:
    promise_already_satisfied() : future_error(system::make_error_code(future_errc::promise_already_satisfied))
    {
    }
};
...
{
  public:
    task_already_started() : future_error(system::make_error_code(future_errc::promise_already_satisfied))
    {
    }
};
...
{
  public:
    promise_moved() : future_error(system::make_error_code(future_errc::no_state))
    {
    }
};
...
template <typename T>
struct future_object : detail::future_object_base
{
...
template <typename T>
struct future_object<T &> : detail::future_object_base
{
    typedef typename future_traits<T>::storage_type storage_type;
    typedef typename future_traits<T>::source_reference_type source_reference_type;
    typedef typename future_traits<T>::rvalue_source_type rvalue_source_type;
...
template <>
struct future_object<void> : detail::future_object_base
{
    typedef void shared_future_get_result_type;
...
struct future_async_object : future_object<Rp>
{
  ...
  public:
    explicit future_async_object(BOOST_THREAD_FWD_REF(Fp) f) : thr_(&future_async_object::run, this, boost::forward<Fp>(f))
    {
        this->set_async();
    }
...
class future_waiter
{
    ...
    struct registered_waiter
    {
        ...
        registered_waiter(boost::shared_ptr<detail::future_object_base> const &a_future,
                          ...
                          count_type index_) : future_(a_future), wait_iterator(wait_iterator_), index(index_)
        {
        }
    };
    ...
    struct all_futures_lock
    {
        ...
        all_futures_lock(std::vector<registered_waiter> &futures) : count(futures.size()), locks(new boost::unique_lock<boost::mutex>[count])
        {
...
class basic_future : public base_future
{
  ...
  public:
    ...
    basic_future(BOOST_THREAD_RV_REF(basic_future) other) BOOST_NOEXCEPT : future_(BOOST_THREAD_RV(other).future_)
    {
    ...
    class shared_future : public detail::basic_future<R>
{
  ...
  public:
    ...
    shared_future(shared_future const &other) : base_type(other)
    {
    }
    ...
    shared_future(BOOST_THREAD_RV_REF(shared_future) other) BOOST_NOEXCEPT : base_type(boost::move(static_cast<base_type &>(BOOST_THREAD_RV(other))))
    {
        BOOST_THREAD_RV(other).future_.reset();
    }
    shared_future(BOOST_THREAD_RV_REF(BOOST_THREAD_FUTURE<R>) other) BOOST_NOEXCEPT : base_type(boost::move(static_cast<base_type &>(BOOST_THREAD_RV(other))))
    {
    }
...
class promise
{
  ...
  public:
    ...
    // Assignment
    promise(BOOST_THREAD_RV_REF(promise) rhs) BOOST_NOEXCEPT : future_(BOOST_THREAD_RV(rhs).future_), future_obtained(BOOST_THREAD_RV(rhs).future_obtained)
    {
    ...
    namespace detail
{
    ...
    struct task_object<F, R(ArgTypes...)> : task_base<R(ArgTypes...)>
#else
    template <typename F, typename R>
    struct task_object<F, R()> : task_base<R()>
#endif
#else
    template <typename F, typename R>
    struct task_object : task_base<R>
#endif
    {
      private:
      ...
      public:
        F f;
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
        task_object(BOOST_THREAD_RV_REF(F) f_) : f(boost::forward<F>(f_))
        {
        }
#else
        task_object(F const &f_) : f(f_)
        {
        }
        task_object(BOOST_THREAD_RV_REF(F) f_) : f(boost::move(f_)) // TODO forward
        {
        }
#endif
...
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
    template <typename R, typename... ArgTypes>
    struct task_object<R (*)(ArgTypes...), R(ArgTypes...)> : task_base<R(ArgTypes...)>
#else
    template <typename R>
    struct task_object<R (*)(), R()> : task_base<R()>
#endif
#else
    template <typename R>
    struct task_object<R (*)(), R> : task_base<R>
    ...
    struct task_object<F, void(ArgTypes...)> : task_base<void(ArgTypes...)>
#else
    template <typename F>
    struct task_object<F, void()> : task_base<void()>
#endif
#else
    template <typename F>
    struct task_object<F, void> : task_base<void>
    ...
    struct task_object<void (*)(ArgTypes...), void(ArgTypes...)> : task_base<void(ArgTypes...)>
#else
    template <>
    struct task_object<void (*)(), void()> : task_base<void()>
#endif
#else
    template <>
    struct task_object<void (*)(), void> : task_base<void>
...
class packaged_task
{
  ...
  public:
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    template <class F>
    explicit packaged_task(BOOST_THREAD_RV_REF(F) f, typename disable_if<is_same<typename decay<F>::type, packaged_task>, dummy *>::type = 0)
    {
...
#else
    template <class F>
    explicit packaged_task(F const &f, typename disable_if<is_same<typename decay<F>::type, packaged_task>, dummy *>::type = 0)
    {
#if defined BOOST_THREAD_PROVIDES_SIGNATURE_PACKAGED_TASK
#if defined(BOOST_THREAD_PROVIDES_VARIADIC_THREAD)
    ...
    // assignment
    packaged_task(BOOST_THREAD_RV_REF(packaged_task) other) BOOST_NOEXCEPT : future_obtained(BOOST_THREAD_RV(other).future_obtained)
    {
...
BOOST_THREAD_FUTURE<typename boost::result_of<typename decay<F>::type(
    typename decay<ArgTypes>::type...)>::type>
async(launch policy, BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(ArgTypes)... args)
{
    ...
    typedef typename boost::result_of<typename decay<F>::type(
        typename decay<ArgTypes>::type...)>::type R;
    typedef packaged_task<R(ArgTypes...)> packaged_task_type;
...
async(launch policy, BOOST_THREAD_FWD_REF(F) f)
{
    ...
    if (int(policy) & int(launch::async))
    {
        ...
        return boost::detail::make_future_async_object<Rp>(
            BF(
                thread_detail::decay_copy(boost::forward<F>(f)), thread_detail::decay_copy(boost::forward<ArgTypes>(args))...));
#else
...
struct future_continuation : future_continuation_base
{
    ...
    future_continuation(F &f, BOOST_THREAD_FWD_REF(C) c) : parent(f),
                                                           continuation(boost::forward<C>(c)),
                                                           policy_(f.launch_policy()),
                                                           next()
    {
    }
    future_continuation(F &f, BOOST_THREAD_FWD_REF(C) c, launch policy) : parent(f),
                                                                          continuation(boost::forward<C>(c)),
                                                                          policy_(policy),
                                                                          next()
    {
...
struct future_continuation<F, R, CR (*)(F &)> : future_continuation_base
{
    ...
    future_continuation(F &f, CR (*c)(F &)) : parent(f),
                                              continuation(c),
                                              policy_(f.launch_policy()),
                                              next()
    {
    }
    future_continuation(F &f, CR (*c)(F &), launch policy) : parent(f),
                                                             continuation(c),
                                                             policy_(policy),
                                                             next()
    {
