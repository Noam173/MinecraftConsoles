...
class stream_guard
{
  ...
  public:
    ...
    stream_guard(externally_locked_stream<Stream, RecursiveMutex> &mtx) : mtx_(&mtx)
    {
        mtx.lock();
    }

    stream_guard(externally_locked_stream<Stream, RecursiveMutex> &mtx, adopt_lock_t) : mtx_(&mtx)
    {
    }
...
class externally_locked_stream : public externally_locked<Stream &, RecursiveMutex>
{
  ...
  public:
     ...
     */
    externally_locked_stream(Stream &stream, RecursiveMutex &mtx) : base_type(stream, mtx)
    {
    }
