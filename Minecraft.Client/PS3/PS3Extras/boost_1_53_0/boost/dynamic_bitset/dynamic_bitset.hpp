...
class dynamic_bitset
{
  ...
  private:
    ...
    BOOST_DYNAMIC_BITSET_PRIVATE :
        ...
        bool
        m_unchecked_test(size_type pos) const;
...
dynamic_bitset<Block, Allocator>::count() const
{
    ...
    enum
    {
        no_padding = bits_per_block == CHAR_BIT * sizeof(Block)
    };
#else
    ...
    enum
    {
        ...
        Allocator > ::bits_per_block == CHAR_BIT * sizeof(Block)
    };
#endif

    enum
    {
        enough_table_width = table_width >= CHAR_BIT
    };
...
dynamic_bitset<Block, Allocator>::find_next(size_type pos) const
{
    ...
    return fore ? blk * bits_per_block + lowest_bit(fore)
           ...
           const dynamic_bitset<Block, Alloc> &b)
{
    ...
    if (cerberos)
    {
        ...
        try
        {
            ...
            size_t npad = os.width() <= 0 // careful: os.width() is signed (and can be < 0)
                                  || (bitsetsize_type)os.width() <= b.size()
                              ? 0
                              : os.width() - b.size();
...
operator>>(std::basic_istream<Ch, Tr> &is, dynamic_bitset<Block, Alloc> &b)
{
    ...
    const size_type limit = 0 < w && static_cast<size_type>(w) < b.max_size() ? w : b.max_size();
