...
class random_access_index : BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS SuperMeta::type
...
#pragma parse_mfunc_templ off
  ...
  public:
    ...
    const_reference at(size_type n) const
    {
        if (n >= size())
        {
            throw_exception(std::out_of_range("random access index"));
        }
        return node_type::from_impl(*ptrs.at(n))->value();
    }
    ...
    BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS : random_access_index(
                                                                 const ctor_args_list &args_list, const allocator_type &al) : super(args_list.get_tail(), al),
                                                                                                                              ptrs(al, header()->impl(), 0)
    {
    }

    random_access_index(const random_access_index<SuperMeta, TagList> &x) : super(x),
