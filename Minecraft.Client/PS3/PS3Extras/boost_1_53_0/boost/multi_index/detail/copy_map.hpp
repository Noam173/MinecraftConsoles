...
class copy_map : private noncopyable
{
  public:
    ...
    copy_map(
        const Allocator &al, std::size_t size, Node *header_org, Node *header_cpy) : al_(al), size_(size), spc(al_, size_), n(0),
    ...
    void clone(Node *node)
    {
        ...
        if (n == size_)
        {
            std::sort(&*spc.data(), &*spc.data() + size_);
        }
    }
    ...
    Node *find(Node *node) const
    {
        ...
        return std::lower_bound(
                   begin(), end(), copy_map_entry<Node>(node, 0))
            ->second;
    }
  ...
  private:
    typedef typename prevent_eti<
        ...
        typename boost::detail::allocator::rebind_to<
            Allocator, Node>::type>::type allocator_type;
