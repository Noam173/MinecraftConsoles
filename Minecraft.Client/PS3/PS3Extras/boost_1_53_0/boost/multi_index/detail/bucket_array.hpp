...
class bucket_array_base : private noncopyable
{
  protected:
    inline static std::size_t next_prime(std::size_t n)
    {
        ...
        if (bound == prime_list + prime_list_size)
        {
            bound--;
        }
        return *bound;
...
class bucket_array : public bucket_array_base
{
  ...
  public:
    ...
    bucket_array(const Allocator &al, pointer end_, std::size_t size) : size_(bucket_array_base::next_prime(size)),
                                                                        spc(al, size_ + 1)
    {
        clear();
        end()->next() = end_;
    ...
    pointer end() const
    {
        return buckets() + size_;
    }
    pointer at(std::size_t n) const
    {
        return buckets() + n;
    }
    ...
    void clear()
    {
        for (pointer x = begin(), y = end(); x != y; ++x)
        {
            x->next() = x;
        }
    }
