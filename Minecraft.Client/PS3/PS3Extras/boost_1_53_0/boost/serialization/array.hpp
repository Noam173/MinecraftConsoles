...
template <class T>
class array : public wrapper_traits<const array<T>>
{
  public:
    typedef T value_type;

    array(value_type *t, std::size_t s) : m_t(t),
                                          m_element_count(s)
    {
    }
    array(const array &rhs) : m_t(rhs.m_t),
    ...
    void serialize(Archive &ar, const unsigned int version)
    {
        typedef BOOST_DEDUCED_TYPENAME
            boost::serialization::use_array_optimization<Archive>::template apply<
                BOOST_DEDUCED_TYPENAME remove_const<T>::type>::type use_optimized;
...
#define BOOST_SERIALIZATION_USE_ARRAY_OPTIMIZATION(Archive)                                                                                   \
    ...
    {                                                                                                                                         \
        ...
        struct apply : boost::mpl::apply1<Archive::use_array_optimization, BOOST_DEDUCED_TYPENAME boost::remove_const<ValueType>::type>::type \
        {                                                                                                                                     \
        };                                                                                                                                    \
    };                                                                                                                                        \
    }                                                                                                                                         \
