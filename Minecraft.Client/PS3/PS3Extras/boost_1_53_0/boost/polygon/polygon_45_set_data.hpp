...
class polygon_45_set_data
{
  public:
    ...
    // copy constructor
    inline polygon_45_set_data(const polygon_45_set_data &that) : error_data_(that.error_data_), data_(that.data_), dirty_(that.dirty_),
                                                                  unsorted_(that.unsorted_), is_manhattan_(that.is_manhattan_)
    {
    }

    template <typename ltype, typename rtype, int op_type>
    inline polygon_45_set_data(const polygon_45_set_view<ltype, rtype, op_type> &that) : error_data_(), data_(), dirty_(false), unsorted_(false), is_manhattan_(true)
    {
        (*this) = that.value();
    }
    ...
    {
        if (input_begin == input_end || (!data_.empty() && &(*input_begin) == &(*(data_.begin()))))
        {
            return;
        }
        dirty_ = true;
    ...
    {
        return !error_data_.empty();
    }
    inline std::size_t error_count() const
    {
        return error_data_.size() / 4;
    }
    ...
    // find out if Polygon set is clean
    inline bool is_manhattan() const
    {
        return is_manhattan_;
    }
  ...
  private:
    ...
    void insert_dispatch(const geometry_type &geometry_object, bool is_hole, polygon_90_with_holes_concept)
    {
        ...
        for (typename polygon_with_holes_traits<geometry_type>::iterator_holes_type itr =
                 begin_holes(geometry_object);
             itr != end_holes(geometry_object);
             ++itr)
        {
...
inline void polygon_45_set_data<Unit>::insert_vertex_half_edge_45_pair(const point_data<Unit> &pt1, point_data<Unit> &pt2,
                                                                       ...
                                                                       direction_1d wdir)
{
    if (insert_vertex_half_edge_45_pair_into_vector(data_, pt1, pt2, pt3, wdir))
    {
        is_manhattan_ = false;
    }
}
...
inline void polygon_45_set_data<Unit>::insert(const polygon_45_set_data<Unit> &polygon_set, bool is_hole)
{
    ...
    dirty_ = true;
    unsorted_ = true;
    if (polygon_set.is_manhattan_ == false)
    {
        is_manhattan_ = false;
    }
    return;
...
inline void polygon_45_set_data<Unit>::insert(const polygon_45_set_data<coord_type> &polygon_set, bool is_hole)
{
    ...
    dirty_ = true;
    unsorted_ = true;
    if (polygon_set.is_manhattan() == false)
    {
        is_manhattan_ = false;
    }
    return;
...
void insert_rectangle_into_vector_45(cT &output, const rT &rect, bool is_hole)
{
    point_data<typename rectangle_traits<rT>::coordinate_type>
        llpt = ll(rect),
        lrpt = lr(rect), ulpt = ul(rect), urpt = ur(rect);
...
template <typename Unit>
inline void handleResizingEdge45_SQRT1OVER2(polygon_45_set_data<Unit> &sizingSet, point_data<Unit> first,
...
template <typename Unit>
inline void handleResizingEdge45(polygon_45_set_data<Unit> &sizingSet, point_data<Unit> first,
...
template <typename Unit>
inline void handleResizingVertex45(polygon_45_set_data<Unit> &sizingSet, const point_data<Unit> &first,
                                   ...
                                   int multiplier)
{
    ...
    if (diffAngle < 4)
    {
        if (resizing > 0)
        {
            return; // accute interior corner
        }
        else
        {
            multiplier *= -1; // make it appear to be an accute exterior angle
        }
    }
                               ...
                               std::vector<typename polygon_45_formation<Unit2>::Vertex45Compact> &rvalue_data)
{
    ...
    typename boolean_op_45<Unit2>::template Scan45<typename boolean_op_45<Unit2>::Count2,
                                                   typename boolean_op_45<Unit2>::template boolean_op_45_output_functor<op>>
        scan45;
    std::vector<typename boolean_op_45<Unit2>::Vertex45> eventOut;
    typedef std::pair<typename boolean_op_45<Unit2>::Point,
                      typename boolean_op_45<Unit2>::template Scan45CountT<typename boolean_op_45<Unit2>::Count2>>
        Scan45Vertex;
    ...
    while (iter1 != end1 || iter2 != end2)
    {
        ...
        if (iter2 != end2)
        {
            currentX = (std::min)(currentX, iter2->pt.x());
        }
        if (currentX != x)
        {
        ...
        {
            // std::cout << "case1 next\n";
            eventIn.push_back(Scan45Vertex(iter2->pt,
        ...
        {
            // std::cout << "case2 next\n";
            eventIn.push_back(Scan45Vertex(iter1->pt,
                                           typename polygon_45_formation<Unit2>::
                                               Scan45Count(
                             ...
                             std::vector<typename polygon_45_formation<Unit2>::Vertex45Compact> &lvalue_data)
{
    ...
    typename boolean_op_45<Unit2>::template Scan45<typename boolean_op_45<Unit2>::Count1,
                                                   typename boolean_op_45<Unit2>::template unary_op_45_output_functor<op>>
        scan45;
    std::vector<typename boolean_op_45<Unit2>::Vertex45> eventOut;
...
void get_error_rects_shell(cT &posE, cT &negE, iT beginr, iT endr)
{
    ...
    while (not_done)
    {
        ...
        if (local_abs(x(pt2)) % 2)
        { // y % 2 should also be odd
            ...
            if (dir == CLOCKWISE)
            {
                posE.push_back(rectangle_data<typename Point::coordinate_type>(x(pt2) - 1, y(pt2) - 1, x(pt2) + 1, y(pt2) + 1));
            }
            else
            {
                negE.push_back(rectangle_data<typename Point::coordinate_type>(x(pt2) - 1, y(pt2) - 1, x(pt2) + 1, y(pt2) + 1));
            }
        }
        pt1 = pt2;
        pt2 = pt3;
                                                             ...
                                                             const polygon_45_set_data<Unit> &rvalue) const
{
    ...
    if (is_manhattan() && rvalue.is_manhattan())
    {
        ...
        for (typename value_type::const_iterator itr = data_.begin(); itr != data_.end(); ++itr)
        {
            if ((*itr).count[3] == 0)
            {
                continue; // skip all non vertical edges
            }
        ...
        catch (std::string str)
        {
            ...
            if (str == msg)
            {
                ...
                for (std::size_t i = 0; i < data_.size(); ++i)
                {
                    ...
                    ci.count = typename polygon_45_formation<Unit2>::Vertex45Count(vi.count[0], vi.count[1], vi.count[2], vi.count[3]);
                ...
                if (!result_is_manhattan)
                {
                    ...
                    for (std::size_t i = 0; i < error_data_out.size(); ++i)
                    {
                        ...
                        ci.count = typename polygon_45_formation<Unit>::Vertex45Count(vi.count[0], vi.count[1], vi.count[2], vi.count[3]);
