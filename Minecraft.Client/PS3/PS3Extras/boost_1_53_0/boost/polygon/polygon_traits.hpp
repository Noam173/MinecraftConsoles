...
struct polygon_traits<T,
                      typename gtl_or_4<
                          ...
                          typename gtl_same_type<typename geometry_concept<T>::type, polygon_45_with_holes_concept>::type>::type> : public polygon_traits_general<T>
{
};
...
struct polygon_traits<T,
                      typename gtl_or<
                          ...
                          typename gtl_same_type<typename geometry_concept<T>::type, polygon_90_with_holes_concept>::type>::type> : public polygon_traits_90<T>
{
};
...
struct polygon_traits<T, typename gtl_or<typename gtl_or_4<
                                             ...
                                             typename gtl_same_type<typename geometry_concept<T>::type, polygon_45_concept>::type,
                                             typename gtl_same_type<typename geometry_concept<T>::type, polygon_with_holes_concept>::type,
                                             typename gtl_same_type<typename geometry_concept<T>::type, polygon_45_with_holes_concept>::type>::type,
                                         typename gtl_or<
                                             typename gtl_same_type<typename geometry_concept<T>::type, polygon_90_concept>::type,
                                             typename gtl_same_type<typename geometry_concept<T>::type, polygon_90_with_holes_concept>::type>::type>::type> : public gtl_ifelse<typename gtl_or<typename gtl_same_type<typename geometry_concept<T>::type, polygon_90_concept>::type,
...
template <>
struct polygon_from_polygon_with_holes_type<polygon_with_holes_concept>
{
    typedef polygon_concept type;
};
template <>
struct polygon_from_polygon_with_holes_type<polygon_45_with_holes_concept>
{
    typedef polygon_45_concept type;
};
template <>
struct polygon_from_polygon_with_holes_type<polygon_90_with_holes_concept>
{
    typedef polygon_90_concept type;
};

template <>
struct geometry_domain<polygon_45_concept>
{
    typedef forty_five_domain type;
};
template <>
struct geometry_domain<polygon_45_with_holes_concept>
{
    typedef forty_five_domain type;
};
template <>
struct geometry_domain<polygon_90_concept>
{
    typedef manhattan_domain type;
};
template <>
struct geometry_domain<polygon_90_with_holes_concept>
{
    typedef manhattan_domain type;
};
...
{
    typedef typename coordinate_traits<coordinate_type>::coordinate_distance type;
};
...
typename enable_if<typename gtl_or<
                       typename is_mutable_polygon_90_type<T>::type,
                       typename is_mutable_polygon_90_with_holes_type<T>::type>::type,
                   T>::type &
...
typename enable_if<
    typename gtl_and<typename is_mutable_polygon_type<T1>::type,
                     typename is_polygon_type<T2>::type>::type,
    T1>::type &
...
typename enable_if<
    typename gtl_and<typename is_mutable_polygon_45_with_holes_type<T1>::type,
                     typename is_polygon_45_with_holes_type<T2>::type>::type,
    T1>::type &
assign(T1 &lvalue, const T2 &rvalue)
{
    polygon_mutable_traits<T1>::set_points(lvalue, polygon_traits<T2>::begin_points(rvalue),
...
typename enable_if<
    typename gtl_and<typename is_mutable_polygon_90_type<T1>::type,
                     typename is_polygon_90_type<T2>::type>::type,
    T1>::type &
assign(T1 &lvalue, const T2 &rvalue)
{
...
typename enable_if<
    typename gtl_and<typename is_mutable_polygon_90_with_holes_type<T1>::type,
                     typename is_polygon_90_with_holes_type<T2>::type>::type,
    T1>::type &
assign(T1 &lvalue, const T2 &rvalue)
{
    polygon_90_mutable_traits<T1>::set_compact(lvalue, polygon_90_traits<T2>::begin_compact(rvalue),
...
move(T &polygon, orientation_2d orient, typename polygon_traits<T>::coordinate_type displacement)
{
    ...
    {
        return convolve(polygon, point_data<Unit>(displacement, Unit(0)));
    }
...
typename enable_if<
    typename gtl_and<typename is_any_mutable_polygon_without_holes_type<polygon_type>::type,
                     typename gtl_not<typename gtl_same_type<forty_five_domain,
...
{
    return value < 0 ? (Unit)-value : value;
}
...
typename enable_if<
    typename gtl_and<typename is_any_mutable_polygon_without_holes_type<polygon_type>::type,
                     typename gtl_same_type<forty_five_domain,
...
template <typename T>
T &scale(T &polygon, double factor,
...
#pragma warning(default : 1572)
            ...
            area += (x2 - x1) * (((area_type)y(*begin_range) - y_base) + ((area_type)y(previous) - y_base)) / 2;
        }
    ...
    if (!equivalence(first, previous))
    {
        ...
        area += (x2 - x1) * (((area_type)y(first) - y_base) + ((area_type)y(previous) - y_base)) / 2;
    }
    return area;
}
...
area(const T &polygon)
{
    ...
    area_type retval = point_sequence_area<typename polygon_traits<T>::iterator_type, area_type>(begin_points(polygon), end_points(polygon));
    if (retval < 0)
    {
        retval *= -1;
    }
    ...
    {
        area_type tmp_area = point_sequence_area<typename polygon_traits<typename polygon_with_holes_traits<T>::hole_type>::iterator_type, area_type>(begin_points(*itr), end_points(*itr));
        if (tmp_area < 0)
        {
            tmp_area *= -1;
        }
...
is_45(const polygon_type &polygon)
{
    ...
    if (!point_sequence_is_45(itr, itr_end))
    {
        return false;
    }
    ...
    for (; itrh != itrh_end; ++itrh)
    {
        ...
        if (!point_sequence_is_45(itr1, itr1_end))
        {
            return false;
        }
    }
...
template <typename T>
typename distance_type_by_domain<typename geometry_domain<typename geometry_concept<T>::type>::type, typename polygon_traits<T>::coordinate_type>::type
perimeter(const T &polygon,
          typename enable_if<
              typename is_polygon_with_holes_type<T>::type>::type * = 0)
{
    typedef typename distance_type_by_domain<typename geometry_domain<typename geometry_concept<T>::type>::type, typename polygon_traits<T>::coordinate_type>::type Unit;
...
winding(const T &polygon)
{
    ...
    typedef typename area_type_by_domain<typename geometry_domain<typename geometry_concept<T>::type>::type,
                                         typename polygon_traits<T>::coordinate_type>::type area_type;
    return point_sequence_area<typename polygon_traits<T>::iterator_type, area_type>(begin_points(polygon), end_points(polygon)) < 0 ? COUNTERCLOCKWISE : CLOCKWISE;
}
...
struct edge_utils
{
    ...
    class less_point : public std::binary_function<Point, Point, bool>
    {
      public:
        ...
        inline bool operator()(const Point &pt1, const Point &pt2) const
        {
            ...
            if (pt1.get(HORIZONTAL) == pt2.get(HORIZONTAL))
            {
                if (pt1.get(VERTICAL) < pt2.get(VERTICAL))
                {
                    return true;
                }
            }
    ...
    static inline bool less_slope(area_type dx1, area_type dy1, area_type dx2, area_type dy2)
    {
        ...
        int cross_1_sign = dx2_sign * dy1_sign;
        int cross_2_sign = dx1_sign * dy2_sign;
        if (cross_1_sign < cross_2_sign)
        {
            return true;
        }
        if (cross_2_sign < cross_1_sign)
        {
            return false;
        }
        if (cross_1_sign == -1)
        {
            return cross_2 < cross_1;
        }
    ...
    static inline int on_above_or_below(Point pt, const half_edge &he)
    {
        ...
        if (!between(pt, he.first, he.second))
        {
            retval *= -1;
        }
        return retval;
...
contains(const T &polygon, const input_point_type &point, bool consider_touch = true)
{
    ...
    bool isInside = contains(view_as<typename polygon_from_polygon_with_holes_type<
                                 typename geometry_concept<T>::type>::type>(polygon),
                             point, consider_touch);
    if (!isInside)
    {
        return false; // no need to check holes
    }
    ...
    T, typename get_void<typename is_any_mutable_polygon_without_holes_type<T>::type>::type>
{
    typedef T hole_type;
    typedef const hole_type *iterator_holes_type;
    static inline iterator_holes_type begin_holes(const hole_type &t)
    {
        return &t;
    }
    static inline iterator_holes_type end_holes(const hole_type &t)
    {
        return &t;
    }
    static inline std::size_t size_holes(const hole_type &t)
    {
        return 0;
    }
};
