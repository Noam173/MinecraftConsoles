...
struct are_not_both_rectangle_concept
{
...
typename enable_if<typename gtl_and<
                       ...
                       typename gtl_same_type<rectangle_concept,
                                              typename geometry_concept<typename std::iterator_traits<typename output_container_type::iterator>::value_type>::type>::type>::type,
                   void>::type
...
empty(const polygon_set_type &polygon_set)
{
    ...
    {
        return begin_90_set_data(polygon_set) == end_90_set_data(polygon_set);
    }
    polygon_90_set_data<typename polygon_90_set_traits<polygon_set_type>::coordinate_type> ps;
    assign(ps, polygon_set);
    ps.clean();
