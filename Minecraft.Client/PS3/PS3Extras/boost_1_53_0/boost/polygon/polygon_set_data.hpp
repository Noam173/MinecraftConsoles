...
class polygon_set_data
{
  public:
    ...
    // copy constructor
    inline polygon_set_data(const polygon_set_data &that) : data_(that.data_), dirty_(that.dirty_), unsorted_(that.unsorted_), is_45_(that.is_45_)
    {
    }
    ...
    inline void insert_clean(const element_type &edge, bool is_hole = false)
    {
        if (!scanline_base<coordinate_type>::is_45_degree(edge.first) &&
            ...
            !scanline_base<coordinate_type>::is_vertical(edge.first))
        {
            is_45_ = false;
        }
    ...
    inline bool downcast(polygon_45_set_data<coordinate_type> &result) const
    {
        ...
        for (iterator_type itr = begin(); itr != end(); ++itr)
        {
            ...
            if (scanline_base<coordinate_type>::is_horizontal(elem.first))
            {
                rise = 0;
            }
            else if (scanline_base<coordinate_type>::is_vertical(elem.first))
            {
                rise = 2;
            }
            else
            {
                ...
                if (elem.first.first.y() > elem.first.second.y())
                {
                    rise = -1; // down sloping 45
                }
            }
    ...
    inline GEOMETRY_CONCEPT_ID concept_downcast() const
    {
        ...
        for (iterator_type itr = begin(); itr != end(); ++itr)
        {
            ...
            if (h_delta != 0 || v_delta != 0)
            {
                ...
                if (v_delta != h_delta && v_delta != -h_delta)
                {
                    return POLYGON_SET_CONCEPT;
                }
                else
                {
                    is_45 = true;
