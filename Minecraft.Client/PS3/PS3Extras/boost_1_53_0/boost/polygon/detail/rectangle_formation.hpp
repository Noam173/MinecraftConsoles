...
class ScanLineToRects
{
  ...
  public:
    ...
    inline ScanLineToRects(orientation_2d orient, rectangle_type model) : scanData_(orientation_2d(orient.to_int() ? VERTICAL : HORIZONTAL)),
...
template <class CT, class ST, class rectangle_type, typename interval_type, typename coordinate_type>
inline CT &
             ...
             bool &haveCurrentRect, rectangle_type &currentRect, coordinate_type currentCoordinate, orientation_2d orient)
{
    ...
    if (!edgeProcessed)
    {
        if (haveCurrentRect)
        {
            if (currentRect.get(orient.get_perpendicular().get_direction(HIGH)) == currentCoordinate &&
...
template <class T>
template <class CT>
inline ScanLineToRects<T> &ScanLineToRects<T>::processEdge(CT &rectangles, const interval_data<coordinate_type> &edge)
{
                     ...
                     orientation_2d orient, rectangle_concept)
{
         ...
         itr != end; ++itr)
    {
        ...
        if (abs((*itr).second.second) > 1)
        {
            itr = tmp_itr; // next edge begins from this vertex
        }
    }
}
} // namespace polygon
