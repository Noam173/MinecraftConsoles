  ...
  public:
    ...
    class iterator
    {
      ...
      public:
        ...
        inline iterator(const ActiveTail *at, bool isHole, orientation_2d orient) : pLine_(), pLineEnd_(), index_(), indexEnd_(), startEnd_()
        {
...
class PolyLineHoleData
{
  ...
  public:
    ...
    {
        return p_->begin(true, (orientT ? VERTICAL : HORIZONTAL));
    }
    ...
    {
        return iterator_type(begin_compact(), end_compact());
    }
    inline iterator_type end() const
    {
        return iterator_type(end_compact(), end_compact());
    }
    inline std::size_t size() const
    {
        return 0;
    }
    inline ActiveTail<Unit> *yield()
    {
        return p_;
    }
    template <class iT>
  ...
  public:
    ...
    class iteratorHoles
    {
      ...
      public:
        ...
        inline PolyLineHoleData<orientT, Unit> operator*()
        {
            return PolyLineHoleData<orientT, Unit>(*itr_);
        }
    };
    ...
    inline compact_iterator_type begin_compact() const
    {
        return p_->begin(false, (orientT ? VERTICAL : HORIZONTAL));
    }
    inline compact_iterator_type end_compact() const
    {
        return p_->end();
    ...
    {
        return iteratorHoles(p_->beginHoles());
    }
    inline iteratorHoles end_holes() const
    {
        return iteratorHoles(p_->endHoles());
    }
    inline ActiveTail<Unit> *yield()
    {
        return p_;
...
template <bool orientT, typename Unit>
struct PolyLineType<orientT, Unit, polygon_90_with_holes_concept>
{
    typedef PolyLinePolygonWithHolesData<orientT, Unit> type;
};
template <bool orientT, typename Unit>
struct PolyLineType<orientT, Unit, polygon_45_with_holes_concept>
{
    typedef PolyLinePolygonWithHolesData<orientT, Unit> type;
};
template <bool orientT, typename Unit>
struct PolyLineType<orientT, Unit, polygon_with_holes_concept>
{
    typedef PolyLinePolygonWithHolesData<orientT, Unit> type;
};
template <bool orientT, typename Unit>
struct PolyLineType<orientT, Unit, polygon_90_concept>
{
    typedef PolyLineHoleData<orientT, Unit> type;
};
template <bool orientT, typename Unit>
struct PolyLineType<orientT, Unit, polygon_45_concept>
{
    typedef PolyLineHoleData<orientT, Unit> type;
};
template <bool orientT, typename Unit>
struct PolyLineType<orientT, Unit, polygon_concept>
{
    typedef PolyLineHoleData<orientT, Unit> type;
};
...
template <class T>
inline bool to_bool(const T &val)
{
    return val != 0;
}
...
// constructor
template <typename Unit>
inline PolyLine<Unit>::PolyLine(orientation_2d orient, Unit coord, Side side) : ptdata_(1, coord),
                                                                                headp_(0),
                                                                                tailp_(0),
...
template <typename Unit>
inline ActiveTail<Unit>::ActiveTail(orientation_2d orient, Unit coord, Side solidToRight, ActiveTail *otherTailp) : tailp_(0), otherTailp_(0), holesList_()
{
...
template <typename Unit>
inline ActiveTail<Unit>::ActiveTail(PolyLine<Unit> *active, ActiveTail<Unit> *otherTailp) : tailp_(active), otherTailp_(otherTailp), holesList_()
{
}
...
inline void ActiveTail<Unit>::writeOutFigure(std::vector<Unit> &outVec, bool isHole) const
{
    ...
    if (isHole)
    {
        startEnd = otherTailp_->tailp_->endConnectivity(HEAD);
    }
    while (nextPolyLinep)
    {
...
inline typename std::map<TKey, T>::iterator findAtNext(std::map<TKey, T> &theMap,
