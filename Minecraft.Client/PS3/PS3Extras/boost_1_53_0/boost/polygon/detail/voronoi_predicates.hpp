                 ...
                 bool recompute_lower_x = true)
        {
            ...
            if (is_zero(orientation))
            {
                ...
                if (recompute_c_y)
                {
                    ...
                    cA[1] = a[0] * a[0] * (static_cast<int_x2_type>(segm_start1.y()) + static_cast<int_x2_type>(segm_start2.y())) -
                            a[0] * b[0] * (static_cast<int_x2_type>(segm_start1.x()) + static_cast<int_x2_type>(segm_start2.x()) - static_cast<int_x2_type>(site1.x()) * 2) +
                ...
                if (recompute_c_x || recompute_lower_x)
                {
                    ...
                    cA[1] = b[0] * b[0] * (static_cast<int_x2_type>(segm_start1.x()) + static_cast<int_x2_type>(segm_start2.x())) -
                            a[0] * b[0] * (static_cast<int_x2_type>(segm_start1.y()) + static_cast<int_x2_type>(segm_start2.y()) - static_cast<int_x2_type>(site1.y()) * 2) +
      ...
      public:
                 ...
                 circle_type &c_event)
        {
            ...
            robust_fpt_type inv_segm_len(to_fpt(1.0) /
                                             get_sqrt(line_a * line_a + line_b * line_b),
                                         to_fpt(3.0));
            robust_dif_type t;
                 ...
                 circle_type &c_event)
        {
            ...
            if (ot::eval(orientation) == ot::COLLINEAR)
            {
                ...
                t /= a;
                robust_dif_type c_x, c_y;
                c_x += robust_fpt_type(to_fpt(0.5) * (to_fpt(segm_start1.x()) + to_fpt(segm_start2.x())));
                c_x += robust_fpt_type(a1) * t;
                c_y += robust_fpt_type(to_fpt(0.5) * (to_fpt(segm_start1.y()) + to_fpt(segm_start2.y())));
