...
class robust_fpt
{
  public:
    ...
    robust_fpt(floating_point_type fpv, relative_error_type error) : fpv_(fpv), re_(error)
    {
    }
...
{
  public:
    robust_dif() : positive_sum_(0),
                   negative_sum_(0)
    {
    }

    explicit robust_dif(const T &value) : positive_sum_((value > 0) ? value : 0),
                                          negative_sum_((value < 0) ? -value : 0)
    {
    }

    robust_dif(const T &pos, const T &neg) : positive_sum_(pos),
                                             negative_sum_(neg)
    {
    }
