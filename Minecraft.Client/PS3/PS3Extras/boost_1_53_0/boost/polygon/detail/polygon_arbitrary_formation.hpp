...
struct scanline_base
{
    ...
    static inline bool intersects_grid(Point pt, const half_edge &he)
    {
        ...
        {
            if (is_vertical(he) || is_horizontal(he))
            {
                return true;
            }
        }
        ...
        if (intersects(widget1, he) && on_above_or_below(pt11, he))
        {
            return true;
        }
        half_edge widget2(pt01, pt10);
        // intersects but not just at pt01 or 10
        if (intersects(widget2, he) && on_above_or_below(pt01, he) && on_above_or_below(pt10, he))
        {
            return true;
        }
        return false;
    ...
    static inline Unit evalAtXforYlazy(Unit xIn, Point pt, Point other_pt)
    {
        long double
            evalAtXforYret,
            evalAtXforYxIn, evalAtXforYx1, evalAtXforYy1, evalAtXforYdx1, evalAtXforYdx,
        ...
        evalAtXforY0 = 0;
        if (evalAtXforYdx1 == evalAtXforY0)
        {
            return (Unit)evalAtXforYy1;
        }
    ...
    static inline typename high_precision_type<Unit>::type evalAtXforY(Unit xIn, Point pt, Point other_pt)
    {
        ...
        if (evalAtXforYdx1 == evalAtXforY0)
        {
            return evalAtXforYret = evalAtXforYy1;
        }
    ...
    struct compute_intersection_pack
    {
                                                     ...
                                                     bool projected = false, bool round_closest = false)
        {
            ...
            if (!projected && !::boost::polygon::intersects(rect1, rect2, true))
            {
                return false;
            }
            if (is_vertical(he1))
            ...
            if (equal_slope_hp(dx1, dy1, dx2, dy2))
            {
                return false;
            }
            ...
            if (!projected && !contains(rect1, result, true))
            {
                return false;
            }
            if (!projected && !contains(rect2, result, true))
            {
                return false;
            }
            if (projected)
                                               ...
                                               bool projected = false, bool round_closest = false)
        {
            ...
            set_points(rect1, he1.first, he1.second);
            set_points(rect2, he2.first, he2.second);
            if (!::boost::polygon::intersects(rect1, rect2, true))
            {
                return false;
            }
            if (is_vertical(he1))
            ...
            // truncate downward if it went up due to negative number
            if (x < (high_precision)x_unit)
            {
                --x_unit;
            }
            if (y < (high_precision)y_unit)
            {
                --y_unit;
            }
            if (is_horizontal(he1))
            ...
            // std::cout << exp_x << " " << exp_y << " " << y1 << " " << y2 << "\n";
            Point result(x_unit, y_unit);
            if (!contains(rect1, result, true))
            {
                return false;
            }
            if (!contains(rect2, result, true))
            {
                return false;
            }
            if (projected)
    ...
    static inline bool intersects(const half_edge &he1, const half_edge &he2)
    {
        ...
        if (::boost::polygon::intersects(rect1, rect2, false))
        {
            ...
            if (oab1 == 0 && between(he1.first, he2.first, he2.second))
            {
                return true;
            }
            int oab2 = on_above_or_below(he1.second, he2);
            if (oab2 == 0 && between(he1.second, he2.first, he2.second))
            {
                return true;
            }
            if (oab1 == oab2 && oab1 != 0)
            {
                return false; // both points of he1 are on same side of he2
            }
            int oab3 = on_above_or_below(he2.first, he1);
            if (oab3 == 0 && between(he2.first, he1.first, he1.second))
            {
                return true;
            }
            int oab4 = on_above_or_below(he2.second, he1);
            if (oab4 == 0 && between(he2.second, he1.first, he1.second))
            {
                return true;
            }
            if (oab3 == oab4)
            {
                return false; // both points of he2 are on same side of he1
            }
    ...
    class vertex_half_edge
    {
      public:
        ...
        inline vertex_half_edge &operator=(const std::pair<Point, Point> &vertex)
        {
            return *this;
        }
        ...
        inline bool operator==(const std::pair<Point, Point> &vertex) const
        {
            return false;
        }
        inline bool operator!=(const std::pair<Point, Point> &vertex) const
...
class polygon_arbitrary_formation : public scanline_base<Unit>
{
  public:
    ...
    class poly_line_arbitrary
    {
      public:
        ...
        inline poly_line_arbitrary() : points()
        {
    ...
    class active_tail_arbitrary
    {
      ...
      public:
        ...
        inline active_tail_arbitrary(Point point, active_tail_arbitrary *otherTailp, bool head = true) : tailp_(), otherTailp_(), holesList_(), head_()
        {
            tailp_ = new poly_line_arbitrary;
        ...
        inline active_tail_arbitrary(active_tail_arbitrary *otherTailp) : tailp_(), otherTailp_(), holesList_(), head_()
        {
        ...
        // copy constructor
        inline active_tail_arbitrary(const active_tail_arbitrary &that) : tailp_(), otherTailp_(), holesList_(), head_()
        {
            (*this) = that;
        }
    ...
    class vertex_arbitrary_compact
    {
      public:
        ...
        inline vertex_arbitrary_compact &operator=(const std::pair<Point, Point> &vertex)
        {
            return *this;
        }
  ...
  public:
    inline polygon_arbitrary_formation() : scanData_(), x_((std::numeric_limits<Unit>::min)()), justBefore_(false), fractureHoles_(0)
    {
        less_vertex_half_edge lessElm(&x_, &justBefore_);
        scanData_ = scanline_data(lessElm);
    }
    inline polygon_arbitrary_formation(bool fractureHoles) : scanData_(), x_((std::numeric_limits<Unit>::min)()), justBefore_(false), fractureHoles_(fractureHoles)
    {
        less_vertex_half_edge lessElm(&x_, &justBefore_);
        scanData_ = scanline_data(lessElm);
    }
    inline polygon_arbitrary_formation(const polygon_arbitrary_formation &that) : scanData_(), x_((std::numeric_limits<Unit>::min)()), justBefore_(false), fractureHoles_(0)
    {
        (*this) = that;
    }
                            ...
                            if (incoming[j] != 0)
                            {
                                if (incoming[j] == 1)
                                {
                                    ...
                                    else
                                    {
                                        elements.push_back(std::pair<vertex_half_edge,
                                                                     active_tail_arbitrary *>(vertex_half_edge(point,
                                                                                                               incoming_count[j].first, incoming[j]),
                                                                                              tails[i]));
                                    }
                                    tails[i] = 0;
                                    counts[i] = 0;
                ...
                if (incoming[i] != 0)
                {
                    for (int j = i + 1; j < i_size; ++j)
                    {
                        if (incoming[j] != 0)
                        {
                            ...
                            elements.push_back(std::pair<vertex_half_edge,
                                                         active_tail_arbitrary *>(vertex_half_edge(point,
                                                                                                   incoming_count[i].first, incoming[i]),
                                                                                  tailPair.second));
                            incoming[i] = 0;
                            incoming[j] = 0;
                            break;
    ...
    inline iT processEvent_(cT &output, iT inputBegin, iT inputEnd)
    {
        ...
        while (currentIter != inputEnd && currentIter->pt.get(HORIZONTAL) == x_)
        {
                    ...
                    (iter->first.other_pt.x() == x_ && iter->first.other_pt.y() == currentY)))
            {
                ...
                counts_from_scanline.push_back(std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arbitrary *>(std::pair<std::pair<Point, Point>, int>(std::pair<Point, Point>(iter->first.pt,
                                                                                                                                                                                           iter->first.other_pt),
                                                                                                                                                                   iter->first.count),
                                                                                                                           iter->second));
                ++iter;
            ...
            if (verticalTail)
            {
                ...
                counts_from_scanline.push_back(std::pair<std::pair<std::pair<Point, Point>, int>, active_tail_arbitrary *>(std::pair<std::pair<Point, Point>, int>(std::pair<Point, Point>(verticalCount.first,
                                                                                                                                                                                           currentPoint),
