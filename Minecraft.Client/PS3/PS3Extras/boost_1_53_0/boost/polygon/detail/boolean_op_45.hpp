...
struct boolean_op_45
{
    ...
    class Count2
    {
      public:
...
#endif
        {
            ...
            counts[1] = count.counts[1];
        }
        inline bool operator==(const Count2 &count) const
        {
            return counts[0] == count.counts[0] && counts[1] == count.counts[1];
        }
        inline bool operator!=(const Count2 &count) const
        {
            return !((*this) == count);
        }
        inline Count2 &operator=(int count)
        {
        ...
        inline int &operator[](bool index)
        {
            return counts[index];
        }
        inline int operator[](bool index) const
        {
            return counts[index];
        }
        inline Count2 &operator+=(const Count2 &count)
    ...
    class Count1
    {
      public:
        ...
        inline bool operator==(const Count1 &count) const
        {
        ...
        inline Count1 &operator=(int count)
        {
            count_ = count;
            return *this;
        }
    ...
    class Scan45ElementT
    {
      public:
        ...
        inline Scan45ElementT(Unit xIn, Unit yIn, int riseIn, CountType countIn = CountType()) : x(xIn), y(yIn), rise(riseIn), count(countIn)
        {
        }
        inline Scan45ElementT(const Scan45ElementT &that) : x(that.x), y(that.y), rise(that.rise), count(that.count)
        {
        }
    ...
    class lessScan45ElementRise : public std::binary_function<Scan45Element, Scan45Element, bool>
    {
      public:
        ...
        {
        } // default constructor is only constructor
    ...
    class lessScan45Element
    {
      ...
      public:
        ...
        inline lessScan45Element &operator=(const lessScan45Element &that)
        {
            x_ = that.x_;
    ...
    class Scan45CountT
    {
      public:
        ...
        inline Scan45CountT(CountType count) : counts()
        {
        ...
        inline CountType &operator[](int index)
        {
            return counts[index];
        }
        inline CountType operator[](int index) const
        {
            return counts[index];
        }
        inline Scan45CountT &operator+=(const Scan45CountT &count)
    ...
    class Vertex45T
    {
      public:
        ...
        inline Vertex45T &operator=(const std::pair<Point, Point> &vertex)
        {
            return *this;
        }
    ...
    class lessVertex45
    {
      ...
      public:
        ...
        inline lessVertex45 &operator=(const lessVertex45 &that)
        {
            x_ = that.x_;
            justBefore_ = that.justBefore_;
            return *this;
...
#pragma warning(default : 4127)
      ...
      public:
        ...
        void scan(cT &output, iT inputBegin, iT inputEnd)
        {
            ...
            while (inputBegin != inputEnd)
            {
                ...
                if (!crossVector_.empty() && crossVector_[0].first.x() < nextX)
                {
                    nextX = crossVector_[0].first.x();
                }
                if (nextX != x_)
                {
                    // std::cout << "3\n";
      ...
      private:
        ...
        inline void advance_(cT &output)
        {
                   ...
                   (*crossQueue_.begin()).x() == x_)
            {
                ...
                while (searchDownItr != scanData_.begin() && searchDownItr->evalAtX(x_) == vertex.first.y())
                {
    ...
    static inline bool testScan45Data(streamtype &stdcout)
    {
        ...
        if (itr1 != itr2)
        {
            stdcout << "test1 failed\n";
        }
        if (itrA == itrB)
        {
            stdcout << "test2 failed\n";
        }
        ...
        if (itr != itr10)
        {
            stdcout << "test3 failed\n";
        }
        ++itr;
        if (itr != itr30)
        {
            stdcout << "test4 failed\n";
        }
        ++itr;
        if (itr != itr20)
        {
            stdcout << "test5 failed\n";
        }
        ++itr;
        if (itr != itr40)
        {
            stdcout << "test6 failed\n";
        }
    ...
    {
        if (!testScan45Rect(stdcout))
        {
            return false;
        }
        if (!testScan45P1(stdcout))
        {
            return false;
        }
        if (!testScan45P2(stdcout))
        {
            return false;
        }
        if (!testScan45And(stdcout))
        {
            return false;
        }
        if (!testScan45Star1(stdcout))
        {
            return false;
        }
        if (!testScan45Star2(stdcout))
        {
            return false;
        }
        if (!testScan45Star3(stdcout))
        {
            return false;
        }
        if (!testScan45Star4(stdcout))
        {
            return false;
        }
        return true;
