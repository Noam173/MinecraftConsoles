...
class line_intersection : public scanline_base<Unit>
{
  ...
  public:
    ...
    static inline void compute_y_cuts(std::vector<Unit> &y_cuts, iT begin, iT end, std::size_t size)
    {
        ...
        if (size < 30)
        {
            return; // 30 is empirically chosen, but the algorithm is not sensitive to this constant
        }
        ...
        for (iT itr = begin; itr != end; ++itr, ++position)
        {
            ...
            if (histogram_size - position < histogram_size / 3)
            {
                break;
            }
                                     ...
                                     iT begin, iT end, Unit min_y)
    {
             ...
             outer != data.end(); ++outer)
        {
            ...
            // itr = pts.begin();
            // itr2 = pts.end();
            while (lfinger != newend && (*lfinger).x() < startpt.x())
            {
                ++lfinger;
            }
...
class merge_output_functor
{
  public:
    ...
    void operator()(result_type &result, const half_edge &edge, const key_type &left, const key_type &right)
    {
        ...
        if (scanline_base<Unit>::is_vertical(edge))
        {
            elem.second *= -1;
        }
        if (!left.empty())
  ...
  protected:
    ...
    class less_vertex_data
    {
      ...
      public:
        ...
        {
            less_point lp;
            if (lp(lvalue.first.first, rvalue.first.first))
            {
                return true;
            }
            if (lp(rvalue.first.first, lvalue.first.first))
            {
                return false;
            }
...
class arbitrary_connectivity_extraction : public scanline_base<Unit>
{
  ...
  public:
    ...
    inline arbitrary_connectivity_extraction(const arbitrary_connectivity_extraction &pm) : pmd(pm.pmd), evalAtXforYPack_(pm.evalAtXforYPack_)
    {
    }
    inline arbitrary_connectivity_extraction &operator=(const arbitrary_connectivity_extraction &pm)
    {
        pmd = pm.pmd;
        return *this;
    }
    ...
    inline void execute(result_type &result)
    {
        ...
        std::pair<std::pair<Unit, std::map<point_data<Unit>, std::set<property_type>>>,
                  result_type *>
            output(std::make_pair(std::make_pair((std::numeric_limits<Unit>::max)(),
                                                 std::map<point_data<Unit>,
                                                          std::set<property_type>>()),
                                  &result));
        connectivity_extraction_output_functor<std::pair<std::pair<Unit,
                                                                   std::map<point_data<Unit>, std::set<property_type>>>,
                                                         result_type *>,
