...
struct polygon_45_touch
{
    ...
    class CountTouch
    {
      public:
        ...
        inline bool operator==(const CountTouch &count) const
        {
    ...
    typedef typename std::pair<Point,
                               typename boolean_op_45<Unit>::template Scan45CountT<CountTouch>>
        Vertex45Compact;
    ...
    static void performTouch(graph_type &graph, TouchSetData &tsd)
    {
        ...
        std::pair<std::pair<Unit, std::map<Unit, std::set<int>>>, graph_type *> output(std::make_pair(std::make_pair((std::numeric_limits<Unit>::max)(), std::map<Unit, std::set<int>>()), &graph));
