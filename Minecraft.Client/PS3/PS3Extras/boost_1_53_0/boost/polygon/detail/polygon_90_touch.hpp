...
struct touch_90_operation
{
      ...
      public:
        ...
        class iterator
        {
          ...
          public:
            ...
            inline iterator(const iterator &that) : itr_(), ivlIds_(), incremented_(false)
            {
                (*this) = that;
            }
        ...
        {
            return iterator(eventData_.end(), 0, 0, std::set<int>());
        }
    ...
    class TouchOp
    {
      ...
      public:
        inline TouchOp() : scanData_(), nextItr_()
        {
            nextItr_ = scanData_.end();
        }
        inline TouchOp(const TouchOp &that) : scanData_(that.scanData_), nextItr_()
        {
        ...
        inline void processInterval(graphT &outputContainer, Interval ivl, const std::set<int> &ids, bool leadingEdge)
        {
            ...
            else
            {
                ...
                if (leadingEdge)
                {
                    evaluateBorder_(outputContainer, highItr->second, ids);
                }
            }
