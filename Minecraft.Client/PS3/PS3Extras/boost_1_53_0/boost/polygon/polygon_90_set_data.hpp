...
class polygon_90_set_data
{
  public:
    ...
    inline polygon_90_set_data(orientation_2d orient, iT input_begin, iT input_end) : orient_(HORIZONTAL), data_(), dirty_(false), unsorted_(false)
    {
        dirty_ = true;
        unsorted_ = true;
    ...
    // copy constructor
    inline polygon_90_set_data(const polygon_90_set_data &that) : orient_(that.orient_), data_(that.data_), dirty_(that.dirty_), unsorted_(that.unsorted_)
    {
    }
    ...
    inline polygon_90_set_data(orientation_2d orient, const polygon_90_set_data &that) : orient_(orient), data_(), dirty_(false), unsorted_(false)
    {
                       ...
                       orientation_2d orient = HORIZONTAL)
    {
        ...
        if (orient != orient_)
        {
            std::swap(data_.back().first, data_.back().second.first);
        }
        if (is_hole)
        {
            data_.back().second.second *= -1;
        }
        dirty_ = true;
        unsorted_ = true;
    }
...
class property_merge_90
{
  ...
  public:
    ...
    inline property_merge_90 &operator=(const property_merge_90 &that)
    {
        pmd_ = that.pmd_;
        return *this;
    }
