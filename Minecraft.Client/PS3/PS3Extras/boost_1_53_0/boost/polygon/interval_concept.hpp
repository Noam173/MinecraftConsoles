...
struct interval_difference_type_by_concept<GeometryType, gtl_yes>
{
    typedef typename coordinate_traits<
        typename interval_traits<GeometryType>::coordinate_type>::coordinate_difference type;
};
...
typename enable_if<
    typename gtl_and<
        y_i_get,
        typename is_interval_concept<
            typename geometry_concept<IntervalType>::type>::type>::type,
...
abuts(
    ...
    direction_1d dir)
{
    return dir.to_int() ? low(interval2) == high(interval1) : low(interval1) == high(interval2);
}
