...
empty(const polygon_set_type &polygon_set)
{
    ...
    {
        return begin_polygon_set_data(polygon_set) == end_polygon_set_data(polygon_set);
    }
    polygon_set_data<typename polygon_set_traits<polygon_set_type>::coordinate_type> ps;
    assign(ps, polygon_set);
    ps.clean();
...
simplify(polygon_set_type &polygon_set, typename coordinate_traits<
                                            typename polygon_set_traits<polygon_set_type>::coordinate_type>::coordinate_distance threshold)
{
    typedef typename polygon_set_traits<polygon_set_type>::coordinate_type Unit;
    typedef polygon_with_holes_data<Unit> p_type;
    std::vector<p_type> polys;
    ...
    for (std::size_t i = 0; i < polys.size(); ++i)
    {
        ...
        for (typename std::list<polygon_data<Unit>>::iterator itrh =
                 polys[i].holes_.begin();
             itrh != (polys[i].holes_.end()); ++itrh)
        {
...
{
    return polygon_set_view<geometry_type_1, geometry_type_2, 0>(lvalue, rvalue);
}
...
typename enable_if<typename gtl_and_4<yes_ps_op,
                                      ...
                                      typename gtl_if<typename is_either_polygon_set_type<geometry_type_1, geometry_type_2>::type>::type>::type,
                   polygon_set_view<geometry_type_1, geometry_type_2, 0>>::type
operator+(const geometry_type_1 &lvalue, const geometry_type_2 &rvalue)
{
    return polygon_set_view<geometry_type_1, geometry_type_2, 0>(lvalue, rvalue);
...
operator*(const geometry_type_1 &lvalue, const geometry_type_2 &rvalue)
{
    return polygon_set_view<geometry_type_1, geometry_type_2, 1>(lvalue, rvalue);
}
...
operator^(const geometry_type_1 &lvalue, const geometry_type_2 &rvalue)
{
    return polygon_set_view<geometry_type_1, geometry_type_2, 2>(lvalue, rvalue);
}
...
operator-(const geometry_type_1 &lvalue, const geometry_type_2 &rvalue)
{
    return polygon_set_view<geometry_type_1, geometry_type_2, 3>(lvalue, rvalue);
}
...
struct view_of<polygon_45_set_concept, T>
{
    ...
    inline orientation_2d orient() const
    {
        return HORIZONTAL;
    }
...
struct get_iterator_type_2<view_of<polygon_45_set_concept, T>, polygon_45_set_concept>
{
    ...
    static type begin(const view_of<polygon_45_set_concept, T> &t)
    {
        return t.begin();
    }
    static type end(const view_of<polygon_45_set_concept, T> &t)
    {
        return t.end();
    }
};
