...
//
template <
    typename T, typename Dest, typename Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(typename Arity /* = ... (see substitute_fwd.hpp) */
                                                                                )>
struct substitute
{
    typedef T type;
...
#define BOOST_VARIANT_AUX_ENABLE_RECURSIVE_IMPL_SUBSTITUTE_TAG(CV_)               \
    ...
    struct substitute<                                                            \
        CV_ Source, Dest, Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(mpl::int_<-1>)> \
    {                                                                             \
...
#define BOOST_VARIANT_AUX_ENABLE_RECURSIVE_IMPL_HANDLE_POINTER(CV_)            \
    ...
    struct substitute<                                                         \
        T * CV_, Dest, Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(mpl::int_<-1>)> \
    {                                                                          \
        typedef typename substitute<                                           \
            T, Dest, Source>::type *CV_ type;                                  \
    };                                                                         \
    /**/
...
template <typename T, typename Dest, typename Source>
struct substitute<
    T &, Dest, Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(mpl::int_<-1>)>
{
    typedef typename substitute<
        T, Dest, Source>::type &type;
};
...
#define BOOST_VARIANT_AUX_SUBSTITUTE_TYPEDEF_IMPL(N)                \
    typedef typename substitute<                                    \
        BOOST_PP_CAT(U, N), Dest, Source>::type BOOST_PP_CAT(u, N); \
    /**/
...
//
template <
    template <BOOST_MPL_PP_PARAMS(i, typename P)> class T, BOOST_MPL_PP_PARAMS(i, typename U), typename Dest, typename Source>
struct substitute<
    T<BOOST_MPL_PP_PARAMS(i, U)>, Dest, Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(mpl::int_<(i)>)>
{
  private:
...
//
template <
    typename R, BOOST_MPL_PP_PARAMS(i, typename U), typename Dest, typename Source>
struct substitute<
    R (*)(BOOST_MPL_PP_PARAMS(i, U)), Dest, Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(mpl::int_<-1>)>
{
  private:
...
struct substitute<
    R (*)(void), Dest, Source BOOST_MPL_AUX_LAMBDA_ARITY_PARAM(mpl::int_<-1>)>
{
  private:
    typedef typename substitute<R, Dest, Source>::type r;
