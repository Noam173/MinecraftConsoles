...
class simple_seq_fit_impl
{
  ...
  private:
    ...
    std::pair<void *, bool> priv_allocation_command(boost::interprocess::allocation_type command, size_type min_size, size_type preferred_size, size_type &received_size, void *reuse_ptr, size_type sizeof_object);
...
simple_seq_fit_impl<MutexFamily, VoidPointer>::priv_first_block_offset(const void *this_ptr, size_type extra_hdr_bytes)
{
    ...
    size_type block1_off =
        ipcdetail::get_rounded_size(sizeof(simple_seq_fit_impl) + extra_hdr_bytes + this_disalignment, Alignment) - this_disalignment;
...
template <class MutexFamily, class VoidPointer>
inline typename simple_seq_fit_impl<MutexFamily, VoidPointer>::size_type
simple_seq_fit_impl<MutexFamily, VoidPointer>::priv_block_end_offset() const
{
    // First align "this" pointer
    ...
    size_type this_disalignment = (uint_this - uint_aligned_this);
    size_type old_end =
        ipcdetail::get_truncated_size(m_header.m_size + this_disalignment, Alignment) - this_disalignment;
    ...
    get_min_size(size_type extra_hdr_bytes)
{
    return ipcdetail::get_rounded_size((size_type)sizeof(simple_seq_fit_impl), Alignment) +
           ipcdetail::get_rounded_size(extra_hdr_bytes, Alignment) + MinBlockSize;
}
...
inline void simple_seq_fit_impl<MutexFamily, VoidPointer>::zero_free_memory()
{
    ...
    do
    {
        ...
        std::memset(priv_get_user_buffer(block), 0, block->get_user_bytes());
        block = ipcdetail::to_raw_pointer(block->m_next);
    ...
    priv_allocate(boost::interprocess::allocation_type command, size_type limit_size, size_type preferred_size, size_type &received_size, void *reuse_ptr)
{
    ...
    if (command & boost::interprocess::allocate_new)
    {
        ...
        if (biggest_block)
        {
            ...
            void *ret = this->priv_check_and_allocate(biggest_block->m_size, prev_biggest_block, biggest_block, received_size);
            BOOST_ASSERT(ret != 0);
            algo_impl_t::assert_alignment(ret);
    ...
    if (reuse_ptr && (command & (boost::interprocess::expand_fwd | boost::interprocess::expand_bwd)))
    {
        return_type ret(priv_expand_both_sides(command, limit_size, preferred_size, received_size, reuse_ptr, false), true);
...
template <class MutexFamily, class VoidPointer>
inline bool simple_seq_fit_impl<MutexFamily, VoidPointer>::priv_is_allocated_block(typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *block)
{
