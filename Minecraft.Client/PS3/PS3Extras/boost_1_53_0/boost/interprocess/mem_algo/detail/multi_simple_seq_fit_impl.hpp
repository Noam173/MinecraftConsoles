...
class simple_seq_fit_impl
{
  ...
  private:
    ...
    typedef typename boost::intrusive::
        pointer_traits<void_pointer>::template rebind_pointer<block_ctrl>::type block_ctrl_ptr;
    ...
    struct block_ctrl
    {
        ...
        {
            return reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(addr) - BlockCtrlBytes);
        }

        void *get_addr() const
        {
            return reinterpret_cast<block_ctrl *>(reinterpret_cast<const char *>(this) + BlockCtrlBytes);
        }
    };
  ...
  private:
    ...
    std::pair<void *, bool> priv_allocate(boost::interprocess::allocation_type command, size_type min_size, size_type preferred_size, size_type &received_size, void *reuse_ptr = 0);
    ...
    bool priv_expand(void *ptr, size_type min_size, size_type preferred_size, size_type &received_size);

    /*!Real expand to both sides implementation*/
    void *priv_expand_both_sides(boost::interprocess::allocation_type command, size_type min_size, size_type preferred_size, size_type &received_size, void *reuse_ptr, bool only_preferred_backwards);

    /*!Real shrink function implementation*/
    bool priv_shrink(void *ptr, size_type max_size, size_type preferred_size, size_type &received_size);
    ...
    void *priv_check_and_allocate(size_type units, block_ctrl *prev, block_ctrl *block, size_type &received_size);
    ...
    simple_seq_fit_impl(size_type size, size_type extra_hdr_bytes)
{
    ...
    m_header.m_root.m_next = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(this) + block1_off);
...
inline void simple_seq_fit_impl<MutexFamily, VoidPointer>::grow(size_type extra_size)
{
    ...
    block_ctrl *new_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(this) + old_end);
...
{
    return ipcdetail::get_rounded_size(sizeof(simple_seq_fit_impl) + extra_hdr_bytes, Alignment) + MinBlockSize;
}
...
inline void simple_seq_fit_impl<MutexFamily, VoidPointer>::clear_free_memory()
{
    ...
    do
    {
        ...
        std::memset(reinterpret_cast<char *>(block) + BlockCtrlBytes, 0, block->m_size * Alignment - BlockCtrlBytes);
    ...
    size(void *ptr) const
{
    ...
    block_ctrl *block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(ptr) - BlockCtrlBytes);
    ...
    multi_allocate(size_type nbytes)
{
    ...
    if (!addr)
    {
        ...
        boost::interprocess::multi_segment_services *p_services =
            static_cast<boost::interprocess::multi_segment_services *>(void_pointer::find_group_data(group));
...
void *simple_seq_fit_impl<MutexFamily, VoidPointer>::
    priv_expand_both_sides(boost::interprocess::allocation_type command, size_type min_size, size_type preferred_size, size_type &received_size, void *reuse_ptr, bool only_preferred_backwards)
{
    ...
    if (command & boost::interprocess::expand_bwd)
    {
        ...
        {
            needs_backwards =
                max_value(ipcdetail::get_rounded_size(min_size - extra_forward, Alignment), min_value(prev->get_user_bytes(), needs_backwards));
        }
        ...
        if ((prev->get_user_bytes()) >= needs_backwards)
        {
            ...
            {
                block_ctrl *new_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(reuse) - needs_backwards - BlockCtrlBytes);
                new_block->m_next = 0;
                new_block->m_size =
...
std::pair<void *, bool> simple_seq_fit_impl<MutexFamily, VoidPointer>::
    priv_allocate(boost::interprocess::allocation_type command, size_type limit_size, size_type preferred_size, size_type &received_size, void *reuse_ptr)
{
    ...
    {
        void *ret = priv_expand_both_sides(command, limit_size, preferred_size, received_size, reuse_ptr, true);
        if (ret)
        {
    ...
    if (command & boost::interprocess::allocate_new)
    {
        ...
        if (biggest_block)
        {
            ...
            void *ret = this->priv_check_and_allocate(nunits, prev_biggest_block, biggest_block, received_size);
            if (ret)
            {
    ...
    if (reuse_ptr && (command & (boost::interprocess::expand_fwd | boost::interprocess::expand_bwd)))
    {
        return return_type(priv_expand_both_sides(command, limit_size, preferred_size, received_size, reuse_ptr, false), true);
    }
    return return_type(0, false);
}
...
simple_seq_fit_impl<MutexFamily, VoidPointer>::
    priv_next_block_if_free(typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *ptr)
{
    // Take the address where the next block should go
    block_ctrl *next_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(ptr) + ptr->m_size * Alignment);
...
template <class MutexFamily, class VoidPointer>
inline std::pair<typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *, typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *>
simple_seq_fit_impl<MutexFamily, VoidPointer>::
    priv_prev_block_if_free(typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *ptr)
{
    ...
    while ((reinterpret_cast<char *>(prev_block) + prev_block->m_size * Alignment) != (reinterpret_cast<char *>(ptr)) && prev_block != root)
    {
...
template <class MutexFamily, class VoidPointer>
inline bool simple_seq_fit_impl<MutexFamily, VoidPointer>::
    priv_expand(void *ptr, size_type min_size, size_type preferred_size, size_type &received_size)
{
    // Obtain the real size of the block
    block_ctrl *block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(ptr) - BlockCtrlBytes);
...
template <class MutexFamily, class VoidPointer>
inline bool simple_seq_fit_impl<MutexFamily, VoidPointer>::
    priv_shrink(void *ptr, size_type max_size, size_type preferred_size, size_type &received_size)
{
    // Obtain the real size of the block
    block_ctrl *block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(ptr) - BlockCtrlBytes);
    ...
    block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(block) + block->m_size * Alignment);
    ...
    priv_allocate_aligned(size_type nbytes, size_type alignment)
{
    ...
    char *aligned_portion = reinterpret_cast<char *>(reinterpret_cast<size_type>(static_cast<char *>(buffer) + alignment - 1) & -alignment);

    char *pos = ((aligned_portion - reinterpret_cast<char *>(buffer)) >= (MinBlockSize * Alignment)) ? aligned_portion : (aligned_portion + alignment);

    block_ctrl *first = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(buffer) - BlockCtrlBytes);
...
template <class MutexFamily, class VoidPointer>
inline void *simple_seq_fit_impl<MutexFamily, VoidPointer>::priv_check_and_allocate(size_type nunits, typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *prev, typename simple_seq_fit_impl<MutexFamily, VoidPointer>::block_ctrl *block, size_type &received_size)
{
    ...
    if (block->m_size > upper_nunits)
    {
        ...
        block->m_size = nunits;
        block_ctrl *new_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(block) + Alignment * nunits);
    ...
    if (found)
    {
        ...
        block->m_next = 0;
        // Check alignment
        BOOST_ASSERT(((reinterpret_cast<char *>(block) - reinterpret_cast<char *>(this)) % Alignment) == 0);
...
void simple_seq_fit_impl<MutexFamily, VoidPointer>::priv_deallocate(void *addr)
{
    ...
    block_ctrl_ptr block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(addr) - BlockCtrlBytes);

    // All used blocks' next is marked with 0 so check it
    BOOST_ASSERT(block->m_next == 0);

    // Check if alignment and block size are right
    BOOST_ASSERT((reinterpret_cast<char *>(addr) - reinterpret_cast<char *>(this)) % Alignment == 0);
    ...
    if ((reinterpret_cast<char *>(ipcdetail::to_raw_pointer(block)) + Alignment * block->m_size) ==
    ...
    if ((reinterpret_cast<char *>(ipcdetail::to_raw_pointer(prev)) + Alignment * prev->m_size) ==
