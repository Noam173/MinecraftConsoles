...
class rbtree_best_fit
{
  ...
  private:
    ...
    typedef typename boost::intrusive::
        pointer_traits<VoidPointer>::template rebind_pointer<block_ctrl>::type block_ctrl_ptr;

    typedef typename boost::intrusive::
        pointer_traits<VoidPointer>::template rebind_pointer<char>::type char_ptr;
  ...
  private:
    typedef typename bi::make_set_base_hook<bi::void_pointer<VoidPointer>, bi::optimize_size<true>, bi::link_mode<bi::normal_link>>::type TreeHook;
          ...
          public TreeHook
    {
        ...
        {
            this->m_size = 0;
            this->m_allocated = 0, this->m_prev_allocated = 0;
        }
    ...
    typedef typename bi::make_multiset<block_ctrl, bi::base_hook<TreeHook>>::type Imultiset;
  ...
  private:
    ...
    std::pair<void *, bool> priv_allocate(boost::interprocess::allocation_type command, size_type limit_size, size_type preferred_size, size_type &received_size, void *reuse_ptr = 0, size_type backwards_multiple = 1);
    ...
    bool priv_expand(void *ptr, const size_type min_size, const size_type preferred_size, size_type &received_size);

    //! Real expand to both sides implementation
    void *priv_expand_both_sides(boost::interprocess::allocation_type command, size_type min_size, size_type preferred_size, size_type &received_size, void *reuse_ptr, bool only_preferred_backwards, size_type backwards_multiple);
    ...
    void *priv_check_and_allocate(size_type units, block_ctrl *block, size_type &received_size);
...
rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_first_block_offset_from_this(const void *this_ptr, size_type extra_hdr_bytes)
{
    ...
    priv_add_segment(void *addr, size_type segment_size)
{
    ...
    block_ctrl *end_block = static_cast<block_ctrl *>(new (reinterpret_cast<char *>(addr) + first_big_block->m_size * Alignment) SizeHolder);
    ...
    BOOST_ASSERT(static_cast<void *>(static_cast<SizeHolder *>(first_big_block)) < static_cast<void *>(static_cast<TreeHook *>(first_big_block)));
...
rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_end_block()
{
    ...
    block_ctrl *end_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(this) + block1_off + original_first_block_size);
    return end_block;
}
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::grow(size_type extra_size)
{
    ...
    block_ctrl *new_end_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(old_end_block) + align_offset * Alignment);
    ...
    get_min_size(size_type extra_hdr_bytes)
{
    return (algo_impl_t::ceil_units(sizeof(rbtree_best_fit)) +
            ...
            MinBlockUnits + EndCtrlBlockUnits) *
           Alignment;
}
    ...
    all_memory_deallocated()
{
    ...
    return m_header.m_allocated == 0 &&
           ...
           (++m_header.m_imultiset.begin()) == m_header.m_imultiset.end() && m_header.m_imultiset.begin()->m_size == (m_header.m_size - block1_off - EndCtrlBlockBytes) / Alignment;
}
...
inline std::pair<T *, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
    allocation_command(boost::interprocess::allocation_type command, size_type limit_size,
                       ...
                       T *reuse_ptr)
{
    std::pair<void *, bool> ret = priv_allocation_command(command, limit_size, preferred_size, received_size, static_cast<void *>(reuse_ptr), sizeof(T));
                           ...
                           void *reuse_ptr, size_type sizeof_object)
{
    ...
    {
        bool success = algo_impl_t::try_shrink(this, reuse_ptr, limit_objects * sizeof_object, preferred_objects * sizeof_object, received_objects);
        received_objects /= sizeof_object;
        return std::pair<void *, bool>((success ? reuse_ptr : 0), true);
    }
    return priv_allocation_command(command, limit_objects, preferred_objects, received_objects, reuse_ptr, sizeof_object);
}
...
template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
void *rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
    priv_expand_both_sides(boost::interprocess::allocation_type command, size_type min_size, size_type preferred_size, size_type &received_size, void *reuse_ptr, bool only_preferred_backwards, size_type backwards_multiple)
{
    ...
    if (command & boost::interprocess::expand_bwd)
    {
        ...
        size_type lcm;
        if (!algo_impl_t::calculate_lcm_and_needs_backwards_lcmed(backwards_multiple, received_size, only_preferred_backwards ? preferred_size : min_size, lcm, needs_backwards_aligned))
        {
            return 0;
        }
        ...
        if (size_type(prev_block->m_size * Alignment) >= needs_backwards_aligned)
        {
            ...
            {
                block_ctrl *new_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(reuse) - needs_backwards_aligned);
...
std::pair<void *, bool> rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
    priv_allocate(boost::interprocess::allocation_type command, size_type limit_size, size_type preferred_size, size_type &received_size, void *reuse_ptr, size_type backwards_multiple)
{
    ...
    {
        void *ret = priv_expand_both_sides(command, limit_size, preferred_size, received_size, reuse_ptr, true, backwards_multiple);
        if (ret)
        {
    ...
    if (command & boost::interprocess::allocate_new)
    {
        ...
        {
            return return_type(this->priv_check_and_allocate(preferred_units, ipcdetail::to_raw_pointer(&*it), received_size), false);
        }

        if (it != m_header.m_imultiset.begin() &&
            (--it)->m_size >= limit_units)
        {
            return return_type(this->priv_check_and_allocate(it->m_size, ipcdetail::to_raw_pointer(&*it), received_size), false);
        }
    }
    ...
    if (reuse_ptr && (command & (boost::interprocess::expand_fwd | boost::interprocess::expand_bwd)))
    {
        return return_type(priv_expand_both_sides(command, limit_size, preferred_size, received_size, reuse_ptr, false, backwards_multiple), true);
    }
...
{
    return const_cast<block_ctrl *>(reinterpret_cast<const block_ctrl *>(reinterpret_cast<const char *>(ptr) - AllocatedCtrlBytes));
}
    ...
    priv_get_total_units(size_type userbytes)
{
    ...
    if (units < BlockCtrlUnits)
    {
        units = BlockCtrlUnits;
    }
    return units;
}
...
template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
bool rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::
    priv_expand(void *ptr, const size_type min_size, const size_type preferred_size, size_type &received_size)
{
    // Obtain the real size of the block
    ...
    size_type intended_user_units = (merged_user_units < preferred_user_units) ? merged_user_units : preferred_user_units;
    ...
    if ((merged_units - intended_units) >= BlockCtrlUnits)
    {
        ...
        block_ctrl *rem_block = new (reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(block) + intended_units * Alignment)) block_ctrl;
...
inline
    typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *
    rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_prev_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *ptr)
{
    BOOST_ASSERT(!ptr->m_prev_allocated);
    return reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(ptr) - ptr->m_prev_size * Alignment);
}

template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline
    typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *
    rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_end_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *first_segment_block)
{
    ...
    block_ctrl *end_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(first_segment_block) + first_segment_block->m_prev_size * Alignment);
    (void)end_block;
...
inline
    typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *
    rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_first_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *end_segment_block)
{
    // The first block's logic is different from the rest of blocks: stores in m_prev_size the absolute
    // distance with the end block
    BOOST_ASSERT(end_segment_block->m_allocated);
    block_ctrl *first_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(end_segment_block) - end_segment_block->m_size * Alignment);
    (void)first_block;
...
inline
    typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *
    rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_next_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *ptr)
{
    return reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(ptr) + ptr->m_size * Alignment);
}

template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline bool rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_is_allocated_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block)
{
    bool allocated = block->m_allocated != 0;
#ifndef NDEBUG
    if (block != priv_end_block())
    {
        block_ctrl *next_block = reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(block) + block->m_size * Alignment);
...
template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline bool rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_is_prev_allocated(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block)
{
...
template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_mark_as_allocated_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block)
{
    block->m_allocated = 1;
    reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(block) + block->m_size * Alignment)->m_prev_allocated = 1;
}

template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_mark_as_free_block(typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block)
{
    block->m_allocated = 0;
...
template <class MutexFamily, class VoidPointer, std::size_t MemAlignment>
inline void *rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_check_and_allocate(size_type nunits, typename rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::block_ctrl *block, size_type &received_size)
{
    ...
    if (block->m_size >= upper_nunits)
    {
        ...
        // This is the remaining block
        block_ctrl *rem_block = new (reinterpret_cast<block_ctrl *>(reinterpret_cast<char *>(block) + Alignment * nunits)) block_ctrl;
        algo_impl_t::assert_alignment(rem_block);
        ...
        if (it_old == m_header.m_imultiset.begin() || (--imultiset_iterator(it_old))->m_size < rem_block->m_size)
        {
...
void rbtree_best_fit<MutexFamily, VoidPointer, MemAlignment>::priv_deallocate(void *addr)
{
    ...
    if (merge_with_prev || merge_with_next)
    {
        ...
        imultiset_iterator free_block_to_check_it(Imultiset::s_iterator_to(only_merge_next ? *next_block : *block_to_insert));
