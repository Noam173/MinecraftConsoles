...
class basic_managed_xsi_shared_memory
    : public ipcdetail::basic_managed_memory_impl<CharType, AllocationAlgorithm, IndexType, ipcdetail::xsishmem_open_or_create<AllocationAlgorithm>::type::ManagedOpenOrCreateUserOffset>,
      private ipcdetail::xsishmem_open_or_create<AllocationAlgorithm>::type
{
  ...
  public: // functions
    ...
    basic_managed_xsi_shared_memory(create_only_t, const xsi_key &key,
        ...
        : base_t(), base2_t(create_only, key, size, read_write, addr,
    ...
    basic_managed_xsi_shared_memory(open_or_create_t,
        ...
        : base_t(), base2_t(open_or_create, key, size, read_write, addr,
    ...
    basic_managed_xsi_shared_memory(open_read_only_t, const xsi_key &key,
                                    const void *addr = 0)
        : base_t(), base2_t(open_only, key, read_only, addr,
                            create_open_func_t(get_this_pointer(),
                                               ipcdetail::DoOpen))
    {
    }
    ...
    basic_managed_xsi_shared_memory(open_only_t, const xsi_key &key,
                                    const void *addr = 0)
        : base_t(), base2_t(open_only, key, read_write, addr,
                            create_open_func_t(get_this_pointer(),
                                               ipcdetail::DoOpen))
    {
    }
