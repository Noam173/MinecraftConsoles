...
class message_queue_t
{
  ...
  public:
    ...
    typedef typename boost::intrusive::
        pointer_traits<void_pointer>::template rebind_pointer<char>::type char_ptr;
...
{
    typedef typename boost::intrusive::
        pointer_traits<VoidPointer>::template rebind_pointer<msg_hdr_t<VoidPointer>>::type msg_hdr_ptr_t;
    ...
    : public ipcdetail::priority_functor<VoidPointer>
{
    ...
    typedef typename boost::intrusive::
        pointer_traits<void_pointer>::template rebind_pointer<msg_header>::type msg_hdr_ptr_t;
    typedef typename boost::intrusive::pointer_traits<msg_hdr_ptr_t>::difference_type difference_type;
    typedef typename boost::make_unsigned<difference_type>::type size_type;
    typedef typename boost::intrusive::
        pointer_traits<void_pointer>::template rebind_pointer<msg_hdr_ptr_t>::type msg_hdr_ptr_ptr_t;
    typedef ipcdetail::managed_open_or_create_impl<shared_memory_object, 0, true, false> open_create_impl_t;
  ...
  public:
    ...
    msg_header &insert_at(iterator where)
    {
        ...
        else
        {
            ...
            if (circ_pos < m_cur_num_msg / 2)
            {
                ...
                if (!unique_segment)
                {
                    std::copy(&mp_index[0] + second_segment_beg, &mp_index[0] + second_segment_end, &mp_index[0] + second_segment_beg - 1);
                    mp_index[m_max_num_msg - 1] = mp_index[0];
                }
                std::copy(&mp_index[0] + first_segment_beg, &mp_index[0] + first_segment_end, &mp_index[0] + first_segment_beg - 1);
                *where = backup;
                m_cur_first_msg = m_cur_first_msg ? m_cur_first_msg : m_max_num_msg;
                --m_cur_first_msg;
                ++m_cur_num_msg;
            ...
            else
            {
                ...
                if (!unique_segment)
                {
                    std::copy_backward(&mp_index[0] + second_segment_beg, &mp_index[0] + second_segment_end, &mp_index[0] + second_segment_end + 1);
                    mp_index[0] = mp_index[m_max_num_msg - 1];
                }
                std::copy_backward(&mp_index[0] + first_segment_beg, &mp_index[0] + first_segment_end, &mp_index[0] + first_segment_end + 1);
                *where = backup;
                ++m_cur_num_msg;
                return **where;
            }
    ...
    void initialize_memory()
    {
        ...
        msg_hdr_ptr_t *index = reinterpret_cast<msg_hdr_ptr_t *>(reinterpret_cast<char *>(this) + r_hdr_size);

        // Pointer to the first message header
        msg_header *msg_hdr = reinterpret_cast<msg_header *>(reinterpret_cast<char *>(this) + r_hdr_size + r_index_size);
        ...
        {
            index[i] = msg_hdr;
            msg_hdr = reinterpret_cast<msg_header *>(reinterpret_cast<char *>(msg_hdr) + r_max_msg_size);
        }
    }
...
class msg_queue_initialization_func_t
{
  public:
    ...
    {
        return mq_hdr_t<VoidPointer>::get_mem_size(m_maxmsgsize, m_maxmsg) - message_queue_t<VoidPointer>::open_create_impl_t::ManagedOpenOrCreateUserOffset;
    }
...
template <class VoidPointer>
inline typename message_queue_t<VoidPointer>::size_type message_queue_t<VoidPointer>::get_mem_size(size_type max_msg_size, size_type max_num_msg)
{
...
template <class VoidPointer>
inline void message_queue_t<VoidPointer>::send(const void *buffer, size_type buffer_size, unsigned int priority)
{
    this->do_send(blocking, buffer, buffer_size, priority, ptime());
}

template <class VoidPointer>
inline bool message_queue_t<VoidPointer>::try_send(const void *buffer, size_type buffer_size, unsigned int priority)
{
    return this->do_send(non_blocking, buffer, buffer_size, priority, ptime());
}

template <class VoidPointer>
inline bool message_queue_t<VoidPointer>::timed_send(const void *buffer, size_type buffer_size, unsigned int priority, const boost::posix_time::ptime &abs_time)
{
