...
struct intersegment_base
{
    ...
    static const std::size_t pow_size_bits =
        (max_segment_size_bits == (std::size_t(1) << pow_size_bits_helper)) ? pow_size_bits_helper : pow_size_bits_helper + 1;
    ...
    static std::size_t calculate_size(std::size_t orig_size, std::size_t &pow, std::size_t &frc)
    {
        ...
        BOOST_ASSERT(pow >= frc_size_bits);
        std::size_t rounded = ipcdetail::get_rounded_size_po2(diff, (std::size_t)(1u << (pow - frc_size_bits)));
        if (rounded == low_size)
        {
            ++pow;
            frc = 0;
    ...
    : public intersegment_base
{
  ...
  private:
    ...
    typedef boost::interprocess::flat_map<const void *, segment_info_t, std::less<const void *>> ptr_to_segment_info_t;
  ...
  public:
    ...
    static void get_segment_info_and_offset(const void *ptr, segment_info_t &segment, std::size_t &offset, void *&base)
    {
        ...
        typename ptr_to_segment_info_t::iterator it = s_map.m_ptr_to_segment_info.upper_bound(ptr);
...
class intersegment_ptr : public flat_map_intersegment<interprocess_mutex>
{
  ...
  public: // Public Functions
    ...
    {
        base_t::inc_offset(offset * sizeof(T));
        return *this;
    }
    ...
    intersegment_ptr &operator-=(std::ptrdiff_t offset)
    {
        base_t::dec_offset(offset * sizeof(T));
        return *this;
    }
...
inline bool operator==(const intersegment_ptr<T1> &left,
...
template <class T1, class T2>
inline bool operator!=(const intersegment_ptr<T1> &pt1,
...
inline std::basic_ostream<E, T> &operator<<(std::basic_ostream<E, T> &os, const intersegment_ptr<U> &p)
{
    return os << p.get();
}
...
//! operator>>
template <class E, class T, class U>
inline std::basic_istream<E, T> &operator>>(std::basic_istream<E, T> &os, intersegment_ptr<U> &p)
{
    U *tmp;
    return os >> tmp;
    p = tmp;
...
template <class T>
inline intersegment_ptr<T> operator+(std::ptrdiff_t diff, const intersegment_ptr<T> &right)
{
    return right + diff;
}
...
inline std::ptrdiff_t operator-(const intersegment_ptr<T> &pt,
                                const intersegment_ptr<T2> &pt2)
{
...
//! swap specialization
template <class T>
inline void swap(boost::interprocess::intersegment_ptr<T> &pt,
...
//! Never throws.
template <class T>
inline T *to_raw_pointer(boost::interprocess::intersegment_ptr<T> const &p)
{
    return p.get();
}
...
inline boost::interprocess::intersegment_ptr<T> static_pointer_cast(const boost::interprocess::intersegment_ptr<U> &r)
{
...
//! Never throws.
template <class T, class U>
inline boost::interprocess::intersegment_ptr<T> const_pointer_cast(const boost::interprocess::intersegment_ptr<U> &r)
{
...
//! Never throws.
template <class T, class U>
inline boost::interprocess::intersegment_ptr<T> dynamic_pointer_cast(const boost::interprocess::intersegment_ptr<U> &r)
{
...
//! Never throws.
template <class T, class U>
inline boost::interprocess::intersegment_ptr<T> reinterpret_pointer_cast(const boost::interprocess::intersegment_ptr<U> &r)
{
...
template <class T>
struct is_multisegment_ptr<boost::interprocess::intersegment_ptr<T>>
{
...
//! for optimizations
template <class T>
struct has_trivial_constructor<boost::interprocess::intersegment_ptr<T>>
    : public true_type
{
};
...
//! for optimizations
template <class T>
struct has_trivial_destructor<boost::interprocess::intersegment_ptr<T>>
    : public true_type
{
};
