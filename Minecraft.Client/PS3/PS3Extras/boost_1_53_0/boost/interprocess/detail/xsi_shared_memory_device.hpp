...
class xsi_shared_memory_device
{
  ...
  private:
    ...
    bool priv_open_or_create_name_only(ipcdetail::create_enum_t type, const char *shmname, mode_t mode, std::size_t size);
    bool priv_open_or_create_name_id(ipcdetail::create_enum_t type, const char *shmname, boost::uint8_t id, mode_t mode, std::size_t size);
...
inline void xsi_shared_memory_device::priv_obtain_index(mapped_region &reg, xsi_named_mutex &mut, std::string &path)
{
...
inline bool xsi_shared_memory_device::priv_remove_dead_memory(xsi_shared_memory_device::info_t *info, const char *path)
{
    bool removed = false;
...
inline bool xsi_shared_memory_device::priv_open_or_create_name_id(ipcdetail::create_enum_t type, const char *filepath, mode_t mode, std::size_t size)
{
    // Set accesses
    ...
    { // if(type == ipcdetail::DoOpenOrCreate){
...
inline bool xsi_shared_memory_device::priv_open_or_create_name_only(ipcdetail::create_enum_t type, const char *shmname, mode_t mode, std::size_t size)
{
    // Set accesses
    if (mode != read_write && mode != read_only)
    {
...

    {
        ...
        if (type == ipcdetail::DoOpen)
        {
            ...
            xsi_shared_memory temp(open_only, xsi_shm_emulation_file_path.c_str(), target_entry + info_constants_t<0>::FirstID, perm);
            m_shm = boost::move(temp);
        }
        else
        {
            ...
            {
                // Try to reuse slot
                xsi_shared_memory temp(create_only, xsi_shm_emulation_file_path.c_str(), target_entry + info_constants_t<0>::FirstID, size, perm);
                std::strcpy(info->names[target_entry].buf, shmname);
                m_shm = boost::move(temp);
            }
            else
            { // if(type == ipcdetail::DoOpenOrCreate){
                xsi_shared_memory temp(open_or_create, xsi_shm_emulation_file_path.c_str(), target_entry + info_constants_t<0>::FirstID, size, perm);
                if (!found)
                {
...
inline bool xsi_shared_memory_device::remove(const char *shmname)
{
    try
    {
        ...
        for (std::size_t i = 0; i != info_constants_t<0>::NumID; ++i)
        {
            ...
            {
                xsi_shared_memory temp(open_only, xsi_shm_emulation_file_path.c_str(), i + info_constants_t<0>::FirstID);
