...
{
    intermodule_singleton_helpers::create_tmp_subdir_and_get_pid_based_filepath(robust_lock_subdir_path(), robust_lock_prefix(), pid, s);
}
    ...
    class other_process_lock_remover
    {
      public:
        void operator()(const char *filepath, const char *filename)
        {
            ...
            if (!intermodule_singleton_helpers::check_if_filename_complies_with_pid(filename, robust_lock_prefix(), get_current_process_id(), pid_str))
            {
...
template <class Mutex>
inline bool robust_spin_mutex<Mutex>::timed_lock(const boost::posix_time::ptime &abs_time)
{
...
inline bool robust_spin_mutex<Mutex>::lock_own_unique_file()
{
    ...
    robust_emulation_helpers::robust_mutex_lock_file *dummy =
        &ipcdetail::intermodule_singleton<robust_emulation_helpers::robust_mutex_lock_file>::get();
    return dummy != 0;
}
