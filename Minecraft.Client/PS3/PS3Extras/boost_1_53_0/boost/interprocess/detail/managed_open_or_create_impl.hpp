    ...
    : public managed_open_or_create_impl_device_holder<StoreDevice, DeviceAbstraction>
{
  ...
  public:
    static const std::size_t
        ManagedOpenOrCreateUserOffset =
            ct_rounded_size < sizeof(boost::uint32_t),
        MemAlignment ? (MemAlignment) : (::boost::alignment_of<::boost::detail::max_align>::value) > ::value;
    ...
    managed_open_or_create_impl(open_or_create_t,
                                ...
                                const permissions &perm)
    {
        priv_open_or_create(DoOpenOrCreate, id, size, mode, addr, perm, null_mapped_region_function());
    }
    ...
    managed_open_or_create_impl(create_only_t,
                                ...
                                const permissions &perm)
    {
        priv_open_or_create(DoCreate, id, size, mode, addr, perm, construct_func);
    }
    ...
    managed_open_or_create_impl(open_only_t,
                                ...
                                const ConstructFunc &construct_func)
    {
        priv_open_or_create(DoOpen, id, 0, mode, addr, permissions(), construct_func);
    }
    ...
    managed_open_or_create_impl(open_or_create_t,
                                ...
                                const permissions &perm)
    {
        priv_open_or_create(DoOpenOrCreate, id, size, mode, addr, perm, construct_func);
    }
  ...
  private:
    ...
    template <class ConstructFunc>
    inline void priv_open_or_create(create_enum_t type,
                                    const device_id_t &id,
                                    std::size_t size,
                                    ...
                                    ConstructFunc construct_func)
    {
        ...
        else
        {
            ...
            construct_func(static_cast<char *>(region.get_address()) + ManagedOpenOrCreateUserOffset, region.get_size() - ManagedOpenOrCreateUserOffset, false);
            // All ok, just move resources to the external mapped region
            m_mapped_region.swap(region);
        }
        if (StoreDevice)
