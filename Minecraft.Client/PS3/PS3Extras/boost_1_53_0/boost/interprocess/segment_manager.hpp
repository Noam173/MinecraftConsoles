    ...
    : private MemoryAlgorithm
{
  public:
    ...
    allocation_command(boost::interprocess::allocation_type command, size_type limit_size,
                       ...
                       T *reuse_ptr = 0)
    {
        std::pair<T *, bool> ret = MemoryAlgorithm::allocation_command(command | boost::interprocess::nothrow_allocation, limit_size, preferred_size, received_size, reuse_ptr);
    ...
    {
        std::pair<void *, bool> ret = MemoryAlgorithm::raw_allocation_command(command | boost::interprocess::nothrow_allocation, limit_objects, preferred_objects, received_objects, reuse_ptr, sizeof_object);
        if (!(command & boost::interprocess::nothrow_allocation) && !ret.first)
        {
            throw bad_alloc();
        }
  ...
  protected:
    void *prot_anonymous_construct(size_type num, bool dothrow, ipcdetail::in_place_interface &table)
    {
        ...
        block_header_t block_info(size_type(table.size * num), size_type(table.alignment), anonymous_type, 1, 0);
    ...
    : public segment_manager_base<MemoryAlgorithm>
{
  ...
  public:
    ...
    explicit segment_manager(size_type segment_size)
        : Base(segment_size, priv_get_reserved_bytes()), m_header(static_cast<Base *>(get_this_pointer()))
    {
    ...
    bool destroy(const ipcdetail::unique_instance_t *)
    {
        ...
        return this->priv_generic_named_destroy<char>(typeid(T).name(), m_header.m_unique_index, dtor, is_intrusive_t());
    }
    ...
    {
        ipcdetail::placement_destroy<T> dtor;
        return this->priv_generic_named_destroy<CharType>(name, m_header.m_named_index, dtor, is_intrusive_t());
    }
    ...
    {
        return make_transform_iterator(m_header.m_named_index.begin(), named_transform());
    }
    ...
    {
        return make_transform_iterator(m_header.m_named_index.end(), named_transform());
    }
    ...
    {
        return make_transform_iterator(m_header.m_unique_index.begin(), unique_transform());
    }
    ...
    {
        return make_transform_iterator(m_header.m_unique_index.end(), unique_transform());
    }
    ...
    {
        return static_cast<T *>(priv_generic_construct(name, num, try2find, dothrow, table));
    }
  ...
  private:
                                 ...
                                 ipcdetail::in_place_interface &table)
    {
        ...
        {
            ret = this->priv_generic_named_construct<char>(unique_type, table.type_name, num, try2find, dothrow, table, m_header.m_unique_index, is_intrusive_t());
        }
        else
        {
            ret = this->priv_generic_named_construct<CharType>(named_type, name, num, try2find, dothrow, table, m_header.m_named_index, is_intrusive_t());
        }
        return ret;
    }
    ...
    void priv_destroy_ptr(const void *ptr, ipcdetail::in_place_interface &dtor)
    {
        ...
        case named_type:
            this->priv_generic_named_destroy<CharType>(ctrl_data, m_header.m_named_index, dtor, is_node_index_t());
            break;

        case unique_type:
            this->priv_generic_named_destroy<char>(ctrl_data, m_header.m_unique_index, dtor, is_node_index_t());
            break;
    ...
    void *priv_generic_find(const CharT *name,
                            ...
                            bool use_lock)
    {
        ...
        //-------------------------------
        // Find name in index
        ipcdetail::intrusive_compare_key<CharT> key(name, std::char_traits<CharT>::length(name));
                            ...
                            bool use_lock)
    {
        ...
        {
            // Get header
            block_header_t *ctrl_data = reinterpret_cast<block_header_t *>(ipcdetail::to_raw_pointer(it->second.m_ptr));
    ...
    template <class CharT>
    bool priv_generic_named_destroy_impl(const typename IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm>>::iterator &it,
                                         IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm>> &index,
                                         ipcdetail::in_place_interface &table)
    {
        typedef IndexType<ipcdetail::index_config<CharT, MemoryAlgorithm>> index_type;
        ...
        {
            index_it *ihdr = block_header_t::template to_first_header<index_it>(ctrl_data);
            ihdr->~index_it();
            memory = ihdr;
        }
        else
                                       ...
                                       ipcdetail::true_ is_intrusive)
    {
        ...
        block_header_t block_info(size_type(table.size * num), size_type(table.alignment), type, sizeof(CharT), namelen);
        ...
        if (dothrow)
        {
            buffer_ptr = this->allocate(block_info.template total_size_with_header<intrusive_value_type>());
        }
        else
        {
            buffer_ptr = this->allocate(block_info.template total_size_with_header<intrusive_value_type>(), std::nothrow_t());
            if (!buffer_ptr)
            {
                return 0;
            }
        ...
        // Build scoped ptr to avoid leaks with constructor exception
        ipcdetail::mem_algo_deallocator<segment_manager_base_type> mem(buffer_ptr, *static_cast<segment_manager_base_type *>(this));
  ...
  private:
        ...
        : public rmutex
    {
        ...
        header_t(Base *restricted_segment_mngr)
            : m_named_index(restricted_segment_mngr), m_unique_index(restricted_segment_mngr)
        {
        }
    } m_header;
