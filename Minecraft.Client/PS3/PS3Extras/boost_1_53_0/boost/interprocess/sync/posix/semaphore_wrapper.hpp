...
inline bool semaphore_open(sem_t *&handle, create_enum_t type, const char *origname,
    ...
    case DoCreate:
        {
            while (1)
            {
                ...
                {
                    oflag = 0;
                    if ((handle = ::sem_open(name.c_str(), oflag)) != BOOST_INTERPROCESS_POSIX_SEM_FAILED || (errno != ENOENT))
                    {
                        break;
                    }
                }
