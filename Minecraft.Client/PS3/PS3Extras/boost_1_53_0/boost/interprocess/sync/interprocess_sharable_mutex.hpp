...
class interprocess_sharable_mutex
{
  ...
  private:
    ...
    {
        static const unsigned max_readers = ~(unsigned(1) << (sizeof(unsigned) * CHAR_BIT - 1));
    };
...
inline bool interprocess_sharable_mutex::try_lock()
{
    ...
    if (!lck.owns() || this->m_ctrl.exclusive_in || this->m_ctrl.num_shared)
    {
        return false;
    }
    this->m_ctrl.exclusive_in = 1;
...
inline bool interprocess_sharable_mutex::timed_lock(const boost::posix_time::ptime &abs_time)
{
...
inline void interprocess_sharable_mutex::lock_sharable()
{
    ...
    while (this->m_ctrl.exclusive_in || this->m_ctrl.num_shared == constants::max_readers)
    {
        this->m_first_gate.wait(lck);
    }
...
inline bool interprocess_sharable_mutex::try_lock_sharable()
{
    ...
    // if an exclusive lock has been acquired
    // or there are too many sharable locks
    if (!lck.owns() || this->m_ctrl.exclusive_in || this->m_ctrl.num_shared == constants::max_readers)
    {
        return false;
    }
...
inline bool interprocess_sharable_mutex::timed_lock_sharable(const boost::posix_time::ptime &abs_time)
{
    if (abs_time == boost::posix_time::pos_infin)
    {
        this->lock_sharable();
    ...
    while (this->m_ctrl.exclusive_in || this->m_ctrl.num_shared == constants::max_readers)
    {
        if (!this->m_first_gate.timed_wait(lck, abs_time))
        {
            if (this->m_ctrl.exclusive_in || this->m_ctrl.num_shared == constants::max_readers)
            {
                return false;
            }
            break;
