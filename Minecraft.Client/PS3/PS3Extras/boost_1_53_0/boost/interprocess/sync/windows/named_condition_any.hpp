    ...
    class named_cond_callbacks : public windows_named_sync_interface
    {
      ...
      public:
        ...
        virtual bool open(create_enum_t, const char *id_name)
        {
            ...
            bool created;
            if (!sem_block_queue.open_or_create(aux_str.c_str(), sem_counts[0], winapi_semaphore_wrapper::MaxCount, perm, created))
            {
                return false;
            }
            aux_str.erase(pos);
            ...
            if (!sem_block_lock.open_or_create(aux_str.c_str(), sem_counts[1], winapi_semaphore_wrapper::MaxCount, perm, created))
            {
                return false;
            }
            aux_str.erase(pos);
