    ...
    : private Deleter
{
  ...
  public:
    ...
    {
        ipcdetail::do_swap<Deleter>(*this, b);
        ipcdetail::do_swap(m_ptr, b.m_ptr);
    }
...
inline void swap(scoped_ptr<T, D> &a, scoped_ptr<T, D> &b)
{
    a.swap(b);
}
...
template <class T, class D>
inline
    typename scoped_ptr<T, D>::pointer
    to_raw_pointer(scoped_ptr<T, D> const &p)
{
    return p.get();
}
...
template <class T, class D>
inline T *to_raw_pointer(boost::interprocess::scoped_ptr<T, D> const &p)
{
    return p.get();
}
#endif
