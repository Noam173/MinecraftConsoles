...
class intrusive_ptr
{
  public:
    ...
    typedef typename boost::intrusive::
        pointer_traits<VoidPointer>::template rebind_pointer<T>::type pointer;
  ...
  public:
    ...
    intrusive_ptr(const pointer &p, bool add_ref = true) : m_ptr(p)
    {
        if (m_ptr != 0 && add_ref)
        {
            intrusive_ptr_add_ref(ipcdetail::to_raw_pointer(m_ptr));
        }
    }
    ...
    ~intrusive_ptr()
    {
        if (m_ptr != 0)
        {
            intrusive_ptr_release(ipcdetail::to_raw_pointer(m_ptr));
        }
    }
...
inline bool operator==(intrusive_ptr<T, VP> const &a,
...
//! Does not throw
template <class T, class VP>
inline bool operator==(const typename intrusive_ptr<T, VP>::pointer &a,
...
inline bool operator<(intrusive_ptr<T, VP> const &a,
                      intrusive_ptr<T, VP> const &b)
{
    return std::less<typename intrusive_ptr<T, VP>::pointer>()(a.get(), b.get());
}

//! Exchanges the contents of the two intrusive_ptrs.
//! Does not throw
template <class T, class VP>
inline void swap(intrusive_ptr<T, VP> &lhs,
...
inline std::basic_ostream<E, T> &operator<<(std::basic_ostream<E, T> &os, intrusive_ptr<Y, VP> const &p)
{
    os << p.get();
    return os;
}
