...
struct block
{
    ...
    SizeType operator()(SizeType i) const
    {
        ...
        {
            return cutoff_processor + (i - cutoff) / (n / p);
        }
    }
    ...
    std::size_t start(ID id) const
    {
        ...
        {
            return estimate - (id - cutoff_processor);
        }
    }
...
struct oned_block_cyclic
{
    ...
    SizeType block_size(ProcessID id, SizeType n) const
    {
        ...
        SizeType my_elements = my_blocks * size + (p == extra_blocks ? extra_elements : 0);
        return my_elements;
    }
...
struct twod_block_cyclic
{
    ...
    SizeType block_size(ProcessID id, SizeType n) const
    {
        ...
        {
            --n;
            if ((*this)(n) == id && (int)local(n) > result)
            {
                result = local(n);
            }
        }
    ...
    SizeType local(SizeType i) const
    {
        ...
        std::size_t block_offset = (data_row % block_rows) * block_columns + (data_col % block_columns);
    ...
    SizeType global(SizeType i) const
    {
        ...
        SizeType result = block_row * block_rows + block_col * block_columns + row_in_block * block_rows + col_in_block;
