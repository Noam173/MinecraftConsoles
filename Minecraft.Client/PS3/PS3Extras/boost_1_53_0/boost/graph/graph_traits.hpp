...
#define BOOST_GRAPH_MEMBER_OR_VOID(name)                                               \
    ...
    template <typename T>                                                              \
    struct BOOST_JOIN(get_opt_member_, name) : boost::mpl::eval_if_c<                  \
...
struct has_vertex_property
    : mpl::not_<
          typename detail::is_no_property<typename vertex_property_type<Graph>::type>>::type
{
};
...
struct has_edge_property
    : mpl::not_<
          typename detail::is_no_property<typename edge_property_type<Graph>::type>>::type
{
};
