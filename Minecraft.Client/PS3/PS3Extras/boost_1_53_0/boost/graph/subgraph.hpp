...
class subgraph
{
  ...
  public:
    ...
    subgraph(const subgraph &x)
        : m_parent(x.m_parent), m_edge_counter(x.m_edge_counter), m_global_vertex(x.m_global_vertex), m_global_edge(x.m_global_edge)
    {
        if (x.is_root())
        {
            m_graph = x.m_graph;
    ...
    typedef indirect_iterator<
        typename ChildrenList::const_iterator, subgraph<Graph>, std::bidirectional_iterator_tag>
        children_iterator;

    typedef indirect_iterator<
        typename ChildrenList::const_iterator, subgraph<Graph> const, std::bidirectional_iterator_tag>
...
get(global_property<Property> p, subgraph<G> &g)
{
    typedef typename property_map<
        subgraph<G>, global_property<Property>>::type Map;
...
{
    typedef typename property_map<
        subgraph<G>, global_property<Property>>::const_type Map;
    return Map(&g, p.value);
}
...
{
    typedef typename property_map<
        subgraph<G>, local_property<Property>>::type Map;
    return Map(&g, p.value);
}
...
{
    typedef typename property_map<
        subgraph<G>, local_property<Property>>::const_type Map;
    return Map(&g, p.value);
}
