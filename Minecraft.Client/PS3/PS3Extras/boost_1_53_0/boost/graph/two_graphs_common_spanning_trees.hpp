...
struct bridges_visitor : public default_dfs_visitor
{
    bridges_visitor(
        ...
        DistMap dist,
        LowMap low,
        Buffer &buffer) : mTree(tree), mPred(pred), mDist(dist), mLow(low), mBuffer(buffer)
    {
        mNum = -1;
    }
    ...
    Seq inL)
{
    ...
    else
    {
        ...
        {
            if (!inL[j] && !get(diG, iG_bimap.left.at(j)) && !get(dvG, vG_bimap.left.at(j)))
            {
        ...
        if (found)
        {
            ...
            // REC
            detail::rec_two_graphs_common_spanning_trees<Graph, Func, Seq, Map>(iG, iG_bimap, aiG_inL, diG, vG, vG_bimap, aiG_inL, dvG, func, inL);

            while (!iG_buf_copy.empty())
            {
                put(diG, iG_buf_copy.top(), false);
                put(dvG, vG_bimap.left.at(iG_bimap.right.at(iG_buf_copy.top())), false);
                iG_buf_copy.pop();
            }
            while (!vG_buf_copy.empty())
            {
                put(dvG, vG_buf_copy.top(), false);
                put(diG, iG_bimap.left.at(vG_bimap.right.at(vG_buf_copy.top())), false);
                vG_buf_copy.pop();
            }
            ...
            while (!iG_buf.empty() && !found)
            {
                if (!inL[iG_bimap.right.at(iG_buf.top())])
                {
                    ...
                    put(avG_inL, vG_bimap.left.at(iG_bimap.right.at(iG_buf.top())), true);
                    ...
                    put(avG_inL, vG_bimap.left.at(iG_bimap.right.at(iG_buf.top())), false);
                }
                iG_buf.pop();
            }
            while (!vG_buf.empty() && !found)
            {
                if (!inL[vG_bimap.right.at(vG_buf.top())])
                {
                    ...
                    put(aiG_inL, iG_bimap.left.at(vG_bimap.right.at(vG_buf.top())), true);
                    ...
                    put(aiG_inL, iG_bimap.left.at(vG_bimap.right.at(vG_buf.top())), false);
                }
                vG_buf.pop();
            }
            ...
            if (!found)
            {
                ...
                while (!iG_buf_copy.empty())
                {
                    ...
                    put(avG_inL, vG_bimap.left.at(iG_bimap.right.at(iG_buf_copy.top())), true);
                ...
                while (!vG_buf_copy.empty())
                {
                    ...
                    put(aiG_inL, iG_bimap.left.at(vG_bimap.right.at(vG_buf_copy.top())), true);
                ...
                detail::rec_two_graphs_common_spanning_trees<
                    Graph, Func, Seq, Map>(iG, iG_bimap, aiG_inL, diG, vG, vG_bimap, aiG_inL, dvG, func, inL);
...
BOOST_CONCEPT_REQUIRES(
    ((RandomAccessContainer<Order>))((IncidenceGraphConcept<Graph>))((UnaryFunction<Func, void, Seq>))((Mutable_RandomAccessContainer<Seq>))((VertexAndEdgeListGraphConcept<Graph>)),
    (void))
    ...
    Seq inL)
{
    ...
    if (iG_buf.empty() && vG_buf.empty())
    {
        ...
        detail::rec_two_graphs_common_spanning_trees<Graph, Func, Seq,
                                                     associative_property_map<std::map<edge_descriptor, bool>>>(iG, iG_bimap, aiG_inL, diG, vG, vG_bimap, aiG_inL, dvG, func, inL);
    }
}
...
template <
    ...
    typename Seq>
BOOST_CONCEPT_REQUIRES(
    ((IncidenceGraphConcept<Graph>))((EdgeListGraphConcept<Graph>)),
    (void))
two_graphs_common_spanning_trees(
    const Graph &iG,
    const Graph &vG,
