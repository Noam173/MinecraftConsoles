...
class isomorphism_algo
{
    ...
    struct edge_cmp
    {
        ...
        bool operator()(const edge1_t &e1, const edge1_t &e2) const
        {
            ...
            return std::make_pair(m1, std::make_pair(u1, v1)) < std::make_pair(m2, std::make_pair(u2, v2));
        }
        const Graph1 &G1;
        DFSNumMap dfs_num;
    };
  ...
  public:
          ...
          index_map1(index_map1), index_map2(index_map2)
    {
        ...
        in_S = make_safe_iterator_property_map(in_S_vec.begin(), in_S_vec.size(), index_map2
#ifdef BOOST_NO_STD_ITERATOR_TRAITS
                                               ,
                                               in_S_vec.front()
#endif /* BOOST_NO_STD_ITERATOR_TRAITS */
...
class degree_vertex_invariant
{
  ...
  public:
    ...
    {
        return (m_max_vertex_in_degree + 1) * out_degree(v, m_g) + get(m_in_degree_map, v);
    }
