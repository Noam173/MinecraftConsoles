...
struct default_construct_iterator : public boost::iterator_facade<default_construct_iterator<T>, T, boost::random_access_traversal_tag, const T &>
{
    ...
    bool equal(default_construct_iterator /*i*/) const
    {
        return true;
    }
    void increment()
    ...
    typename base_type::difference_type distance_to(default_construct_iterator) const
    {
        return 0;
    }
};
                                               ...
                                               VertexProperty, Vertex, typed_identity_property_map<Vertex>>
{
  ...
  public:
        ...
        : inherited_vertex_properties(numverts), m_property(prop)
    {
        ...
        boost::graph::detail::split_into_separate_coords(edge_begin, edge_end, sources, targets);
        m_forward.assign_sources_and_targets_global(sources, targets, numverts, boost::typed_identity_property_map<vertices_size_type>());
    }
    ...
    {
        std::vector<vertex_descriptor> sources, targets;
        boost::graph::detail::split_into_separate_coords_filtered(edge_begin, edge_end, sources, targets, source_pred);
        m_forward.assign_sources_and_targets_global(sources, targets, numlocalverts, global_to_local);
    }
        ...
        : inherited_vertex_properties(numlocalverts), m_property(prop)
    {
        ...
        boost::graph::detail::split_into_separate_coords_filtered(edge_begin, edge_end, ep_iter, sources, targets, edge_props, source_pred);
        m_forward.assign_sources_and_targets_global(sources, targets, edge_props, numlocalverts, global_to_local);
    }
                       ...
                       const GlobalToLocal &global_to_local)
    {
        ...
        typedef std::vector<
            boost::tuple<vertex_pair,
                         edge_bundled>>
            edge_vector_t;
        edge_vector_t new_edges(boost::make_zip_iterator(boost::make_tuple(first, ep_iter)),
        ...
        m_forward.add_edges_sorted_internal(boost::make_transform_iterator(
                                                new_edges.begin(),
    ...
    const typename forward_type::inherited_edge_properties &edge_properties() const
    {
        return m_forward;
    }
                                               ...
                                               VertexProperty, Vertex, typed_identity_property_map<Vertex>>
{
  ...
  private:
    void set_up_backward_property_links()
    {
        ...
        m_backward.assign_unsorted_multi_pass_edges(detail::transpose_edges(
                                                        detail::make_edge_to_index_pair_iter(*this, get(vertex_index, *this), e.first)),
                                                    detail::transpose_edges(
                                                        detail::make_edge_to_index_pair_iter(*this, get(vertex_index, *this), e.second)),
...
template <BOOST_CSR_GRAPH_TEMPLATE_PARMS>
std::pair<counting_iterator<Vertex>, counting_iterator<Vertex>> inline vertices(const BOOST_CSR_GRAPH_TYPE &g)
{
...
template <BOOST_CSR_GRAPH_TEMPLATE_PARMS, typename Tag>
struct property_map<BOOST_CSR_GRAPH_TYPE, Tag, typename disable_if<detail::is_distributed_selector<Vertex>>::type> : csr_property_map_helper<
                                                                                                                         BOOST_CSR_GRAPH_TYPE,
                                                                                                                         Tag,
                                                                                                                         typename detail::property_kind_from_graph<BOOST_CSR_GRAPH_TYPE, Tag>::type>
{
};
