...
class mutate_graph_impl : public mutate_graph
{
  ...
  public:
    ...
    set_graph_property(const std::string &name, const std::string &value, const std::string &value_type)
    {
        ...
        try
        {
            mpl::for_each<value_types>(put_property<MutableGraph, value_types>(name, m_dp, m_g, value, value_type, m_type_names, type_found));
        }
    ...
    set_vertex_property(const std::string &name, any vertex, const std::string &value, const std::string &value_type)
    {
        ...
        try
        {
            mpl::for_each<value_types>(put_property<vertex_descriptor, value_types>(name, m_dp, any_cast<vertex_descriptor>(vertex),
    ...
    set_edge_property(const std::string &name, any edge, const std::string &value, const std::string &value_type)
    {
        ...
        try
        {
            mpl::for_each<value_types>(put_property<edge_descriptor, value_types>(name, m_dp, any_cast<edge_descriptor>(edge),
                                                                                  value, value_type, m_type_names, type_found));
        }
        catch (bad_lexical_cast)
        {
...
void write_graphml(std::ostream &out, const Graph &g, VertexIndexMap vertex_index,
