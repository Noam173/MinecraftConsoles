...
{
    return graph_attributes_writer<GAttrMap, NAttrMap, EAttrMap>(g_attr, n_attr, e_attr);
}

template <typename Graph>
graph_attributes_writer<typename graph_property<Graph, graph_graph_attribute_t>::type,
...
inline void
write_graphviz(std::ostream &out, const Graph &g, VertexWriter vw BOOST_GRAPH_ENABLE_IF_MODELS_PARM(Graph, vertex_list_graph_tag))
{
    default_writer dw;
    default_writer gw;
...
inline void
write_graphviz(std::ostream &out, const Graph &g,
               VertexWriter vw, EdgeWriter ew BOOST_GRAPH_ENABLE_IF_MODELS_PARM(Graph, vertex_list_graph_tag))
{
    default_writer gw;
...
struct bad_parallel_edge : public graph_exception
{
    ...
    bad_parallel_edge(const std::string &i, const std::string &j) : from(i), to(j)
    {
    }
    ...
    const char *what() const throw()
    {
        ...
        {
            statement =
                std::string("Failed to add parallel edge: (") + from + "," + to + ")\n";
        }
...
class mutate_graph
{
  public:
    ...
    virtual void
    do_add_edge(const edge_t &edge, const node_t &source, const node_t &target) = 0;
