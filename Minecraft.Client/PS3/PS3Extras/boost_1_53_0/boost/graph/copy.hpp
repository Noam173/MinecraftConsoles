...
{
    typedef Desc type;
    static Desc convert(const Desc &d, const Graph &)
    {
        return d;
    }
};
...
struct choose_graph_copy
{
    ...
    {
        algo =
            (is_convertible<Trv, vertex_list_graph_tag>::value && is_convertible<Trv, edge_list_graph_tag>::value)
                ? 0
                : is_convertible < Dr,
        directed_tag > ::value ? 1 : 2
    };
...
{
    typedef typename detail::choose_edge_copier_helper<Param, G1, G2>::type Choice;
...
{
    typedef typename detail::choose_vertex_copier_helper<Param, G1, G2>::type Choice;
    return Choice::apply(params, g_in, g_out);
}
                    ...
                    const Params &params)
{
    ...
    typename graph_traits<MutableGraph>::vertex_descriptor src_copy = vis.copy_one_vertex(src);
