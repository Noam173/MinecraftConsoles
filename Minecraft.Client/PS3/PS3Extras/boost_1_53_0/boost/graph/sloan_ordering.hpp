                         ...
                         DegreeMap degree)
{
    ...
    do
    {
        ...
        for (typename std::vector<typename graph_traits<Graph>::vertices_size_type>::iterator iter = dist.begin(); iter != dist.end(); ++iter)
        {
            *iter = 0;
        }

        // generating the RLS (rooted level structure)
        breadth_first_search(G, s, visitor(make_bfs_visitor(record_distances(dist_pmap, on_tree_edge()))));
        ...
        while (!degree_queue.empty())
        {
            ...
            breadth_first_search(G, i, boost::visitor(make_bfs_visitor(record_distances(dist_pmap, on_tree_edge()))));
               ...
               Weight W2)
{
    ...
    breadth_first_search(g, e, visitor(make_bfs_visitor(record_distances(dist_pmap, on_tree_edge()))));
