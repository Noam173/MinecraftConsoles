...
class Numbering
{
  ...
  public:
    ...
    {
        data[get(id, node)] = -num;
    }
    bool all_done(number_type i = 0) const
    {
        return num + i > max_num;
    }
...
class mmd_impl
{
    ...
    typedef iterator_property_map<vertex_t *,
                                  identity_property_map, vertex_t, vertex_t &>
        IndexVertexMap;
  ...
  public:
    ...
    void eliminate(vertex_t node)
    {
        ...
        {
            vertex_t v_node = *v;
            if (!degree_lists_marker.need_update(v_node) && !degree_lists_marker.outmatched_or_done(v_node))
            {
                           ...
                           PermutationMap prev)
    {
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            {
                prev[i] = next[i];
                supernode_size[get(index_vertex_map, i)] = next[i] + 1; // record the supernode info
            }
            else
            {
                prev[i] = -next[i];
...
void minimum_degree_ordering(Graph &G,
                             DegreeMap degree,
                             InversePermutationMap inverse_perm,
                             PermutationMap perm,
                             SuperNodeMap supernode_size,
