...
{
  public:
    component_value_allocator(int num, int size) : last(0), num(num), size(size)
    {
    }
...
struct update_reducer
{
    ...
    update_reducer(work_queue *q,
                   ...
                   processor_id_type pg_id) : q(q), collisions(collisions), pg_id(pg_id)
    {
    }
...
connected_components_ps(const Graph &g, ComponentMap c)
{
    ...
    typedef typename property_map<Graph, vertex_owner_t>::const_type vertex_owner_map;
