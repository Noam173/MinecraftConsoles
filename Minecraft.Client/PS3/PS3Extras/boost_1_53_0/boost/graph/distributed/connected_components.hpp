...
void build_local_metagraph(const Graph &g, ParentMap p, RootIterator r,
                           RootIterator r_end, AdjacencyMap &adj)
{
    ...
    if (id == 0)
    {
        ...
        while (optional<std::pair<process_id_type, int>> m = probe(pg))
        {
            ...
            for (typename std::vector<vertex_descriptor>::iterator iter = ++adjs.begin(); iter != adjs.end(); ++iter)
            {
        ...
        for (typename std::map<vertex_descriptor, meta_vertex_descriptor>::iterator
                 iter = vertex_map.begin();
             iter != vertex_map.end(); ++iter)
        {
            vertex_map[iter->first] = add_vertex(metaVertex<vertex_descriptor>(iter->first), mg);
        }
        ...
        typename std::vector<std::pair<vertex_descriptor, vertex_descriptor>>::iterator edge_iter = edges.begin();
...
void request_parent_map_entries(const Graph &g, ParentMap p,
...
void parallel_connected_components(DistributedGraph &g, ParentMap p)
{
    ...
    typedef typename boost::graph::parallel::process_group_type<DistributedGraph>::type process_group_type;
    typedef typename process_group_type::process_id_type process_id_type;
    ...
    for (liter = roots.begin(); liter != roots.end(); ++liter)
    {
        ...
        my_adj.erase(std::remove_if(my_adj.begin(), my_adj.end(),
    ...
    do
    {
        ...
        do
        {
            ...
            for (liter = old_roots.begin(); liter != old_roots.end(); ++liter)
            {
                ...
                if (get(owner, p1) != id)
                {
                    parent_requests[get(owner, p1)].insert(p1);
                }
                vertex_descriptor p2 = get(p, p1);
                if (get(owner, p2) != id)
                {
                    parent_requests[get(owner, p2)].insert(p2);
                }
            }
        ...
        if (id == 0)
        {
            std::cerr << double_steps << " doubling steps.\n";
        }
#endif
        //
        ...
        while (optional<std::pair<process_id_type, int>> m = probe(pg))
        {
            ...
            typename std::vector<vertex_descriptor>::iterator aviter = incoming_edges.begin();
            ++aviter;
...
number_components_from_parents(const Graph &g, ParentMap p, ComponentMap c)
{
    ...
    {
        if (std::find(my_roots.begin(), my_roots.end(), get(p, v)) == my_roots.end())
        {
...
int number_components_from_parents(const Graph &g, ParentMap p,
                                   dummy_property_map)
{
    using boost::parallel::all_reduce;
                                         ...
                                         BOOST_GRAPH_ENABLE_IF_MODELS_PARM(Graph, distributed_graph_tag))
{
    ...
    return connected_components(g, c,
