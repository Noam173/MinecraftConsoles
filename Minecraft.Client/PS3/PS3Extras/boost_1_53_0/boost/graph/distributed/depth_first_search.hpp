            ...
            case discover_msg:
                {
                    ...
                    if (get(color, p.first) == Color::white())
                    {
                        ...
                        if (shift_center_of_activity(p.first))
                        {
                            break;
                        }
                        ...
                        for (boost::tie(ei, ei_end) = out_edges(p.first, g); ei != ei_end; ++ei)
                        {
                            ...
                            if (get(parent, p.first) != target(*ei, g) && get(explore, p.first) != target(*ei, g))
                            {
...
template <typename DistributedGraph, typename ColorMap, typename ParentMap,
          typename ExploreMap, typename VertexIndexMap, typename DFSVisitor>
void tsin_depth_first_visit(const DistributedGraph &g,
                            ...
                            VertexIndexMap index_map)
{
    ...
    graph::distributed::detail::parallel_dfs<DistributedGraph, ColorMap, ParentMap, ExploreMap, VertexIndexMap,
                                             DFSVisitor>
        do_dfs(g, color, parent, explore, index_map, vis);
    do_dfs.run(s);
...
template <typename DistributedGraph, typename DFSVisitor>
void depth_first_visit(const DistributedGraph &g,
                       typename graph_traits<DistributedGraph>::vertex_descriptor s,
                       DFSVisitor vis)
{
