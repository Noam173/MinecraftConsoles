...
template <typename T>
void send(const mpi_process_group &pg, mpi_process_group::process_id_type dest,
...
T *all_reduce(const mpi_process_group &pg, T *first, T *last, T *out,
              BinaryOperation bin_op);

template <typename T, typename BinaryOperation>
T *scan(const mpi_process_group &pg, T *first, T *last, T *out,
        BinaryOperation bin_op);

template <typename InputIterator, typename T>
void all_gather(const mpi_process_group &pg,
...
template <typename T>
void broadcast(const mpi_process_group &pg, T &val,
         ...
         int tag, const T &value, int block = -1)
{
    ...
    std::pair<boost::mpi::communicator, int> actual = pg.actual_communicator_and_tag(tag, block);
