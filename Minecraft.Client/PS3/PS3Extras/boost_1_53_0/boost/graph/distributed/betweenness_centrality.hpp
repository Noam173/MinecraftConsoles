...
class get_owner_of_first_tuple_element
{
  ...
  public:
    ...
    owner_type get_owner(Tuple t)
    {
        return get(owner, boost::tuples::get<0>(t));
    }
...
class get_owner_of_first_pair_element
{
  ...
  public:
    ...
    owner_type get_owner(std::pair<Vertex, T> p)
    {
        return get(owner, p.first);
    }
...
#endif
          >
void betweenness_centrality_delta_stepping_impl<
    Graph, DistanceMap, IncomingMap, EdgeWeightMap, PathCountMap
#ifdef COMPUTE_PATH_COUNTS_INLINE
    ,
    IsSettledMap, VertexIndexMap
    ...
    run(Vertex s)
{
    ...
    do
    {
        ...
        while (current_bucket < buckets.size() && (!buckets[current_bucket] || buckets[current_bucket]->empty()))
        {
            ++current_bucket;
        }
        ...
        do
        {
            ...
            nonempty_bucket = (current_bucket < buckets.size() && buckets[current_bucket] && !buckets[current_bucket]->empty());
    ...
    if (x <= get(distance, v))
    {
        ...
        if (get(owner, v) == process_id(pg))
        {
            if (x < get(distance, v))
            {
                ...
                {
                    buckets.resize(new_index + 1, 0);
                }

                // Make sure that we have allocated the bucket itself.
                if (!buckets[new_index])
                {
                    buckets[new_index] = new Bucket;
                }

                if (get(distance, v) != (std::numeric_limits<Dist>::max)() && !vertex_was_deleted[get(local, v)])
                {
                    // We're moving v from an old bucket into a new one. Compute
                    // the old index, then splice it in.
                    BucketIndex old_index = static_cast<BucketIndex>(get(distance, v) / delta);
                ...
                if (u != v)
                {
                    put(incoming, v, IncomingType(1, u));
                }
                put(distance, v, x);
...
void do_brandes_sssp(const Graph &g,
                     ...
                     typename graph_traits<Graph>::vertex_descriptor s)
{
    ...
    BGL_FORALL_VERTICES_T(v, g, Graph)
    {
        if (get(distance, v) < (std::numeric_limits<distance_type>::max)() && get(dependency_count, v) == 0)
        {
    ...
    while (!Q.empty())
    {
        ...
        if (get(dependency_count, w) == 0)
        {
            ...
            for (incoming_iterator vw = el.begin(); vw != el.end(); ++vw)
            {
                ...
                dependency_type factor = dependency_type(get(path_count, v)) / dependency_type(get(path_count, w));
...
template <typename Graph, typename CentralityMap, typename EdgeCentralityMap,
          ...
          typename PathCountMap, typename VertexIndexMap, typename ShortestPaths,
          typename Stack>
void do_sequential_brandes_sssp(const Graph &g,
                                CentralityMap centrality,
                                EdgeCentralityMap edge_centrality_map,
                                IncomingMap incoming,
                                DistanceMap distance,
...
template <typename ProcessGroup, typename Graph,
          ...
          typename VertexIndexMap, typename ShortestPaths,
          typename Buffer>
void non_distributed_brandes_betweenness_centrality_impl(const ProcessGroup &pg,
                                                         const Graph &g,
                                                         CentralityMap centrality,
                                                         EdgeCentralityMap edge_centrality_map,
...
template <typename ProcessGroup, typename Graph, typename CentralityMap,
          ...
          typename Buffer>
void non_distributed_brandes_betweenness_centrality(const ProcessGroup &pg,
                                                    const Graph &g,
                                                    CentralityMap centrality,
                                                    EdgeCentralityMap edge_centrality_map,
                                                    IncomingMap incoming,
...
template <typename ProcessGroup, typename Graph, typename CentralityMap,
          ...
          typename Buffer>
void non_distributed_brandes_betweenness_centrality_dispatch2(const ProcessGroup &pg,
                                                              const Graph &g,
                                                              CentralityMap centrality,
                                                              EdgeCentralityMap edge_centrality_map,
                                                              WeightMap weight_map,
...
void non_distributed_brandes_betweenness_centrality(const ProcessGroup &pg, const Graph &g,
                                                    const bgl_named_params<Param, Tag, Rest> &params)
{
    typedef bgl_named_params<Param, Tag, Rest> named_params;
