...
boman_et_al_graph_coloring(const DistributedGraph &g,
                           ColorMap color,
                           ...
                           VertexOrdering ordering, VertexIndexMap vertex_index)
{
    ...
    std::size_t rounds = vertices_to_color.size() / s + (vertices_to_color.size() % s == 0 ? 0 : 1);
    ...
    while (rounds > 0)
    {
                ...
                for (vertex_set_iterator vi = start; vi != first; ++vi)
                {
                    ...
                    BGL_FORALL_OUTEDGES_T(v, e, g, DistributedGraph)
                    {
                        ...
                        if (get(owner, w) != process_id(pg) // boundary vertex
                            && marked_conflicting[get(vertex_index, v)] != outer_iter_num && get(color, v) == get(color, w) && ordering(v, w))
                        {
        ...
        rounds = vertices_to_color.size() / s + (vertices_to_color.size() % s == 0 ? 0 : 1);
        rounds = all_reduce(pg, rounds, boost::parallel::maximum<std::size_t>());
...
boman_et_al_graph_coloring(const DistributedGraph &g, ColorMap color,
                           typename graph_traits<DistributedGraph>::vertices_size_type s,
