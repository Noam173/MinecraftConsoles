...
struct redistributed_descriptor : maybe_store_property<DescriptorProperty>
{
  ...
  private:
    ...
    {
        ar &boost::serialization::base_object<inherited>(*this) & unsafe_serialize(descriptor);
    }
};
...
struct source_migrated_t
{
    ...
    {
        return get(vertex_to_processor, DVertex(e.source_processor, source(e.e, g))) != e.source_processor;
    }
...
struct source_or_target_migrated_t
{
    ...
    bool operator()(Edge e) const
    {
        return get(vertex_to_processor, source(e, g)) != source(e, g).owner || get(vertex_to_processor, target(e, g)) != target(e, g).owner;
    }
...
void PBGL_DISTRIB_ADJLIST_TYPE ::request_in_neighbors(vertex_descriptor v,
                                                      VertexProcessorMap vertex_to_processor,
                                                      bidirectionalS)
{
...
template <PBGL_DISTRIB_ADJLIST_TEMPLATE_PARMS>
template <typename VertexProcessorMap>
void PBGL_DISTRIB_ADJLIST_TYPE ::remove_migrated_in_edges(vertex_descriptor v,
                                                          VertexProcessorMap vertex_to_processor,
                                                          bidirectionalS)
{
...
void PBGL_DISTRIB_ADJLIST_TYPE ::redistribute(VertexProcessorMap vertex_to_processor)
{
    ...
    const bool has_stable_descriptors =
        is_same<typename config_type::vertex_list_selector, listS>::value || is_same<typename config_type::vertex_list_selector, setS>::value || is_same<typename config_type::vertex_list_selector, multisetS>::value;
    ...
    {
        if (!has_stable_descriptors || get(vertex_to_processor, *vi) != vi->owner)
        {
    ...
    for (boost::tie(ei, ei_end) = edges(*this); ei != ei_end; ++ei)
    {
        ...
        if (!has_stable_descriptors || get(vertex_to_processor, src) != src.owner || get(vertex_to_processor, tgt) != tgt.owner)
        {
    ...
    // all that much given the cost of redistribution.
    {
        ...
        for (process_id_type src = 0; src < p; ++src)
        {
                 ...
                 ei != redistributed_edges[src].end(); ++ei)
            {
                ...
                if (old_to_new_vertex_map.find(need_vertex) == old_to_new_vertex_map.end())
                {
