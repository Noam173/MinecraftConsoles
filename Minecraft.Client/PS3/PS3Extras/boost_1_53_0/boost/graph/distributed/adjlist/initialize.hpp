...
void PBGL_DISTRIB_ADJLIST_TYPE::
               ...
               VertexListS)
{
    ...
    while (first != last)
    {
        ...
        {
            if (distribution(first->second) != id)
            {
                break;
            }
            vertex_descriptor source(id, index_to_vertex[distribution.local(first->first)]);
            vertex_descriptor target(distribution(first->second),
    ...
    while (first != last)
    {
        if (distribution(first->first) == id)
        {
            ...
            delayed_edges.push_back(delayed_edge_t(st_pair(source, first->second), *ep_iter));
        }
        ++first;
        ++ep_iter;
    }
    ...
    for (typename std::vector<delayed_edge_t>::iterator i = delayed_edges.begin(); i != delayed_edges.end(); ++i)
    {
