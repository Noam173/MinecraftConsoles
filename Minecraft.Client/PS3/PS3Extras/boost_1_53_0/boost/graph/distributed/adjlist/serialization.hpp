...
{
    pending_edge(
        Vertex source, Vertex target, Properties properties, void *property_ptr)
        : source(source), target(target), properties(properties), property_ptr(property_ptr)
    {
    }
...
struct graph_loader
{
    ...
    typedef typename mpl::if_<
        is_same<VertexListS, defaultS>, vecS, VertexListS>::type vertex_list_selector;
    ...
    graph_loader(Graph &g, Archive &ar)
        : m_g(g), m_ar(ar), m_pg(g.process_group()), m_requested_vertices(num_processes(m_pg)), m_remote_vertices(num_processes(m_pg)), m_property_ptrs(num_processes(m_pg))
    {
        g.clear();
        load_prefix();
        load_vertices();
  ...
  private:
    struct pending_in_edge
    {
        pending_in_edge(
            ...
            : u(u), v(v), property_ptr(property_ptr)
        {
        }
    ...
    template <class Anything>
    void add_edge(
        vertex_descriptor u, vertex_descriptor v, edge_property_type const &property, void *property_ptr, Anything);
    void add_edge(
        vertex_descriptor u, vertex_descriptor v, edge_property_type const &property, void *property_ptr, vecS);
    ...
    void *maybe_load_property_ptr(directedS)
    {
        return 0;
    }
    ...
    vertex_descriptor u, vertex_descriptor v, edge_property_type const &property, void *property_ptr, vecS)
{
    std::pair<local_edge_descriptor, bool> inserted =
        detail::parallel::add_local_edge(
            local(u), local(v), m_g.build_edge_property(property), m_g.base());
    ...
    edge_descriptor const &edge, void *old_property_ptr, bidirectionalS)
{
    if (owner(target(edge, m_g)) == m_g.processor())
    {
        ...
        boost::graph_detail::push(get(
                                      vertex_in_edges, m_g.base())[local(target(edge, m_g))],
                                  e);
    }
    else
    {
...
void graph_loader<Graph, Archive, VertexListS>::commit_pending_in_edges(bidirectionalS)
{
    ...
    BOOST_FOREACH (pending_in_edge const &e, m_pending_in_edges)
    {
        ...
        std::vector<unsafe_pair<void *, void *>>::iterator i = std::lower_bound(
            m_property_ptrs[owner(u)].begin(), m_property_ptrs[owner(u)].end(), unsafe_pair<void *, void *>(e.property_ptr, 0));

        if (i == m_property_ptrs[owner(u)].end() || i->first != e.property_ptr)
        {
            BOOST_ASSERT(false);
        }
    ...
    vertex_descriptor u) const
{
    ...
    typename std::vector<serializable_vertex_descriptor>::const_iterator
        i = std::lower_bound(
            m_requested_vertices[owner(u)].begin(), m_requested_vertices[owner(u)].end(), serializable_vertex_descriptor(local(u)));

    if (i == m_requested_vertices[owner(u)].end() || *i != local(u))
    {
        BOOST_ASSERT(false);
    }
...
void save_edges(Archive &ar, Graph const &g, DirectedS)
{
    ...
    BGL_FORALL_EDGES_T(e, g, Graph)
    {
        ...
        maybe_save_properties(
            ar, "edge_property", static_cast<edge_property_type const &>(get(edge_all_t(), g, e)));
...
void PBGL_DISTRIB_ADJLIST_TYPE::load(std::string const &filename)
{
    ...
    std::set_difference(
        disk_files.begin(), disk_files.end(), consumed_files.begin(), consumed_files.end(), std::back_inserter(available_files));
...
void PBGL_DISTRIB_ADJLIST_TYPE::save(std::string const &filename) const
{
    ...
    BGL_FORALL_VERTICES_T(v, *this, graph_type)
    {
        ...
        detail::parallel::maybe_save_properties(
            ar, "vertex_property", static_cast<vertex_property_type const &>(get(vertex_all_t(), *this, v)));
    }
