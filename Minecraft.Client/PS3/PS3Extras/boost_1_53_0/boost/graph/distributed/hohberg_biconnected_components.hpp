...
struct hohberg_message
{
    ...
    bool is_path() const
    {
        return gamma == graph_traits<Graph>::null_vertex();
    }
    bool is_tree() const
    {
        return gamma != graph_traits<Graph>::null_vertex();
...
template <typename Graph>
void hohberg_vertex_processor<Graph>::initialize_leader(Vertex alpha,
                                                        const Graph &g)
{
...
template <typename Graph>
void hohberg_vertex_processor<Graph>::operator()(Edge e, path_t &path,
                                                 const Graph &g)
{
    using namespace hohberg_detail;
...
template <typename Graph>
void hohberg_vertex_processor<Graph>::operator()(Edge e, Vertex gamma,
...
template <typename Graph>
void hohberg_vertex_processor<Graph>::operator()(Edge e, edges_size_type name,
                                                 const Graph &g)
{
    using namespace hohberg_detail;
    ...
    BGL_FORALL_OUTEDGES_T(alpha, e, g, Graph)
    {
        ...
        if (edata.is_tree_edge && find(edata.M.begin(), edata.M.end(), parent) != edata.M.end() && target(e, g) != parent)
        {
    ...
    num_starting_bicomponents(Vertex alpha, const Graph &g)
{
    ...
    {
        per_edge_data &edata = edge_data[idx++];
        if (edata.is_tree_edge && find(edata.M.begin(), edata.M.end(), parent) == edata.M.end())
        {
            // Map from local partition numbers to global bicomponent numbers
...
template <typename Graph>
template <typename ComponentMap>
void hohberg_vertex_processor<Graph>::
...
template <typename Graph>
void hohberg_vertex_processor<Graph>::echo_phase(Vertex alpha, const Graph &g)
{
    using namespace hohberg_detail;
        ...
        BGL_FORALL_OUTEDGES_T(alpha, e, g, Graph)
        {
            ...
            else
            {
                ...
                if (edata.msg.is_path())
                {
                    ...
                    {
                        ++pos;
                        if (pos != edata.msg.path_or_bicomp.end() && find(bicomp.begin(), bicomp.end(), *pos) == bicomp.end())
                        {
#if defined(PBGL_HOHBERG_DEBUG) && PBGL_HOHBERG_DEBUG > 1
...
hohberg_biconnected_components(const Graph &g,
                               ...
                               VertexProcessorMap vertex_processor)
{
    ...
    do
    {
        ...
        if (minimum_phase == 4 && all_reduce(pg, maximum_phase, boost::mpi::maximum<char>()) == 4)
        {
...

{
    ...
    return hohberg_biconnected_components(g, component, first, last,
...
hohberg_biconnected_components(const Graph &g, ComponentMap component)
{
    ...
    return hohberg_biconnected_components(g, component, make_iterator_property_map(parents.begin(), get(vertex_index, g)));
}
