...
void sorted_mutating_kruskal(const Graph &g,
                                        ...
                                        bool already_local_msf)
{
    ...
    if (!already_local_msf)
    {
        ...
        detail::sorted_mutating_kruskal(g, first_vertex, last_vertex,
                                    ...
                                    SupervertexMap supervertex_map)
{
    ...
    kruskal_minimum_spanning_tree(make_vertex_and_edge_range(g, vertices(g).first, vertices(g).second,
                                                             edges(g).first, edges(g).second),
                                  std::back_inserter(edge_list),
                                  boost::weight_map(weight_map).vertex_index_map(index_map));
                                    ...
                                    OutputIterator out, VertexIndex i_map)
{
    ...
    return dense_boruvka_minimum_spanning_tree(g, weight_map, out, i_map,
                                   ...
                                   GlobalIndexMap global_index)
{
    ...
    {
        pg = detail::merge_local_minimum_spanning_trees_step(pg, g, vertices(g).first, vertices(g).second,
                   ...
                   ParentMap parent_map, SupervertexMap supervertex_map)
{
    ...
    kruskal_minimum_spanning_tree(make_vertex_and_edge_range(g, vertices(g).first, vertices(g).second,
                                                             edges(g).first, edges(g).second),
                                  std::back_inserter(edge_list),
                                  boost::weight_map(weight).vertex_index_map(index));
    ...
    double log_d_p = log((double)num_processes(process_group(g))) / log((double)tree_factor);
    ...
    while (pg && num_processes(pg) > 1)
    {
        pg = detail::merge_local_minimum_spanning_trees_step(pg, g, supervertices.begin(), supervertices.end(),
                   ...
                   GlobalIndexMap index)
{
    ...
    return boruvka_then_merge(g, weight, out, index,
                    ...
                    GlobalIndexMap index)
{
    ...
    return boruvka_mixed_merge(g, weight, out, index,
                               make_iterator_property_map(ranks.begin(), index),
                               make_iterator_property_map(parents.begin(), index),
                               make_iterator_property_map(supervertex_indices.begin(), index));
}
