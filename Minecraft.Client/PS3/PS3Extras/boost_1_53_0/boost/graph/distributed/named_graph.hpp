...
template <typename InDistribution, typename VertexProperty, typename VertexSize,
          ...
          typename ExtractName = typename internal_vertex_name<VertexProperty>::type>
...
class named_graph
{
  ...
  protected:
    ...
    template <typename U, typename V>
    boost::parallel::detail::untracked_pair<edge_descriptor, bool>
    handle_add_edge_with_reply_and_property(int source, int tag,
                                            const pair_with_property<U, V, edge_property_type> &msg,
                                            trigger_receive_context);
...
BGL_NAMED_GRAPH::lazy_add_edge::commit() const
{
    ...
    else
    {
        ...
        u_owner = get(vertex_owner, self.derived(),
...
template <BGL_NAMED_GRAPH_PARAMS>
void BGL_NAMED_GRAPH::setup_triggers()
{
    ...
    simple_trigger(process_group_, msg_add_edge_name_name_with_reply, this,
                   &named_graph::template handle_add_edge_with_reply<vertex_name_type, vertex_name_type>);
    ...
    simple_trigger(process_group_, msg_add_edge_name_vertex_with_reply, this,
                   &named_graph::template handle_add_edge_with_reply<vertex_name_type, vertex_descriptor>);
    ...
    simple_trigger(process_group_, msg_add_edge_vertex_name_with_reply, this,
                   &named_graph::template handle_add_edge_with_reply<vertex_descriptor, vertex_name_type>);
    ...
    simple_trigger(process_group_,
                   ...
                   &named_graph::template handle_add_edge_with_reply_and_property<vertex_name_type, vertex_name_type>);
    ...
    simple_trigger(process_group_,
                   ...
                   &named_graph::template handle_add_edge_with_reply_and_property<vertex_name_type, vertex_descriptor>);
    ...
    simple_trigger(process_group_,
                   ...
                   &named_graph::template handle_add_edge_with_reply_and_property<vertex_descriptor, vertex_name_type>);
}
            ...
            const BGL_NAMED_GRAPH &g)
{
    ...
    typename BGL_NAMED_GRAPH::process_id_type owner = g.named_distribution()(name);

    if (owner == process_id(g.process_group()))
    {
        // The vertex is local, so search for a mapping here
        optional<local_vertex_descriptor> result = find_vertex(name, g.derived().base());
        if (result)
        {
 ...
 */
template <typename Graph, typename Vertex, typename Edge, typename Config,
          typename ExtractName = typename internal_vertex_name<typename Config::vertex_property_type>::type>
