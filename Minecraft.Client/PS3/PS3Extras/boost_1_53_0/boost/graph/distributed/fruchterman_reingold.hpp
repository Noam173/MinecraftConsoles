...
template <typename PositionMap, typename DisplacementMap, typename LocalForces>
inline distributed_force_pairs_proxy<PositionMap, DisplacementMap, LocalForces>
make_distributed_force_pairs(const PositionMap &position,
                             const DisplacementMap &displacement,
                             const LocalForces &local_forces)
{
    typedef distributed_force_pairs_proxy<PositionMap, DisplacementMap, LocalForces>
        result_type;
...
class neighboring_tiles_force_pairs
{
  public:
    ...
    void operator()(const Graph &g, ApplyForce apply_force)
    {
        ...
        {
            if (position[v][0] <= cutoffs[left])
            {
                my_vertices[left].push_back(v);
            }
            if (position[v][1] <= cutoffs[top])
            {
                my_vertices[top].push_back(v);
            }
            if (position[v][0] >= cutoffs[right])
            {
                my_vertices[right].push_back(v);
            }
            if (position[v][1] >= cutoffs[bottom])
            {
                my_vertices[bottom].push_back(v);
            }
        }
...
make_neighboring_tiles_force_pairs(PositionMap position,
                                                ...
                                                DisplacementMap displacement)
{
    ...
    boost::fruchterman_reingold_force_directed_layout(g, position, origin, extent,
