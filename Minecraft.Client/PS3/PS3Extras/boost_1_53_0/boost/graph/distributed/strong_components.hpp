...
void marshal_set(std::vector<std::vector<typename graph_traits<Graph>::vertex_descriptor>> in,
...
template <typename Graph>
void unmarshal_set(std::vector<typename graph_traits<Graph>::vertex_descriptor> in,
                   std::vector<std::vector<typename graph_traits<Graph>::vertex_descriptor>> &out)
{
    ...
    while (!in.empty())
    {
        typename std::vector<vertex_descriptor>::iterator end = std::find(in.begin(), in.end(), graph_traits<Graph>::null_vertex());
...
void fleischer_hendrickson_pinar_strong_components(const Graph &g,
    ...
    do
    {
        ...
        queue_t Q(process_group(g),
                  ...
                  make_filtered_queue(queue<vertex_descriptor>(),
                                      boost::detail::has_not_been_seen<VertexIndexMap>(num_vertices(g), vertex_index_map)),
                  false);

        rev_queue_t Qr(process_group(gr),
                       get(vertex_owner, gr),
                       make_filtered_queue(queue<rev_vertex_descriptor>(),
                                           boost::detail::has_not_been_seen<VertexIndexMap>(num_vertices(gr), vertex_index_map)),
                       false);
        ...
        if (active_vertices < 0.05 * n)
        {
            breadth_first_search(fgr, get(fr, local_start[0]),
                                 ...
                                 detail::scc_discovery_visitor<filtered_graph<const ReverseGraph, keep_all,
                                                                              detail::in_subset<Rev_VertexSet>>,
                                                               Rev_ParentMap>(pred_map),
                                 color_map);
        }
        else
        {
        ...
        if (std::size_t(id) < vertex_sets.size())
        { // If this proc has one or more unique starting points
            ...
            for (std::size_t j = 0; j < vertex_sets[i].size(); ++j)
            {
                ...
                if (get(owner, v) == id)
                {
                    ...
                    while (estart != eend && find(vertex_sets[i].begin(), vertex_sets[i].end(),
                                                  target(*estart, g)) == vertex_sets[i].end())
                    {
                        estart++;
                    }
                    if (estart != eend)
                    {
                        ...
                        while (restart != reend && find(vertex_sets[i].begin(), vertex_sets[i].end(),
                                                        get(rf, target(*restart, gr))) == vertex_sets[i].end())
                        {
                            restart++;
                        }
                        if (restart != reend)
...
number_components(const Graph &g, VertexComponentMap r, ComponentMap c)
{
    ...
    if (process_id(process_group(g)) != 0)
    {
        receive(process_group(g), 0, 0, c_num);
    }
...
template <typename Graph, typename ComponentMap, typename VertexComponentMap,
          typename VertexIndexMap>
typename property_traits<ComponentMap>::value_type
fleischer_hendrickson_pinar_strong_components_impl(const Graph &g,
                                                   ComponentMap c,
                                              ...
                                              VertexIndexMap vertex_index_map)
{
    ...
    return fleischer_hendrickson_pinar_strong_components_impl(g, c, r, vertex_index_map,
