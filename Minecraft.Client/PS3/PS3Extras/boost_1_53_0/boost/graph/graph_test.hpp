...
struct graph_test
{
    ...
    struct ignore_vertex
    {
        ...
        bool operator()(vertex_t x) const
        {
            return x != v;
        }
        vertex_t v;
                              ...
                              const Graph &g)
    {
        ...
        typedef typename std::vector<std::pair<vertex_t, vertex_t>>::const_iterator edge_iter;
        typedef typename graph_traits<Graph>::out_edge_iterator out_edge_iter;
        ...
        for (vertex_iter ui = vertex_set.begin(); ui != vertex_set.end(); ++ui)
        {
            ...
            BOOST_CHECK(deg_size_t(std::distance(p.first, p.second)) == out_degree(u, g));
                                  ...
                                  const Graph &g)
    {
        ...
        for (vertex_iter vi = vertex_set.begin(); vi != vertex_set.end(); ++vi)
        {
            ...
            BOOST_CHECK(deg_size_t(std::distance(p.first, p.second)) == in_degree(v, g));
            for (; p.first != p.second; ++p.first)
            {
                edge_t e = *p.first;
                               ...
                               const Graph &g)
    {
        ...
        for (typename std::vector<std::pair<vertex_t, vertex_t>>::const_iterator i = edge_set.begin();
             i != edge_set.end(); ++i)
        {
    ...
    void test_add_vertex(Graph &g)
    {
        ...
        BOOST_CHECK((verify_isomorphism(make_filtered_graph(g, keep_all(), ignore_vertex(v)), cpy,
                                        iso_map)));
    }
    ...
    void test_add_edge(vertex_t u, vertex_t v, Graph &g)
    {
        ...
        {
            BOOST_CHECK(allows_parallel_edges(g) && added == true || !allows_parallel_edges(g) && added == false);
        }
        else
        {
        ...
        { // edge added
            ...
            BOOST_CHECK((verify_isomorphism(make_filtered_graph(g, ignore_edge(e)), cpy, iso_map)));
        }
        else
        { // edge not added
    ...
    void test_remove_edge(vertex_t u, vertex_t v, Graph &g)
    {
        ...
        BOOST_CHECK((verify_isomorphism(g, make_filtered_graph(cpy, ignore_edges(u, v, cpy)),
                                        iso_map)));
    }
    ...
    void test_remove_edge(edge_t e, Graph &g)
    {
        ...
        BOOST_CHECK((verify_isomorphism(g, make_filtered_graph(cpy, ignore_edge(e)),
                                        iso_map)));
    }
    ...
    void test_clear_vertex(vertex_t v, Graph &g)
    {
        ...
        BOOST_CHECK((verify_isomorphism(g, make_filtered_graph(cpy, keep_all(), ignore_vertex(v)),
                                        iso_map)));
    }
