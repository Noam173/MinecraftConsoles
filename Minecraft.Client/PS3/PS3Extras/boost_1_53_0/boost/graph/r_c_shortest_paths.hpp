...
template <class Graph, class Resource_Container>
inline bool operator==(const r_c_shortest_paths_label<Graph, Resource_Container> &l1,
...
inline bool operator>(const r_c_shortest_paths_label<Graph, Resource_Container> &l1,
                      const r_c_shortest_paths_label<Graph, Resource_Container> &l2)
{
    return l2.cumulated_resource_consumption < l1.cumulated_resource_consumption;
}
                                 ...
                                 Visitor vis)
{
    ...
    typedef
        typename Label_Allocator::template rebind<r_c_shortest_paths_label<Graph, Resource_Container>>::other LAlloc;
    LAlloc l_alloc;
    ...
    l_alloc.construct(first_label,
                      r_c_shortest_paths_label<Graph, Resource_Container>(i_label_num++,
                                                                          rc,
                                                                          0,
    ...
    while (unprocessed_labels.size())
    {
        ...
        if (!cur_label->b_is_dominated)
        {
            ...
            if (static_cast<int>(list_labels_cur_vertex.size()) >= 2 && vec_last_valid_index_for_dominance[i_cur_resident_vertex_num] < static_cast<int>(list_labels_cur_vertex.size()))
            {
        ...
        if (!cur_label->b_is_dominated)
        {
                 ...
                 ++oei)
            {
                ...
                else
                {
                    ...
                    vec_vertex_labels[vertex_index_map[new_sp_label->resident_vertex]].push_back(new_sp_label);
                        ...
                        ed_last_extended_arc)
{
    ...
    if (b_result_must_be_equal_to_desired_final_resource_levels)
    {
        b_correctly_extended =
            actual_final_resource_levels == desired_final_resource_levels ? true : false;
    }
    else
    {
        if (actual_final_resource_levels < desired_final_resource_levels || actual_final_resource_levels == desired_final_resource_levels)
        {
