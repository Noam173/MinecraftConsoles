                                         ...
                                         VertexIndexMap vm)
{
    ...
    typedef boost::iterator_property_map<typename vsize_vector_t::iterator, VertexIndexMap>
        vertex_to_vsize_map_t;
    typedef boost::iterator_property_map<typename bool_vector_t::iterator, VertexIndexMap>
    ...
    for (ForwardIterator itr_end = ordering_end; itr != itr_end; ++itr)
    {
        ...
        {
            vertex_t curr_vertex = source(*pi, g) == v ? target(*pi, g) : source(*pi, g);
    ...
    graph::detail::accumulate_offsets(*ordering_begin, 0, g, x, delta_x, left, right);
