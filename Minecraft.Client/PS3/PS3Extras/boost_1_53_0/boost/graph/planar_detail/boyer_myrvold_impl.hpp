...
class boyer_myrvold_impl
{
    ...
    typedef typename graph_traits<Graph>::out_edge_iterator
        out_edge_iterator_t;
    typedef graph::detail::face_handle<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy> face_handle_t;
    ...
    {
        typedef iterator_property_map<typename std::vector<T>::iterator, VertexIndexMap> type;
    };
  ...
  public:
    boyer_myrvold_impl(const Graph &arg_g, VertexIndexMap arg_vm) : g(arg_g),
                                                                    vm(arg_vm),
                                                                    ...
                                                                    separated_node_in_parent_list(separated_node_in_parent_list_vector.begin(), vm),
...

    {
        ...
        planar_dfs_visitor<vertex_to_v_size_map_t, vertex_to_vertex_map_t,
            ...
            vis(low_point, dfs_parent, dfs_number, least_ancestor, dfs_parent_edge);
             ...
             itr != vertices_by_lowpoint.end(); ++itr)
        {
            ...
            if (v != parent)
            {
                separated_node_in_parent_list[v] =
                    separated_dfs_child_list[parent]->insert(separated_dfs_child_list[parent]->end(), v);
            }
        }
  ...
  private:
        ...
        for (boost::tie(oi, oi_end) = out_edges(v, g); oi != oi_end; ++oi)
        {
            ...
            while (true)
            {
                ...
                if (walkup_itr == walkup_end)
                {
                    ...
                    visited[dfs_child_handles[dfs_child].first_vertex()] = timestamp;
                    visited[dfs_child_handles[dfs_child].second_vertex()] = timestamp;

                    if (low_point[dfs_child] < dfs_number[v] ||
                        least_ancestor[dfs_child] < dfs_number[v])
                    {
                        pertinent_roots[parent]->push_back(dfs_child_handles[dfs_child]);
                    }
                    else
                    {
                        pertinent_roots[parent]->push_front(dfs_child_handles[dfs_child]);
                    }
    ...
    bool walkdown(vertex_t v)
    {
        ...
        while (!pertinent_roots[v]->empty())
        {
            ...
            while (true)
            {
                ...
                first_face_itr = typename face_vertex_iterator<>::type(curr_face_handle, face_handles, first_side());
                second_face_itr = typename face_vertex_iterator<>::type(curr_face_handle, face_handles, second_side());
                ...
                else
                {
                    ...
                    if (first_side_vertex == second_side_vertex)
                    {
                        if (first_tail != v)
                        {
                            vertex_t first = face_handles[first_tail].first_vertex();
                            vertex_t second = face_handles[first_tail].second_vertex();
                            boost::tie(first_side_vertex, first_tail) = make_tuple(first_tail,
                                                                                   first == first_side_vertex ? second : first);
                        }
                        else if (second_tail != v)
                        {
                            vertex_t first = face_handles[second_tail].first_vertex();
                            vertex_t second = face_handles[second_tail].second_vertex();
                            boost::tie(second_side_vertex, second_tail) = make_tuple(second_tail,
                                                                                     first == second_side_vertex ? second : first);
                        }
                        else
                        {
                            break;
                    ...
                    canonical_dfs_child[first_side_vertex] = canonical_dfs_child[root_face_handle.first_vertex()];
                    canonical_dfs_child[second_side_vertex] = canonical_dfs_child[root_face_handle.second_vertex()];
                ...
                {
                    merge_stack.push_back(make_tuple(chosen, chose_first_upper_path, chose_first_lower_path));
                ...
                while (!merge_stack.empty())
                {
                    ...
                    boost::tie(merge_point,
                               ...
                               top_path_follows_first) = merge_stack.back();
                    merge_stack.pop_back();

                    face_handle_t top_handle(face_handles[merge_point]);
                    face_handle_t bottom_handle(*pertinent_roots[merge_point]->begin());
                ...
                canonical_dfs_child[w] = canonical_dfs_child[root_face_handle.first_vertex()];
    ...
    {
        for (typename std::vector<vertex_t>::iterator mp_itr = current_merge_points.begin();
        ...
        for (boost::tie(xi, xi_end) = vertices(g); xi != xi_end; ++xi)
        {
            if (!separated_dfs_child_list[*xi]->empty())
            {
                     ...
                     yi != yi_end; ++yi)
                {
                    ...
                    face_handles[*xi].glue_first_to_second(dfs_child_handles[*yi]);
                }
            }
        }
    ...
    {
        typename vertex_list_t::iterator to_delete = separated_node_in_parent_list[v];
  ...
  public:
                               ...
                               EdgeContainer &path_edges)
    {
        ...
        for (boost::tie(oi, oi_end) = out_edges(v, g); oi != oi_end; ++oi)
        {
            ...
            edge_t e(*oi);
            current_endpoint = target(*oi, g) == v ? source(*oi, g) : target(*oi, g);
            ...
            while (true)
            {
                       ...
                       !forbidden_edge[*walkup_itr])
                {
                    ...
                    current_endpoint =
                        source(f, g) == current_endpoint ? target(f, g) : source(f, g);
                    ++walkup_itr;
                }
                ...
                walkup_itr = walkup_itr_t(current_endpoint, face_handles, first_side());
            }
    ...
    void extract_kuratowski_subgraph(OutputIterator o_itr, EdgeIndexMap em)
    {
        ...
        typedef iterator_property_map<typename std::vector<bool>::iterator, EdgeIndexMap>
            edge_to_bool_map_t;
        ...
        vertex_to_bool_map_t upper_face_vertex(upper_face_vertex_vector.begin(), vm);

        std::vector<bool> lower_face_vertex_vector(num_vertices(g), false);
        vertex_to_bool_map_t lower_face_vertex(lower_face_vertex_vector.begin(), vm);
        ...
        v_dfchild_handle = dfs_child_handles[canonical_dfs_child[previous_vertex]];
        ...
        for (face_itr = x_lower_itr; *face_itr != y; ++face_itr)
        {
            ...
            if (w == graph_traits<Graph>::null_vertex()) // haven't found a w yet
            {
                ...
                {
                    if (low_point[canonical_dfs_child[roots_itr->first_vertex()]] < dfs_number[v])
                    {
                        w = current_vertex;
                        w_handle = *roots_itr;
                        break;
        ...
        {
            edge_t e(*ei);
            goal_edge[e] = !outer_face_edge[e] && (source(e, g) == x || target(e, g) == x);
        ...
        {
            edge_t e(*ei);
            goal_edge[e] = !outer_face_edge[e] && (source(e, g) == y || target(e, g) == y);
            forbidden_edge[*ei] = outer_face_edge[*ei];
        }
        ...
        else if (w != graph_traits<Graph>::null_vertex())
        {
                 ...
                 z_v_path.back() == w_handle.first_edge()))
            {
                ...
                vertex_t anchor = source(final_edge, g) == w_handle.get_anchor() ? target(final_edge, g) : source(final_edge, g);
        ...
        else
        {
            ...
            typedef typename face_edge_iterator<single_side, previous_iteration>::type old_face_iterator_t;
            ...
            vertex_to_bool_map_t x_y_path_vertex(x_y_path_vertex_vector.begin(), vm);
                 ...
                 of_itr != of_itr_end; ++of_itr)
            {
                ...
                for (; old_face_itr != old_face_end; ++old_face_itr)
                {
                    ...
                    previous_vertex = current_vertex;
                    current_vertex = source(e, g) == current_vertex ? target(e, g) : source(e, g);
                 ...
                 v_edge_itr != v_edge_end; ++v_edge_itr)
            {
                ...
                vertex_t current_vertex = source(embedded_edge, g) == v ? target(embedded_edge, g) : source(embedded_edge, g);
                ...
                {
                    internal_face_itr = typename face_edge_iterator<>::type(current_vertex, face_handles, second_side());
                }
                else
                {
                    internal_face_itr = typename face_edge_iterator<>::type(current_vertex, face_handles, first_side());
                }
        ...
        else if (chosen_case == detail::BM_CASE_C)
        {
            ...
            {
                face_itr = typename face_vertex_iterator<single_side, follow_visitor>::type(v_dfchild_handle.first_vertex(), face_handles, second_side());
            }
            else
            {
                face_itr = typename face_vertex_iterator<single_side, follow_visitor>::type(v_dfchild_handle.first_vertex(), face_handles, first_side());
            }
        ...
        else if (chosen_case == detail::BM_CASE_E)
        {
                ...
                (second_x_y_path_endpoint != x && second_x_y_path_endpoint != y))
            {
                ...
                for (face_itr = typename face_edge_iterator<>::type(deletion_endpoint, face_handles, first_side());
                ...
                {
                    is_in_subgraph[face_handles[deletion_endpoint].first_edge()] = false;
                }
                else
                {
                    is_in_subgraph[face_handles[deletion_endpoint].second_edge()] = false;
                }
            }
        }
