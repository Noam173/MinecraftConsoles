...
struct lazy_list_node
{
    ...
    lazy_list_node(const DataType &data) : m_reversed(false),
                                           m_data(data),
                                           m_has_data(true)
    {
    }

    lazy_list_node(ptr_t left_child, ptr_t right_child) : m_reversed(false),
                                                          m_has_data(false),
...
struct face_handle_impl
{
    ...
    face_handle_impl() : cached_first_vertex(graph_traits<Graph>::null_vertex()),
...
class face_handle
{
  public:
    typedef typename graph_traits<Graph>::vertex_descriptor vertex_t;
    typedef typename graph_traits<Graph>::edge_descriptor edge_t;
    typedef face_handle_impl<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy> impl_t;
    typedef face_handle<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy> self_t;

    face_handle(vertex_t anchor = graph_traits<Graph>::null_vertex()) : pimpl(new impl_t())
    {
        pimpl->anchor = anchor;
    }

    face_handle(vertex_t anchor, edge_t initial_edge, const Graph &g) : pimpl(new impl_t())
    {
