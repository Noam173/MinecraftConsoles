...
class format_info
{
  ...
  private:
    ...
    {
        static const int stream_index[number_of_manipulators] = {std::ios::xalloc(), std::ios::xalloc(), std::ios::xalloc()};
    ...
    std::basic_istream<CharType, CharTrait> &is, format_info::manipulator_type del)
{
    ...
    const bool is_delimiter = !std::use_facet<std::ctype<CharType>>(is.getloc()).is(std::ctype_base::space, d);
#else
    const bool is_delimiter = (!std::isspace(d, is.getloc()));
#endif
...
template <class CharType, class CharTrait>
inline std::basic_istream<CharType, CharTrait> &
read(std::basic_istream<CharType, CharTrait> &i, const null_type &)
{
    return i;
}
