...
inline const null_type cnull_type()
{
    return null_type();
}
...
template <typename T>
struct add_const_reference : add_reference<typename add_const<T>::type>
{
};
...
struct cons
{
  ...
  public:
    ...
    head_cref get_head() const
    {
        return head;
    }
    tail_cref get_tail() const
    {
        return tail;
    }
...
#else
    template <typename T>
    explicit cons(head_cref h, const T &t) : head(h), tail(t.head, t.tail)
    {
    }

    explicit cons(head_cref h = head_type(),
                  tail_cref t = tail_type()) : head(h), tail(t)
    {
    }
#endif
    ...
    template <class U>
    cons(const U &u)
        : head(detail::init_head(u, 0)), tail(detail::init_tail<Tail>::execute(u, 0L))
    {
    }
...
struct build_cons
{
  private:
    enum
    {
        tail_is_null_type = is_null_type<Tail>::RET
    };
...
struct element_ref
{
  private:
    ...
    enum
    {
        is_ref = is_reference<elt_type>::value
    };
...
template <
    ...
    typename T9,
    typename T10>
class tuple : public detail::map_tuple_to_cons<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>::cons1
{
  private:
  ...
  public:
    ...
    tuple(
        ...
        t8_cref t8 = T8(),
        t9_cref t9 = T9(),
        t10_cref t10 = T10()) : cons1(t1, cons2(t2, cons3(t3, cons4(t4, cons5(t5, cons6(t6, cons7(t7, cons8(t8, cons9(t9, cons10(t10))))))))))
    {
    }
    ...
    template <typename Head, typename Tail>
    tuple(const cons<Head, Tail> &other) : cons1(other.head, other.tail)
    {
    }
...
template <typename T1>
inline tuple<detail::assign_to_pointee<T1>>
tie(T1 &t1)
{
