...
struct hull_to_geometry
{
    ...
    {
        hull_insert<
            geometry::point_order<OutputGeometry>::value>::apply(geometry,
                                                                 std::back_inserter(
                                                                     // Handle linestring, ring and polygon the same:
                                                                     detail::as_range<
                                                                         typename range_type<OutputGeometry>::type>(out)),
                                                                 strategy);
    }
};
...
struct convex_hull<Box, box_tag>
{
    ...
    static inline void apply(Box const &box, OutputGeometry &out,
                             Strategy const &)
    {
        static bool const Close = geometry::closure<OutputGeometry>::value == closed;
        static bool const Reverse = geometry::point_order<OutputGeometry>::value == counterclockwise;
                                         ...
                                         OutputIterator out, Strategy const &strategy)
{
    ...
    return dispatch::convex_hull_insert<
        geometry::point_order<Geometry>::value>::apply(geometry, out, strategy);
}
