...
static inline bool is_entering(Turn const & /* TODO remove this parameter */, Operation const &op)
{
    ...
    return op.operation == operation_intersection || op.operation == operation_continue || op.operation == operation_blocked;
}
                                   ...
                                   LineString const &linestring, Polygon const &polygon)
{
    ...
    detail::point_on_border::midpoint_helper<
        point_type,
        0, dimension<point_type>::value>::apply(point_in_between, linestring[op.seg_id.segment_index], turn.point);
                              ...
                              LineString const &linestring, Polygon const &polygon)
{
    ...
    {
        return entered || turn.method == method_crosses || (first && last_covered_by(turn, op, linestring, polygon));
    }
    return false;
}
