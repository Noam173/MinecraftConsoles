...
struct ring_identifier
{
    ...
    inline ring_identifier(int src, int mul, int rin)
        : source_index(src), multi_index(mul), ring_index(rin)
    {
    }
    ...
    inline bool operator==(ring_identifier const &other) const
    {
        return source_index == other.source_index && ring_index == other.ring_index && multi_index == other.multi_index;
    }
    ...
    friend std::ostream &operator<<(std::ostream &os, ring_identifier const &ring_id)
    {
        ...
        if (ring_id.ring_index >= 0)
        {
            os << ", r:" << ring_id.ring_index;
        }
        if (ring_id.multi_index >= 0)
        {
            os << ", m:" << ring_id.multi_index;
        }
        os << ")";
        return os;
    }
