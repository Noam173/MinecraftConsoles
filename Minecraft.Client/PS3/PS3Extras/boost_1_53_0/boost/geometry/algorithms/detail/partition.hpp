...
inline void divide_box(Box const &box, Box &lower_box, Box &upper_box)
{
    ...
    ctype two = 2;
    ctype mid = (geometry::get<min_corner, Dimension>(box) + geometry::get<max_corner, Dimension>(box)) / two;
                                       ...
                                       index_vector_type &exceeding)
{
    typedef boost::range_iterator<
        index_vector_type const>::type index_iterator_type;
...
class partition_two_collections
{
                                  ...
                                  Policy &policy, VisitBoxPolicy &box_policy)
    {
        ...
        {
            if (std::size_t(boost::size(input1)) > min_elements && std::size_t(boost::size(input2)) > min_elements && level < 100)
            {
...
class partition
{
    ...
    {
        std::size_t index = 0;
        for (typename boost::range_iterator<InputCollection const>::type it = boost::begin(collection);
  ...
  public:
                             ...
                             VisitBoxPolicy box_visitor = visit_no_policy())
    {
        if (std::size_t(boost::size(collection)) > min_elements)
        {
            ...
            detail::partition::partition_one_collection<
                0, Box,
                OverlapsPolicy,
                VisitBoxPolicy>::apply(total, collection, index_vector, 0, min_elements,
                                       visitor, box_visitor);
        }
        else
        {
            typedef typename boost::range_iterator<
                InputCollection const>::type iterator_type;
    ...
    static inline void apply(InputCollection const &collection1,
                             ...
                             VisitBoxPolicy box_visitor = visit_no_policy())
    {
        if (std::size_t(boost::size(collection1)) > min_elements && std::size_t(boost::size(collection2)) > min_elements)
        {
            ...
            detail::partition::partition_two_collections<
                0, Box, OverlapsPolicy, VisitBoxPolicy>::apply(total,
