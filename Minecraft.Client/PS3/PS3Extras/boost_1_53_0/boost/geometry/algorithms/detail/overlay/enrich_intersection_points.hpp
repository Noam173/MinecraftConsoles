...
struct indexed_turn_operation
{
    ...
    inline indexed_turn_operation(int i, int oi, TurnOperation const &s)
        : index(i), operation_index(oi), discarded(false), subject(s)
    {
    }
};
...
{
    inline sort_on_segment_and_distance(TurnPoints const &turn_points, Geometry1 const &geometry1, Geometry2 const &geometry2, Strategy const &strategy, bool *clustered)
        : m_turn_points(turn_points), m_geometry1(geometry1), m_geometry2(geometry2), m_strategy(strategy), m_clustered(clustered)
    {
    }
  ...
  private:
                                        ...
                                        Indexed const &right) const
    {
        ...
        int const order = get_relative_order<
            point_type>::apply(pi, pj, ri, rj, si, sj);
        // debug("r/o", order == -1);
        return order == -1;
    }
  ...
  public:
    ...
    inline bool operator()(Indexed const &left, Indexed const &right) const
    {
        ...
        if (sl == sr && geometry::math::equals(left.subject.enriched.distance, right.subject.enriched.distance))
        {
            // Both left and right are located on the SAME segment.

            // First check "real" intersection (crosses)
            // -> distance zero due to precision, solve it by sorting
            if (m_turn_points[left.index].method == method_crosses && m_turn_points[right.index].method == method_crosses)
            {
                        ...
                        Strategy const &strategy)
{
    ...
    std::sort(boost::begin(operations),
              ...
              sort_on_segment_and_distance<
                  ...
                  Geometry1, Geometry2,
                  Reverse1, Reverse2,
                  Strategy>(turn_points, geometry1, geometry2, strategy, &clustered));
    ...
    if (clustered)
    {
             ...
             prev = it++)
        {
            ...
            if (prev_op.seg_id == op.seg_id && (turn_points[prev->index].method != method_crosses || turn_points[it->index].method != method_crosses) && geometry::math::equals(prev_op.enriched.distance, op.enriched.distance))
            {
                          ...
                          Strategy const &)
{
    ...
    if (operations.size() > 0)
    {
        ...
        for (iterator_type prev = it + (boost::size(operations) - 1);
             ...
             prev = it++)
        {
            operations_type &prev_op = turn_points[prev->index].operations[prev->operation_index];
            operations_type &op = turn_points[it->index].operations[it->operation_index];

            prev_op.enriched.travels_to_ip_index = it->index;
            prev_op.enriched.travels_to_vertex_index = it->subject.seg_id.segment_index;
                                       ...
                                       Strategy const &strategy)
{
    ...
    for (typename mapped_vector_type::iterator mit = mapped_vector.begin();
