...
struct assign_visitor
{
    ...
    inline assign_visitor(Geometry1 const &g1, Geometry2 const &g2, Collection const &c,
        ...
        : m_geometry1(g1), m_geometry2(g2), m_collection(c), m_ring_map(map), m_check_for_orientation(check)
    {
    }
    ...
    inline void apply(Item const &outer, Item const &inner, bool first = true)
    {
        ...
        if (math::larger(outer.real_area, 0))
        {
            if (inner.real_area < 0 || m_check_for_orientation)
            {
                ...
                if (geometry::within(inner_in_map.point, outer.envelope) && within_selected_input(inner_in_map, outer.id, m_geometry1, m_geometry2, m_collection))
                {
                    // Only assign parent if that parent is smaller (or if it is the first)
                    if (inner_in_map.parent.source_index == -1 || outer.abs_area < inner_in_map.parent_area)
                    {
                           ...
                           bool check_for_orientation = false)
{
...

    {
        ...
        geometry::partition<
            box_type, ring_info_helper_get_box, ring_info_helper_ovelaps_box>::apply(vector, visitor);
