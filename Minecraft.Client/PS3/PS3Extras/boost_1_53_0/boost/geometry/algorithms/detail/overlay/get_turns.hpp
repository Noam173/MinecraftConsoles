...
class get_turns_in_sections
{
    typedef typename closeable_view<
        ...
        closure<Geometry1>::value>::type cview_type1;
    typedef typename closeable_view<
        typename range_type<Geometry2>::type const,
        closure<Geometry2>::value>::type cview_type2;

    typedef typename reversible_view<
        cview_type1 const,
        Reverse1 ? iterate_reverse : iterate_forward>::type view_type1;
    typedef typename reversible_view<
        cview_type2 const,
        Reverse2 ? iterate_reverse : iterate_forward>::type view_type2;

    typedef typename boost::range_iterator<
        view_type1 const>::type range1_iterator;

    typedef typename boost::range_iterator<
        view_type2 const>::type range2_iterator;
  ...
  public:
        ...
        InterruptPolicy &interrupt_policy)
    {
        ...
        bool const same_source =
            source_id1 == source_id2 && sec1.ring_id.multi_index == sec2.ring_id.multi_index && sec1.ring_id.ring_index == sec2.ring_id.ring_index;
             ...
             ++prev1, ++it1, ++index1, ++next1, ++ndi1)
        {
                 ...
                 ++prev2, ++it2, ++index2, ++next2, ++ndi2)
            {
                ...
                if (skip)
                {
                    ...
                    skip = (skip_larger && index1 >= index2) || ndi2 == ndi1 + 1 || neighbouring<Geometry1>(sec1, index1, index2);
                }
  ...
  private:
    ...
    {
        return (dir == 1 && get<Dim>(point) < get<min_corner, Dim>(box)) || (dir == -1 && get<Dim>(point) > get<max_corner, Dim>(box));
    }

    template <size_t Dim, typename Point, typename Box>
    static inline bool exceeding(int dir, Point const &point, Box const &box)
    {
        return (dir == 1 && get<Dim>(point) > get<max_corner, Dim>(box)) || (dir == -1 && get<Dim>(point) < get<min_corner, Dim>(box));
    }
                                                     ...
                                                     RangeIterator const &it, Section const &section)
    {
        ...
        std::size_t check = 0;
        while (!detail::disjoint::disjoint_point_point(*it, *next) && check++ < section.range_count)
        {
            next++;
        }
    }
...
struct section_visitor
{
    ...
    section_visitor(int id1, Geometry1 const &g1,
        ...
        : m_source_id1(id1), m_geometry1(g1), m_source_id2(id2), m_geometry2(g2), m_turns(turns), m_interrupt_policy(ip)
    {
    }
    ...
    inline bool apply(Section const &sec1, Section const &sec2)
    {
        if (!detail::disjoint::disjoint_box_box(sec1.bounding_box, sec2.bounding_box))
        {
            return get_turns_in_sections<
                ...
                Turns,
                TurnPolicy,
                InterruptPolicy>::apply(m_source_id1, m_geometry1, sec1,
...
class get_turns_generic
{
  ...
  public:
        ...
        Turns &turns, InterruptPolicy &interrupt_policy)
    {
        ...
        geometry::partition<
            box_type, get_section_box, ovelaps_section_box>::apply(sec1, sec2, visitor);
    }
};
...
struct get_turns_cs
{
    ...
    typedef typename closeable_view<
        Range const,
        closure<Range>::value>::type cview_type;

    typedef typename reversible_view<
        cview_type const,
        ReverseRange ? iterate_reverse : iterate_forward>::type view_type;

    typedef typename boost::range_iterator<
        view_type const>::type iterator_type;
  ...
  private:
    ...
    static inline int get_side(Box const &box, Point const &point)
    {
        ...
        if (geometry::math::equals(c, left))
        {
            return -2;
        }
        else if (geometry::math::equals(c, right))
        {
            return 2;
        }
        else if (c < left)
...
struct get_turns_polygon_cs
{
        ...
        int multi_index = -1)
    {
        ...
        typename interior_return_type<Polygon const>::type rings = interior_rings(polygon);
...
struct get_turns<
    ...
    Turns,
    TurnPolicy,
    InterruptPolicy> : detail::get_turns::get_turns_polygon_cs<Polygon, Box,
                                                               ReversePolygon, ReverseBox,
                                                               Turns, TurnPolicy, InterruptPolicy>
{
};
...
struct get_turns<
    ...
    Turns,
    TurnPolicy,
    InterruptPolicy> : detail::get_turns::get_turns_cs<Ring, Box, ReverseRing, ReverseBox,
                                                       Turns, TurnPolicy, InterruptPolicy>
...
struct get_turns_reversed
{
        ...
        Turns &turns, InterruptPolicy &interrupt_policy)
    {
        get_turns<
            ...
            Reverse2, Reverse1,
            Turns, TurnPolicy,
            InterruptPolicy>::apply(source_id2, g2, source_id1, g1, turns, interrupt_policy);
    }
};
                      ...
                      InterruptPolicy &interrupt_policy)
{
    ...
    typedef typename strategy_intersection<
        ...
        Geometry1,
        Geometry2,
        typename boost::range_value<Turns>::type>::segment_intersection_strategy_type segment_intersection_strategy_type;
    ...
    boost::mpl::if_c<
        ...
        dispatch::get_turns<
            ...
            Reverse1, Reverse2,
            Turns, TurnPolicy,
            InterruptPolicy>>::type::apply(0, geometry1,
                                           1, geometry2,
                                           turns, interrupt_policy);
}
