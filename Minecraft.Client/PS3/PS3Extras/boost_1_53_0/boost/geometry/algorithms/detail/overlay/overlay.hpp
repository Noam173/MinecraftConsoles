...
{
    return (turn_info.discarded || turn_info.both(operation_union)) && !turn_info.any_blocked() && !turn_info.both(operation_intersection);
}
                                                   ...
                                                   OutputIterator out)
{
    ...
    if (Direction == overlay_intersection || (Direction == overlay_difference && geometry::num_points(geometry1) == 0))
    {
        return out;
    }
        ...
        Strategy const &)
    {
        ...
        if (geometry::num_points(geometry1) == 0 || geometry::num_points(geometry2) == 0)
        {
            return return_if_one_input_is_empty<
                GeometryOut, Direction, ReverseOut>(geometry1, geometry2, out);
        }
        ...
        geometry::get_turns<
            Reverse1, Reverse2,
            detail::overlay::calculate_distance_policy>(geometry1, geometry2, turn_points, policy);
