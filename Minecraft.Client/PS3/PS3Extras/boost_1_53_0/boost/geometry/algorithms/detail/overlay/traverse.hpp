...
#if defined(BOOST_GEOMETRY_DEBUG_INTERSECTION) || defined(BOOST_GEOMETRY_OVERLAY_REPORT_WKT) || defined(BOOST_GEOMETRY_DEBUG_TRAVERSE)
...
inline void set_visited_for_continue(Info &info, Turn const &turn)
{
    ...
    if (turn.operation == detail::overlay::operation_continue)
    {
        for (typename boost::range_iterator<
                 typename Info::container_type>::type it = boost::begin(info.operations);
                           ...
                           segment_identifier &seg_id)
{
    ...
    {
        if (info.enriched.travels_to_vertex_index < 0 || info.enriched.travels_to_ip_index < 0)
        {
            return false;
        }
...
{
    return (operation == operation_intersection && source1 != source2) || (operation == operation_union && source1 == source2);
}
                           ...
                           Iterator &selected)
{
         ...
         ++it)
    {
        ...
        //           take that one.
        if ((it->operation == operation_continue && (!has_tp || it->visited.started())) || (it->operation == operation && !it->visited.finished() && (!has_tp || select_source(operation, it->seg_id.source_index, seg_id.source_index))))
        {
            selected = it;
  ...
  public:
                             ...
                             Turns &turns, Rings &rings)
    {
        ...
        typedef typename boost::range_iterator<
            typename turn_type::container_type>::type turn_operation_iterator_type;

        std::size_t const min_num_points = core_detail::closure::minimum_ring_size<
            geometry::closure<ring_type>::value>::value;
                 ...
                 ++it)
            {
                ...
                if (!(it->is_discarded() || it->blocked()))
                {
                    for (turn_operation_iterator_type iit = boost::begin(it->operations);
                         ...
                         ++iit)
                    {
                        if (iit->visited.none() && !iit->visited.rejected() && (iit->operation == operation || iit->operation == detail::overlay::operation_continue))
                        {
