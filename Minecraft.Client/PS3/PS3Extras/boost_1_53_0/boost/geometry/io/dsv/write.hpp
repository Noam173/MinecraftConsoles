...
struct dsv_settings
{
    ...
    dsv_settings(std::string const &sep, std::string const &open, std::string const &close, std::string const &psep, std::string const &lopen, std::string const &lclose, std::string const &lsep)
        : coordinate_separator(sep), point_open(open), point_close(close), point_separator(psep), list_open(lopen), list_close(lclose), list_separator(lsep)
    {
    }
};
...
struct stream_coordinate
{
                             ...
                             dsv_settings const &settings)
    {
        ...
        stream_coordinate<
            Point, Dimension + 1, Count>::apply(os, point, settings);
    }
};
...
struct stream_indexed
{
                             ...
                             dsv_settings const &settings)
    {
        ...
        stream_indexed<
            Geometry, Index, Dimension + 1, Count>::apply(os, geometry, settings);
    }
};
...
struct dsv_range
{
                             ...
                             dsv_settings const &settings)
    {
             ...
             ++it)
        {
            ...
            stream_coordinate<
                point_type, 0, dimension<point_type>::type::value>::apply(os, *it, settings);
            os << settings.point_close;
...
struct dsv_per_index
{
                             ...
                             dsv_settings const &settings)
    {
        os << settings.point_open;
        stream_indexed<
            Geometry, Index, 0, dimension<Geometry>::type::value>::apply(os, geometry, settings);
        os << settings.point_close;
    }
};
...
class dsv_manipulator
{
  public:
    inline dsv_manipulator(Geometry const &g,
                           dsv_settings const &settings)
        : m_geometry(g), m_settings(settings)
    {
    }
        ...
        dsv_manipulator const &m)
    {
        dispatch::dsv<
            typename tag_cast<
                ...
                multi_tag>::type,
            Geometry>::apply(os, m.m_geometry, m.m_settings);
        os.flush();
        return os;
    }
...
*/
template <typename Geometry>
inline detail::dsv::dsv_manipulator<Geometry> dsv(Geometry const &geometry, std::string const &coordinate_separator = ", ", std::string const &point_open = "(", std::string const &point_close = ")", std::string const &point_separator = ", ", std::string const &list_open = "(", std::string const &list_close = ")", std::string const &list_separator = ", ")
{
