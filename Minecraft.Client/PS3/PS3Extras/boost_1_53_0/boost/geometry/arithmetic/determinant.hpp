...
class calculate_determinant
{
  ...
  public:
    static inline ReturnType apply(U const &ux, U const &uy, V const &vx, V const &vy)
    {
...
template <typename ReturnType, typename U, typename V>
inline ReturnType determinant(U const &ux, U const &uy, V const &vx, V const &vy)
{
    return calculate_determinant<
        ReturnType, U, V>::apply(ux, uy, vx, vy);
}
...
inline ReturnType determinant(U const &u, V const &v)
{
    ...
    return calculate_determinant<
        ...
        typename geometry::coordinate_type<V>::type>::apply(get<0>(u), get<1>(u), get<0>(v), get<1>(v));
}
