...
#define BOOST_PROTO_DEFINE_ENV_VAR(TAG, NAME)                                                                                          \
    ...
    {                                                                                                                                  \
        ...
        typename boost::disable_if_c<                                                                                                  \
            boost::is_const<Value>::value, boost::proto::env<TAG, typename boost::proto::detail::value_type<Value>::type>>::type const \
    ...
    : private Base
{
  ...
  public:
    ...
    struct lookup
        : mpl::if_c<
              is_same<OtherKey, Key>::value, found, typename Base::template lookup<OtherKey, OtherValue>>::type
    {
    };
#else
    /// INTERNAL ONLY
    ...
    : detail::poly_function<has_env_var<Key>>
{
    ...
    struct impl
    {
        typedef mpl::not_<
            is_same<
                typename remove_reference<Env>::type::template lookup<Key>::type, key_not_found>>
            result_type;
    ...
    : detail::poly_function<has_env_var<data_type>>
{
    ...
    struct impl
    {
        typedef mpl::not_<
            is_same<
                typename remove_reference<Env>::type::template lookup<data_type>::type, key_not_found>>
            result_type;
...
inline typename disable_if_c<
    is_const<T>::value, env<T1, V1, BOOST_PROTO_UNCVREF(typename result_of::as_env<T &>::type)>>::type const
operator,
