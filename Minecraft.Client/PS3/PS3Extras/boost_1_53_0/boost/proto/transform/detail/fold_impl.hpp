...
#define BOOST_PROTO_FOLD_STATE_TYPE(Z, N, DATA)                                                    \
    typedef                                                                                        \
        typename when<_, Fun>::template impl<                                                      \
            typename result_of::child_c<Expr, N>::type, BOOST_PP_CAT(state, N), Data>::result_type \
...
#define BOOST_PROTO_FOLD_STATE(Z, N, DATA)                                           \
    ...
    BOOST_PP_CAT(s, BOOST_PP_INC(N)) = typename when<_, Fun>::template impl<         \
        typename result_of::child_c<Expr, N>::type, BOOST_PP_CAT(state, N), Data>()( \
        proto::child_c<N>(e), BOOST_PP_CAT(s, N), d);                                \
    /**/
    ...
    typedef                                                                \
        typename when<_, Fun>::template impl<                              \
            typename result_of::child_c<                                   \
                Expr, BOOST_PP_SUB(DATA, BOOST_PP_INC(N))>::type,          \
...
#define BOOST_PROTO_REVERSE_FOLD_STATE(Z, N, DATA)                                                          \
    ...
    BOOST_PP_CAT(s, BOOST_PP_SUB(DATA, BOOST_PP_INC(N))) = typename when<_, Fun>::template impl<            \
        ...
        proto::child_c<BOOST_PP_SUB(DATA, BOOST_PP_INC(N))>(e), BOOST_PP_CAT(s, BOOST_PP_SUB(DATA, N)), d); \
    /**/
