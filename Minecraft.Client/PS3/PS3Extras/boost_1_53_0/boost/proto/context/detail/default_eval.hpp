...
struct default_eval<Expr, Context, proto::tag::function, N>
{
    ...
    typedef
        typename BOOST_PROTO_RESULT_OF<
            function_type(BOOST_PP_ENUM_SHIFTED(N, BOOST_PROTO_DEFAULT_EVAL_TYPE, Expr))>::type
            result_type;
  ...
  private:
    ...
    result_type invoke(Expr &expr, Context &context, mpl::true_) const
    {
        ...
        return (
            BOOST_PROTO_GET_POINTER(class_type, (BOOST_PROTO_DEFAULT_EVAL(~, 1, expr)))->*BOOST_PROTO_DEFAULT_EVAL(~, 0, expr))(BOOST_PP_ENUM(BOOST_PP_SUB(N, 2), BOOST_PROTO_DEFAULT_EVAL_SHIFTED, expr));
    }
};
