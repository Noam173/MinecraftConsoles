...
{
template <
    typename Expr, typename Context, typename Tag // = typename Expr::proto_tag
    ,
...
#define BOOST_PROTO_BINARY_DEFAULT_EVAL(OP, TAG, LMAKE, RMAKE)                              \
    ...
    {                                                                                       \
      ...
      public:                                                                               \
        ...
        {                                                                                   \
            return proto::eval(                                                             \
                proto::child_c<0>(expr), ctx) OP proto::eval(proto::child_c<1>(expr), ctx); \
        }                                                                                   \
    };                                                                                      \
    /**/
...
struct default_eval<Expr, Context, proto::tag::function, 1>
{
    typedef
        typename proto::detail::result_of_fixup<
            BOOST_PROTO_DEFAULT_EVAL_TYPE(~, 0, Expr)>::type
            function_type;
...
struct default_eval<Expr, Context, proto::tag::function, 2>
{
    ...
    typedef
        typename detail::result_of_<
            function_type(BOOST_PROTO_DEFAULT_EVAL_TYPE(~, 1, Expr))>::type
            result_type;

    result_type operator()(Expr &expr, Context &context) const
    {
        return this->invoke(
            expr, context, is_member_function_pointer<function_type>(), is_member_object_pointer<function_type>());
    }
  ...
  private:
    ...
    result_type invoke(Expr &expr, Context &context, mpl::true_, mpl::false_) const
    {
        ...
        return (
            BOOST_PROTO_GET_POINTER(class_type, (BOOST_PROTO_DEFAULT_EVAL(~, 1, expr)))->*BOOST_PROTO_DEFAULT_EVAL(~, 0, expr))();
    }
