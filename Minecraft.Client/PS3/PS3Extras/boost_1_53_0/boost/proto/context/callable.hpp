...
/// \endcode
template <
    typename Expr, typename Context, long Arity // = Expr::proto_arity_c
    >
...
struct callable_eval<Expr, Context, 0>
{
    ...
    typedef
        typename BOOST_PROTO_RESULT_OF<
            Context(typename Expr::proto_tag, value_type)>::type
            result_type;
...
/// \endcode
template <
    typename Context, typename DefaultCtx // = default_context
    >
struct callable_context
{
    ...
    struct eval
        : mpl::if_c<
              detail::is_expr_handled<Expr, Context>::value, callable_eval<Expr, ThisContext>, typename DefaultCtx::template eval<Expr, Context>>::type
    {
    };
};
} // namespace context
