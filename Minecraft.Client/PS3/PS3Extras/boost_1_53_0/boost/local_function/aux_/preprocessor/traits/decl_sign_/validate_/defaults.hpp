...
#define BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_SIGN_VALIDATE_DEFAULTS_IS_UNBIND_( \
    ...
    BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_DETAIL_PP_KEYWORD_IS_DEFAULT_FRONT(sign),    \
                 0, BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_DETAIL_PP_KEYWORD_IS_CONST_BIND_FRONT(sign), 0, BOOST_PP_IIF(BOOST_LOCAL_FUNCTION_DETAIL_PP_KEYWORD_IS_BIND_FRONT(sign), 0, 1)))
...
#define BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_SIGN_VALIDATE_DEFAULTS_INDEX_(        \
    ...
    BOOST_PP_IF(index, /* can't use IIF because index can be any number */            \
                ...
                ERROR_default_value_cannot_be_specified_as_the_first_element          \
                    BOOST_PP_EMPTY /* because error might not be present */           \
                        BOOST_PP_TUPLE_EAT(3))(sign, index, error)
...
#define BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_SIGN_VALIDATE_DEFAULTS_PRED_DATA_( \
    sign, index, error)                                                            \
    BOOST_PP_BITAND(                                                               \
        BOOST_PP_IS_EMPTY(error(/* expand empty */)), BOOST_PP_LESS(index, BOOST_PP_LIST_SIZE(sign)))
...
#define BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_SIGN_VALIDATE_DEFAULTS(sign) \
    BOOST_PP_TUPLE_ELEM(3, 2, BOOST_PP_WHILE(BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_SIGN_VALIDATE_DEFAULTS_PRED_, BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_SIGN_VALIDATE_DEFAULTS_OP_, (sign, 0, BOOST_PP_EMPTY)))
