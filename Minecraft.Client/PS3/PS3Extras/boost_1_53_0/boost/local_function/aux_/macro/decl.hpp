...
#define BOOST_LOCAL_FUNCTION_AUX_DECL_ERROR_(id, typename01, decl_traits)                                                                                                         \
    BOOST_PP_IIF(BOOST_PP_LIST_IS_CONS(                                                                                                                                           \
                     BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_RETURNS(decl_traits)),                         /* return specified, so no result type before this macro expansion */ \
                 BOOST_PP_TUPLE_EAT(1), /* even if error, must declare result type to prevent additional */ /* error due to result type appearing before this macro expansion */  \
                 BOOST_LOCAL_FUNCTION_AUX_CODE_RESULT_DECL)(id);                                            /* close eventual previous statements, otherwise it has no effect */  \
    BOOST_MPL_ASSERT_MSG(false,                                                                             /* always fails (there's an error) */                                 \
                         BOOST_LOCAL_FUNCTION_AUX_PP_DECL_TRAITS_ERROR_MSG(decl_traits), ());               /* must close ASSERT macro for eventual use within class scope */
...
#define BOOST_LOCAL_FUNCTION_AUX_DECL(id, typename01, decl_traits)                    \
    BOOST_PP_IIF(BOOST_PP_IS_EMPTY(                                                   \
                 ...
                 BOOST_LOCAL_FUNCTION_AUX_DECL_ERROR_)(id, typename01, decl_traits)
