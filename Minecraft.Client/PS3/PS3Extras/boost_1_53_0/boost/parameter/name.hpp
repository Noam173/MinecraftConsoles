...
#if !defined(BOOST_NO_SFINAE) && !defined(BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION) && !BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x592))
...
struct lambda<
    T, typename boost::enable_if<
           parameter::aux::is_name_tag<T>, parameter::aux::lambda_tag>::type>
{
    typedef true_ is_le;
...
#define BOOST_PARAMETER_NAME_OBJECT(tag, name) \
    static ::boost::parameter::keyword<tag> const &name = ::boost::parameter::keyword<tag>::instance;
#else
#define BOOST_PARAMETER_NAME_OBJECT(tag, name)                                                 \
    namespace                                                                                  \
    {                                                                                          \
    ::boost::parameter::keyword<tag> const &name = ::boost::parameter::keyword<tag>::instance; \
    }
#endif
...
#define BOOST_PARAMETER_COMPLEX_NAME(name)                                                                                \
    BOOST_PARAMETER_BASIC_NAME(                                                                                           \
        BOOST_PARAMETER_COMPLEX_NAME_NAMESPACE(name), BOOST_PP_TUPLE_EAT(2) name, BOOST_PARAMETER_COMPLEX_NAME_TAG(name)) \
    /**/
...
#define BOOST_PARAMETER_NAME(name) \
    BOOST_PP_IIF(                  \
        BOOST_PARAMETER_IS_BINARY(name), BOOST_PARAMETER_COMPLEX_NAME, BOOST_PARAMETER_SIMPLE_NAME)(name) /**/
