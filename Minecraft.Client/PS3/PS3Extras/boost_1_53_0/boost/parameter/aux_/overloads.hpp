...
#define BOOST_PARAMETER_arg_list(n) \
    aux::make_arg_list<             \
        BOOST_PP_ENUM(N, BOOST_PARAMETER_open_list, _), void_ BOOST_PP_REPEAT(N, BOOST_PARAMETER_close_list, _), deduced_list, aux::tag_keyword_arg>
...
typename mpl::first<
    typename BOOST_PARAMETER_arg_list(N)::type>::type
    ...
    return result_type(
        BOOST_PP_ENUM(N, BOOST_PARAMETER_arg_pack_init, BOOST_PP_DEC(N))
            BOOST_PP_ENUM_TRAILING_PARAMS(
                BOOST_PP_SUB(BOOST_PARAMETER_MAX_ARITY, N), aux::void_reference() BOOST_PP_INTERCEPT));
}
