...
#define BOOST_PARAMETER_FLATTEN_SPEC0(r, n, elem, data)                                                                 \
    ((                                                                                                                  \
        BOOST_PP_TUPLE_ELEM(3, 2, data), BOOST_PP_TUPLE_REM(BOOST_PP_TUPLE_ELEM(3, 0, data)) elem                       \
                                             BOOST_PP_ENUM_TRAILING(                                                    \
                                                 BOOST_PP_SUB(                                                          \
                                                     BOOST_PP_TUPLE_ELEM(3, 1, data), BOOST_PP_TUPLE_ELEM(3, 0, data)), \
...
#define BOOST_PARAMETER_FLATTEN_SPEC_AUX(r, arity, max_arity, spec, transform) \
    BOOST_PARAMETER_FOR_EACH_R(                                                \
        r, arity, BOOST_PARAMETER_FLATTEN_ARGS(spec), (arity, max_arity, transform(BOOST_PARAMETER_FLATTEN_QUALIFIER(spec))), BOOST_PARAMETER_FLATTEN_SPEC0)
...
#define BOOST_PARAMETER_FLATTEN_SPEC_optional(r, arities, spec) \
    BOOST_PARAMETER_FLATTEN_SPEC_AUX(                           \
        r, BOOST_PP_CAT(BOOST_PARAMETER_FLATTEN_ARITY_, BOOST_PARAMETER_FLATTEN_QUALIFIER(spec))(arities), BOOST_PP_TUPLE_ELEM(3, 2, arities), spec, BOOST_PARAMETER_FLATTEN_IDENTITY)
...
#define BOOST_PARAMETER_FLATTEN_SPEC_deduced_M(r, arities, n, spec) \
    BOOST_PARAMETER_FLATTEN_SPEC_AUX(                               \
        r, BOOST_PP_CAT(BOOST_PARAMETER_FLATTEN_ARITY_, BOOST_PARAMETER_FLATTEN_QUALIFIER(spec))(arities), BOOST_PP_TUPLE_ELEM(3, 2, arities), spec, BOOST_PARAMETER_FLATTEN_SPEC_AS_DEDUCED)

#define BOOST_PARAMETER_FLATTEN_SPEC_deduced(r, arities, spec) \
    BOOST_PP_SEQ_FOR_EACH_I_R(                                 \
        r, BOOST_PARAMETER_FLATTEN_SPEC_deduced_M, arities, BOOST_PARAMETER_FLATTEN_ARGS(spec))

#define BOOST_PARAMETER_FLATTEN_SPEC(r, arities, spec) \
    BOOST_PP_CAT(                                      \
        BOOST_PARAMETER_FLATTEN_SPEC_, BOOST_PARAMETER_FLATTEN_QUALIFIER(spec))(r, arities, spec)

#define BOOST_PARAMETER_FLATTEN(optional_arity, required_arity, wanted_arity, specs) \
    BOOST_PP_SEQ_FOR_EACH(                                                           \
        BOOST_PARAMETER_FLATTEN_SPEC, (optional_arity, required_arity, wanted_arity), specs)
