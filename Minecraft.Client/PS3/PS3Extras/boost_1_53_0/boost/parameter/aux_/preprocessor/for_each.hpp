...
#define BOOST_PARAMETER_FOR_EACH_pred_aux0(n, x)       \
    BOOST_PP_CAT(BOOST_PARAMETER_FOR_EACH_pred_aux, n) \
    x
...
#define BOOST_PARAMETER_FOR_EACH_pred(r, state) \
    BOOST_PARAMETER_FOR_EACH_pred_SPLIT_FIRST(  \
        BOOST_PARAMETER_FOR_EACH_pred_aux0(     \
            BOOST_PP_TUPLE_ELEM(5, 3, state), BOOST_PP_TUPLE_ELEM(5, 0, state)))
#else
#define BOOST_PARAMETER_FOR_EACH_pred(r, state) \
    BOOST_PP_SPLIT(                             \
        0, BOOST_PARAMETER_FOR_EACH_pred_aux0(  \
               BOOST_PP_TUPLE_ELEM(5, 3, state), BOOST_PP_TUPLE_ELEM(5, 0, state)))
#endif
...
#define BOOST_PARAMETER_FOR_EACH_op(r, state)                \
    (                                                        \
        ...
        BOOST_PP_TUPLE_ELEM(5, 1, state), BOOST_PP_TUPLE_ELEM(5, 2, state), BOOST_PP_TUPLE_ELEM(5, 3, state), BOOST_PP_INC(BOOST_PP_TUPLE_ELEM(5, 4, state)))

#define BOOST_PARAMETER_FOR_EACH_macro(r, state) \
    BOOST_PP_TUPLE_ELEM(5, 2, state)(            \
        r, BOOST_PP_TUPLE_ELEM(5, 4, state), BOOST_PARAMETER_FOR_EACH_head(BOOST_PP_TUPLE_ELEM(5, 3, state), BOOST_PP_TUPLE_ELEM(5, 0, state)), BOOST_PP_TUPLE_ELEM(5, 1, state))

#define BOOST_PARAMETER_FOR_EACH_build_end_sentinel(z, n, text) \
    BOOST_PP_COMMA_IF(n)                                        \
    BOOST_PARAMETER_FOR_EACH_END_SENTINEL
...
#define BOOST_PARAMETER_FOR_EACH_R(r, arity, list, data, macro) \
    BOOST_PP_CAT(BOOST_PP_FOR_, r)(                             \
        (list BOOST_PARAMETER_FOR_EACH_build_end_sentinel_tuple(arity), data, macro, arity, 0), BOOST_PARAMETER_FOR_EACH_pred, BOOST_PARAMETER_FOR_EACH_op, BOOST_PARAMETER_FOR_EACH_macro)
