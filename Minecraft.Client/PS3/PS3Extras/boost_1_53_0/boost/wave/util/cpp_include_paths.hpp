...
struct load_filepos
{
    ...
    static unsigned int get_column()
    {
        return 0;
    }
...
//  include path).
inline bool include_paths::add_include_path(
...
inline bool include_paths::find_include_file(std::string &s, std::string &dir,
                                 ...
                                 bool is_system, char const *current_file) const
{
    ...
    if (!is_system)
    {
        ...
        {   // set_sys_include_delimiter() not called
...
inline void save(Archive &ar,
                 const typename boost::wave::util::bidirectional_map<
                     std::string, std::string>::type &t,
                 const unsigned int /* file_version */)
{
    boost::serialization::stl::save_collection<
        Archive,
        typename boost::wave::util::bidirectional_map<
            std::string, std::string>::type>(ar, t);
}
                 ...
                 const unsigned int /* file_version */)
{
    typedef typename boost::wave::util::bidirectional_map<
        std::string, std::string>::type map_type;
