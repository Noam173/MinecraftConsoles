...
struct BOOST_PP_CAT(apply_wrap, i_)
{
    ...
    typedef typename BOOST_PP_CAT(msvc_apply, i_)<F>::template result_<
        AUX778076_APPLY_WRAP_PARAMS(i_, T)>::type type;
};
...
struct BOOST_PP_CAT(apply_wrap, i_)
    : BOOST_PP_CAT(apply_wrap_impl, i_)<
          ::boost::mpl::aux::arity<F, i_>::value, F
                                                      BOOST_PP_COMMA_IF(i_) AUX778076_APPLY_WRAP_PARAMS(i_, T)>::type
{
};
...
#if i_ == 0 && j_ == 0 && defined(BOOST_MPL_CFG_BCC590_WORKAROUNDS) && !defined(BOOST_MPL_CFG_NO_HAS_APPLY)
