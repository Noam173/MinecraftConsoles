...
template <
    typename BOOST_MPL_AUX_NA_PARAM(Sequence), typename BOOST_MPL_AUX_NA_PARAM(T), typename Predicate = less<>, typename pred_ = typename lambda<Predicate>::type>
struct upper_bound
...
template <
    typename Distance, typename Predicate, typename T, typename DeferredIterator>
...
struct upper_bound_step
{
    typedef typename eval_if<
        Distance, upper_bound_step_impl<Distance, Predicate, T, DeferredIterator>, DeferredIterator>::type type;
};
...
struct upper_bound_step_impl
{
    ...
    typedef typename apply2<
        Predicate, T, typename deref<middle_>::type>::type cond_;
    ...
    typedef typename eval_if<
        cond_, step_forward_, step_backward_>::type type;
};

} // namespace aux

template <
    typename BOOST_MPL_AUX_NA_PARAM(Sequence), typename BOOST_MPL_AUX_NA_PARAM(T), typename Predicate = less<>>
struct upper_bound
{
  private:
  ...
  public:
    typedef typename aux::upper_bound_step<
        size_, pred_, T, begin<Sequence>>::type type;
};
