...
struct at_impl<aux::map_tag>
{
    ...
    struct apply
    {
        ...
        typedef typename eval_if<
            is_void_<item_>, void_, second<item_>>::type type;
    };
};
...
struct is_item_masked
{
    BOOST_STATIC_CONSTANT(bool, value =
                                    sizeof(BOOST_MPL_AUX_OVERLOAD_CALL_IS_MASKED(
                                        Map, BOOST_MPL_AUX_STATIC_CAST(long_<order> *, 0))) == sizeof(aux::yes_tag));
};
...
struct item_by_order
{
    typedef typename eval_if_c<
        is_item_masked<Map, order>::value, void_, m_at<Map, (order - 2)>>::type type;
};
