...
struct BOOST_PP_CAT(vector, i_)
    : v_item<
          BOOST_PP_CAT(T, BOOST_PP_DEC(i_)), AUX778076_VECTOR_TAIL(vector, BOOST_PP_DEC(i_), T)>
{
...
struct at_impl<aux::vector_tag<i_>>
{
    ...
    struct apply
    {
        typedef typename aux::v_at_impl<BOOST_MPL_AUX_VALUE_WKND(N)::value>::template result_<V_>::type type;
    };
};
