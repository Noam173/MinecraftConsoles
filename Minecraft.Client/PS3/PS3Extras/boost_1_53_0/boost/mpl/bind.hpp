...
#if !defined(BOOST_MPL_CFG_NO_PREPROCESSED_HEADERS) && !defined(BOOST_MPL_PREPROCESSING_MODE)
...
#define AUX778076_BIND_PARAMS(param)               \
    BOOST_MPL_PP_PARAMS(                           \
        BOOST_MPL_LIMIT_METAFUNCTION_ARITY, param) \
    /**/

#define AUX778076_BIND_DEFAULT_PARAMS(param, value)       \
    BOOST_MPL_PP_DEFAULT_PARAMS(                          \
        BOOST_MPL_LIMIT_METAFUNCTION_ARITY, param, value) \
    /**/
...
struct resolve_arg_impl<true>
{
    ...
    struct result_
    {
        typedef typename AUX778076_APPLY<
            T, AUX778076_BIND_PARAMS(U)>::type type;
    };
};
...
template <
    typename T, AUX778076_BIND_PARAMS(typename U)>
struct resolve_bind_arg
    : resolve_arg_impl<is_bind_template<T>::value>::template result_<T, AUX778076_BIND_PARAMS(U)>
{
};
...
template <BOOST_MPL_AUX_NTTP_DECL(int, arity_)>
struct bind_chooser;
...
template <typename T>
struct is_bind_template
    : is_bind_template_impl<::boost::detail::is_reference_impl<T>::value>::template result_<T>
{
};
...
#if defined(BOOST_NO_TEMPLATE_PARTIAL_SPECIALIZATION) && !defined(BOOST_MPL_CFG_NO_BIND_TEMPLATE)
...
struct bind
    : aux::bind_chooser<
          aux::bind_count_args<AUX778076_BIND_PARAMS(T)>::value>::template result_<F, AUX778076_BIND_PARAMS(T)>::type
{
};
...
template <BOOST_MPL_PP_PARAMS(i_, typename T)>
struct AUX778076_SPEC_NAME;

template <
    typename Tag AUX778076_BIND_N_PARAMS(i_, typename T)>
    struct BOOST_PP_CAT(bind, i_)<
        BOOST_PP_CAT(quote, i_) < AUX778076_SPEC_NAME, Tag> AUX778076_BIND_N_PARAMS(i_, T) >
{
    template <
...
#include BOOST_PP_ITERATE()
        ...
        typedef typename AUX778076_SPEC_NAME<
            typename t1::type, BOOST_MPL_PP_EXT_PARAMS(2, BOOST_PP_INC(i_), t)>::type f_;
...
#define AUX778076_ARG(unused, i_, t) \
    ...
    typename BOOST_PP_CAT(t, BOOST_PP_INC(i_))::type /**/
        ...
        typedef typename BOOST_PP_CAT(apply_wrap, i_)<
            f_
                BOOST_PP_COMMA_IF(i_) BOOST_MPL_PP_REPEAT(i_, AUX778076_ARG, t)>::type type;
