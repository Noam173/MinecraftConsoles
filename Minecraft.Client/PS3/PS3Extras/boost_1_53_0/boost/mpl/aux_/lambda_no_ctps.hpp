...
struct lambda
{
    ...
    typedef typename aux::lambda_impl<arity_>::template result_<T, Tag, Protect> l_;
...
#define AUX778076_LAMBDA_TYPEDEF(unused, i_, F)                       \
    typedef lambda<                                                   \
        typename F::BOOST_PP_CAT(arg, BOOST_PP_INC(i_)), Tag, false_> \
...
struct lambda_impl<int_<i_>>
{
    ...
    struct result_
    {
        ...
        typedef BOOST_PP_CAT(bind, i_)<
            typename F::rebind
                BOOST_MPL_PP_REPEAT(i_, AUX778076_LAMBDA_RESULT, unused)> bind_;

        typedef typename if_<
            is_le, if_<Protect, mpl::protect<bind_>, bind_>, identity<F>>::type type_;
