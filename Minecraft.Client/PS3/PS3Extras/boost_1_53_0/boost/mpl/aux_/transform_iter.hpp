...
template <
    typename Iterator, typename LastIterator, typename F>
struct transform_iter
{
    ...
    typedef typename apply1<
        F, typename deref<base>::type>::type type;
};

template <
    typename LastIterator, typename F>
...
struct transform_iter_impl
{
    ...
    struct result_
    {
        ...
        typedef typename apply1<
            F, typename deref<Iterator>::type>::type type;
    };
};
...
struct transform_iter
    : transform_iter_impl<
          ::boost::is_same<Iterator, LastIterator>::value>::template result_<Iterator, LastIterator, F>
{
};
