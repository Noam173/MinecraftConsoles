...
struct FindAttr
    : or_<
          ...
          when<subscript<_, _>, _state>, when<terminal<_>, _state>, when<proto::assign<terminal<detail::attribute_placeholder<Nbr>>, _>, call<_value(_right)>>, otherwise<fold<_, _state, FindAttr<Nbr>>>>
{
};
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        ...
        typedef
            typename FindAttr<typename expr_type::proto_child0::nbr_type>::template impl<
                State, mpl::void_, int>::result_type
                attr_type;

        typedef
            typename proto::terminal<
                detail::read_attr<
                    typename expr_type::proto_child0::nbr_type, BOOST_PROTO_UNCVREF(attr_type)>>::type
                result_type;
...
struct DeepCopy
    : or_<
          when<terminal<detail::attribute_placeholder<_>>, as_read_attr>, when<terminal<_>, proto::_deep_copy>, otherwise<nary_expr<_, vararg<DeepCopy>>>>
{
};
...
struct MaxAttr
    : or_<
          when<terminal<detail::attribute_placeholder<_>>, attr_nbr>, when<terminal<_>, make<mpl::int_<0>>>
          // Ignore nested actions, because attributes are scoped:
          ,
          when<subscript<_, _>, make<mpl::int_<0>>>, otherwise<fold<_, make<mpl::int_<0>>, max_attr>>>
{
};
...
struct max_attr : proto::transform<max_attr>
{
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        typedef
            typename mpl::max<
                typename impl::state, typename MaxAttr::template impl<Expr, State, Data>::result_type>::type
                result_type;
    };
};
...
struct as_attr_matcher : proto::transform<as_attr_matcher>
{
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        ...
        typedef detail::attr_matcher<
            typename proto::result_of::value<typename expr_type::proto_child1>::type, typename data_type::traits_type, typename data_type::icase_type>
            result_type;

        result_type operator()(
            typename impl::expr_param expr, typename impl::state_param, typename impl::data_param data) const
        {
            return result_type(
                proto::value(proto::left(expr)).nbr(), proto::value(proto::right(expr)), data.traits());
        }
    };
};
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        ...
        typedef
            typename shift_right<
                typename terminal<begin_type>::type, typename shift_right<
                                                         Expr, terminal<detail::attr_end_matcher>::type>::type>::type
                result_type;

        result_type operator()(
            typename impl::expr_param expr, typename impl::state_param, typename impl::data_param) const
        {
            begin_type begin;
...
struct as_action : proto::transform<as_action>
{
    ...
    struct impl : proto::transform_impl<Expr, State, Data>
    {
        ...
        typedef
            typename mpl::if_c<
                proto::matches<action_type, CheckAssertion>::value, detail::predicate_matcher<action_copy_type>, detail::action_matcher<action_copy_type>>::type
                matcher_type;

        typedef
            typename proto::shift_right<
                marked_expr_type, typename proto::terminal<matcher_type>::type>::type
                no_attr_type;
        ...
        result_type operator()(
            typename impl::expr_param expr, typename impl::state_param state, typename impl::data_param data) const
        {
            int dummy = 0;
            ...
            no_attr_type that = {
                marked_expr, {matcher_type(DeepCopy::impl<action_type, expr_type, int>()(proto::right(expr), proto::left(expr), dummy), proto::value(proto::left(marked_expr)).mark_number_)}};
