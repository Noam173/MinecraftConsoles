...
struct regex_traits_type
{
    ...
    typedef typename mpl::if_c<
        is_same<Locale, std::locale>::value, cpp_regex_traits<char_type>, Locale>::type type;
...
struct xpression_linker
{
    ...
    explicit xpression_linker(Traits const &tr)
        : back_stack_(), traits_(&tr), traits_type_(&typeid(Traits)), has_backrefs_(false)
    {
    }
  ...
  private:
    ...
    struct alt_link_pred
    {
        ...
        alt_link_pred(
            xpression_linker<Char> *linker, xpression_peeker<Char> *peeker, void const *next)
            : linker_(linker), peeker_(peeker), next_(next)
        {
        }
    ...
    void alt_link(
        alternates_vector<BidiIter> const &alternates, void const *next, xpression_peeker<Char> *peeker)
    {
    ...
    void alt_link(
        fusion::sequence_base<Alternates> const &alternates, void const *next, xpression_peeker<Char> *peeker)
    {
#if BOOST_VERSION >= 103500
