...
ret(const lambda_functor<Arg> &a1)
{
    return lambda_functor_base<
        explicit_return_type_action<RET>,
        tuple<lambda_functor<Arg>>>(tuple<lambda_functor<Arg>>(a1));
}
...
template <class T>
inline const T &protect(const T &t)
{
    return t;
}
...
class non_lambda_functor
{
  ...
  public:
    ...
    struct sig
    {
        typedef typename LambdaFunctor::inherited::
    ...
    operator()() const
    {
        return lf.template call<typename LambdaFunctor::nullary_return_type>(cnull_type(), cnull_type(), cnull_type(), cnull_type());
    }
...
template <class Arg>
inline const Arg &unlambda(const Arg &a)
{
    return a;
}
...
struct const_incorrect_lambda_functor
{
  ...
  public:
    ...
    struct sig
    {
        typedef typename LambdaFunctor::inherited::template sig<typename SigArgs::tail_type>::type type;
    };
...
struct const_parameter_lambda_functor
{
  ...
  public:
    ...
    template <class A, class B, class C>
    typename sig<tuple<const const_parameter_lambda_functor, const A &, const B &, const C &>>::type
    operator()(const A &a, const B &b, const C &c) const
    {
...
template <class Args>
struct return_type_N<voidifier_action, Args>
{
    typedef void type;
};
...
make_void(const lambda_functor<Arg1> &a1)
{
    return lambda_functor_base<
        action<1, voidifier_action>,
        tuple<lambda_functor<Arg1>>>(tuple<lambda_functor<Arg1>>(a1));
}
...
{
    return lambda_functor_base<do_nothing_action, null_type>();
}
...
template <class F>
inline result_type_to_sig<F> std_functor(const F &f)
{
    return f;
}
