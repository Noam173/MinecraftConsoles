...
class identity
{
  ...
  public:
    ...
    typedef typename boost::add_reference<
        typename boost::add_const<T>::type>::type par_t;
    ...
    struct sig
    {
        typedef typename boost::remove_const<element_t>::type type;
    };
...
template <class T>
inline lambda_functor<identity<T &>> var(T &t)
{
    return identity<T &>(t);
}
...
template <class T>
lambda_functor<T> var(const lambda_functor<T> &t)
{
    return t;
}
...
template <class T>
inline lambda_functor<identity<typename bound_argument_conversion<const T>::type>>
constant(const T &t)
{
    return identity<typename bound_argument_conversion<const T>::type>(t);
}
template <class T>
lambda_functor<T> constant(const lambda_functor<T> &t)
{
    return t;
}
...
template <class T>
lambda_functor<T> constant_ref(const lambda_functor<T> &t)
{
    return t;
}
...
{
    typedef lambda_functor<identity<const T &>> type;
};
...
{
    typedef typename detail::remove_reference_and_cv<T>::type plain_T;
    typedef typename detail::IF<is_lambda_functor<plain_T>::value,
                                plain_T,
                                lambda_functor<
                                    identity<typename bound_argument_conversion<T>::type>>>::RET type;
};
...
{
    template <class U>
    static inline const U &go(const U &u)
    {
        return u;
    }
};
...
template <class T>
struct constify_rvals<T &>
{
    template <class U>
    static inline U &go(U &u)
    {
        return u;
    }
};
...
template <>
struct has_null_type<null_type>
{
    BOOST_STATIC_CONSTANT(bool, value = false);
};
...
class deduce_argument_types
{
  ...
  public:
    typedef typename detail::IF<
        has_null_type<t1>::value, null_type, t1>::RET type;
};
...
template <int N>
struct element_or_null<N, null_type>
{
    typedef null_type type;
};
...
#define BOOST_LAMBDA_LAMBDA_FUNCTOR_BASE_FIRST_PART(ARITY)                              \
    ...
    {                                                                                   \
      public:                                                                           \
        ...
        struct sig                                                                      \
        {                                                                               \
            typedef typename detail::deduce_argument_types<Args, SigArgs>::type rets_t; \
                                                                                        \
          public:                                                                       \
            typedef typename return_type_N_prot<Act, rets_t>::type type;                \
        };                                                                              \
                                                                                        \
        template <class RET, CALL_TEMPLATE_ARGS>                                        \
        RET call(CALL_FORMAL_ARGS) const                                                \
...
typedef typename deduce_argument_types<Args, tuple<CALL_REFERENCE_TYPES>>::type rets_t;
