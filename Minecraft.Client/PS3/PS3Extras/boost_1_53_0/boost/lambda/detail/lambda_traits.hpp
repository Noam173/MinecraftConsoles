...
struct IF_type
{
    typedef typename IF_type_<typename IF<C, T, E>::RET>::type type;
};
...
template <class T>
class remove_reference_if_valid
{
...
struct tuple_element_as_reference
{
    typedef typename boost::tuples::access_traits<
        typename boost::tuples::element<N, T>::type>::non_const_type type;
};

// returns the cv and reverence stripped type of a tuple element
template <int N, class T>
struct tuple_element_stripped
{
    typedef typename remove_reference_and_cv<
        typename boost::tuples::element<N, T>::type>::type type;
};

// is_lambda_functor -------------------------------------------------

template <class T>
struct is_lambda_functor_
{
    BOOST_STATIC_CONSTANT(bool, value = false);
};

template <class Arg>
struct is_lambda_functor_<lambda_functor<Arg>>
{
    ...
    BOOST_STATIC_CONSTANT(bool,
                          value =
                              detail::is_lambda_functor_<
                                  typename detail::remove_reference_and_cv<T>::type>::value);
};
...
{
    typedef typename detail::parameter_traits_<
        T,
        typename detail::IF<boost::is_function<T>::value, T &, const T>::RET>::type type;
};
...
struct const_copy_argument<T[n]>
{
    typedef const T (&type)[n];
};
template <class T, int n>
...
template <class T, int n>
struct bind_traits<volatile T[n]>
{
    typedef const volatile T (&type)[n];
};
...
{
    typedef tuple<typename bind_traits<T0>::type,
...
class bind_type_generator
{
    ...
    typedef typename detail::bind_tuple_mapper<
        T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>::type args_t;
...
template <class T>
inline const T &make_const(const T &t)
{
    return t;
}
