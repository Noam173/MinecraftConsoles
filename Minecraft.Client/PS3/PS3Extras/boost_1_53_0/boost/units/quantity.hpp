...
template <class Source, class Destination>
struct is_non_narrowing_conversion : mpl::and_<
...
// on sun.
    ...
    quantity(const quantity<Unit, YY> &source,
             typename boost::enable_if<detail::is_non_narrowing_conversion<YY, Y>>::type * = 0) : val_(source.value())
    {
        BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
    }
    ...
    explicit quantity(const quantity<Unit, YY> &source,
                      typename boost::disable_if<detail::is_non_narrowing_conversion<YY, Y>>::type * = 0) : val_(static_cast<Y>(source.value()))
    {
        BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
    }
    ...
    /// implicit conversion between value types is allowed if allowed for value types themselves
    template <class YY>
    quantity(const quantity<Unit, YY> &source) : val_(source.value())
    {
        BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
...
class quantity<BOOST_UNITS_DIMENSIONLESS_UNIT(System), Y>
{
  public:
    ...
    /// implicit conversion between value types is allowed if allowed for value types themselves
    template <class YY>
    quantity(const quantity<unit<dimension_type, system_type>, YY> &source) : val_(source.value())
    {
        BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
        BOOST_STATIC_ASSERT((boost::is_convertible<YY, Y>::value == true));
    }
    ...
    /// implicit conversion between different unit systems is allowed
    template <class System2, class Y2>
    quantity(const quantity<unit<dimensionless_type, homogeneous_system<System2>>, Y2> &source) : val_(source.value())
    {
        BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
    ...
    template <class System2, class Y2>
    explicit quantity(const quantity<unit<dimensionless_type, System2>, Y2> &source,
                      typename boost::disable_if<detail::is_dimensionless_system<System2>>::type * = 0) : val_(conversion_helper<quantity<unit<dimensionless_type, System2>, Y2>, this_type>::convert(source).value())
    {
        BOOST_UNITS_CHECK_LAYOUT_COMPATIBILITY(this_type, Y);
    }
...
struct quantity_cast_helper<Y, quantity<Unit, X>>
{
    ...
    type operator()(quantity<Unit, X> &source)
    {
