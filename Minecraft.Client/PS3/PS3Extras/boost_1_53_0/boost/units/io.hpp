...
{
    typedef double type;
    static double call(const T &arg)
    {
        return std::abs(arg);
    }
};
...
struct print_scale_t
{
    ...
    void operator()(Prefix, const T &t) const
    {
        ...
        *os << t / Prefix::value() << ' ';
        ...
        {
        case name_fmt:
            do_print(*os, Prefix::name());
            break;
        case raw_fmt:
        case symbol_fmt:
            do_print(*os, Prefix::symbol());
            break;
        case typename_fmt:
...
void do_print_prefixed_impl(std::basic_ostream<CharT, Traits> &os, const quantity<Unit, T> &q, F default_)
{
    ...
    if (detail::find_matching_scale<Prefixes>(q.value(), autoprefix_norm(q.value()), detail::print_scale(os, prefixed)))
    {
        if (prefixed)
        {
            ...
            {
            case symbol_fmt:
                do_print(os, maybe_parenthesize(Unit(), format_symbol_impl()));
                break;
            case raw_fmt:
                do_print(os, maybe_parenthesize(Unit(), format_raw_symbol_impl()));
                break;
            case name_fmt:
                do_print(os, maybe_parenthesize(Unit(), format_name_impl()));
                break;
            case typename_fmt:
                do_print(os, simplify_typename(Unit()));
                break;
            }
        }
        else
...
typename base_unit_info<
    scaled_base_unit<BaseUnit, Scale>>::base_unit_info_primary_template
do_print_prefixed(
...
inline mpl::true_ test_norm(double)
{
    return mpl::true_();
}
inline mpl::false_ test_norm(one)
{
    return mpl::false_();
}
