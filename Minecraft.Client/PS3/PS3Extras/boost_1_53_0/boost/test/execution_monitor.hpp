...
class BOOST_TEST_DECL execution_monitor
{
  public:
    // Constructor
    execution_monitor()
        : p_catch_system_errors(true), p_auto_start_dbg(false), p_timeout(0), p_use_alt_stack(true), p_detect_fp_exceptions(false)
    {
    }
...
template <typename Exception, typename ExceptionTranslator>
void execution_monitor::register_exception_translator(ExceptionTranslator const &tr, boost::type<Exception> *)
{
...
#define BOOST_TEST_SYS_ASSERT(exp) \
    if ((exp))                     \
        ...
        throw ::boost::system_error(BOOST_STRINGIZE(exp))
