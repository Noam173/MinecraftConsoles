...
struct config_file_iterator::Impl : noncopyable
{
    ...
    bool is_active_line()
    {
        return m_inactive_ifdef_level == 0;
    }
...
config_file_iterator::Impl::Impl()
    : m_path_separators(BOOST_RT_PARAM_LITERAL("/\\")), m_line_delimeter(BOOST_RT_PARAM_LITERAL('\n')), m_sl_comment_delimeter(BOOST_RT_PARAM_LITERAL("#")), m_command_delimeter(BOOST_RT_PARAM_LITERAL("$")), m_line_beak(BOOST_RT_PARAM_LITERAL("\\")), m_macro_ref_begin(BOOST_RT_PARAM_LITERAL("$")), m_macro_ref_end(BOOST_RT_PARAM_LITERAL("$"))

      ,
      m_include_kw(BOOST_RT_PARAM_LITERAL("include")), m_define_kw(BOOST_RT_PARAM_LITERAL("define")), m_undef_kw(BOOST_RT_PARAM_LITERAL("undef")), m_ifdef_kw(BOOST_RT_PARAM_LITERAL("ifdef")), m_ifndef_kw(BOOST_RT_PARAM_LITERAL("ifndef")), m_else_kw(BOOST_RT_PARAM_LITERAL("else")), m_endif_kw(BOOST_RT_PARAM_LITERAL("endif"))

      ,
      m_buffer_size(8192)

      ,
      m_trim_trailing_spaces(true), m_trim_leading_spaces(false), m_skip_empty_lines(true), m_detect_missing_macro(true)
...
bool config_file_iterator::Impl::get_next_line(cstring &line)
{
    ...
    while (!m_curr_level->m_stream.eof() || !!m_curr_level->m_parent.get())
    {
        ...
        if (match_front(line, m_command_delimeter))
        { // 90 //
...
void config_file_iterator::Impl::process_command_line(cstring line)
{
...
void config_file_iterator::Impl::process_include(cstring line)
{
    using namespace unit_test;
...
void config_file_iterator::Impl::process_define(cstring line)
{
    using namespace unit_test;
...
void config_file_iterator::Impl::process_undef(cstring line)
{
    if (!is_active_line())
    {
        return;
...
void config_file_iterator::Impl::process_ifdef(cstring line)
{
...
void config_file_iterator::Impl::process_ifndef(cstring line)
{
    m_conditional_states.push_back(true);
    if (!is_active_line())
    {
...
void config_file_iterator::Impl::process_else(cstring line)
{
...
void config_file_iterator::Impl::process_endif(cstring line)
{
...
void config_file_iterator::Impl::substitute_macros(cstring &where)
{
...
void config_file_iterator::construct()
{
...
void config_file_iterator::load(cstring file_name)
{
...
void config_file_iterator::register_command_handler(cstring command_kw, command_handler const &ch)
{
...
void config_file_iterator::set_parameter(rtti::id_t id, cstring value)
{
...
void config_file_iterator::set_parameter(rtti::id_t id, char_type value)
{
    BOOST_RTTI_SWITCH(id)
    {
...
void config_file_iterator::set_parameter(rtti::id_t id, std::size_t value)
{
    BOOST_RTTI_SWITCH(id)
    {
