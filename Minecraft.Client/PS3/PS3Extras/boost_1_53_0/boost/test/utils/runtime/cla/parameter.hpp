...
class parameter : public BOOST_RT_PARAM_NAMESPACE::parameter
{
  public:
    parameter(identification_policy &ID, argument_factory &F, bool optional_value = false)
        : p_optional(false), p_multiplicable(false), p_optional_value(optional_value), m_id_policy(ID), m_arg_factory(F)
    {
    }
    ...
    {
        return m_actual_argument != 0;
    }
    argument const &actual_argument() const
    {
        return *m_actual_argument;
    }
    ...
    {
        return m_id_policy.responds_to(name);
    }
    ...
    {
        return m_id_policy.id_2_report();
    }
