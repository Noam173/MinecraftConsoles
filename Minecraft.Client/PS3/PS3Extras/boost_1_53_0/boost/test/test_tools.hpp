...
#define FUNC_PARAMS(z, m, dummy) \
    , BOOST_JOIN(Arg, m) const &BOOST_JOIN(arg, m), char const *BOOST_JOIN(BOOST_JOIN(arg, m), _descr) /**/

#define PRED_PARAMS(z, m, dummy) BOOST_PP_COMMA_IF(m) BOOST_JOIN(arg, m)

#define ARG_INFO(z, m, dummy) \
    , BOOST_JOIN(BOOST_JOIN(arg, m), _descr), &static_cast<const unit_test::lazy_ostream &>(unit_test::lazy_ostream::instance() << ::boost::test_tools::tt_detail::print_helper(BOOST_JOIN(arg, m))) /**/
...
#define IMPL_FRWD(z, n, dummy)                                                                       \
    ...
    check_frwd(Pred P, unit_test::lazy_ostream const &check_descr,                                   \
               const_string file_name, std::size_t line_num,                                         \
               tool_level tl, check_type ct BOOST_PP_REPEAT_##z(BOOST_PP_ADD(n, 1), FUNC_PARAMS, _)) \
    {                                                                                                \
        return check_impl(P(BOOST_PP_REPEAT_##z(BOOST_PP_ADD(n, 1), PRED_PARAMS, _)),                \
...
inline predicate_result equal_impl(char *left, char const *right)
{
    return equal_impl(static_cast<char const *>(left), static_cast<char const *>(right));
}
inline predicate_result equal_impl(char const *left, char *right)
{
    return equal_impl(static_cast<char const *>(left), static_cast<char const *>(right));
}
inline predicate_result equal_impl(char *left, char *right)
{
    return equal_impl(static_cast<char const *>(left), static_cast<char const *>(right));
}
...
inline predicate_result equal_impl(wchar_t *left, wchar_t const *right)
{
    return equal_impl(static_cast<wchar_t const *>(left), static_cast<wchar_t const *>(right));
}
inline predicate_result equal_impl(wchar_t const *left, wchar_t *right)
{
    return equal_impl(static_cast<wchar_t const *>(left), static_cast<wchar_t const *>(right));
}
inline predicate_result equal_impl(wchar_t *left, wchar_t *right)
{
    return equal_impl(static_cast<wchar_t const *>(left), static_cast<wchar_t const *>(right));
}
#endif
