...
{
template <class T>
T boost_numeric_ublas_sqrt(const T &t)
{
    using namespace std;
...
template <class T>
T boost_numeric_ublas_abs(const T &t)
{
    using namespace std;
...
template <>
unsigned int boost_numeric_ublas_abs(const unsigned int &t)
{
    return t;
}
// unsigned types are always non-negative
template <>
unsigned long boost_numeric_ublas_abs(const unsigned long &t)
{
    return t;
}
} // namespace
...
struct promote_traits
{
    ...
    static const std::size_t size = sizeof(
        type_deduction_detail::test<
            typename base_type::x_type, typename base_type::y_type>(x + y) // Use x+y to stand of all the arithmetic actions
    );
...
template <class I>
BOOST_UBLAS_INLINE void increment(I &it, const I & /* it_end */, typename I::difference_type /* compare */, sparse_bidirectional_iterator_tag)
{
    ++it;
}
template <class I>
...
struct container_traits
    : container_view_traits<E>,
      mutable_container_traits<E>
{
};
...
struct matrix_traits
    : matrix_view_traits<MATRIX>,
      mutable_matrix_traits<MATRIX>
{
};
