...
class permutation_matrix : public vector<T, A>
{
  public:
    ...
    BOOST_UBLAS_INLINE
    explicit permutation_matrix(size_type size) : vector<T, A>(size)
    {
    ...
    BOOST_UBLAS_INLINE
    explicit permutation_matrix(const vector_type &init)
        : vector_type(init)
    {
    }
    BOOST_UBLAS_INLINE
...
typename M::size_type lu_factorize(M &m)
{
    ...
    for (size_type i = 0; i < size; ++i)
    {
        ...
        project(m, range(i + 1, size1), range(i + 1, size2)).minus_assign(outer_prod(project(mci, range(i + 1, size1)), project(mri, range(i + 1, size2))));
    }
#if BOOST_UBLAS_TYPE_CHECK
...
typename M::size_type axpy_lu_factorize(M &m, PM &pm)
{
    ...
    for (size_type i = 0; i < size; ++i)
    {
        ...
        project(v, range(i, size1)).assign(project(column(m, i), range(i, size1)) - axpy_prod<vector_type>(project(mr, range(i, size1), range(0, i)), urr));
        ...
        if (v(i_norm_inf) != value_type /*zero*/ ())
        {
            ...
            project(column(mr, i), range(i + 1, size1)).assign(project(v, range(i + 1, size1)) / v(i));
            if (i_norm_inf != i)
            {
    ...
    for (size_type i = 0; i < size; ++i)
    {
        ...
        project(v, range(i, size1)).assign(project(column(m, i), range(i, size1)) - axpy_prod<vector_type>(project(lr, range(i, size1), range(0, i)), urr));
        size_type i_norm_inf = i + index_norm_inf(project(v, range(i, size1)));
        BOOST_UBLAS_CHECK(i_norm_inf < size1, external_logic());
        if (v(i_norm_inf) != value_type /*zero*/ ())
        {
            ...
            project(column(lr, i), range(i + 1, size1)).assign(project(v, range(i + 1, size1)) / v(i));
            if (i_norm_inf != i)
            {
