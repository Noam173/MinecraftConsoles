    ...
    : public storage_array<c_array_view<T>>
{
  ...
  public:
    ...
    c_array_view(size_type size, array_type data) : size_(size), data_(data)
    {
    }
...
class compressed_matrix_view : public matrix_expression<compressed_matrix_view<L, IB, IA, JA, TA>>
{
  ...
  public:
    BOOST_UBLAS_INLINE
    compressed_matrix_view(index_type n_rows, index_type n_cols, array_size_type nnz, const rowptr_array_type &iptr, const index_array_type &jptr, const value_array_type &values) : matrix_expression<self_type>(),
    ...
    BOOST_UBLAS_INLINE
    compressed_matrix_view(const compressed_matrix_view &o) : size1_(size1_), size2_(size2_),
                                                              nnz_(nnz_),
  ...
  private:
    ...
    {
        return std::lower_bound(it_start, it_end, k_based(index));
    }
