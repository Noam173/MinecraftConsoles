  ...
  public:
    ...
    controlled_step_result try_step(System system, const StateIn &in, const DerivIn &dxdt, time_type &t, StateOut &out, time_type &dt)
    {
        ...
        if (m_max_rel_error > 1.0)
        {
            ...
            dt *= max BOOST_PREVENT_MACRO_SUBSTITUTION(static_cast<value_type>(9) / static_cast<value_type>(10) * pow(m_max_rel_error, static_cast<value_type>(-1) / (m_stepper.error_order() - 1)),
        ...
        else
        {
            if (m_max_rel_error < 0.5)
            {
                ...
                t += dt;
                dt *= static_cast<value_type>(9) / static_cast<value_type>(10) * pow(m_max_rel_error, static_cast<value_type>(-1) / m_stepper.stepper_order());
                return success;
            }
            else
            {
