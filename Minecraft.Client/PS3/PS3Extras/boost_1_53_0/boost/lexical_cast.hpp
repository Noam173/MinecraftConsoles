...

{
  public:
    ...
    bad_lexical_cast(
        ...
        const std::type_info &target_type_arg) BOOST_NOEXCEPT : source(&source_type_arg),
...
struct deduce_char_traits<Char, std::basic_string<Char, Traits, Alloc>, Source>
{
    typedef Traits type;
};

template <class Char, class Target, class Traits, class Alloc>
struct deduce_char_traits<Char, Target, std::basic_string<Char, Traits, Alloc>>
{
    typedef Traits type;
};

template <class Char, class Traits, class Alloc, class Source>
struct deduce_char_traits<Char, boost::container::basic_string<Char, Traits, Alloc>, Source>
{
    typedef Traits type;
};

template <class Char, class Target, class Traits, class Alloc>
struct deduce_char_traits<Char, Target, boost::container::basic_string<Char, Traits, Alloc>>
{
    typedef Traits type;
};

template <class Char, class Traits, class Alloc1, class Alloc2>
struct deduce_char_traits<Char, std::basic_string<Char, Traits, Alloc1>, std::basic_string<Char, Traits, Alloc2>>
{
    typedef Traits type;
};

template <class Char, class Traits, class Alloc1, class Alloc2>
struct deduce_char_traits<Char, boost::container::basic_string<Char, Traits, Alloc1>, boost::container::basic_string<Char, Traits, Alloc2>>
{
    typedef Traits type;
};

template <class Char, class Traits, class Alloc1, class Alloc2>
struct deduce_char_traits<Char, boost::container::basic_string<Char, Traits, Alloc1>, std::basic_string<Char, Traits, Alloc2>>
{
    typedef Traits type;
};

template <class Char, class Traits, class Alloc1, class Alloc2>
struct deduce_char_traits<Char, std::basic_string<Char, Traits, Alloc1>, boost::container::basic_string<Char, Traits, Alloc2>>
{
    typedef Traits type;
};
} // namespace detail
...
struct is_this_float_conversion_optimized
{
    typedef boost::type_traits::ice_and<
        ...
        boost::type_traits::ice_or<
            ...
            boost::is_same<Char, wchar_t>::value>::value
#else
        boost::type_traits::ice_eq<sizeof(Char), sizeof(char)>::value
#endif
        >
...
struct lexical_cast_stream_traits
{
    ...
    BOOST_STATIC_ASSERT_MSG((!boost::is_same<char16_t, src_char_t>::value && !boost::is_same<char16_t, target_char_t>::value),
                            "Your compiler does not have full support for char16_t");
#endif
#if !defined(BOOST_NO_CXX11_CHAR32_T) && defined(BOOST_NO_CXX11_UNICODE_LITERALS)
    BOOST_STATIC_ASSERT_MSG((!boost::is_same<char32_t, src_char_t>::value && !boost::is_same<char32_t, target_char_t>::value),
                            "Your compiler does not have full support for char32_t");
#endif

    typedef BOOST_DEDUCED_TYPENAME boost::detail::deduce_char_traits<
        char_type, Target, no_cv_src>::type traits;
    ...
    BOOST_STATIC_CONSTANT(bool, requires_stringbuf =
                                    (boost::type_traits::ice_or<
                                        is_string_widening_required_t::value, is_source_input_not_optimized_t::value>::value));
...
{
template <class T>
inline BOOST_DEDUCED_TYPENAME make_unsigned<T>::type lcast_to_unsigned(T value) BOOST_NOEXCEPT
{
...
CharT *lcast_put_unsigned(const T n_param, CharT *finish)
{
    ...
    BOOST_DEDUCED_TYPENAME boost::mpl::if_c<
        (sizeof(int_type) > sizeof(T)), int_type, T>::type n = n_param;
        ...
        if (grouping_size && grouping[0] > 0)
        {
            ...
            for (; end >= begin; --end)
            {
                if (remained)
                {
                    ...
                    if (multiplier_10 / 10 != multiplier)
                    {
                        multiplier_overflowed = true;
                    }
                    ...
                    if (*end < czero || *end >= czero + 10
                        /* detecting overflow */
                        || (dig_value && new_sub_value / dig_value != multiplier_10) || static_cast<T>((std::numeric_limits<T>::max)() - new_sub_value) < value || (multiplier_overflowed && dig_value))
                    {
                        return false;
                    }
...
inline bool parse_inf_nan_impl(const CharT *begin, const CharT *end, T &value, const CharT *lc_NAN, const CharT *lc_nan, const CharT *lc_INFINITY, const CharT *lc_infinity, const CharT opening_brace, const CharT closing_brace) BOOST_NOEXCEPT
{
    using namespace std;
    if (begin == end)
    {
    ...
    if (lc_iequal(begin, lc_nan, lc_NAN, 3))
    {
        ...
        {
            value = (boost::math::changesign)(std::numeric_limits<T>::quiet_NaN());
        }
        return true;
    }
    else if ((/* 'INF' or 'inf' */
              ...
              lc_iequal(begin, lc_infinity, lc_INFINITY, inifinity_size)))
    {
        ...
        {
            value = (boost::math::changesign)(std::numeric_limits<T>::infinity());
        }
        return true;
    }
...
template <class CharT, class T>
bool put_inf_nan_impl(CharT *begin, CharT *&end, const T &value, const CharT *lc_nan, const CharT *lc_infinity) BOOST_NOEXCEPT
{
    using namespace std;
    const CharT minus = lcast_char_constants<CharT>::minus;
...
{
    return parse_inf_nan_impl(begin, end, value, u"NAN", u"nan", u"INFINITY", u"infinity", u'(', u')');
}
...
inline bool lcast_ret_float(T &value, const CharT *begin, const CharT *end)
{
    ...
    if (parse_inf_nan(begin, end, value))
    {
        return true;
    }
    ...
    while (begin != end)
    {
        if (found_decimal)
        {
            ...
            if (Traits::eq(*begin, lowercase_e) || Traits::eq(*begin, capital_e))
            {
                break;
            }
            ...
            if (is_mantissa_full || tmp_mantissa / 10u != mantissa || (std::numeric_limits<mantissa_type>::max)() - (*begin - zero) < tmp_mantissa)
            {
        ...
        else
        {
            ...
            else if (Traits::eq(*begin, decimal_point) || Traits::eq(*begin, lowercase_e) || Traits::eq(*begin, capital_e))
            {
                 ...
                 *    ) then return error
                 */
                if (grouping_size && found_grouping && (length_since_last_delim != grouping[0] || last_grouping_pos > 1 || (last_grouping_pos == 0 && grouping_size > 1)))
                {
                    return false;
                }
#endif
                ...
                if (Traits::eq(*begin, decimal_point))
                {
                    ++begin;
                    found_decimal = true;
                    if (!found_number_before_exp && begin == end)
                    {
                        return false;
                    }
                    continue;
            ...
            else if (grouping_size && Traits::eq(*begin, thousands_sep))
            {
                if (found_grouping)
                {
                    ...
                    if (length_since_last_delim != grouping[last_grouping_pos])
                    {
                        ...
                        if (grouping_size > 1u && last_grouping_pos + 1 < grouping_size)
                        {
                            return false;
                        }
                }
    ...
    if (begin != end && (Traits::eq(*begin, lowercase_e) || Traits::eq(*begin, capital_e)))
    {
        ...
        while (begin != end)
        {
            if (*begin < czero || *begin >= czero + 10 || exp_pow_of_10 * 10 < exp_pow_of_10) /* Overflows are checked lower more precisely*/
            {
                return false;
            }
    ...
    if ((boost::math::isinf)(value) || (boost::math::isnan)(value))
    {
        return false;
    }
...
class stl_buf_unlocker : public BufferType
{
  public:
    ...
    CharT *pptr() const
    {
        return base_class::pptr();
    }
    CharT *pbase() const
    {
        return base_class::pbase();
    }
    void setg(CharT *gbeg, CharT *gnext, CharT *gend)
    {
        return base_class::setg(gbeg, gnext, gend);
    }
    void setp(CharT *pbeg, CharT *pend)
    {
        return setp(pbeg, pend);
    }
#endif
};
} // namespace detail
...
class lexical_stream_limited_src
{
  ...
  private:
    ...
    bool shl_input_streamable(InputStreamable &input)
    {
        ...
        const unlocked_but_t *const p = static_cast<unlocked_but_t *>(out_stream.rdbuf());
        start = p->pbase();
        finish = p->pptr();
        return result;
    }
  ...
  public:
    ...
    bool operator<<(std::array<CharT, N> const &input) BOOST_NOEXCEPT
    {
        if (input.size())
        {
            return shl_char_array_limited(&input[0], N);
        }
        else
        {
            return true;
  ...
  public:
    ...
    bool operator>>(bool &output) BOOST_NOEXCEPT
    {
        ...
        case 2:
            ...
            {
                output = false;
                return Traits::eq(minus, *start) && Traits::eq(zero, start[1]);
            }
        default:
...
struct is_arithmetic_and_not_xchars
{
    BOOST_STATIC_CONSTANT(bool, value =
                                    (boost::type_traits::ice_and<
                                        ...
                                        boost::type_traits::ice_not<
                                            detail::is_char_or_wchar<Target>::value>::value,
                                        boost::type_traits::ice_not<
                                            detail::is_char_or_wchar<Source>::value>::value>::value));
};
...
struct detect_precision_loss
{
    ...
    typedef BOOST_DEDUCED_TYPENAME mpl::if_<
        boost::is_arithmetic<Source>, Source, Source const &>::type argument_type;
...
struct lexical_cast_dynamic_num_not_ignoring_minus
{
    static inline Target lexical_cast_impl(const Source &arg)
    {
        return boost::numeric::converter<
            ...
            detect_precision_loss<Source, Target>>::convert(arg);
    }
};
...
struct lexical_cast_dynamic_num_ignoring_minus
{
    static inline Target lexical_cast_impl(const Source &arg)
    {
        typedef BOOST_DEDUCED_TYPENAME boost::mpl::eval_if_c<
            ...
            boost::make_unsigned<Source>>::type usource_t;
        ...
        typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_c<
            boost::type_traits::ice_and<
                boost::type_traits::ice_or<
                    ...
                    boost::is_float<Source>::value>::value,
                boost::type_traits::ice_not<
                    boost::is_same<Source, bool>::value>::value,
                boost::type_traits::ice_not<
                    boost::is_same<Target, bool>::value>::value,
                boost::is_unsigned<Target>::value>::value,
            lexical_cast_dynamic_num_ignoring_minus<Target, Source>,
            lexical_cast_dynamic_num_not_ignoring_minus<Target, Source>>::type caster_type;
...
inline Target lexical_cast(const Source &arg)
{
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::type_traits::ice_or<
        ...
        boost::type_traits::ice_and<
            ...
            boost::detail::is_stdstring<Target>::value>::value>
        shall_we_copy_t;

    typedef BOOST_DEDUCED_TYPENAME
        boost::detail::is_arithmetic_and_not_xchars<Target, src>
    ...
    typedef BOOST_DEDUCED_TYPENAME boost::mpl::if_c<
        ...
        BOOST_DEDUCED_TYPENAME boost::mpl::if_c<
            ...
            boost::detail::lexical_cast_do_cast<Target, src>>::type>::type caster_type;
...
Target lexical_cast(Source arg)
{
    typedef typename detail::widest_char<
        BOOST_DEDUCED_TYPENAME detail::stream_char<Target>::type, BOOST_DEDUCED_TYPENAME detail::stream_char<Source>::type>::type char_type;
