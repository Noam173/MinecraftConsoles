...
struct duration_style_io_saver
{
     ...
     */
    explicit duration_style_io_saver(state_type &s) : s_save_(s)
    {
     ...
     */
    duration_style_io_saver(state_type &s, aspect_type new_value) : s_save_(s), a_save_(new_value)
    {
    }
...
operator>>(std::basic_istream<CharT, Traits> &is, duration<Rep, Period> &d)
{
    ...
    BOOST_TRY
    {
        ...
        if (bool(ipfx))
        {
            ...
            {
                std::use_facet<duration_get<CharT>>(is.getloc()).get(is, std::istreambuf_iterator<CharT, Traits>(), is, err, d);
            }
        }
    }
    BOOST_CATCH(...)
