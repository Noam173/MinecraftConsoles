...
class time_duration : private boost::less_than_comparable<T, boost::equality_comparable<T>>
 ...
 * either (haven't tried) */
{
  public:
    ...
    time_duration(hour_type hours_in,
                  ...
                  fractional_seconds_type frac_sec_in = 0) : ticks_(rep_type::to_tick_count(hours_in, minutes_in, seconds_in, frac_sec_in))
    {
    }
...
class subsecond_duration : public base_duration
{
  ...
  public:
    explicit subsecond_duration(boost::int64_t ss) : base_duration(impl_type(traits_type::ticks_per_second >= frac_of_second ? ss * adjustment_ratio : ss / adjustment_ratio))
    {
    }
};
