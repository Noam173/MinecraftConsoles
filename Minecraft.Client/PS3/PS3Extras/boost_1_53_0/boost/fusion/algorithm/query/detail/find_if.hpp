...
{
    typedef typename main_find_if<
        typename result_of::next<First>::type, Last, Pred>::type
        type;
};
...
{
    typedef mpl::or_<
        result_of::equal_to<First, Last>, apply_filter<First, Pred>>
        filter;

    typedef typename mpl::eval_if<
        filter, mpl::identity<First>, recursive_find_if<First, Last, Pred>>::type
        type;
};
...
struct static_find_if
{
    typedef typename choose_find_if<
        First, Last, typename mpl::lambda<Pred>::type, is_base_of<random_access_traversal_tag, typename traits::category_of<First>::type>::value>::type
        type;
...
{
    typedef static_find_if<
        typename result_of::begin<Sequence>::type, typename result_of::end<Sequence>::type, Pred>
        filter;
