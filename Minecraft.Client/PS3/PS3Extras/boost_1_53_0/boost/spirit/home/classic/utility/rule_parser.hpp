...
#define BOOST_SPIRIT_RP_IMPL_III(name, name_t, pars, np, acts, na, mbrs, nm, x)                             \
    ...
    (name, name_t, np, na)                                                                                  \
        BOOST_SPIRIT_RP_REGISTER_TEMPLATE(name_t, BOOST_PP_INC(BOOST_PP_ADD(np, na)))
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// RP_OPAQUE_IMPL_I
//
...
#define BOOST_SPIRIT_RP_GEN_FUNC(name, name_t, np, na)                                                                                                                                             \
    ...
    {                                                                                                                                                                                              \
        return name_t<void BOOST_PP_ENUM_TRAILING_PARAMS(np, T)>(BOOST_PP_ENUM_PARAMS(np, p) BOOST_PP_ENUM_TRAILING_PARAMS(na,                                                                     \
...
#define BOOST_SPIRIT_RP__PM_OPAQUE_STATIC(r, data, i, elem) \
    static ::boost::call_traits<                            \
        BOOST_SPIRIT_RP_TYPE(BOOST_PP_TUPLE_ELEM(2, 0, elem))>::reference BOOST_PP_TUPLE_ELEM(2, 1, elem);

// PM_OPAQUE_CTOR_PARAMS
#define BOOST_SPIRIT_RP__PM_OPAQUE_CTOR_PARAMS(r, data, i, elem) \
    BOOST_PP_COMMA_IF(i)::boost::call_traits<                    \
        BOOST_SPIRIT_RP_TYPE(BOOST_PP_TUPLE_ELEM(2, 0, elem))>::param_type BOOST_PP_TUPLE_ELEM(2, 1, elem)

// PM_OPAQUE_GEN_PARAMS
#define BOOST_SPIRIT_RP__PM_OPAQUE_GEN_PARAMS(r, data, i, elem) \
    BOOST_PP_COMMA_IF(i)::boost::call_traits<                   \
        BOOST_SPIRIT_RP_TYPE(BOOST_PP_TUPLE_ELEM(2, 0, elem))>::param_type data##i
...
#define BOOST_SPIRIT_RP__MV_STATIC(r, data, i, elem) \
    static data()::boost::call_traits<               \
        data() BOOST_SPIRIT_RP_TYPE(BOOST_PP_TUPLE_ELEM(3, 0, elem))>::reference BOOST_PP_TUPLE_ELEM(3, 1, elem);
...
{
    return action_chain<PH, M, typename action_concatenator<A1, A2>::type>(concatenate_actions(chain.action(), a));
}
...
#define BOOST_SPIRIT_RP_TYPE(x) \
    ::BOOST_SPIRIT_CLASSIC_NS::type_of::remove_special_fptr<::BOOST_SPIRIT_CLASSIC_NS::type_of::special_result &(*)x>::type
