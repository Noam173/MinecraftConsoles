...
{
    return unrolled_byte_loops<T, n_bytes>::load_big(static_cast<const unsigned char *>(bytes) + n_bytes);
}

template <>
inline float load_big_endian<float, 4>(const void *bytes)
{
...
template <>
inline double load_big_endian<double, 8>(const void *bytes)
{
    const unsigned char *b = reinterpret_cast<const unsigned char *>(
        bytes);
    b += 7;
...
{
    return unrolled_byte_loops<T, n_bytes>::load_little(static_cast<const unsigned char *>(bytes));
}

template <>
inline float load_little_endian<float, 4>(const void *bytes)
{
    const unsigned char *b = reinterpret_cast<const unsigned char *>(
        bytes);
...
template <>
inline double load_little_endian<double, 8>(const void *bytes)
{
    const unsigned char *b = reinterpret_cast<const unsigned char *>(
        bytes);
...
{
    unrolled_byte_loops<T, n_bytes>::store_big(static_cast<char *>(bytes) + n_bytes, value);
}

template <>
inline void store_big_endian<float, 4>(void *bytes, float value)
{
...
template <>
inline void store_big_endian<double, 8>(void *bytes, double value)
{
    unsigned char *b = reinterpret_cast<unsigned char *>(bytes);
    b += 7;
...
{
    unrolled_byte_loops<T, n_bytes>::store_little(static_cast<char *>(bytes), value);
}
    ...
    : cover_operators<endian<endianness::big, T, n_bits, alignment::aligned>, T>
{
  ...
  public:
    ...
    operator T() const
    {
        return detail::load_big_endian<T, sizeof(T)>(&m_value);
    }
#endif
  private:
    T m_value;
