...
struct get_local_or_void
{
    typedef typename mpl::eval_if<
        mpl::less<Index, mpl::size<Locals>>, fusion::result_of::at<Locals, Index>, mpl::identity<fusion::void_>>::type
        type;
};
...
{
    typedef typename mpl::eval_if<
        mpl::equal_to<Index, mpl::int_<-1>>, outer_local<Local, Env>, get_local_or_void<typename Env::locals_type, Index>>::type
        type;
};
...
struct get_local
{
    typedef typename get_index<
        typename Env::map_type, typename Local::key_type>::type
        index_type;

    typedef typename get_local_from_index<Local, Env, index_type>::type
        type;
};
...
struct eval_local
{
    ...
    {
        typedef typename get_index<typename Env::outer_env_type::map_type, Key>::type
            index_type;

        return get<RT>(
            env.outer_env, index_type(), mpl::equal_to<index_type, mpl::int_<-1>>());
    }
    ...
    {
        return get<RT>(
            env, index, mpl::equal_to<Index, mpl::int_<-1>>());
    }
};
} // namespace detail
} // namespace phoenix
