...
struct function_eval<0>
{
    ...
    struct result
    {
        typedef typename remove_reference<
            typename F::template result<Env>::type>::type
            fn;
...
#define PHOENIX_GET_ARG(z, n, data)                                    \
    typedef typename remove_reference<                                 \
        typename BOOST_PP_CAT(A, n)::template result<Env>::type>::type \
        BOOST_PP_CAT(a, n);
...
{
    template <typename Env, typename F, BOOST_PP_ENUM_PARAMS(N, typename A)>
    struct result
    {
        typedef typename remove_reference<
            typename F::template result<Env>::type>::type
        ...
        typedef BOOST_PP_CAT(mpl::vector, N)<BOOST_PP_ENUM_PARAMS(N, a)> args;

        typedef typename fn::template result<BOOST_PP_ENUM_PARAMS(N, a)>
            function_apply;

        typedef typename mpl::eval_if<
            is_same<
                typename mpl::find<args, fusion::void_>::type, typename mpl::end<args>::type>,
            function_apply, mpl::identity<fusion::void_>>::type
            type;
    };

    template <typename RT, typename Env, typename F, BOOST_PP_ENUM_PARAMS(N, typename A)>
    static RT
    eval(Env const &env, F const &f, BOOST_PP_ENUM_BINARY_PARAMS(N, A, &_))
    {
