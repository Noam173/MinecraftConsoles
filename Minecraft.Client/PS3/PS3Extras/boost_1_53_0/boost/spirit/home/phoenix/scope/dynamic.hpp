...
struct dynamic_frame : noncopyable
{
    ...
    dynamic_frame(DynamicScope const &scope)
        : tuple(), save(scope.frame), scope(scope)
    {
        scope.frame = this;
    }
    ...
    dynamic_frame(DynamicScope const &scope, Tuple const &init)
        : tuple(init), save(scope.frame), scope(scope)
    {
        scope.frame = this;
    }
...
struct dynamic_member
{
    ...
    struct result
    {
        typedef typename fusion::result_of::at_c<tuple_type, N>::type
            type;
    };
...
#define PHOENIX_DYNAMIC_I(name, seq)                                      \
    struct name : ::boost::phoenix::dynamic<                              \
...
#define PHOENIX_DYNAMIC_B(r, _, i, xy) \
    BOOST_PP_COMMA_IF(i)               \
    BOOST_PP_TUPLE_ELEM(2, 1, xy)(*this)
