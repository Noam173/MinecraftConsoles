...
struct switch_gen
{
    ...
    typename lazy_enable_if<
        fusion::traits::is_sequence<Cases>, detail::switch_composite_actor<Cond, Cases>>::type
    operator[](Cases const &cases) const
    {
        typedef typename detail::switch_composite<Cond, Cases>
            switch_composite;
    ...
    {
        typedef typename detail::switch_composite<Cond, fusion::vector<actor<D>>>
            switch_composite;
...
case_(A0 const &_0)
{
    return switch_case<typename as_actor<A0>::type, int, N>(as_actor<A0>::convert(_0));
}
...
{
    return default_case<typename as_actor<A0>::type>(as_actor<A0>::convert(_0));
}
...
operator,
    (
        switch_case_base<D0, A0> const &_0, switch_case_base<D1, A1> const &_1)
{
    return detail::compose_case_a<D0, D1>::eval(
        static_cast<D0 const &>(_0), static_cast<D1 const &>(_1));
}

template <typename Seq, typename D, typename A>
inline typename lazy_enable_if<
    fusion::traits::is_sequence<Seq>, detail::compose_case_b<Seq, D>>::type
operator,
    (Seq const &seq, switch_case_base<D, A> const &case_)
{
...
{
    actor<default_case<actor<null_actor>>> default_ = default_case<actor<null_actor>>(nothing);
