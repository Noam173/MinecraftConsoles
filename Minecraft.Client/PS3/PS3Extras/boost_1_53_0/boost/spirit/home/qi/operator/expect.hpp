...
struct expectation_failure : std::runtime_error
{
    expectation_failure(Iterator first, Iterator last, info const &what)
        : std::runtime_error("boost::spirit::qi::expectation_failure"), first(first), last(last), what_(what)
    {
    }
  ...
  private:
        ...
        Iterator &first, Iterator const &last, Context &context, Skipper const &skipper)
    {
        return detail::expect_function<
            Iterator, Context, Skipper, expectation_failure<Iterator>>(first, last, context, skipper);
    }
