    ...
    : nary_parser<Derived>
{
    ...
    struct attribute
    {
        ...
        typedef typename traits::build_attribute_sequence<
            Elements, Context, traits::sequence_attribute_transform, Iterator, qi::domain>::type all_attributes;
    ...
    bool parse_impl(Iterator &first, Iterator const &last, Context &context, Skipper const &skipper, Attribute &attr_, mpl::false_) const
    {
        ...
        if (spirit::any_if(elements, attr, Derived::fail_function(iter, last, context, skipper), predicate()))
        {
            return false;
        }
        first = iter;
    ...
    bool parse_impl(Iterator &first, Iterator const &last, Context &context, Skipper const &skipper, Attribute &attr_, mpl::true_) const
    {
        ...
        Iterator iter = first;
        // return false if *any* of the parsers fail
        if (fusion::any(elements, detail::make_sequence_pass_container(
    ...
    {
        return parse_impl(first, last, context, skipper, attr, traits::is_container<Attribute>());
    }
