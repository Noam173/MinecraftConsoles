...
template <
    typename Iterator, typename T1, typename T2, typename T3, typename T4>
struct rule
    : proto::extends<
...
// VC7.1 has problems to resolve 'rule' without explicit template parameters
    ...
    bool parse(Iterator &first, Iterator const &last, Context &caller_context, Skipper const &skipper, Attribute &attr, Params const &params) const
    {
        if (f)
        {
            // do a preskip if this is an implied lexeme
