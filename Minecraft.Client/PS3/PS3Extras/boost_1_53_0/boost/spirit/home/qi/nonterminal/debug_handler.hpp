...
{
template <
    typename Iterator, typename Context, typename Skipper, typename F>
struct debug_handler
{
    ...
    debug_handler(
        function_type subject, F f, std::string const &rule_name)
...
void debug(rule<Iterator, T1, T2, T3, T4> &r, F f)
{
    ...
    typedef debug_handler<
        Iterator, typename rule_type::context_type, typename rule_type::skipper_type, F>
        debug_handler;
...
void debug(rule<Iterator, T1, T2, T3, T4> &r)
{
    ...
    typedef debug_handler<
        Iterator, typename rule_type::context_type, typename rule_type::skipper_type, simple_trace>
        debug_handler;
