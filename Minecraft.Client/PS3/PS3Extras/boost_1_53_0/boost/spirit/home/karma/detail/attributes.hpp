...
struct transform_attribute<boost::optional<Exposed> const, Transformed, typename disable_if<is_same<boost::optional<Exposed>, Transformed>>::type>
{
...
struct transform_attribute<Attribute const, Attribute>
{
    ...
    static Attribute const &pre(Attribute const &val)
    {
        return val;
    }
    // Karma only, no post() and no fail() required
