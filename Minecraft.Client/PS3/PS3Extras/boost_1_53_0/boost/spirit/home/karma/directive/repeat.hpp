...
template <typename T>
struct use_directive<karma::domain, terminal_ex<tag::repeat // enables repeat(exact)[p]
                                                ,
...
template <typename T>
struct use_directive<karma::domain, terminal_ex<tag::repeat // enables repeat(min, max)[p]
                                                ,
...
template <typename T>
struct use_directive<karma::domain, terminal_ex<tag::repeat // enables repeat(min, inf)[p]
                                                ,
...
struct use_lazy_directive<
    karma::domain, tag::repeat, 1 // arity
    > : mpl::true_
{
};
...
struct repeat_generator
    : base_repeat_generator<
          Subject, LoopIter, mpl::false_, repeat_generator<Subject, LoopIter>>
{
...
struct strict_repeat_generator
    : base_repeat_generator<
          Subject, LoopIter, mpl::true_, strict_repeat_generator<Subject, LoopIter>>
{
    typedef base_repeat_generator<
...
struct make_directive<tag::repeat, Subject, Modifiers>
{
    typedef typename mpl::if_<
        detail::get_stricttag<Modifiers>, strict_kleene<Subject>, kleene<Subject>>::type result_type;
    ...
    terminal_ex<tag::repeat, fusion::vector1<T>>, Subject, Modifiers>
{
    ...
    typedef typename mpl::if_<
        detail::get_stricttag<Modifiers>, strict_repeat_generator<Subject, iterator_type>, repeat_generator<Subject, iterator_type>>::type result_type;
...
template <typename T, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<tag::repeat, fusion::vector2<T, inf_type>>, Subject, Modifiers>
{
...
struct handles_container<
    karma::repeat_generator<Subject, LoopIter>, Attribute, Context, Iterator>
    : mpl::true_
{
};
...
template <typename Subject, typename LoopIter, typename Attribute, typename Context, typename Iterator>
struct handles_container<
    karma::strict_repeat_generator<Subject, LoopIter>, Attribute, Context, Iterator>
    : mpl::true_
{
};
} // namespace traits
