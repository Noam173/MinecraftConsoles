      ...
      noncopyable
{
    ...
    grammar(start_type const &start, std::string const &name_ = "unnamed-grammar")
        : proto::extends<terminal, base_type>(terminal::make(reference_(start))), name_(name_)
    {
    }
    ...
    grammar(rule<Iterator_, T1_, T2_, T3_, T4_> const &, std::string const & = "unnamed-grammar")
    {
        ...
        BOOST_SPIRIT_ASSERT_MSG(
            (is_same<start_type, rule<Iterator_, T1_, T2_, T3_, T4_>>::value), incompatible_start_rule, (rule<Iterator_, T1_, T2_, T3_, T4_>));
    }
...
struct handles_container<
    karma::grammar<IteratorA, T1, T2, T3, T4>, Attribute, Context, IteratorB>
    : detail::nonterminal_handles_container<
          typename attribute_of<
              karma::grammar<IteratorA, T1, T2, T3, T4>, Context, IteratorB>::type,
          Attribute>
{
};
} // namespace traits
