...
struct columns_delimiter
{
    columns_delimiter(Delimiter const &delim, ColumnDelimiter const &cdelim, unsigned int const numcols)
        : delimiter(delim), column_delimiter(cdelim), numcolumns(numcols), count(0)
    {
    }
    ...
    : unary_generator<columns_generator<Subject, NumColumns, ColumnsDelimiter>>
{
    ...
    bool generate(OutputIterator &sink, Context &ctx, Delimiter const &delimiter, Attribute const &attr) const
    {
...
struct make_directive<
    terminal_ex<tag::columns, fusion::vector1<T>>, Subject, Modifiers, typename enable_if_c<integer_traits<T>::is_integral>::type>
{
    typedef typename result_of::compile<karma::domain, eol_type, Modifiers>::type
        columns_delimiter_type;
    typedef columns_generator<
...
template <typename T, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<tag::columns, fusion::vector1<T>>, Subject, Modifiers, typename enable_if<mpl::and_<spirit::traits::matches<karma::domain, T>, mpl::not_<mpl::bool_<integer_traits<T>::is_integral>>>>::type>
{
    ...
    result_type operator()(Terminal const &term, Subject const &subject, unused_type) const
    {
        return result_type(subject, detail::default_columns(), compile<karma::domain>(fusion::at_c<0>(term.args)));
    }
};
...
struct make_directive<
    terminal_ex<tag::columns, fusion::vector2<T1, T2>>, Subject, Modifiers>
{
    ...
    result_type operator()(Terminal const &term, Subject const &subject, unused_type) const
    {
        return result_type(subject, fusion::at_c<0>(term.args), compile<karma::domain>(fusion::at_c<1>(term.args)));
    }
};
...
///////////////////////////////////////////////////////////////////////////
template <typename Subject, typename T1, typename T2, typename Attribute, typename Context, typename Iterator>
struct handles_container<
    karma::columns_generator<Subject, T1, T2>, Attribute, Context, Iterator>
