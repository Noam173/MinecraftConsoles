...
struct lazy_generator : generator<lazy_generator<Function, Modifiers>>
{
    ...
    struct attribute
    {
        typedef typename boost::result_of<karma_modify(tag::lazy_eval, Modifiers)>::type
            modifier;
        ...
        typedef typename result_of::compile<karma::domain, expr_type, modifier>::type
            generator_type;

        typedef typename traits::attribute_of<generator_type, Context, Iterator>::type
            type;
    };
                  ...
                  Delimiter const &d, Attribute const &attr) const
    {
        return detail::lazy_generate_impl_main(
            compile<karma::domain>(func(unused, context), karma_modify()(tag::lazy_eval(), modifiers)), sink, context, d, attr);
    }

    template <typename Context>
    info what(Context &context) const
    {
        return info("lazy", compile<karma::domain>(func(unused, context), karma_modify()(tag::lazy_eval(), modifiers))
                                .what(context));
    }
    ...
    : unary_generator<lazy_directive<Function, Subject, Modifiers>>
{
    ...
    {
        return info("lazy-directive", compile<karma::domain>(
                                          proto::make_expr<proto::tag::subscript>(
                                              function(unused, ctx), subject),
                                          karma_modify()(tag::lazy_eval(), modifiers))
                                          .what(ctx));
...
template <
    typename Terminal, typename Actor, int Arity, typename Subject, typename Modifiers>
