...
struct symbols_lookup
{
    ...
    typedef typename mpl::eval_if<traits::is_container<T>, traits::container_value<T>, sequence_type>::type type;
...
{
    typedef mpl::eval_if<fusion::traits::is_sequence<T>, traits::detail::value_at_c<T, 1>, mpl::identity<unused_type>> sequence_type;
    typedef typename mpl::eval_if<traits::is_container<T>, traits::container_value<T>, sequence_type>::type type;
...
struct symbols_lookup
    : mpl::if_<
          traits::not_is_unused<T>, std::map<Attribute, T>, std::set<Attribute>>
{
};
...
template <
    typename Attribute = char, typename T = unused_type, typename Lookup = typename symbols_lookup<T, Attribute>::type, typename CharEncoding = unused_type, typename Tag = unused_type>
struct symbols
    : proto::extends<
          typename proto::terminal<
              reference<symbols<Attribute, T, Lookup, CharEncoding, Tag>>>::type,
          ...
          symbols<Attribute, T, Lookup, CharEncoding, Tag>>
{
    ...
    symbols(std::string const &name = "symbols")
        : base_type(terminal::make(reference_(*this))), add(*this), remove(*this), lookup(new Lookup()), name_(name)
    {
    }

    symbols(symbols const &syms)
        : base_type(terminal::make(reference_(*this))), add(*this), remove(*this), lookup(syms.lookup), name_(syms.name_)
    {
    }
    ...
    bool generate(OutputIterator &sink, Context &, Delimiter const &d, Attr const &attr) const
    {
        ...
        return karma::detail::generate_encoded<CharEncoding, Tag>::call(
                   sink, (*it).second, traits::symbols_value<Attribute, Attr>::call(attr)) &&
...
template <
    typename Attribute, typename Lookup, typename CharEncoding, typename Tag>
struct symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>
    : proto::extends<
          typename proto::terminal<
              spirit::karma::reference<
                  symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>>>::type,
          ...
          symbols<Attribute, unused_type, Lookup, CharEncoding, Tag>>
{
    ...
    bool generate(OutputIterator &sink, Context &, Delimiter const &d, Attr const &attr) const
    {
        ...
        return karma::detail::generate_encoded<CharEncoding, Tag>::
                   call(sink, traits::symbols_lookup<Attr, Attribute>::call(attr), unused) &&
               karma::delimit_out(sink, d);
    }
...
///////////////////////////////////////////////////////////////////////////
template <typename Attribute, typename T, typename Lookup, typename CharEnconding, typename Tag, typename Modifiers>
    ...
    reference<symbols<Attribute, T, Lookup, CharEnconding, Tag>>, Modifiers>
{
    ...
    typedef typename mpl::if_c<
        lower || upper, symbols<Attribute, T, Lookup, typename spirit::detail::get_encoding_with_case<Modifiers, unused_type, lower || upper>::type, typename detail::get_casetag<Modifiers, lower || upper>::type>, reference_>::type
        result_type;
...
{
///////////////////////////////////////////////////////////////////////////
template <typename Attribute, typename T, typename Lookup, typename CharEncoding, typename Tag, typename Attr, typename Context, typename Iterator>
