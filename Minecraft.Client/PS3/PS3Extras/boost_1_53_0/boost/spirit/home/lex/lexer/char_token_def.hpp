...
struct use_terminal<lex::domain, terminal_ex<
    ...
    : primitive_lexer<char_token_def<CharEncoding, IdType>>
{
    ...
    char_token_def(char_type ch, IdType const &id)
        : ch(ch), id_(id), unique_id_(std::size_t(~0)), token_state_(std::size_t(~0))
    {
    }
...
struct make_primitive<
    terminal_ex<
        tag::char_code<tag::char_, CharEncoding>, fusion::vector1<Char (&)[2]> // single char strings
        >,
    Modifiers>
{
    typedef char_token_def<CharEncoding> result_type;
...
struct make_primitive<
    terminal_ex<
        tag::char_code<tag::char_, CharEncoding>, fusion::vector2<Char (&)[2], A1> // single char strings
        >,
    Modifiers>
{
    typedef char_token_def<CharEncoding> result_type;
