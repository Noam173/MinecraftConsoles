      ...
      lex::lexer_type<lexer_def_<LexerDef>>
{
  ...
  private:
    ...
    struct adder
    {
        ...
        adder const &operator()(char_type c, id_type token_id = id_type()) const
        {
            ...
            def.def.add_token(def.state.c_str(), c, token_id, def.targetstate.empty() ? 0 : def.targetstate.c_str());
            return *this;
        }
        ...
        adder const &operator()(string_type const &s, id_type token_id = id_type()) const
        {
            ...
            def.def.add_token(def.state.c_str(), s, token_id, def.targetstate.empty() ? 0 : def.targetstate.c_str());
            return *this;
        }

        template <typename Attribute>
        adder const &operator()(
            token_def<Attribute, char_type, id_type> &tokdef, id_type token_id = id_type()) const
        {
  ...
  public:
    ...
    lexer_def_(LexerDef &def_, string_type const &state_, string_type const &targetstate_ = string_type())
        : proto_base_type(terminal_type::make(alias())), add(this_()), add_pattern(this_()), def(def_), state(state_), targetstate(targetstate_)
    {
    }
...
class lexer : public Lexer
{
  ...
  public:
    ...
    lexer(unsigned int flags = match_flags::match_default, id_type first_id = id_type(min_token_id))
        : lexer_type(flags), next_token_id(first_id), self(this_(), lexer_type::initial_state())
    {
    }
