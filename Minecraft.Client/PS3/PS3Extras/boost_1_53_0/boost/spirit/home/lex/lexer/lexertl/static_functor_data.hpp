...
class static_data<Iterator, mpl::false_, mpl::false_, TokenValue>
{
  ...
  public:
    ...
    static_data(IterData const &data, Iterator &first, Iterator const &last)
        : first_(first), last_(last), next_token_(data.next_), get_state_name_(data.get_state_name_), bol_(data.bol_)
    {
    }
    ...
    : public static_data<Iterator, mpl::false_, mpl::false_, TokenValue>
{
  ...
  public:
    ...
    {
        std::size_t state_id = lexertl::detail::get_state_id(new_state, this->get_state_name_, num_states_);
    ...
    : public static_data<Iterator, mpl::false_, HasState, TokenValue>
{
  ...
  public:
    ...
    typedef detail::wrap_action<functor_wrapper_type, Iterator, static_data, std::size_t> wrap_action_type;

    template <typename IterData>
    static_data(IterData const &data, Iterator &first, Iterator const &last)
        : base_type(data, first, last), actions_(data.actions_), hold_(), value_(iterator_range<Iterator>(first, last)), has_value_(false), has_hold_(false)
    {
    }
    ...
    : public static_data<Iterator, mpl::false_, HasState, TokenValue>
{
  ...
  public:
    ...
    static_data(IterData const &data_, Iterator &first, Iterator const &last)
        : base_type(data_, first, last), actions_(data_.actions_), hold_(), has_value_(false), has_hold_(false)
    {
