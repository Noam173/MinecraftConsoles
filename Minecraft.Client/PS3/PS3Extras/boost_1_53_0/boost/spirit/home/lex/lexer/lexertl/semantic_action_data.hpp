...
struct semantic_actions<Iterator, mpl::false_, Data>
{
    ...
    invoke_actions(std::size_t /*state*/
                   ,
                   std::size_t &id, std::size_t unique_id, Iterator &end, Data &data) const
    {
...
struct semantic_actions<Iterator, mpl::true_, Data>
{
    ...
    // try to invoke a semantic action for the given token (unique_id)
    BOOST_SCOPED_ENUM(pass_flags)
    invoke_actions(std::size_t state, std::size_t &id, std::size_t unique_id, Iterator &end, Data &data) const
    {
