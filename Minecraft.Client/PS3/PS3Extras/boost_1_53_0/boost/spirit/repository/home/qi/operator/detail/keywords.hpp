...
template <typename Elements, typename Iterator, typename Context, typename Skipper, typename Flags, typename Counters, typename Attribute, typename NoCasePass>
    ...
    : public boost::static_visitor<bool>
{
  ...
  public:
    parse_dispatcher(const Elements &elements, Iterator &first, Iterator const &last, Context &context, Skipper const &skipper, Flags &flags, Counters &counters, attr_reference attr) : elements(elements), first(first), last(last), context(context), skipper(skipper), flags(flags), counters(counters), attr(attr)
    {
    }
    ...
    bool call_subject_unused(
        Subject const &subject, Iterator &first, Iterator const &last, Context &context, Skipper const &skipper, Index &idx) const
    {
        Iterator save = first;
    ...
    bool call(T &idx, mpl::false_) const
    {
        ...
        if (
            (!is_distinct<ElementType>::value) || skipper.parse(first, last, unused, unused, unused))
        {
...
struct string_keywords
{
    ...
    typedef typename spirit::detail::as_variant<
    ...
    struct build_char_type_sequence
    {
        ...
        typedef typename fusion::result_of::transform<Sequence, element_char_type>::type
            type;
    };
    ...
    typedef typename get_keyword_char_type<
        typename mpl::if_<
            mpl::equal_to<
                typename mpl::size<char_types>::type, mpl::int_<0>>,
            mpl::vector<boost::spirit::standard::char_type>, char_types>::type>::type char_type;
    ...
    struct build_case_type_sequence
    {
        ...
        // Compute the list of character types of the child kwd directives
        typedef typename fusion::result_of::transform<Sequence, element_case_type>::type
            type;
    };
    ...
    struct get_nb_case_types
    {
        ...
        typedef typename mpl::count_if<
            Sequence, mpl::equal_to<mpl::_, CaseType>>::type type;
    };
    ...
    typedef typename mpl::and_<
        typename mpl::greater<nb_elements, mpl::int_<0>>::type, typename mpl::equal_to<ikwd_count, nb_elements>::type>::type all_ikwd;

    typedef typename mpl::and_<
        typename mpl::greater<nb_elements, mpl::int_<0>>::type, typename mpl::equal_to<kwd_count, nb_elements>::type>::type all_kwd;
    ...
    typedef typename mpl::or_<
        no_case_modifier,
        mpl::and_<
            all_directives_of_same_type, all_ikwd>>::type
        no_case;
    ...
    typedef typename mpl::if_<
        no_case, nc_filter, spirit::qi::tst_pass_through>::type
        first_pass_filter_type;

    typedef typename mpl::or_<
        all_directives_of_same_type, no_case_modifier>::type requires_one_pass;
    ...
    struct keyword_entry_adder
    {
        ...
        keyword_entry_adder(shared_ptr<keywords_type> lookup, FlagsType &flags, Elements &elements) : lookup(lookup), flags(flags), elements(elements)
        {
        }
        ...
        const Skipper &skipper) const
    {
        ...
        else if (parser_index_type *val_ptr = lookup->find(saved_first, last, nc_filter()))
        {
            first = saved_first;
...
struct complex_kwd_function
{
    ...
    complex_kwd_function(
        Iterator &first, Iterator const &last, Context &context, Skipper const &skipper, ParseDispatcher &dispatcher)
        : first(first), last(last), context(context), skipper(skipper), dispatcher(dispatcher)
    {
    }
    ...
    bool operator()(Component const &component)
    {
        ...
        if (
            extract_keyword::call(
                fusion::at_c<
                    Component::value, typename ParseDispatcher::elements_type>(dispatcher.elements))
                .keyword.parse(
                    first, last, context, skipper, unused))
        {
