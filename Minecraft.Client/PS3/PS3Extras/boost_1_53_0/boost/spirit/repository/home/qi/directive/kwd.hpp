...
template <typename T>
struct use_directive<qi::domain, terminal_ex<repository::tag::kwd // enables kwd(key)[p]
                                             ,
...
template <typename T>
struct use_directive<qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(key)[p]
                                             ,
                                             fusion::vector1<T>>> : mpl::true_
{
};
...
template <typename T>
struct use_directive<qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(key)[p]
                                             ,
                                             fusion::vector1<T>>> : mpl::true_
{
};
...
template <typename T>
struct use_directive<qi::domain, terminal_ex<repository::tag::idkwd // enables idkwd(key)[p]
                                             ,
                                             fusion::vector1<T>>> : mpl::true_
{
};
...
struct use_directive<qi::domain, terminal_ex<repository::tag::kwd // enables kwd(key,exact)[p]
                                             ,
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(key,exact)[p]
                                             ,
                                             fusion::vector2<T1, T2>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(key,exact)[p]
                                             ,
                                             fusion::vector2<T1, T2>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::idkwd // enables idkwd(key,exact)[p]
                                             ,
                                             fusion::vector2<T1, T2>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::kwd // enables kwd(min, max)[p]
                                             ,
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(min, max)[p]
                                             ,
                                             fusion::vector3<T1, T2, T2>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(min, max)[p]
                                             ,
                                             fusion::vector3<T1, T2, T2>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::idkwd // enables idkwd(min, max)[p]
                                             ,
                                             fusion::vector3<T1, T2, T2>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::kwd // enables kwd(min, inf)[p]
                                             ,
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::ikwd // enables ikwd(min, inf)[p]
                                             ,
                                             fusion::vector3<T1, T2, inf_type>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::dkwd // enables dkwd(min, inf)[p]
                                             ,
                                             fusion::vector3<T1, T2, inf_type>>> : mpl::true_
{
};
...
template <typename T1, typename T2>
struct use_directive<qi::domain, terminal_ex<repository::tag::idkwd // enables idkwd(min, inf)[p]
                                             ,
                                             fusion::vector3<T1, T2, inf_type>>> : mpl::true_
{
};
...
struct kwd_finite_iterator // handles kwd(min, max)[p]
{
    kwd_finite_iterator(T const min, T const max)
        : min BOOST_PREVENT_MACRO_SUBSTITUTION(min), max BOOST_PREVENT_MACRO_SUBSTITUTION(max)
    {
    }
...
struct skipper_keyword_marker
{
    ...
    skipper_keyword_marker(Skipper const &skipper, bool &flag, int &counter) : skipper(skipper), flag(flag), counter(counter)
    {
    }
...
struct kwd_parser : spirit::qi::unary_parser<kwd_parser<Subject, KeywordType, LoopIter, NoCase, Distinct>>
{
    ...
    typedef typename remove_const<typename traits::char_type_of<KeywordType>::type>::type
        char_type;
    ...
    struct attribute
    {
        typedef typename traits::build_std_vector<
            typename traits::attribute_of<
                Subject, Context, Iterator>::type>::type
            type;
    };
    ...
    bool parse(Iterator &first, Iterator const &last, Context &context, skipper_keyword_marker<Skipper, NoCasePass> const &skipper, Attribute &attr) const
    {
        ...
        typedef typename mpl::and_<
            traits::is_container<Attribute>, mpl::not_<traits::is_weak_substitute<subject_attribute, Attribute>>>::type predicate;
  ...
  private:
    ...
    {
        return spirit::qi::detail::fail_function<Iterator, Context, Skipper>(first, last, context, skipper);
    }
};
...
struct make_directive_internal_2_args
{
    ...
    typedef typename mpl::if_<is_string_kwd_type,
                              T1,
                              typename result_of::compile<qi::domain, T1>::type>::type keyword_type;
    ...
    typedef typename mpl::if_<
        ...
        repository::qi::complex_kwd_parser<Subject, const_keyword, iterator_type, Distinct>>::type result_type;
    ...
    {
        return result_type(subject, fusion::at_c<0>(term.args), MakeDirectiveHelper::make_iterator(term.args));
    }
    template <typename Terminal>
    ...
    {
        return result_type(subject, compile<qi::domain>(fusion::at_c<0>(term.args), modifiers), MakeDirectiveHelper::make_iterator(term.args));
    }
...
struct make_directive_internal
{
    ...
    typedef typename mpl::if_<is_string_kwd_type,
                              T1,
                              typename result_of::compile<qi::domain, T1, Modifiers>::type>::type keyword_type;
    ...
    result_type create_kwd(Terminal const &term, Subject const &subject, Modifiers const &modifiers, boost::mpl::false_) const
    {
        return result_type(subject, compile<qi::domain>(fusion::at_c<0>(term.args), modifiers), iterator_type());
    }
    // Select which object type to return
    template <typename Terminal>
    result_type operator()(
...
{
    typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers, mpl::false_, make_exact_helper<T2>> make_directive_type;
    typedef typename make_directive_type::result_type result_type;
    template <typename Terminal>
    result_type operator()(
        Terminal const &term, Subject const &subject, Modifiers const &modifiers) const
...
{
    typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers, mpl::true_, make_exact_helper<T2>> make_directive_type;
...
{
    typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers, mpl::false_, make_finite_helper<T2>> make_directive_type;
    ...
    terminal_ex<repository::tag::dkwd, fusion::vector3<T1, T2, T2>>, Subject, Modifiers>
{
    ...
    typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers, mpl::true_, make_finite_helper<T2>> make_directive_type;
    ...
    terminal_ex<repository::tag::ikwd, fusion::vector3<T1, T2, T2>>, Subject, Modifiers>
{
        ...
        Terminal const &term, Subject const &subject, unused_type) const
    {
        ...
        return result_type(subject, fusion::at_c<0>(term.args),
                           iterator_type(
                               fusion::at_c<1>(term.args), fusion::at_c<2>(term.args), encoding));
    }
};
...
{
    typedef make_directive_internal_2_args<T1, T2, Subject, Modifiers, mpl::false_, make_infinite_helper<T2>> make_directive_type;
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::ikwd, fusion::vector3<T1, T2, inf_type>>, Subject, Modifiers>
{
    typedef typename add_const<T1>::type const_keyword;
...
template <typename T1, typename T2, typename Subject, typename Modifiers>
struct make_directive<
    terminal_ex<repository::tag::idkwd, fusion::vector3<T1, T2, inf_type>>, Subject, Modifiers>
{
    typedef typename add_const<T1>::type const_keyword;
    typedef repository::qi::kwd_infinite_iterator<T2> iterator_type;
...
template <typename Subject, typename KeywordType, typename LoopIter, typename NoCase, typename Attribute, typename Context, typename Iterator, typename Distinct>
...
template <typename Subject, typename KeywordType, typename LoopIter, typename Attribute, typename Context, typename Iterator, typename Distinct>
