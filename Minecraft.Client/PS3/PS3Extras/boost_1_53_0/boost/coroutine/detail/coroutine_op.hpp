...
struct coroutine_op<Signature, D, Result, 0>
{
    class iterator : public std::iterator<std::input_iterator_tag, typename remove_reference<Result>::type>
    {
      ...
      public:
        ...
        explicit iterator(D *dp) : dp_(dp), val_()
        {
            fetch_();
        }

        iterator(iterator const &other) : dp_(other.dp_), val_(other.val_)
        {
        }
    ...
    class const_iterator : public std::iterator<std::input_iterator_tag, typename remove_reference<const Result>::type>
    {
      ...
      public:
        ...
        const_iterator() : dp_(0), val_()
        {
        }

        explicit const_iterator(D *dp) : dp_(dp), val_()
        {
            fetch_();
        }

        const_iterator(const_iterator const &other) : dp_(other.dp_), val_(other.val_)
        {
        }
