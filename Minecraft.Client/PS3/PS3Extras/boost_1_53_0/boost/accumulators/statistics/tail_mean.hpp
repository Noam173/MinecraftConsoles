    ...
    : accumulator_base
{
    ...
    result_type result(Args const &args) const
    {
        ...
        return some_non_coherent_tail_mean(args) + numeric::average(quantile(args), n) * ((is_same<LeftRight, left>::value) ? args[quantile_probability] : 1. - args[quantile_probability] - numeric::average(n, count(args)));
    }
};
    ...
    result_type result(Args const &args) const
    {
        ...
        if (n <= static_cast<std::size_t>(tail(args).size()))
        {
            return numeric::average(
                std::accumulate(
                    tail(args).begin(), tail(args).begin() + n, Sample(0)),
                n);
        }
        else
        {
