    ...
    : accumulator_base
{
    ...
    weighted_density_impl(Args const &args)
        : cache_size(args[density_cache_size]), cache(cache_size), num_bins(args[density_num_bins]), samples_in_bin(num_bins + 2, 0.), bin_positions(num_bins + 2), histogram(
                                                                                                                                                                        num_bins + 2, std::make_pair(
                                                                                                                                                                                          numeric::average(args[sample | Sample()], (std::size_t)1), numeric::average(args[sample | Sample()], (std::size_t)1))),
          is_dirty(true)
    {
    }
    ...
    void operator()(Args const &args)
    {
            ...
            for (typename histogram_type::const_iterator iter = this->cache.begin(); iter != this->cache.end(); ++iter)
            {
                ...
                else
                {
                    typename array_type::iterator it = std::upper_bound(
                        this->bin_positions.begin(), this->bin_positions.end(), iter->first);
        ...
        else if (cnt > this->cache_size)
        {
            ...
            {
                typename array_type::iterator it = std::upper_bound(
                    this->bin_positions.begin(), this->bin_positions.end(), args[sample]);
