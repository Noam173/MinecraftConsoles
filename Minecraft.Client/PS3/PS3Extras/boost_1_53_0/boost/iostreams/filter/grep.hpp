...
class basic_grep_filter : public basic_line_filter<Ch, Alloc>
{
  ...
  private:
    virtual string_type do_filter(const string_type &line)
    {
        ...
        bool matches = (options_ & grep::whole_line) ? regex_match(line, re_, match_flags_) : regex_search(line, re_, match_flags_);
...
basic_grep_filter<Ch, Tr, Alloc>::basic_grep_filter(const regex_type &re, match_flag_type match_flags, int options)
