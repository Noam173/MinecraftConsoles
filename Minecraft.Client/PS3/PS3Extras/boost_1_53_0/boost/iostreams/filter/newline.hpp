...
class newline_base
{
  public:
    ...
    {
        return (flags_ & newline::posix) != 0;
    }
    bool is_mixed_dos() const
    {
        return (flags_ & newline::dos) != 0;
    }
    bool is_mixed_mac() const
    {
        return (flags_ & newline::mac) != 0;
    }
    bool is_mixed() const
    {
        int platform =
            (flags_ & newline::posix) != 0 ? newline::posix : (flags_ & newline::dos) != 0 ? newline::dos
                                                          : (flags_ & newline::mac) != 0   ? newline::mac
                                                                                           : 0;
...
class newline_filter
{
  public:
    ...
    int get(Source &src)
    {
        ...
        int c =
            (flags_ & f_has_CR) == 0 ? iostreams::get(src) : CR;
    ...
    bool put(Sink &dest, char c)
    {
        ...
        {
            return c == LF ? newline(dest) : newline(dest) && this->put(dest, c);
        }
        ...
        {
            return newline(dest) ? this->put(dest, c) : false;
        }
  ...
  private:
    ...
    template <typename Sink>
    void newline_if_sink(Sink &dest, mpl::true_)
    {
        newline(dest);
    }
...
class newline_checker : public detail::newline_base
{
  ...
  private:
    void fail()
    {
        boost::throw_exception(newline_error(source()));
    }
    int &source()
    {
        return flags_;
