...
class range_adapter
{
  ...
  public:
    ...
    typedef typename mpl::if_<
        ...
        std::random_access_iterator_tag,
        std::forward_iterator_tag>::type tag;
...
template <typename Mode, typename Range>
inline std::streamsize range_adapter<Mode, Range>::read(char_type *s, std::streamsize n)
{
    return impl::read(cur_, last_, s, n);
}

template <typename Mode, typename Range>
inline std::streamsize range_adapter<Mode, Range>::write(const char_type *s, std::streamsize n)
{
    return impl::write(cur_, last_, s, n);
}

template <typename Mode, typename Range>
std::streampos range_adapter<Mode, Range>::seek(stream_offset off, BOOST_IOS::seekdir way)
{
...
struct range_adapter_impl<std::forward_iterator_tag>
{
    ...
    static std::streamsize read(Iter &cur, Iter &last, Ch *s, std::streamsize n)
    {
        std::streamsize rem = n; // No. of chars remaining.
        while (cur != last && rem-- > 0)
        {
            *s++ = *cur++;
        }
    ...
    template <typename Iter, typename Ch>
    static std::streamsize write(Iter &cur, Iter &last, const Ch *s, std::streamsize n)
    {
        while (cur != last && n-- > 0)
        {
            *cur++ = *s++;
...
struct range_adapter_impl<std::random_access_iterator_tag>
{
    ...
    static void seek(Iter &first, Iter &cur, Iter &last, stream_offset off,
