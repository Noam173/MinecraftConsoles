...
inline std::streamsize check_eof(std::streamsize n)
{
    return n != 0 ? n : -1;
}
    ...
    : mpl::if_<
          ...
          read_device_impl<
              BOOST_DEDUCED_TYPENAME
                  detail::dispatch<
                      T, istream_tag, streambuf_tag, input>::type>>::type
{
};
...
struct read_device_impl<streambuf_tag>
{
    ...
    {
        std::streamsize amt;
        return (amt = t.sgetn(s, n)) != 0 ? amt : detail::true_eof(t) ? -1
                                                                      : 0;
    }
...
struct read_device_impl<input>
{
    ...
    { // gcc 2.95 needs namespace qualification for char_traits.
        ...
        char_type c;
        std::streamsize amt;
        return (amt = t.read(&c, 1)) == 1 ? traits_type::to_int_type(c) : amt == -1 ? traits_type::eof()
                                                                                    : traits_type::would_block();
    }
...
struct read_filter_impl
    : mpl::if_<
          ...
          read_filter_impl<
              BOOST_DEDUCED_TYPENAME
                  detail::dispatch<
                      T, multichar_tag, any_tag>::type>>::type
{
};
...
{
    template <typename T, typename Source>
    static std::streamsize read(T &t, Source &src, typename char_type_of<T>::type *s, std::streamsize n)
    {
