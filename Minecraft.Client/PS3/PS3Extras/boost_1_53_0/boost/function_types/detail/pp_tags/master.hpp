...
template <bits_t Flags, bits_t CCID>
...
struct encode_charr_impl
{
    BOOST_STATIC_CONSTANT(std::size_t, value = (std::size_t)(1 +
                                                                 Flags |
                                                             (BOOST_FT_default_cc * CCID) << 1 | Arity << BOOST_FT_arity_shift));
};
...
struct decode_bits
{
    ...
    BOOST_STATIC_CONSTANT(std::size_t, arity = (std::size_t)(Bits >> BOOST_FT_arity_shift));
};
