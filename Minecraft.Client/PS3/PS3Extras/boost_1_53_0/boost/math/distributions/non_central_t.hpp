...
T non_central_t2_p(T v, T delta, T x, T y, const Policy &pol, T init_val)
{
    ...
    pois = gamma_p_derivative(T(k + 1), d2, pol) * tgamma_delta_ratio(T(k + 1), T(0.5f)) * delta / constants::root_two<T>();
    if (pois == 0)
    {
        return init_val;
    }
...
T non_central_t_pdf(T n, T delta, T t, const Policy &pol)
{
    ...
    if (t == 0)
    {
        ...
        // 1F1(a,b,0) = 1.
        //
        return tgamma_delta_ratio(n / 2 + 0.5f, T(0.5f)) * sqrt(n / constants::pi<T>()) * exp(-delta * delta / 2) / 2;
    }
