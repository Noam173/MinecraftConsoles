...
{ // mode.
    ...
    {
        result = guess[d - static_cast<diff_type>(1)] + (guess[d] - guess[d - static_cast<diff_type>(1)]) / (shapes[d] - shapes[d - static_cast<diff_type>(1)]) * (shape - shapes[d - static_cast<diff_type>(1)]);
    }
...
inline RealType quantile(const skew_normal_distribution<RealType, Policy> &dist, const RealType &p)
{
    ...
    if (shape != 0)
    {
        ...
        x = x + (x * x - static_cast<RealType>(1)) * skew / static_cast<RealType>(6) + x * (x * x - static_cast<RealType>(3)) * exk / static_cast<RealType>(24) - x * (static_cast<RealType>(2) * x * x - static_cast<RealType>(5)) * skew * skew / static_cast<RealType>(36);
    } // if(shape != 0)
