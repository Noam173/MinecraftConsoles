...
class geometric_distribution
{
  public:
        ...
        RealType alpha) // alpha 0.05 equivalent to 95% for one-sided test.
    {
        ...
        if (false == detail::check_probability(function, alpha, &result, Policy()) && geometric_detail::check_dist_and_k(
...
{ // Variance of Binomial distribution = (1-p) / p^2.
    return (1 - dist.success_fraction()) / (dist.success_fraction() * dist.success_fraction());
} // variance
...
{ // Quantile, percentile/100 or Percent Point geometric function.
    ...
    // Check dist and x.
    RealType result = 0;
    if (false == geometric_detail::check_dist_and_prob(function, success_fraction, x, &result, Policy()))
    {
        return result;
    }
