...
{
    return check_success_fraction(function, p, result, pol) && check_successes(function, r, result, pol);
}
template <class RealType, class Policy>
...
class negative_binomial_distribution
{
  public:
        ...
        RealType alpha) // alpha 0.05 equivalent to 95% for one-sided test.
    {
        ...
        if (false == detail::check_probability(function, alpha, &result, Policy()) && negative_binomial_detail::check_dist_and_k(
...
{ // Variance of Binomial distribution = r (1-p) / p^2.
    return dist.successes() * (1 - dist.success_fraction()) / (dist.success_fraction() * dist.success_fraction());
} // variance
...
{ // Quantile, percentile/100 or Percent Point Negative Binomial function.
    ...
    // Check dist and P.
    RealType result = 0;
    if (false == negative_binomial_detail::check_dist_and_prob(function, r, p, P, &result, Policy()))
    {
        return result;
    }
