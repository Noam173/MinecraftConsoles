...
inline bool check_inverse_chi_squared( // Check both distribution parameters.
    ...
    const Policy &pol)
{
    return check_scale(function, scale, result, pol) && check_df(function, degrees_of_freedom,
                                                                 result, pol);
...
RealType pdf(const inverse_chi_squared_distribution<RealType, Policy> &dist, const RealType &x)
{
    ...
    if (false == detail::check_inverse_chi_squared(function, df, scale, &error_result, Policy()))
    { // Bad distribution.
        return error_result;
    }
