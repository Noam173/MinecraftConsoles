...
#if !defined(__BORLANDC__) && !(defined(__GNUC__) && (__GNUC__ == 3) && (__GNUC_MINOR__ <= 2))
#define BOOST_MATH_META_INT(type, name, Default)                                                            \
    ...
    template <class T>                                                                                      \
    struct is_##name : public boost::mpl::bool_<::boost::math::policies::detail::is_##name##_imp<T>::value> \
    {                                                                                                       \
    };
...
struct precision
{
    ...
    typedef typename mpl::if_c<
        ...
        precision_type,
        typename mpl::if_c<
            ((::std::numeric_limits<Real>::digits <= precision_type::value) || (Policy::precision_type::value <= 0)),
            // Default case, full precision for RealType:
            digits2<::std::numeric_limits<Real>::digits>,
            // User customised precision:
            precision_type>::type>::type type;
