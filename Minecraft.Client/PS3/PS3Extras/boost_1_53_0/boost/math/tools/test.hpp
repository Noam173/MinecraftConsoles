...
T relative_error(T a, T b)
{
    ...
    //
    T min_val = (std::max)(tools::min_value<T>(),
                           static_cast<T>((std::numeric_limits<double>::min)()));
    T max_val = (std::min)(tools::max_value<T>(),
...
test_result<typename calculate_result_type<A>::value_type> test(const A &a, F1 test_func, F2 expect_func)
{
    ...
    for (unsigned i = 0; i < a.size(); ++i)
    {
        ...
        {
            point = std::numeric_limits<value_type>::has_infinity ? std::numeric_limits<value_type>::infinity()
