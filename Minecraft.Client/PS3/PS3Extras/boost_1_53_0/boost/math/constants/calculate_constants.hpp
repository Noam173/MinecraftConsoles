...
{
    BOOST_MATH_STD_USING
    return euler<T, policies::policy<policies::digits2<N>>>() * euler<T, policies::policy<policies::digits2<N>>>();
}
...
{
    BOOST_MATH_STD_USING
    return static_cast<T>(1) / euler<T, policies::policy<policies::digits2<N>>>();
}
...
{
    BOOST_MATH_STD_USING
    return pi<T, policies::policy<policies::digits2<N>>>() * pi<T, policies::policy<policies::digits2<N>>>(); //
}
...
{
    BOOST_MATH_STD_USING
    return pi<T, policies::policy<policies::digits2<N>>>() * pi<T, policies::policy<policies::digits2<N>>>() / static_cast<T>(6); //
}
...
{
    BOOST_MATH_STD_USING
    return pi<T, policies::policy<policies::digits2<N>>>() * pi<T, policies::policy<policies::digits2<N>>>() * pi<T, policies::policy<policies::digits2<N>>>(); //
}
...
{
    BOOST_MATH_STD_USING
    return static_cast<T>(1) / pow(pi<T, policies::policy<policies::digits2<N>>>(), static_cast<T>(1) / static_cast<T>(3));
}
...
{
    BOOST_MATH_STD_USING
    return pi<T, policies::policy<policies::digits2<N>>>() / static_cast<T>(180); //
}
...
{
    BOOST_MATH_STD_USING
    return static_cast<T>(180) / pi<T, policies::policy<policies::digits2<N>>>(); //
}
...
{ // http://oeis.org/A006752/constant
  ...
  //"149374281672134266498119621763019776254769479356512926115106248574");
    ...
    return boost::math::constants::pi<T, boost::math::policies::policy<>>() * log(2 + boost::math::constants::root_three<T, boost::math::policies::policy<>>()) / 8 + 3 * sum / 8;
}
...
{ // from e_float constants.cpp
  // Mathematica: N[12 Sqrt[6]  Zeta[3]/Pi^3, 1101]
    BOOST_MATH_STD_USING
    T ev(12 * sqrt(static_cast<T>(6)) * zeta_three<T, policies::policy<policies::digits2<N>>>() / pi_cubed<T, policies::policy<policies::digits2<N>>>());
...
inline T zeta_derivative_2(unsigned n)
{
    ...
    return zeta_series_derivative_2<T>(n) * zeta_series_lead_2<T>() + zeta_series_derivative_lead_2<T>() * zeta_series_2<T>(n);
}
...
{ // From e_float
  ...
  // Mathematica: N[2 Sqrt[Pi] (Pi - 3)/((4 - Pi)^(3/2)), 1100]
    ...
    BOOST_MATH_STD_USING
    T rs(2 * root_pi<T, policies::policy<policies::digits2<N>>>() * pi_minus_three<T, policies::policy<policies::digits2<N>>>() / pow(four_minus_pi<T, policies::policy<policies::digits2<N>>>(), static_cast<T>(3. / 2)));
...
{ // - (6 Pi^2 - 24 Pi + 16)/((Pi - 4)^2)
  // Might provide and calculate this using pi_minus_four.
    BOOST_MATH_STD_USING
    return -(((static_cast<T>(6) * pi<T, policies::policy<policies::digits2<N>>>() * pi<T, policies::policy<policies::digits2<N>>>()) - (static_cast<T>(24) * pi<T, policies::policy<policies::digits2<N>>>()) + static_cast<T>(16)) /
             ((pi<T, policies::policy<policies::digits2<N>>>() - static_cast<T>(4)) * (pi<T, policies::policy<policies::digits2<N>>>() - static_cast<T>(4))));
}
...
{ // 3 - (6 Pi^2 - 24 Pi + 16)/((Pi - 4)^2)
  // Might provide and calculate this using pi_minus_four.
    BOOST_MATH_STD_USING
    return static_cast<T>(3) - (((static_cast<T>(6) * pi<T, policies::policy<policies::digits2<N>>>() * pi<T, policies::policy<policies::digits2<N>>>()) - (static_cast<T>(24) * pi<T, policies::policy<policies::digits2<N>>>()) + static_cast<T>(16)) /
                                ((pi<T, policies::policy<policies::digits2<N>>>() - static_cast<T>(4)) * (pi<T, policies::policy<policies::digits2<N>>>() - static_cast<T>(4))));
}
