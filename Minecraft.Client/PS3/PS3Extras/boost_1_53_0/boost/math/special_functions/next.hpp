...
{
    return val < direction ? boost::math::float_next(val, pol) : val == direction ? val
                                                                                  : boost::math::float_prior(val, pol);
}
...
T float_distance(const T &a, const T &b, const Policy &pol)
{
    ...
    {
        return 2 + fabs(float_distance(static_cast<T>((b < 0) ? -detail::get_smallest_value<T>() : detail::get_smallest_value<T>()), b, pol)) + fabs(float_distance(static_cast<T>((a < 0) ? -detail::get_smallest_value<T>() : detail::get_smallest_value<T>()), a, pol));
    }
    //
