...
T ibeta_inv_imp(T a, T b, T p, T q, const Policy &pol, T *py)
{
    ...
    else if ((a > 1) && (b > 1))
    {
        ...
        if ((b < a) && (x < 0.2))
        {
            ...
            ap1 *= (a + 1);
            terms[4] = bm1 * (33 * a * b_2 + 31 * b_2 + 8 * a_2 * b_2 - 30 * a * b - 47 * b + 11 * a_2 * b + 6 * a_3 * b + 18 + 4 * a - a_3 + a_2 * a_2 - 10 * a_2) / (3 * (a + 3) * (a + 2) * ap1);
...
ibeta_inv(T1 a, T2 b, T3 p, T4 *py, const Policy &pol)
{
    ...
    if (py)
    {
        *py = policies::checked_narrowing_cast<T4, forwarding_policy>(ry, function);
    }
