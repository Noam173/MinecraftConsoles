...
inline T asymptotic_bessel_j_large_x(T v, T x)
{
    ...
    return sqrt(2 / (constants::pi<T>() * x)) * (asymptotic_bessel_j_large_x_P(v, x) * cos(chi) - asymptotic_bessel_j_large_x_Q(v, x) * sin(chi));
}
...
inline T asymptotic_bessel_y_large_x(T v, T x)
{
    ...
    BOOST_MATH_STD_USING // ADL of std names
        T chi = fabs(x) - constants::pi<T>() * (2 * v + 1) / 4;
    return sqrt(2 / (constants::pi<T>() * x)) * (asymptotic_bessel_j_large_x_P(v, x) * sin(chi) - asymptotic_bessel_j_large_x_Q(v, x) * cos(chi));
}
    ...
    typedef typename mpl::if_<
        ...
        typename mpl::if_<
            ...
            typename mpl::if_<
                ...
                mpl::int_<64>,
                mpl::int_<53>>::type>::type>::type type;
};
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy &pol)
{
    ...
    return (boost::math::isfinite)(s) ? s : policies::raise_overflow_error<T>("boost::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", 0, pol);
}
