...
int temme_ik(T v, T x, T *K, T *K1, const Policy &pol)
{
    ...
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < tools::epsilon<T>() ? T(1) : T(boost::math::sin_pi(v) / (v * pi<T>()));
    d = abs(sigma) < tools::epsilon<T>() ? T(1) : T(sinh(sigma) / sigma);
    gamma1 = abs(v) < tools::epsilon<T>() ? T(-euler<T>()) : T((0.5f / v) * (gp - gm) * c);
    gamma2 = (2 + gp + gm) * c / 2;
...
int bessel_ik(T v, T x, T *I, T *K, int kind, const Policy &pol)
{
    ...
    if (x == 0)
    {
        ...
        {
            T z = (u + n % 2);
            Iv = boost::math::sin_pi(z, pol) == 0 ? Iv : policies::raise_overflow_error<T>(function, 0, pol); // reflection formula
        }
