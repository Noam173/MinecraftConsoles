...
T cyl_bessel_j_imp(T v, T x, const bessel_no_int_tag &t, const Policy &pol)
{
    ...
    {
        return (v == 0) ? 1 : (v > 0) ? 0
                                      : policies::raise_domain_error<T>(function, "Got v = %1%, but require v >= 0 or a negative integer: the result would be complex.", v, pol);
    }
...
inline T sph_bessel_j_imp(unsigned n, T x, const Policy &pol)
{
    ...
    //
    return sqrt(constants::pi<T>() / (2 * x)) * cyl_bessel_j_imp(T(T(n) + T(0.5f)), x, bessel_no_int_tag(), pol);
}
...
inline T cyl_neumann_imp(T v, T x, const bessel_no_int_tag &, const Policy &pol)
{
    ...
    if (x <= 0)
    {
        return (v == 0) && (x == 0) ? policies::raise_overflow_error<T>(function, 0, pol)
                                    : policies::raise_domain_error<T>(
                                          function,
