...
T expint_1_rational(const T &z, const mpl::int_<53> &)
{
    ...
    if (z <= 1)
    {
        ...
        result = tools::evaluate_polynomial(P, z) / tools::evaluate_polynomial(Q, z);
        result += z - log(z) - Y;
    }
    else if (z < -boost::math::tools::log_min_value<T>())
    {
        ...
        T recip = 1 / z;
        result = 1 + tools::evaluate_polynomial(P, recip) / tools::evaluate_polynomial(Q, recip);
        result *= exp(-z) * recip;
    }
    else
    {
...
T expint_1_rational(const T &z, const mpl::int_<113> &)
{
    ...
    else if (z <= 4)
    {
        ...
        T recip = 1 / z;
        result = Y + tools::evaluate_polynomial(P, recip) / tools::evaluate_polynomial(Q, recip);
        result *= exp(-z) * recip;
    }
    else if (z < -boost::math::tools::log_min_value<T>())
    {
...
inline T expint_as_series(unsigned n, T z, const Policy &pol)
{
    ...
    BOOST_MATH_INSTRUMENT_VARIABLE(result)
    result += pow(-z, static_cast<T>(n - 1)) * (boost::math::digamma(static_cast<T>(n)) - log(z)) / fact;
    BOOST_MATH_INSTRUMENT_VARIABLE(result)
...
T expint_i_imp(T z, const Policy &pol, const mpl::int_<53> &tag)
{
    ...
    else if (z <= 10)
    {
        ...
        T t = z / 2 - 4;
        result = Y + tools::evaluate_polynomial(P, t) / tools::evaluate_polynomial(Q, t);
        result *= exp(z) / z;
        result += z;
    }
    else if (z <= 20)
