                    ...
                    const Policy &pol)
{
    ...
    else
    {
        ...
        if ((l1 >= tools::log_max_value<T>()) || (l1 <= tools::log_min_value<T>()) || (l2 >= tools::log_max_value<T>()) || (l2 <= tools::log_min_value<T>()))
        {
                    ...
                    const Policy &pol)
{
    ...
    if ((lb1 >= tools::log_max_value<T>()) || (lb1 <= tools::log_min_value<T>()) || (lb2 >= tools::log_max_value<T>()) || (lb2 <= tools::log_min_value<T>()) || (e1 >= tools::log_max_value<T>()) || (e1 <= tools::log_min_value<T>()))
    {
...
T ibeta_imp(T a, T b, T x, const Policy &pol, bool inv, bool normalised, T *p_derivative)
{
    ...
    if (x == 0)
    {
        if (p_derivative)
        {
            *p_derivative = (a == 1) ? (T)1 : (a < 1) ? T(tools::max_value<T>() / 2)
    ...
    if (x == 1)
    {
        if (p_derivative)
        {
            *p_derivative = (b == 1) ? T(1) : (b < 1) ? T(tools::max_value<T>() / 2)
                                                      : T(tools::min_value<T>() * 2);
        }
...
T ibeta_derivative_imp(T a, T b, T x, const Policy &pol)
{
    ...
    if (x == 0)
    {
        return (a > 1) ? 0 : (a == 1) ? 1 / boost::math::beta(a, b, pol)
                                      : policies::raise_overflow_error<T>(function, 0, pol);
    }
    else if (x == 1)
    {
        return (b > 1) ? 0 : (b == 1) ? 1 / boost::math::beta(a, b, pol)
                                      : policies::raise_overflow_error<T>(function, 0, pol);
    }
    //
