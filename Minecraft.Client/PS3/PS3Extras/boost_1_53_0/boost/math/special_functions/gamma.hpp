...
T tgammap1m1_imp(T dz, Policy const &pol, const Lanczos &l)
{
    ...
    if (dz < 0)
    {
        ...
        {
            // Use expm1 on lgamma:
            result = boost::math::expm1(-boost::math::log1p(dz, pol) + lgamma_small_imp<T>(dz + 2, dz + 1, dz, tag_type(), pol, l));
            BOOST_MATH_INSTRUMENT_CODE(result);
        }
    }
    else
            ...
            typedef typename mpl::if_<
                ...
                typename mpl::if_<
                    ...
                    typename mpl::if_<
                        mpl::less_equal<precision_type, mpl::int_<64>>,
                        mpl::int_<64>,
                        mpl::int_<113>>::type>::type>::type tag_type;
...
T gamma_p_derivative_imp(T a, T x, const Policy &pol)
{
    ...
    {
        return (a > 1) ? 0 : (a == 1) ? 1
                                      : policies::raise_overflow_error<T>("boost::math::gamma_p_derivative<%1%>(%1%, %1%)", 0, pol);
    }
    //
    // Normal case:
    //
