...
struct avl_set_defaults
    : pack_options<none, base_hook<detail::default_avl_set_hook>, constant_time_size<true>, size_type<std::size_t>, compare<std::less<T>>>::type
{
};
    ...
    : private detail::clear_on_destructor_base<avltree_impl<Config>>
{
  ...
  public:
    ...
    explicit avltree_impl(const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
        : data_(cmp, v_traits)
    {
    ...
    avltree_impl(bool unique, Iterator b, Iterator e, const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
        : data_(cmp, v_traits)
    {
        node_algorithms::init_header(this->priv_header_ptr());
        this->priv_size_traits().set_size(size_type(0));
    ...
    iterator insert_before(const_iterator pos, reference value)
    {
        ...
        iterator ret(node_algorithms::insert_before(this->priv_header_ptr(), pos.pointed_node(), to_insert), this);
        this->priv_size_traits().increment();
        return ret;
    }
    ...
    {
        node_algorithms::clear_and_dispose(this->priv_header_ptr(), detail::node_disposer<Disposer, avltree_impl>(disposer, this));
        node_algorithms::init_header(this->priv_header_ptr());
        this->priv_size_traits().set_size(0);
    }
    ...
    void clone_from(const avltree_impl &src, Cloner cloner, Disposer disposer)
    {
        ...
        if (!src.empty())
        {
            ...
            node_algorithms::clone(src.priv_header_ptr(), this->priv_header_ptr(), detail::node_cloner<Cloner, avltree_impl>(cloner, this), detail::node_disposer<Disposer, avltree_impl>(disposer, this));
