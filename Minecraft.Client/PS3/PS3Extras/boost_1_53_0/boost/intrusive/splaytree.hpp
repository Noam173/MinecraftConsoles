...
struct splay_set_defaults
    : pack_options<none, base_hook<detail::default_splay_set_hook>, constant_time_size<true>, size_type<std::size_t>, compare<std::less<T>>>::type
{
};
    ...
    : private detail::clear_on_destructor_base<splaytree_impl<Config>>
{
  ...
  public:
    ...
    explicit splaytree_impl(const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
        : data_(cmp, v_traits)
    {
    ...
    iterator insert_equal(reference value)
    {
        ...
        iterator ret(node_algorithms::insert_equal_lower_bound(this->priv_header_ptr(), to_insert, key_node_comp), this);
    ...
    iterator insert_equal(const_iterator hint, reference value)
    {
        ...
        iterator ret(node_algorithms::insert_equal(this->priv_header_ptr(), hint.pointed_node(), to_insert, key_node_comp), this);
        this->priv_size_traits().increment();
        return ret;
    }
    ...
    {
        node_algorithms::clear_and_dispose(this->priv_header_ptr(), detail::node_disposer<Disposer, splaytree_impl>(disposer, this));
        this->priv_size_traits().set_size(0);
    }
    ...
    const_iterator lower_bound_dont_splay(const KeyType &key, KeyValueCompare comp) const
    {
        detail::key_nodeptr_comp<KeyValueCompare, splaytree_impl>
            key_node_comp(comp, this);
        return const_iterator(node_algorithms::lower_bound(this->priv_header_ptr(), key, key_node_comp, false), this);
    }
    ...
    const_iterator upper_bound_dont_splay(const KeyType &key, KeyValueCompare comp) const
    {
        detail::key_nodeptr_comp<KeyValueCompare, splaytree_impl>
            key_node_comp(comp, this);
        return const_iterator(node_algorithms::upper_bound_dont_splay(this->priv_header_ptr(), key, key_node_comp, false), this);
    }
    ...
    const_iterator find_dont_splay(const KeyType &key, KeyValueCompare comp) const
    {
        detail::key_nodeptr_comp<KeyValueCompare, splaytree_impl>
            key_node_comp(comp, this);
        return const_iterator(node_algorithms::find(this->priv_header_ptr(), key, key_node_comp, false), this);
    }
    ...
    equal_range_dont_splay(const KeyType &key, KeyValueCompare comp) const
    {
        detail::key_nodeptr_comp<KeyValueCompare, splaytree_impl>
            key_node_comp(comp, this);
        std::pair<node_ptr, node_ptr> ret(node_algorithms::equal_range(this->priv_header_ptr(), key, key_node_comp, false));
    ...
    std::pair<const_iterator, const_iterator> bounded_range(const KeyType &lower_key, const KeyType &upper_key, KeyValueCompare comp, bool left_closed, bool right_closed) const
    {
        detail::key_nodeptr_comp<KeyValueCompare, splaytree_impl>
            key_node_comp(comp, this);
        std::pair<node_ptr, node_ptr> ret(node_algorithms::bounded_range(this->priv_header_ptr(), lower_key, upper_key, key_node_comp, left_closed, right_closed, false));
        return std::pair<const_iterator, const_iterator>(const_iterator(ret.first, this), const_iterator(ret.second, this));
    }
    ...
    void clone_from(const splaytree_impl &src, Cloner cloner, Disposer disposer)
    {
        ...
        if (!src.empty())
        {
            ...
            node_algorithms::clone(src.priv_header_ptr(), this->priv_header_ptr(), detail::node_cloner<Cloner, splaytree_impl>(cloner, this), detail::node_disposer<Disposer, splaytree_impl>(disposer, this));
  ...
  private:
    static splaytree_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
    {
        ...
        node_plus_pred_t *n = detail::parent_from_member<node_plus_pred_t, header_plus_size>(r, &node_plus_pred_t::header_plus_size_);
...
{
    typedef typename pack_options<splay_set_defaults<T>,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                  O1, O2, O3, O4
#else
                                  Options...
...
{
    /// @cond
    typedef splaytree_impl<typename make_splaytree_opt<T,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                                       O1, O2, O3, O4
#else
                                                       Options...
...
class splaytree
    : public make_splaytree<T,
                            ...
                            >::type
{
    typedef typename make_splaytree<T,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                    O1, O2, O3, O4
#else
                                    Options...
  ...
  public:
    ...
    template <class Iterator>
    splaytree(bool unique, Iterator b, Iterator e, const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
