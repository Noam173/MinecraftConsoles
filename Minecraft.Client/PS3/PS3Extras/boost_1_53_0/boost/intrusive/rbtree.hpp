...
template <class T>
struct set_defaults
    : pack_options<none, base_hook<detail::default_set_hook>, constant_time_size<true>, size_type<std::size_t>, compare<std::less<T>>>::type
{
};
    ...
    : private detail::clear_on_destructor_base<rbtree_impl<Config>>
{
  ...
  public:
    ...
    explicit rbtree_impl(const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
        : data_(cmp, v_traits)
    {
    ...
    iterator insert_equal(reference value)
    {
        ...
        iterator ret(node_algorithms::insert_equal_upper_bound(this->priv_header_ptr(), to_insert, key_node_comp), this);
    ...
    iterator insert_before(const_iterator pos, reference value)
    {
        ...
        this->priv_size_traits().increment();
        return iterator(node_algorithms::insert_before(this->priv_header_ptr(), pos.pointed_node(), to_insert), this);
    }
    ...
    {
        node_algorithms::clear_and_dispose(this->priv_header_ptr(), detail::node_disposer<Disposer, rbtree_impl>(disposer, this));
        node_algorithms::init_header(this->priv_header_ptr());
        this->priv_size_traits().set_size(0);
    }
    ...
    void clone_from(const rbtree_impl &src, Cloner cloner, Disposer disposer)
    {
        ...
        if (!src.empty())
        {
            ...
            node_algorithms::clone(const_node_ptr(src.priv_header_ptr()), node_ptr(this->priv_header_ptr()), detail::node_cloner<Cloner, rbtree_impl>(cloner, this), detail::node_disposer<Disposer, rbtree_impl>(disposer, this));
...
{
    typedef typename pack_options<set_defaults<T>,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                  O1, O2, O3, O4
#else
                                  Options...
    ...
    typedef setopt<value_traits, typename packed_options::compare, typename packed_options::size_type, packed_options::constant_time_size> type;
};
/// @endcond
...
{
    /// @cond
    typedef rbtree_impl<typename make_rbtree_opt<T,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                                 O1, O2, O3, O4
#else
                                                 Options...
...
class rbtree
    : public make_rbtree<T,
                         ...
                         >::type
{
    typedef typename make_rbtree<T,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                 O1, O2, O3, O4
#else
                                 Options...
  ...
  public:
    ...
    rbtree(const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
        : Base(cmp, v_traits)
    {
    }

    template <class Iterator>
    rbtree(bool unique, Iterator b, Iterator e, const value_compare &cmp = value_compare(), const value_traits &v_traits = value_traits())
