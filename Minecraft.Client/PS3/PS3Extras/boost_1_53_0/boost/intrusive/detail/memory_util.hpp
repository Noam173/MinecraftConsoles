...
{
    return static_cast<T *>(static_cast<void *>(const_cast<char *>(&reinterpret_cast<const char &>(obj))));
}
...
#define BOOST_INTRUSIVE_INSTANTIATE_DEFAULT_TYPE_TMPLT(TNAME)                                                                                                                        \
    ...
    {                                                                                                                                                                                \
        ...
        static const bool value = (1 == sizeof(test<T>(0, 0)));                                                                                                                      \
                                                                                                                                                                                     \
        typedef typename ::boost::intrusive::detail::if_c<value, T, DefaultWrap>::type::TNAME type;                                                                                  \
    };                                                                                                                                                                               \
                                                                                                                                                                                     \
    template <typename T, typename DefaultType>                                                                                                                                      \
    ...
    {                                                                                                                                                                                \
        ...
        static const bool value = (1 == sizeof(test<T>(0, 0)));                                                                                                                      \
                                                                                                                                                                                     \
        typedef typename ::boost::intrusive::detail::eval_if_c<value, ::boost::intrusive::detail::identity<T>, ::boost::intrusive::detail::identity<DefaultWrap>>::type::TNAME type; \
    };                                                                                                                                                                               \
    //
...
#define BOOST_PP_LOCAL_MACRO(n)                                                                                                                                               \
    template <template <typename BOOST_PP_ENUM_TRAILING(n, BOOST_INTRUSIVE_PP_IDENTITY, typename)> class TemplateClass, typename T BOOST_PP_ENUM_TRAILING_PARAMS(n, class P)> \
    struct first_param<TemplateClass<T BOOST_PP_ENUM_TRAILING_PARAMS(n, P)>>                                                                                                  \
    {                                                                                                                                                                         \
        typedef T type;                                                                                                                                                       \
    };                                                                                                                                                                        \
//
...
#define BOOST_PP_LOCAL_MACRO(n)                                                                                                                                              \
    template <template <typename BOOST_PP_ENUM_TRAILING(n, BOOST_INTRUSIVE_PP_IDENTITY, typename)> class Ptr, typename T BOOST_PP_ENUM_TRAILING_PARAMS(n, class P), class U> \
    struct type_rebinder<Ptr<T BOOST_PP_ENUM_TRAILING_PARAMS(n, P)>, U, 0u>                                                                                                  \
    {                                                                                                                                                                        \
