...
struct treap_set_defaults
    : pack_options<none, base_hook<detail::default_bs_set_hook>, constant_time_size<true>, size_type<std::size_t>, compare<std::less<T>>, priority<boost::intrusive::priority_compare<T>>>::type
{
};
    ...
    : private detail::clear_on_destructor_base<treap_impl<Config>>
{
  ...
  private:
    ...
    struct node_plus_pred_t : public detail::ebo_functor_holder<value_compare>
    {
        node_plus_pred_t(const value_compare &comp, const priority_compare &p_comp)
            : detail::ebo_functor_holder<value_compare>(comp), header_plus_priority_size_(p_comp)
        {
        }
  ...
  public:
    ...
    explicit treap_impl(const value_compare &cmp = value_compare(), const priority_compare &pcmp = priority_compare(), const value_traits &v_traits = value_traits())
    ...
    treap_impl(bool unique, Iterator b, Iterator e, const value_compare &cmp = value_compare(), const priority_compare &pcmp = priority_compare(), const value_traits &v_traits = value_traits())
        : data_(cmp, pcmp, v_traits)
    {
        node_algorithms::init_header(this->priv_header_ptr());
        this->priv_size_traits().set_size(size_type(0));
    ...
    treap_impl(BOOST_RV_REF(treap_impl) x)
        : data_(::boost::move(x.priv_comp()), ::boost::move(x.priv_pcomp()), ::boost::move(x.priv_value_traits()))
    {
        node_algorithms::init_header(this->priv_header_ptr());
        this->priv_size_traits().set_size(size_type(0));
        this->swap(x);
    ...
    iterator insert_equal(reference value)
    {
        ...
        iterator ret(node_algorithms::insert_equal_upper_bound(this->priv_header_ptr(), to_insert, key_node_comp, key_node_pcomp), this);
    ...
    iterator insert_equal(const_iterator hint, reference value)
    {
        ...
        iterator ret(node_algorithms::insert_equal(this->priv_header_ptr(), hint.pointed_node(), to_insert, key_node_comp, key_node_pcomp), this);
        this->priv_size_traits().increment();
        return ret;
    }
    ...
    std::pair<iterator, bool> insert_unique_check(const KeyType &key, KeyValueCompare key_value_comp, KeyValuePrioCompare key_value_pcomp, insert_commit_data &commit_data)
    {
        ...
        std::pair<node_ptr, bool> ret =
            (node_algorithms::insert_unique_check(this->priv_header_ptr(), key, comp, pcomp, commit_data));
    ...
    std::pair<iterator, bool> insert_unique_check(const_iterator hint, const KeyType &key, KeyValueCompare key_value_comp, KeyValuePrioCompare key_value_pcomp, insert_commit_data &commit_data)
    {
        ...
        detail::key_nodeptr_comp<KeyValuePrioCompare, treap_impl>
            pcomp(key_value_pcomp, this);
        std::pair<node_ptr, bool> ret =
            (node_algorithms::insert_unique_check(this->priv_header_ptr(), hint.pointed_node(), key, comp, pcomp, commit_data));
        return std::pair<iterator, bool>(iterator(ret.first, this), ret.second);
    }
    ...
    iterator insert_before(const_iterator pos, reference value)
    {
        ...
        iterator ret(node_algorithms::insert_before(this->priv_header_ptr(), pos.pointed_node(), to_insert, pcomp), this);
        this->priv_size_traits().increment();
        return ret;
    }
    ...
    {
        node_algorithms::clear_and_dispose(this->priv_header_ptr(), detail::node_disposer<Disposer, treap_impl>(disposer, this));
        node_algorithms::init_header(this->priv_header_ptr());
        this->priv_size_traits().set_size(0);
    }
    ...
    void clone_from(const treap_impl &src, Cloner cloner, Disposer disposer)
    {
        ...
        if (!src.empty())
        {
            ...
            node_algorithms::clone(src.priv_header_ptr(), this->priv_header_ptr(), detail::node_cloner<Cloner, treap_impl>(cloner, this), detail::node_disposer<Disposer, treap_impl>(disposer, this));
  ...
  private:
    ...
    {
        header_plus_size *r = detail::parent_from_member<header_plus_size, node>(boost::intrusive::detail::to_raw_pointer(end_iterator.pointed_node()), &header_plus_size::header_);
        typename node_plus_pred_t::header_plus_priority_size *n =
            detail::parent_from_member<typename node_plus_pred_t::header_plus_priority_size, header_plus_size>(r, &node_plus_pred_t::header_plus_priority_size::header_plus_size_);
        node_plus_pred_t *pn = detail::parent_from_member<node_plus_pred_t, typename node_plus_pred_t::header_plus_priority_size>(n, &node_plus_pred_t::header_plus_priority_size_);
...
{
    typedef typename pack_options<treap_set_defaults<T>,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                  O1, O2, O3, O4
#else
                                  Options...
...
{
    /// @cond
    typedef treap_impl<typename make_treap_opt<T,
#if !defined(BOOST_INTRUSIVE_VARIADIC_TEMPLATES)
                                               O1, O2, O3, O4
#else
                                               Options...
                       ...
                       >::type
{
  ...
  public:
    ...
    template <class Iterator>
    treap(bool unique, Iterator b, Iterator e, const value_compare &cmp = value_compare(), const priority_compare &pcmp = priority_compare(), const value_traits &v_traits = value_traits())
