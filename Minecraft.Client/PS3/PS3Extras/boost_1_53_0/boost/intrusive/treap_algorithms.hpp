...
class treap_algorithms
{
  ...
  private:
    ...
    {
        for (node_ptr p_parent = NodeTraits::get_parent(p); n--; p_parent = NodeTraits::get_parent(p))
        {
            // Check if left child
  ...
  public:
    ...
    static node_ptr insert_equal_upper_bound(const node_ptr &h, const node_ptr &new_node, NodePtrCompare comp, NodePtrPriorityCompare pcomp)
    {
    ...
    //! <b>Throws</b>: If "comp" throws.
    template <class NodePtrCompare, class NodePtrPriorityCompare>
    static node_ptr insert_equal_lower_bound(const node_ptr &h, const node_ptr &new_node, NodePtrCompare comp, NodePtrPriorityCompare pcomp)
    {
        insert_commit_data commit_data;
    ...
    //! <b>Throws</b>: If "comp" throw or "pcomp" throw.
    template <class NodePtrCompare, class NodePtrPriorityCompare>
    static node_ptr insert_equal(const node_ptr &h, const node_ptr &hint, const node_ptr &new_node, NodePtrCompare comp, NodePtrPriorityCompare pcomp)
    {
        insert_commit_data commit_data;
    ...
    template <class NodePtrPriorityCompare>
    static node_ptr insert_before(const node_ptr &header, const node_ptr &pos, const node_ptr &new_node, NodePtrPriorityCompare pcomp)
    {
        insert_commit_data commit_data;
    ...
    static std::pair<node_ptr, bool> insert_unique_check(const const_node_ptr &header, const KeyType &key, KeyNodePtrCompare comp, KeyNodePtrPrioCompare pcomp, insert_commit_data &commit_data)
    {
    ...
    //!   if no more objects are inserted or erased from the set.
    template <class KeyType, class KeyNodePtrCompare, class KeyNodePtrPrioCompare>
    static std::pair<node_ptr, bool> insert_unique_check(const const_node_ptr &header, const node_ptr &hint, const KeyType &key, KeyNodePtrCompare comp, KeyNodePtrPrioCompare pcomp, insert_commit_data &commit_data)
    {
        std::pair<node_ptr, bool> ret =
  ...
  private:
    ...
    template <class NodePtrPriorityCompare>
    static void rebalance_check_and_commit(const node_ptr &h, const node_ptr &new_node, NodePtrPriorityCompare pcomp, insert_commit_data &commit_data)
    {
    ...
    static void rebalance_after_insertion_check(const const_node_ptr &header, const const_node_ptr &up, const Key &k, KeyNodePriorityCompare pcomp, std::size_t &num_rotations)
    {
