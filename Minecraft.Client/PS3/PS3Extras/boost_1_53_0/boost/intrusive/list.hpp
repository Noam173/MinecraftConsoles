...
struct list_defaults
    : pack_options<none, base_hook<detail::default_list_hook>, constant_time_size<true>, size_type<std::size_t>>::type
{
};
  ...
  public:
    ...
    void splice(const_iterator p, list_impl &x)
    {
        if (!x.empty())
        {
            ...
            node_algorithms::transfer(p.pointed_node(), x.begin().pointed_node(), x.end().pointed_node());
    ...
    {
        if (node_traits::get_next(this->get_root_node()) != node_traits::get_previous(this->get_root_node()))
        {
...
{
    /// @cond
    typedef typename pack_options<list_defaults<T>,
    ...
    typedef list_impl<
        listopt<value_traits, typename packed_options::size_type, packed_options::constant_time_size>>
