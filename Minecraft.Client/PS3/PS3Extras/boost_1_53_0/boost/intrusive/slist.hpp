...
struct slist_defaults
    : pack_options<none, base_hook<detail::default_slist_hook>, constant_time_size<true>, linear<false>, size_type<std::size_t>, cache_last<false>>::type
{
};
    ...
    : private detail::clear_on_destructor_base<slist_impl<Config>>
{
  ...
  public:
    ...
    typedef typename detail::if_c<Config::linear, linear_slist_algorithms<node_traits>, circular_slist_algorithms<node_traits>>::type node_algorithms;
  ...
  private:
    ...
    {
        return const_node_ptr(linear ? const_node_ptr() : this->get_root_node());
    }
  ...
  public:
    ...
    slist_impl(const node_ptr &f, const node_ptr &before_l, size_type n, const value_traits &v_traits = value_traits())
        : data_(v_traits)
    {
        if (n)
        {
    ...
    {
        if (node_traits::get_next(node_traits::get_next(this->get_root_node())) != this->get_root_node())
        {
            ...
            while (!this->empty())
            {
                ...
                if (constant_time_size)
                {
                    counter[i].splice_after(counter[i].cbefore_begin(), carry, carry.cbefore_begin(), last_element, carry.size());
                }
                else
                {
                    counter[i].splice_after(counter[i].cbefore_begin(), carry, carry.cbefore_begin(), last_element);
                }
                if (i == fill)
                {
                    ++fill;
            ...
            if (constant_time_size)
            {
                this->splice_after(cbefore_begin(), counter[fill], counter[fill].cbefore_begin(), last_element, counter[fill].size());
            }
            else
            {
                this->splice_after(cbefore_begin(), counter[fill], counter[fill].cbefore_begin(), last_element);
            }
        }
    }
    ...
    const_iterator previous(const_iterator prev_from, const_iterator i) const
    {
        ...
        return const_iterator(node_algorithms::get_previous_node(prev_from.pointed_node(), i.pointed_node()), this);
    }
  ...
  private:
    ...
    {
        node_ptr new_first = node_algorithms::reverse(node_traits::get_next(this->get_root_node()));
    ...
    void priv_shift_backwards(size_type n, detail::bool_<true>)
    {
        std::pair<node_ptr, node_ptr> ret(
            node_algorithms::move_first_n_forward(node_traits::get_next(this->get_root_node()), (std::size_t)n));
        if (ret.first)
        {
    ...
    {
        std::pair<node_ptr, node_ptr> ret(
            node_algorithms::move_first_n_backwards(node_traits::get_next(this->get_root_node()), (std::size_t)n));
        if (ret.first)
        {
            node_traits::set_next(this->get_root_node(), ret.first);
            if (cache_last)
    ...
    static slist_impl &priv_container_from_end_iterator(const const_iterator &end_iterator)
    {
        ...
        root_plus_size *r = detail::parent_from_member<root_plus_size, node>(boost::intrusive::detail::to_raw_pointer(end_iterator.pointed_node()), (&root_plus_size::root_));
        data_t *d = detail::parent_from_member<data_t, root_plus_size>(r, &data_t::root_plus_size_);
...
{
    /// @cond
    typedef typename pack_options<slist_defaults<T>,
    ...
    typedef slist_impl<
        slistopt<value_traits, typename packed_options::size_type, packed_options::constant_time_size, packed_options::linear, packed_options::cache_last>>
