...
class basic_string_ref
{
  public:
...
#else
        return pos > size() ? throw std::out_of_range("string_ref::substr") : basic_string_ref(data() + pos, n == npos || pos + n > size() ? size() - pos : n);
#endif
    }
    ...
    int compare(basic_string_ref x) const
    {
        ...
        return cmp != 0 ? cmp : (len_ == x.len_ ? 0 : len_ < x.len_ ? -1
                                                                    : 1);
    }
