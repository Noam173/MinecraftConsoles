...
contains(const Type &super, const typename Type::segment_type &inter_val)
{
    ...
    std::pair<const_iterator, const_iterator> exterior = super.equal_range(inter_val);
    ...
    return icl::contains(hull(*(exterior.first), *last_overlap), inter_val) && Interval_Set::is_joinable(super, exterior.first, last_overlap);
}
                 ...
                 const typename Type::segment_type &segment)
{
    ...
    std::pair<const_iterator, const_iterator> exterior = object.equal_range(segment);
    if (exterior.first == exterior.second)
    {
        return;
    }
