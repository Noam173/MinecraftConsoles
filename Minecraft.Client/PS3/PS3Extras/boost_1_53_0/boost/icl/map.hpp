...
template <>
inline std::string type_to_string<partial_absorber>::apply()
{
    return "@0";
}
...
template <>
inline std::string type_to_string<partial_enricher>::apply()
{
    return "e0";
}
...
template <>
inline std::string type_to_string<total_absorber>::apply()
{
    return "^0";
}
...
template <>
inline std::string type_to_string<total_enricher>::apply()
{
    return "e^0";
}
                                        ...
                                        Alloc<std::pair<const DomainT, CodomainT>>>
{
  ...
  public:
    ...
    {
        on_definedness<type, Traits::is_total>::add_intersection(section, *this, key_value_pair);
    }
  ...
  private:
    ...
    { // !is_total, absorbs_identities
        ...
        static void flip(Type &object, const element_type &operand)
        {
            ...
            if (!insertion.second)
            {
                on_codomain_model<Type, has_set_semantics<codomain_type>::value, true>::subtract(object, insertion.first, operand.second);
            }
        }
    };
    ...
    { // !is_total  !absorbs_identities
        ...
        static void flip(Type &object, const element_type &operand)
        {
            ...
            if (!insertion.second)
            {
                on_codomain_model<Type, has_set_semantics<codomain_type>::value, false>::subtract(object, insertion.first, operand.second);
            }
        }
    };
...
map<DomainT, CodomainT, Traits, Compare, Combine, Section, Alloc>::_add(const element_type &addend)
{
    ...
    std::pair<iterator, bool> insertion = base_type::insert(value_type(addend.first, version<Combiner>()(co_val)));
...
map<DomainT, CodomainT, Traits, Compare, Combine, Section, Alloc>::_add(iterator prior_, const value_type &addend)
{
    typedef typename on_absorbtion<type, Combiner, absorbs_identities<type>::value>::type on_absorbtion_;
    ...
    iterator inserted_ = base_type::insert(prior_,
...
struct type_to_string<icl::map<DomainT, CodomainT, Traits, Compare, Combine, Section, Alloc>>
{
    ...
    {
        return "map<" + type_to_string<DomainT>::apply() + "," + type_to_string<CodomainT>::apply() + "," + type_to_string<Traits>::apply() + ">";
    }
};
