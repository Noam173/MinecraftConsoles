...
class mapped_reference
{
  ...
  public:
    ...
    typedef typename mpl::if_<is_const<second_type>,
                              second_type &,
    ...
    mapped_reference(const mapped_reference<FstT, SndT> &source) : first(source.first), second(source.second)
    {
    }

    template <class FstT, class SndT>
    operator std::pair<FstT, SndT>()
    {
