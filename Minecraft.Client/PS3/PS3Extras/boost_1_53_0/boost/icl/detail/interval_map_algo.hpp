                 ...
                 typename IntervalMapT::const_iterator past)
{
    ...
    const typename IntervalMapT::codomain_type &co_value = icl::co_value<IntervalMapT>(first);
    while (it_ != past)
    {
         ...
         const typename IntervalMapT::interval_type &sub_interval)
{
    ...
    return icl::contains(hull(exterior.first->first, last_overlap->first), sub_interval) && Interval_Set::is_joinable(container, exterior.first, last_overlap);
}
...
typename enable_if<mpl::and_<mpl::not_<is_total<IntervalMapT>>, is_interval_set<IntervalSetT>>, bool>::type
...
template <class IntervalMapT, class IntervalSetT>
typename enable_if<mpl::and_<is_total<IntervalMapT>, is_interval_set<IntervalSetT>>, bool>::type
              ...
              const typename IntervalMapT::segment_type sub_segment)
{
    ...
    return icl::contains(hull(exterior.first->first, last_overlap->first), sub_interval) && Interval_Map::is_joinable(container, exterior.first, last_overlap);
}
