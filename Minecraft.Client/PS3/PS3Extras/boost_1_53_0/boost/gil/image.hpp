...
class image
{
  ...
  private:
    ...
    std::size_t total_allocated_size_in_bytes(const point_t &dimensions) const
    {
        ...
        const std::size_t _channels_in_image = mpl::eval_if<is_pixel<value_type>, num_channels<view_t>, mpl::int_<1>>::type::value;
        ...
        return (size_in_units + byte_to_memunit<x_iterator>::value - 1) / byte_to_memunit<x_iterator>::value + (_align_in_bytes > 0 ? _align_in_bytes - 1 : 0); // add extra padding in case we need to align the first image pixel
    }
    ...
    void allocate_(const point_t &dimensions, mpl::false_)
    ...
    void allocate_(const point_t &dimensions, mpl::true_)
    { // if it throws and _memory!=0 the client must deallocate _memory
...
template <typename Pixel, bool IsPlanar, typename Alloc>
inline const typename image<Pixel, IsPlanar, Alloc>::const_view_t const_view(const image<Pixel, IsPlanar, Alloc> &img)
{
