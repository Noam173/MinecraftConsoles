...
class jpeg_reader : public file_mgr
{
  ...
  public:
    jpeg_reader(FILE *file) : file_mgr(file)
    {
        init();
    }
    jpeg_reader(const char *filename) : file_mgr(filename, "rb")
    {
        init();
    }
...
class jpeg_writer : public file_mgr
{
  ...
  public:
    jpeg_writer(FILE *file) : file_mgr(file)
    {
        init();
    }
    jpeg_writer(const char *filename) : file_mgr(filename, "wb")
    {
        init();
    }
    ~jpeg_writer()
