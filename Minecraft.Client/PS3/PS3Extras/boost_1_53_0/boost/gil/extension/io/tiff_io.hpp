...
class tiff_reader_color_convert : public tiff_reader
{
  ...
  public:
    tiff_reader_color_convert(const char *filename, tdir_t dirnum = 0) : tiff_reader(filename, dirnum)
    {
    }
    tiff_reader_color_convert(const char *filename, CC cc_in, tdir_t dirnum = 0) : tiff_reader(filename, dirnum), _cc(cc_in)
    {
    }
    template <typename View>
    void apply(const View &view)
...
class tiff_writer
{
  ...
  public:
    ...
    void apply(const View &view)
    {
        ...
        io_error_if(TIFFSetField(_tp, TIFFTAG_PHOTOMETRIC, tiff_write_support_private<typename channel_type<View>::type, typename color_space_type<View>::type>::color_type) != 1);
        ...
        io_error_if(TIFFSetField(_tp, TIFFTAG_BITSPERSAMPLE, tiff_write_support_private<typename channel_type<View>::type, typename color_space_type<View>::type>::bit_depth) != 1);
