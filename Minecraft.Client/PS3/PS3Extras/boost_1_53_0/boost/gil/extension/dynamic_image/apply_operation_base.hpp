...
#define GIL_FWD_CONST_CASE(z, N, SUM) \
    case N:                           \
        return op(*gil_reinterpret_cast_c<const typename mpl::deref<T##N>::type *>(&bits));

#define GIL_FWD_CASE_WITH_INFO(z, N, SUM) \
    case N:                               \
        return op(*gil_reinterpret_cast<typename mpl::deref<T##N>::type *>(&bits), info);
#define GIL_FWD_CONST_CASE_WITH_INFO(z, N, SUM) \
    case N:                                     \
        return op(*gil_reinterpret_cast_c<const typename mpl::deref<T##N>::type *>(&bits), info);

#define GIL_APPLY_FWD_OP(z, N, text)                                                                                  \
    template <>                                                                                                       \
    struct apply_operation_fwd_fn<BOOST_PP_ADD(N, 1)>                                                                 \
    {                                                                                                                 \
        template <typename Types, typename Bits, typename UnaryOp>                                                    \
        typename UnaryOp::result_type apply(Bits &bits, std::size_t index, UnaryOp op) const                          \
        {                                                                                                             \
...
struct reduce_bind1
{
    ...
    GIL_FORCEINLINE result_type operator()(const T1 &t1)
    {
        return _op(t1, _t2);
    }
};
