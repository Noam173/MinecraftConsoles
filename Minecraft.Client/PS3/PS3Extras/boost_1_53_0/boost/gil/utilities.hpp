...
GIL_FORCEINLINE
    point2<T>
    operator+(const point2<T> &p1, const point2<T> &p2)
{
...
GIL_FORCEINLINE
    point2<double>
    operator/(const point2<T> &p, double t)
{
...
GIL_FORCEINLINE
    point2<T>
    operator*(const point2<T> &p, std::ptrdiff_t t)
{
...
GIL_FORCEINLINE
    point2<T>
    operator*(std::ptrdiff_t t, const point2<T> &p)
{
    return point2<T>(p.x * t, p.y * t);
}
                          ...
                          typename D2::argument_type, typename D1::result_type, D1::is_mutable && D2::is_mutable>
{
  public:
    ...
    deref_compose(const deref_compose<_D1, _D2> &dc) : _fn1(dc._fn1), _fn2(dc._fn2)
    {
    }
