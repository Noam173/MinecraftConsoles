    ...
    : public detail::homogeneous_color_base<ChannelReference, layout<ColorSpace>, mpl::size<ColorSpace>::value>
{
  ...
  public:
    ...
    template <typename P>
    planar_pixel_reference(const P &p) : parent_t(p)
    {
        check_compatible<P>();
    }
    ...
    planar_pixel_reference(const planar_pixel_iterator<ChannelPtr, ColorSpace> &p, std::ptrdiff_t diff) : parent_t(p, diff)
    {
    }
...
inline void swap(typename boost::gil::planar_pixel_reference<CR, CS>::value_type &x, const boost::gil::planar_pixel_reference<CR, CS> y)
{
    boost::gil::swap_proxy<typename boost::gil::planar_pixel_reference<CR, CS>::value_type>(x, y);
}
...
/// \ingroup PixelModelPlanarRef
template <typename CR, typename CS>
inline void swap(const boost::gil::planar_pixel_reference<CR, CS> x, const boost::gil::planar_pixel_reference<CR, CS> y)
{
    boost::gil::swap_proxy<typename boost::gil::planar_pixel_reference<CR, CS>::value_type>(x, y);
}
} // namespace std
