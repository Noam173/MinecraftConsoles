...
GIL_FORCEINLINE
    boost::gil::planar_pixel_iterator<IC2, Cs>
...
class copy_and_convert_pixels_fn : public binary_operation_obj<copy_and_convert_pixels_fn<CC>>
{
  ...
  public:
    ...
    GIL_FORCEINLINE
        result_type
        apply_incompatible(const V1 &src, const V2 &dst) const
    {
    ...
    GIL_FORCEINLINE
        result_type
        apply_compatible(const V1 &src, const V2 &dst) const
    {
        copy_pixels(src, dst);
    }
};
...
template <typename It, typename P>
GIL_FORCEINLINE void uninitialized_fill_aux(It first, It last,
                                            const P &p, mpl::true_)
{
    int channel = 0;
    try
...
GIL_FORCEINLINE void uninitialized_copy_aux(It1 first1, It1 last1,
