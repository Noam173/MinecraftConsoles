...
class strided_iterator
    : public iterator_adaptor<
          strided_iterator<BaseIterator, Category>, BaseIterator, use_default, boost::forward_traversal_tag>
{
  ...
  public:
    ...
    strided_iterator(base_iterator first, base_iterator it, base_iterator last, difference_type stride)
        : super_t(it), m_last(last), m_stride(stride)
    {
    }
    ...
    strided_iterator(const strided_iterator<OtherIterator, Category> &other,
        ...
        : super_t(other), m_last(other.base_end()), m_stride(other.get_stride())
    {
    }
...
class strided_iterator<BaseIterator, bidirectional_traversal_tag>
    : public iterator_adaptor<
          strided_iterator<BaseIterator, bidirectional_traversal_tag>, BaseIterator, use_default, bidirectional_traversal_tag>
{
    friend class ::boost::iterator_core_access;
  ...
  public:
    ...
    strided_iterator()
        : m_first(), m_last(), m_stride()
    {
    }

    strided_iterator(base_iterator first, base_iterator it, base_iterator last, difference_type stride)
        : super_t(it), m_first(first), m_last(last), m_stride(stride)
    {
    }
    ...
    strided_iterator(const strided_iterator<OtherIterator, bidirectional_traversal_tag> &other,
                     BOOST_DEDUCED_TYPENAME enable_if_convertible<OtherIterator, base_iterator>::type * = 0)
        : super_t(other.base()), m_first(other.base_begin()), m_last(other.base_end()), m_stride(other.get_stride())
    {
    }
...
class strided_iterator<BaseIterator, random_access_traversal_tag>
    : public iterator_adaptor<
          strided_iterator<BaseIterator, random_access_traversal_tag>, BaseIterator, use_default, random_access_traversal_tag>
{
    friend class ::boost::iterator_core_access;
  ...
  public:
    ...
    strided_iterator()
        : m_first(), m_last(), m_index(0), m_stride()
    {
    }

    strided_iterator(BaseIterator first, BaseIterator it, BaseIterator last, difference_type stride)
        : super_t(it), m_first(first), m_last(last), m_index(stride ? (it - first) / stride : 0), m_stride(stride)
    {
    }
    ...
    strided_iterator(const strided_iterator<OtherIterator, random_access_traversal_tag> &other,
        ...
        : super_t(other.base()), m_first(other.base_begin()), m_last(other.base_end()), m_index(other.get_index()), m_stride(other.get_stride())
    {
    }
...
template <class BaseIterator, class Difference>
inline strided_iterator<BaseIterator, BOOST_DEDUCED_TYPENAME iterator_traversal<BaseIterator>::type>
...
template <class Rng, class Category = BOOST_DEDUCED_TYPENAME iterator_traversal<
                         BOOST_DEDUCED_TYPENAME range_iterator<Rng>::type>::type>
class strided_range
