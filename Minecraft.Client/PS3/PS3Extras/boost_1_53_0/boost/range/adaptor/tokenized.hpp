...
template <class R>
struct tokenized_range : public boost::iterator_range<
                             boost::regex_token_iterator<
                                 BOOST_DEDUCED_TYPENAME range_iterator<R>::type>>
{
  private:
    typedef boost::regex_token_iterator<
        BOOST_DEDUCED_TYPENAME range_iterator<R>::type>
        regex_iter;
...
struct regex_holder
{
    ...
    regex_holder(const T &rex, const U &subm, V flag) : re(rex), sub(subm), f(flag)
    {
    }
