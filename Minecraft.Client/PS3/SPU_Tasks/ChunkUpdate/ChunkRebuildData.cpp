    ...
    for (int iX = startX; iX < (startX + 16); iX++)
    {
        for (int iY = startY; iY < (startY + 16); iY++)
        {
            for (int iZ = startZ; iZ < (startZ + 16); iZ++)
            {
                ...
                if ((iX - startX == 0) || (iX - startX == 15))
                {
                    continue;
                }
                if ((iZ - startZ == 0) || (iZ - startZ == 15))
                {
                    continue;
                }
                ...
                if (!((tileID == Tile_SPU::rock_Id) || (tileID == Tile_SPU::dirt_Id) || (tileID == Tile_SPU::unbreakable_Id) || (flags & e_flag_NoRender)))
                {
                    continue;
                }
...
int ChunkRebuildData::getMaterialID(Tile *pTile)
{
    ...
    if (m == Material::grass)
    {
        return Material_SPU::grass_Id;
    }
    if (m == Material::dirt)
    {
        return Material_SPU::dirt_Id;
    }
    if (m == Material::wood)
    {
        return Material_SPU::wood_Id;
    }
    if (m == Material::stone)
    {
        return Material_SPU::stone_Id;
    }
    if (m == Material::metal)
    {
        return Material_SPU::metal_Id;
    }
    if (m == Material::water)
    {
        return Material_SPU::water_Id;
    }
    if (m == Material::lava)
    {
        return Material_SPU::lava_Id;
    }
    if (m == Material::leaves)
    {
        return Material_SPU::leaves_Id;
    }
    if (m == Material::plant)
    {
        return Material_SPU::plant_Id;
    }
    if (m == Material::replaceable_plant)
    {
        return Material_SPU::replaceable_plant_Id;
    }
    if (m == Material::sponge)
    {
        return Material_SPU::sponge_Id;
    }
    if (m == Material::cloth)
    {
        return Material_SPU::cloth_Id;
    }
    if (m == Material::fire)
    {
        return Material_SPU::fire_Id;
    }
    if (m == Material::sand)
    {
        return Material_SPU::sand_Id;
    }
    if (m == Material::decoration)
    {
        return Material_SPU::decoration_Id;
    }
    if (m == Material::clothDecoration)
    {
        return Material_SPU::clothDecoration_Id;
    }
    if (m == Material::glass)
    {
        return Material_SPU::glass_Id;
    }
    if (m == Material::explosive)
    {
        return Material_SPU::explosive_Id;
    }
    if (m == Material::coral)
    {
        return Material_SPU::coral_Id;
    }
    if (m == Material::ice)
    {
        return Material_SPU::ice_Id;
    }
    if (m == Material::topSnow)
    {
        return Material_SPU::topSnow_Id;
    }
    if (m == Material::snow)
    {
        return Material_SPU::snow_Id;
    }
    if (m == Material::cactus)
    {
        return Material_SPU::cactus_Id;
    }
    if (m == Material::clay)
    {
        return Material_SPU::clay_Id;
    }
    if (m == Material::vegetable)
    {
        return Material_SPU::vegetable_Id;
    }
    if (m == Material::egg)
    {
        return Material_SPU::egg_Id;
    }
    if (m == Material::portal)
    {
        return Material_SPU::portal_Id;
    }
    if (m == Material::cake)
    {
        return Material_SPU::cake_Id;
    }
    if (m == Material::web)
    {
        return Material_SPU::web_Id;
    }
    if (m == Material::piston)
    {
        return Material_SPU::piston_Id;
    }
    if (m == Material::buildable_glass)
    {
        return Material_SPU::buildable_glass_Id;
    }
    if (m == Material::heavyMetal)
    {
        return Material_SPU::heavyMetal_Id;
    }
    assert(0);
    return Material_SPU::air_Id;
}
