...
bool TileRenderer_SPU::tesselateTorchInWorld(Tile_SPU *tt, int x, int y, int z)
{
    ...
    else
    {
        ...
        if (level->m_tileData.lightEmission[tt->id] > 0)
        {
            br = 1.0f;
        }
        t->color(br, br, br);
...
bool TileRenderer_SPU::tesselateLeverInWorld(Tile_SPU *tt, int x, int y, int z)
{
    ...
    if (!hadFixed)
    {
        this->setFixedTexture(getTexture(Tile_SPU::stoneBrick));
    }
    float w1 = 4.0f / 16.0f;
    float w2 = 3.0f / 16.0f;
    float h = 3.0f / 16.0f;
    ...
    if (Tile_SPU::lightEmission[tt->id] > 0)
    {
        br = 1.0f;
    }
    t->color(br, br, br);
...
bool TileRenderer_SPU::tesselateFireInWorld(FireTile_SPU *tt, int x, int y, int z)
{
    ...
    else
    {
        ...
        if (FireTile_SPU::canBurn(level, x, y + 1.0f, z))
        {
            ...
            if (((x + y + z) & 1) == 0)
            {
                t->vertexUV((float)(x0_), (float)(y + h), static_cast<float>(z + 0), (float)(u1), (float)(v0));
                t->vertexUV((float)(x0), static_cast<float>(y + 0), static_cast<float>(z + 0), (float)(u1), (float)(v1));
                t->vertexUV((float)(x0), static_cast<float>(y + 0), static_cast<float>(z + 1), (float)(u0), (float)(v1));
                t->vertexUV((float)(x0_), (float)(y + h), static_cast<float>(z + 1), (float)(u0), (float)(v0));
                ...
                t->vertexUV((float)(x1_), (float)(y + h), (float)(z + 1.0f), (float)(u1), (float)(v0));
                t->vertexUV((float)(x1), (float)(y + 0.0f), (float)(z + 1.0f), (float)(u1), (float)(v1));
                t->vertexUV((float)(x1), (float)(y + 0.0f), static_cast<float>(z + 0), (float)(u0), (float)(v1));
                t->vertexUV((float)(x1_), (float)(y + h), static_cast<float>(z + 0), (float)(u0), (float)(v0));
            }
            else
            {
                t->vertexUV((float)(x + 0.0f), (float)(y + h), (float)(z1_), (float)(u1), (float)(v0));
                t->vertexUV((float)(x + 0.0f), (float)(y + 0.0f), (float)(z1), (float)(u1), (float)(v1));
                t->vertexUV((float)(x + 1.0f), (float)(y + 0.0f), (float)(z1), (float)(u0), (float)(v1));
                t->vertexUV((float)(x + 1.0f), (float)(y + h), (float)(z1_), (float)(u0), (float)(v0));
                ...
                t->vertexUV((float)(x + 1.0f), (float)(y + h), (float)(z0_), (float)(u1), (float)(v0));
                t->vertexUV((float)(x + 1.0f), (float)(y + 0.0f), (float)(z0), (float)(u1), (float)(v1));
                t->vertexUV((float)(x + 0.0f), (float)(y + 0.0f), (float)(z0), (float)(u0), (float)(v1));
                t->vertexUV((float)(x + 0.0f), (float)(y + h), (float)(z0_), (float)(u0), (float)(v0));
            }
        }
    }
    return true;
...
bool TileRenderer_SPU::tesselateDustInWorld(Tile_SPU *tt, int x, int y, int z)
{
    ...
    bool w = RedStoneDustTile_SPU::shouldConnectTo(level, x - 1, y, z, Direction::WEST) || (!level->isSolidBlockingTile(x - 1, y, z) && RedStoneDustTile_SPU::shouldConnectTo(level, x - 1, y - 1, z,
                                                                                                                                                                              Direction::UNDEFINED));
    bool e = RedStoneDustTile_SPU::shouldConnectTo(level, x + 1, y, z, Direction::EAST) || (!level->isSolidBlockingTile(x + 1, y, z) && RedStoneDustTile_SPU::shouldConnectTo(level, x + 1, y - 1, z,
                                                                                                                                                                              Direction::UNDEFINED));
    bool n = RedStoneDustTile_SPU::shouldConnectTo(level, x, y, z - 1, Direction::NORTH) || (!level->isSolidBlockingTile(x, y, z - 1) && RedStoneDustTile_SPU::shouldConnectTo(level, x, y - 1, z - 1,
                                                                                                                                                                               Direction::UNDEFINED));
    bool s = RedStoneDustTile_SPU::shouldConnectTo(level, x, y, z + 1, Direction::SOUTH) || (!level->isSolidBlockingTile(x, y, z + 1) && RedStoneDustTile_SPU::shouldConnectTo(level, x, y - 1, z + 1,
                                                                                                                                                                               Direction::UNDEFINED));
    ...
    if (pic == 0)
    {
        ...
        if (!w)
        {
            x0 += cutDistance / (float)SharedConstants::WORLD_RESOLUTION;
        }
        if (!w)
        {
            u0 += cutDistance;
        ...
        if (!e)
        {
            x1 -= cutDistance / (float)SharedConstants::WORLD_RESOLUTION;
        }
        if (!e)
        {
            u1 -= cutDistance;
        ...
        if (!n)
        {
            z0 += cutDistance / (float)SharedConstants::WORLD_RESOLUTION;
        }
        if (!n)
        {
            v0 += cutDistance;
        ...
        if (!s)
        {
            z1 -= cutDistance / (float)SharedConstants::WORLD_RESOLUTION;
        }
        if (!s)
        {
            v1 -= cutDistance;
                                                                             ...
                                                                             float pBaseBlue)
{
    ...
    if (faceFlags & 0x01)
    {
        if (blsmooth > 0)
        {
            if (tileShapeY0 <= 0)
            {
                pY--; // 4J - condition brought forwardEnterCriticalSection from 1.2.3
            }
...
{
    if (renderShape == Tile_SPU::SHAPE_BLOCK)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_TREE)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_QUARTZ)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_CACTUS)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_STAIRS)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_FENCE)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_EGG)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_ENTITYTILE_ANIMATED)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_FENCE_GATE)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_PISTON_BASE)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_PORTAL_FRAME)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_WALL)
    {
        return true;
    }
    if (renderShape == Tile_SPU::SHAPE_ANVIL)
    {
        return true;
    }
    return false;
