...
class PPUStoreArray
{
  ...
  public:
    PPUStoreArray(uintptr_t pDataPPU)
    {
        m_pDataPPU = (int *)pDataPPU;
        m_cachePos = 0;
        m_ppuPos = 0;
    }
...
bool LevelRenderer_FindNearestChunk_DataIn::MultiplayerChunkCache::getChunkEmpty(int lowerOffset, int upperOffset, int x, int y, int z)
{
    ...
    if ((ix < 0) || (ix >= XZSIZE))
    {
        return false; // ( waterChunk ? waterChunk : emptyChunk );
    }
    if ((iz < 0) || (iz >= XZSIZE))
    {
        return false; //( waterChunk ? waterChunk : emptyChunk );
...
void LevelRenderer_FindNearestChunk_DataIn::findNearestChunk()
{
    ...
    for (int p = 0; p < 4; p++)
    {
        ...
        if (chunkLengths[p] != xChunks * zChunks * CHUNK_Y_COUNT)
        {
            continue;
        }
        int px = (int)player->x;
