...
HRESULT CConsoleMinecraftApp::NavigateBack(int iPad, bool forceUsePad /*= false*/, EUIScene eScene /*= eUIScene_COUNT*/)
{
    ...
    if (m_hCurrentScene[idx] == this->m_hFirstScene[idx])
    {
        SetMenuDisplayed(idx, false);
    }
...
{
    if (m_hFirstChatScene[iPad] == nullptr || m_hCurrentChatScene[iPad] == nullptr)
    {
        return;
    }
...
{
    if (m_hFirstHudScene[iPad] == nullptr || m_hCurrentHudScene[iPad] == nullptr)
    {
        return;
    }
