...
{
    return L"Queue messages: " + std::to_wstring(static_cast<NetworkPlayerXbox *>(GetHostPlayer())->GetQNetPlayer()->GetSendQueueSize(nullptr, QNET_GETSENDQUEUESIZE_MESSAGES)) + L" Queue bytes: " + std::to_wstring(static_cast<NetworkPlayerXbox *>(GetHostPlayer())->GetQNetPlayer()->GetSendQueueSize(nullptr, QNET_GETSENDQUEUESIZE_BYTES));
}
    ...
    if (bMultiplayerAllowed)
    {
        ...
        for (unsigned int i = 0; i < itemsReturned; i++)
        {
            ...
            if ((friends[i].dwFriendState & flagPlayingOnline) == flagPlayingOnline &&
                ...
                (friends[i].dwFriendState & (XONLINE_FRIENDSTATE_FLAG_SENTREQUEST | XONLINE_FRIENDSTATE_FLAG_RECEIVEDREQUEST)) == 0 && friends[i].dwTitleID == TITLEID_MINECRAFT)
            {
...
vector<FriendSessionInfo *> *CPlatformNetworkManagerXbox::GetSessionList(int iPad, int localPlayers, bool partyOnly)
{
    ...
    if (m_currentSearchResultsCount[iPad] > 0)
    {
        ...
        for (DWORD dwResult = 0; dwResult < m_currentSearchResultsCount[iPad]; dwResult++)
        {
            ...
            if (pSearchResult == nullptr || pSearchResult->dwOpenPublicSlots < localPlayers)
            {
                continue;
            }
...
bool CPlatformNetworkManagerXbox::GetGameSessionInfo(int iPad, SessionID sessionId, FriendSessionInfo *foundSessionInfo)
{
    ...
    if (m_currentSearchResultsCount[iPad] > 0)
    {
        ...
        for (DWORD dwResult = 0; dwResult < m_currentSearchResultsCount[iPad]; dwResult++)
        {
            ...
            if (memcmp(&pSearchResult->info.sessionID, &sessionId, sizeof(SessionID)) != 0)
            {
                continue;
            }
            ...
            if (pxnqi->bFlags & XNET_XNQOSINFO_TARGET_CONTACTED)
            {
                    ...
                    sessionInfo->data.isJoinable)
                {
                    ...
                    if (foundSessionInfo->displayLabel != nullptr)
                    {
