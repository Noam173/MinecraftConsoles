...
HRESULT XUI_FontRenderer::CreateFont(const TypefaceDescriptor *pTypefaceDescriptor, float fPointSize, DWORD dwStyle, DWORD dwReserved, HFONTOBJ *phFont)
{
    ...
    while (xuiSize % 8 != 0 && xuiSize % 12 != 0)
    {
        xuiSize -= 2;
    }
    ...
    if (font == nullptr)
    {
        ...
        if (fontData == nullptr)
        {
            ...
            switch (efontdata)
            {
            case eFontData_Mojangles_7:
                sfontdata = &SFontData::Mojangles_7;
                break;
            case eFontData_Mojangles_11:
...
VOID XUI_FontRenderer::ReleaseFont(HFONTOBJ hFont)
{
    ...
    if (xuiFont != nullptr)
    {
        ...
        if (xuiFont->refCount <= 0)
        {
            ...
            if (it != m_loadedFonts[xuiFont->m_iFontData].end())
            {
                m_loadedFonts[xuiFont->m_iFontData].erase(it);
            }
            delete hFont;
        }
    }
...
HRESULT XUI_FontRenderer::DrawCharsToDevice(HFONTOBJ hFont, CharData *pCharData, DWORD dwCount, RECT *pClipRect, HXUIDC hDC, D3DXMATRIX *pWorldViewProj)
{
    ...
    while (i < dwCount)
    {
        ...
        if ((style & XUI_FONT_STYLE_DROPSHADOW) == XUI_FONT_STYLE_DROPSHADOW)
        {
            dropShadow = true;
        }
