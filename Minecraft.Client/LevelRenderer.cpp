...
void LevelRenderer::renderEntities(Vec3 *cam, Culler *culler, float a)
{
    ...
    for (auto &entity : entities)
    {
        ...
        if (shouldRender)
        {
            ...
            if (localplayer && entity == mc->cameraTargetPlayer && !localplayer->ThirdPersonView() && !mc->cameraTargetPlayer->isSleeping())
            {
                continue;
            }
...
int LevelRenderer::renderChunks(int from, int to, int layer, double alpha)
{
    ...
    for (int i = 0; i < chunks[playerIndex].length; i++, pClipChunk++)
    {
        ...
        if (!(globalChunkFlags[pClipChunk->globalIdx] & LevelRenderer::CHUNK_FLAG_CUT_OUT))
        {
...
void LevelRenderer::renderAdvancedClouds(float alpha)
{
    ...
    if (activePlayers() > 2)
    {
        radius = 2; // 4J - reduce the cloud render distance a bit for 3 & 4 player split screen
    }
    float e = 1 / 1024.0f;
    glScalef(ss, 1, ss);
...
bool LevelRenderer::updateDirtyChunks()
{
    ...
    */
    ...
    do
    {
        ...
        if (index == 1)
        {
            dirtyChunkPresent = true; // 1 is a special value passed to let this thread know that a chunk which isn't on this stack has been set to dirty
        }
        else if (index > 1)
        {
            int i2 = index - 2;
...
bool inline clip(float *bb, float *frustum)
{
    ...
    {
        if (frustum[0] * (bb[0]) + frustum[1] * (bb[1]) + frustum[2] * (bb[2]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[3]) + frustum[1] * (bb[1]) + frustum[2] * (bb[2]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[0]) + frustum[1] * (bb[4]) + frustum[2] * (bb[2]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[3]) + frustum[1] * (bb[4]) + frustum[2] * (bb[2]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[0]) + frustum[1] * (bb[1]) + frustum[2] * (bb[5]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[3]) + frustum[1] * (bb[1]) + frustum[2] * (bb[5]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[0]) + frustum[1] * (bb[4]) + frustum[2] * (bb[5]) + frustum[3] > 0)
        {
            continue;
        }
        if (frustum[0] * (bb[3]) + frustum[1] * (bb[4]) + frustum[2] * (bb[5]) + frustum[3] > 0)
        {
            continue;
        }
...
int LevelRenderer::getGlobalIndexForChunk(int x, int y, int z, int dimensionId)
{
    ...
    if ((xx < 0) || (xx >= MAX_LEVEL_RENDER_SIZE[dimIdx]))
    {
        return -1;
    }
    if ((zz < 0) || (zz >= MAX_LEVEL_RENDER_SIZE[dimIdx]))
    {
        return -1;
    }
    if ((yy < 0) || (yy >= CHUNK_Y_COUNT))
    {
        return -1;
    }
