...
void MultiplayerLocalPlayer::tick()
{
    ...
    if (!level->hasChunkAt(Mth::floor(x), 0, Mth::floor(z)))
    {
        return;
    }
...
void MultiplayerLocalPlayer::sendPosition()
{
    ...
    if (sprinting != lastSprinting)
    {
        if (sprinting)
        {
            connection->send(std::make_shared<PlayerCommandPacket>(shared_from_this(), PlayerCommandPacket::START_SPRINTING));
        }
        else
        {
            connection->send(std::make_shared<PlayerCommandPacket>(shared_from_this(), PlayerCommandPacket::STOP_SPRINTING));
        }
    ...
    if (sneaking != lastSneaked)
    {
        if (sneaking)
        {
            connection->send(std::make_shared<PlayerCommandPacket>(shared_from_this(), PlayerCommandPacket::START_SNEAKING));
        }
        else
        {
            connection->send(std::make_shared<PlayerCommandPacket>(shared_from_this(), PlayerCommandPacket::STOP_SNEAKING));
        }
    ...
    if (idle != lastIdle)
    {
        if (idle)
        {
            connection->send(std::make_shared<PlayerCommandPacket>(shared_from_this(), PlayerCommandPacket::START_IDLEANIM));
        }
        else
        {
            connection->send(std::make_shared<PlayerCommandPacket>(shared_from_this(), PlayerCommandPacket::STOP_IDLEANIM));
        }
...
void MultiplayerLocalPlayer::setAndBroadcastCustomSkin(DWORD skinId)
{
    ...
    {
        connection->send(std::make_shared<TextureAndGeometryChangePacket>(shared_from_this(), app.GetPlayerSkinName(GetXboxPad())));
    }
}
...
void MultiplayerLocalPlayer::setAndBroadcastCustomCape(DWORD capeId)
{
    ...
    {
        connection->send(std::make_shared<TextureChangePacket>(shared_from_this(), TextureChangePacket::e_TextureChange_Cape, app.GetPlayerCapeName(GetXboxPad())));
    }
}
