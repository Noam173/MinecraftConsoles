...
{
    if (!shouldProcessForEntity(packet->itemId))
    {
        return;
    }
...
void ClientConnection::handleAnimate(shared_ptr<AnimatePacket> packet)
{
    ...
    else if (packet->action == AnimatePacket::WAKE_UP)
    {
        ...
        {
            dynamic_pointer_cast<Player>(e)->stopSleepInBed(false, false, false);
        }
    }
...
//	printf("Client: handlePreLogin\n");
        ...
        for (DWORD i = 0; i < packet->m_dwPlayerCount; ++i)
        {
            ...
            for (DWORD idx = 0; idx < XUSER_MAX_COUNT; ++idx)
            {
                if (ProfileManager.IsSignedInLive(idx))
                {
                    ...
                    if (ProfileManager.AreXUIDSEqual(playerXUID, packet->m_playerXuids[i]))
                    {
                        localPlayer = true;
                    }
                }
    ...
    if (!canPlay || !canPlayLocal || !isAtLeastOneFriend || cantPlayContentRestricted)
    {
        ...
        else
        {
            ...
            {
                reason = DisconnectPacket::eDisconnect_ContentRestricted_Single_Local;
            }
            ...
            if (!isFriendsWithHost)
            {
                ui.RequestErrorMessage(IDS_CANTJOIN_TITLE, IDS_NOTALLOWED_FRIENDSOFFRIENDS, uiIDA, 1, m_userIndex);
            }
            else
            {
                ui.RequestErrorMessage(IDS_CANTJOIN_TITLE, IDS_NO_USER_CREATED_CONTENT_PRIVILEGE_SINGLE_LOCAL, uiIDA, 1, m_userIndex);
            }
...
{
    shared_ptr<Entity> e = getEntity(packet->id);
    if ((e == nullptr) || !e->instanceof(eTYPE_PLAYER))
    {
        return;
    }
    shared_ptr<Player> player = dynamic_pointer_cast<Player>(e);
...
void ClientConnection::handleGameEvent(shared_ptr<GameEventPacket> gameEventPacket)
{
    ...
    else if (event == GameEventPacket::WIN_GAME)
    {
        ...
        {
            minecraft->localgameModes[ProfileManager.GetPrimaryPad()]->getTutorial()->showTutorialPopup(false);
        }
...
{
    if (!shouldProcessForPosition(packet->x, packet->z))
    {
        return;
    }
...
{
    shared_ptr<Entity> e = getEntity(packet->entityId);
    if ((e == nullptr) || !e->instanceof(eTYPE_LIVINGENTITY))
    {
        return;
    }
        ...
        if (Player::getPlayerGamePrivilege(newPrivileges, priv) != Player::getPlayerGamePrivilege(oldPrivileges, priv))
        {
            ...
            if (app.GetGameHostOption(eGameHostOption_TrustPlayers) == 0)
            {
                ...
                case Player::ePlayerGamePrivilege_CanUseDoorsAndSwitches:
                    ...
                    {
                        message = app.GetString(IDS_PRIV_USE_DOORS_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_CanUseContainers:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_USE_CONTAINERS_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_USE_CONTAINERS_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_CannotAttackAnimals:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_ATTACK_ANIMAL_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_ATTACK_ANIMAL_TOGGLE_OFF);
                    }
                    break;
                ...
                case Player::ePlayerGamePrivilege_CannotAttackPlayers:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_ATTACK_PLAYER_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_ATTACK_PLAYER_TOGGLE_OFF);
                    }
                    break;
                };
            }
            ...
            if (app.GetGameHostOption(eGameHostOption_CheatsEnabled) != 0)
            {
                ...
                case Player::ePlayerGamePrivilege_Invisible:
                    ...
                    {
                        message = app.GetString(IDS_PRIV_INVISIBLE_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_Invulnerable:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_INVULNERABLE_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_INVULNERABLE_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_CanToggleInvisible:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_CAN_INVISIBLE_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_CAN_INVISIBLE_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_CanToggleFly:
                    ...
                    {
                        message = app.GetString(IDS_PRIV_CAN_FLY_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_CanToggleClassicHunger:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_CAN_EXHAUSTION_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_CAN_EXHAUSTION_TOGGLE_OFF);
                    }
                    break;
                case Player::ePlayerGamePrivilege_CanTeleport:
                    if (privOn)
                    {
                        message = app.GetString(IDS_PRIV_CAN_TELEPORT_TOGGLE_ON);
                    }
                    else
                    {
                        message = app.GetString(IDS_PRIV_CAN_TELEPORT_TOGGLE_OFF);
                    }
                    break;
                };
            }
...
void ClientConnection::handleUpdateProgress(shared_ptr<UpdateProgressPacket> packet)
{
    if (!g_NetworkManager.IsHost())
    {
        Minecraft::GetInstance()->progressRenderer->progressStagePercentage(packet->m_percentage);
    }
}
...
{
    if (deferredEntityLinkPackets.empty())
    {
        return;
    }
