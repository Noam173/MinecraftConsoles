...
void PlayerConnection::teleport(double x, double y, double z, float yRot, float xRot, bool sendPacket /*= true*/)
{
    ...
    if (sendPacket)
    {
        player->connection->send(std::make_shared<MovePlayerPacket::PosRot>(x, y + 1.62f, y, z, yRot, xRot, false, false));
    }
}
...
void PlayerConnection::handlePlayerAction(shared_ptr<PlayerActionPacket> packet)
{
    ...
    if (packet->action == PlayerActionPacket::START_DESTROY_BLOCK)
    {
        if (true)
        {
            player->gameMode->startDestroyBlock(x, y, z, packet->face); // 4J - condition was !server->isUnderSpawnProtection(level, x, y, z, player) (from Java 1.6.4) but putting back to old behaviour
        }
        else
        {
...
void PlayerConnection::handleContainerSetSlot(shared_ptr<ContainerSetSlotPacket> packet)
{
    ...
    { // Still allow creative players to change slots manually with packets(?) -- might want this different.
        ...
        if (serverPlayer != nullptr)
        {
            ...
            if (serverPlayer == player)
            {
                ...
                if (cheats)
                {
                    ...
                    if (canBeInvisible)
                    {
                        serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invisible, Player::getPlayerGamePrivilege(packet->m_playerPrivileges, Player::ePlayerGamePrivilege_Invisible));
                    }
                    if (canBeInvisible)
                    {
                        serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invulnerable, Player::getPlayerGamePrivilege(packet->m_playerPrivileges, Player::ePlayerGamePrivilege_Invulnerable));
                    }
                    ...
                    if (!inCreativeMode)
                    {
                        ...
                        if (canFly)
                        {
                            serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_CanFly, Player::getPlayerGamePrivilege(packet->m_playerPrivileges, Player::ePlayerGamePrivilege_CanFly));
                        }
                        if (canChangeHunger)
                        {
                            serverPlayer->setPlayerGamePrivilege(Player::ePlayerGamePrivilege_ClassicHunger, Player::getPlayerGamePrivilege(packet->m_playerPrivileges, Player::ePlayerGamePrivilege_ClassicHunger));
                        }
                    }
                }
            }
...
void PlayerConnection::handleCraftItem(shared_ptr<CraftItemPacket> packet)
{
    ...
    case Item::pickAxe_wood_Id:
        player->awardStat(GenericStats::buildPickaxe(), GenericStats::param_buildPickaxe());
        break;
    case Tile::furnace_Id:
        player->awardStat(GenericStats::buildFurnace(), GenericStats::param_buildFurnace());
        break;
    case Item::hoe_wood_Id:
        player->awardStat(GenericStats::buildHoe(), GenericStats::param_buildHoe());
        break;
    case Item::bread_Id:
        player->awardStat(GenericStats::makeBread(), GenericStats::param_makeBread());
        break;
    case Item::cake_Id:
        player->awardStat(GenericStats::bakeCake(), GenericStats::param_bakeCake());
        break;
    case Item::pickAxe_stone_Id:
        player->awardStat(GenericStats::buildBetterPickaxe(), GenericStats::param_buildBetterPickaxe());
        break;
    case Item::sword_wood_Id:
        player->awardStat(GenericStats::buildSword(), GenericStats::param_buildSword());
        break;
    case Tile::dispenser_Id:
        player->awardStat(GenericStats::dispenseWithThis(), GenericStats::param_dispenseWithThis());
        break;
    case Tile::enchantTable_Id:
        player->awardStat(GenericStats::enchantments(), GenericStats::param_enchantments());
        break;
    case Tile::bookshelf_Id:
        player->awardStat(GenericStats::bookcase(), GenericStats::param_bookcase());
        break;
    }
    //}
