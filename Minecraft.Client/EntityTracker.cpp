...
void EntityTracker::addEntity(shared_ptr<Entity> e)
{
    ...
    {
        addEntity(e, 16 * 4, 1, true);
    }
    else if (e->instanceof(eTYPE_WITHERBOSS))
    {
        addEntity(e, 16 * 5, 1, false);
    }
    else if (e->instanceof(eTYPE_BAT))
    {
        addEntity(e, 16 * 5, 1, false);
    }
    else if (dynamic_pointer_cast<Creature>(e) != nullptr)
    {
        addEntity(e, 16 * 5, 1, true);
    }
    else if (e->instanceof(eTYPE_ENDERDRAGON))
    {
        addEntity(e, 16 * 10, 1, true);
    }
...
void EntityTracker::tick()
{
    ...
    for (unsigned int i = 0; i < server->getPlayers()->players.size(); i++)
    {
        ...
        if (ep->dimension != level->dimension->id)
        {
            continue;
        }

        if (ep->connection == nullptr)
        {
            continue;
        }
