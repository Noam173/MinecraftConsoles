...
void PlayerChunkMap::PlayerChunk::add(shared_ptr<ServerPlayer> player, bool sendPacket /*= true*/)
{
    ...
    if (sendPacket)
    {
        player->connection->send(std::make_shared<ChunkVisibilityPacket>(pos.x, pos.z, true));
    }
    ...
    {
        {
            ...
            if (it != parent->knownChunks.end())
            {
                parent->knownChunks.erase(it);
            }
        }
...
void PlayerChunkMap::PlayerChunk::broadcast(shared_ptr<Packet> packet)
{
    ...
    for (size_t i = 0; i < allPlayers.size(); i++)
    {
        ...
        if (player->connection == nullptr || player->connection->isLocal())
        {
            continue;
        }
        ...
        if (!g_NetworkManager.SystemFlagGet(thisPlayer, flagIndex))
        {
            continue;
        }
...
bool PlayerChunkMap::chunkInRange(int x, int z, int xc, int zc)
{
    ...
    int xd = x - xc;
    int zd = z - zc;
    if (xd < -radius || xd > radius)
    {
        return false;
    }
    if (zd < -radius || zd > radius)
    {
        return false;
    }
    return true;
