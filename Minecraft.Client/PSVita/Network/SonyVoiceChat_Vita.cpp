...
void SQRVoiceConnection::SendInternal(const void *data, unsigned int dataSize)
{
    ...
    while (dataRemaining)
    {
        ...
        if (dataSize > SNP_MAX_PAYLOAD)
        {
            dataSize = SNP_MAX_PAYLOAD;
        }
...
SQRVoiceConnection::SQRVoiceConnection(int rudpCtx, SceNpMatching2RoomMemberId remoteRoomMemberId)
    : m_rudpCtx(rudpCtx), m_remoteRoomMemberId(remoteRoomMemberId), m_bConnected(false), m_headsetConnectionMask(0), m_playRingBuffer(sc_ringBufferSize)
{
