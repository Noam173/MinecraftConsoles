...
#define WRAPPED_READFILE(hFile, lpBuffer, nNumberOfBytesToRead, lpNumberOfBytesRead, lpOverlapped)       \
    {                                                                                                    \
        if (ReadFile(hFile, lpBuffer, nNumberOfBytesToRead, lpNumberOfBytesRead, lpOverlapped) == FALSE) \
        {                                                                                                \
            return FALSE;                                                                                \
        }                                                                                                \
    }
    ...
    if (dialogStatus == SCE_COMMON_DIALOG_STATUS_RUNNING)
    {
        ...
        else if (dialogSubStatus == SCE_COMMON_DIALOG_STATUS_FINISHED)
        {
            ...
            if (ret == SCE_OK)
            {
                ...
                if (dialogResult.result == SCE_COMMON_DIALOG_RESULT_USER_CANCELED || dialogResult.result == SCE_COMMON_DIALOG_RESULT_ABORTED)
                {
                ...
                if (finishDialog)
                {
                    ...
                    if (ret == SCE_OK)
                    {
                        app.DebugPrintf("[SaveDataDeleteDialog] Successfully finished saveDataDialog.\n");
                    }
                    else
                    {
                        app.DebugPrintf("[SaveDataDeleteDialog] ERROR: UPDATING DIALOG, sceSaveDataDialogFinish() (0x%x).\n", ret);
                    }
                }
            }
            else
