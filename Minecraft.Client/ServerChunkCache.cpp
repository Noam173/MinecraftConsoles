    ...
    if ((chunk == nullptr) || (chunk->x != x) || (chunk->z != z))
    {
...
#endif // _DURANGO
        {
            ...
            if (asyncPostProcess)
            {
                ...
                if (((chunk->terrainPopulated & LevelChunk::sTerrainPopulatedFromHere) == 0) && hasChunk(x + 1, z + 1) && hasChunk(x, z + 1) && hasChunk(x + 1, z))
                {
                    MinecraftServer::getInstance()->addPostProcessRequest(this, x, z);
                }
                if (hasChunk(x - 1, z) && ((getChunk(x - 1, z)->terrainPopulated & LevelChunk::sTerrainPopulatedFromHere) == 0) && hasChunk(x - 1, z + 1) && hasChunk(x, z + 1) && hasChunk(x - 1, z))
                {
                    MinecraftServer::getInstance()->addPostProcessRequest(this, x - 1, z);
                }
                if (hasChunk(x, z - 1) && ((getChunk(x, z - 1)->terrainPopulated & LevelChunk::sTerrainPopulatedFromHere) == 0) && hasChunk(x + 1, z - 1) && hasChunk(x, z - 1) && hasChunk(x + 1, z))
                {
                    MinecraftServer::getInstance()->addPostProcessRequest(this, x, z - 1);
                }
                if (hasChunk(x - 1, z - 1) && ((getChunk(x - 1, z - 1)->terrainPopulated & LevelChunk::sTerrainPopulatedFromHere) == 0) && hasChunk(x - 1, z - 1) && hasChunk(x, z - 1) && hasChunk(x - 1, z))
                {
                    MinecraftServer::getInstance()->addPostProcessRequest(this, x - 1, z - 1);
                }
            }
            else
            {
            ...
            if (hasChunk(x - 1, z) && hasChunk(x - 2, z) && hasChunk(x - 1, z + 1) && hasChunk(x - 1, z - 1))
            {
                chunk->checkChests(this, x - 1, z);
            }
            if (hasChunk(x, z + 1) && hasChunk(x, z + 2) && hasChunk(x - 1, z + 1) && hasChunk(x + 1, z + 1))
            {
            ...
            if (hasChunk(x - 1, z) && hasChunk(x + 1, z) && hasChunk(x, z - 1) && hasChunk(x, z + 1))
            {
                ...
                while (!chunkSet && i < sortedChunkList.size())
                {
                    ...
                    if (chunk->shouldSave(force) || threadData[j].saveEntities)
                    {
                        ...
                        if (saveThreads[j] == nullptr)
                        {
                            ...
                            if (j == 0)
                            {
                                saveThreads[j]->SetProcessor(CPU_CORE_SAVE_THREAD_A); // XSetThreadProcessor( saveThreads[j], 1);
                            }
                            else if (j == 1)
                            {
                                saveThreads[j]->SetProcessor(CPU_CORE_SAVE_THREAD_B); // XSetThreadProcessor( saveThreads[j], 3);
                            }
                            else if (j == 2)
                            {
                                saveThreads[j]->SetProcessor(CPU_CORE_SAVE_THREAD_C); // XSetThreadProcessor( saveThreads[j], 5);
                            }
