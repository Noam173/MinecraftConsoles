...
DWORD TlsAlloc(VOID)
{
    return TLSStorageOrbis::Instance()->Alloc();
}
BOOL TlsFree(DWORD dwTlsIndex)
{
    return TLSStorageOrbis::Instance()->Free(dwTlsIndex);
}
LPVOID TlsGetValue(DWORD dwTlsIndex)
{
    return TLSStorageOrbis::Instance()->GetValue(dwTlsIndex);
}
BOOL TlsSetValue(DWORD dwTlsIndex, LPVOID lpTlsValue)
{
    return TLSStorageOrbis::Instance()->SetValue(dwTlsIndex, lpTlsValue);
}
...
class OrbisVAlloc
{
  public:
    class PageInfo
    {
      public:
        ...
        PageInfo(off_t physAddr, void *virtualAddr, uint64_t size)
            : m_physAddr(physAddr), m_virtualAddr(virtualAddr), m_size(size)
        {
        }
    };
    void *m_virtualAddr;
    ...
    OrbisVAlloc(void *addr, uint64_t size)
        : m_virtualAddr(addr), m_virtualSize(size), m_allocatedSize(0)
    {
    }
...
HANDLE CreateFileA(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
    ...
    case GENERIC_WRITE:
        ...
        break;
    default:
        openParams.openFlags = SCE_FIOS_O_READ | SCE_FIOS_O_WRITE;
        break;
    }
    ...
    {
    case CREATE_ALWAYS:
        openParams.openFlags |= SCE_FIOS_O_CREAT;
        break;
    case CREATE_NEW:
        openParams.openFlags |= SCE_FIOS_O_CREAT;
        break;
