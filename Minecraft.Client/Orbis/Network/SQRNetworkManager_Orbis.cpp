...
bool SQRNetworkManager_Orbis::CreateSocket()
{
    ...
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_SOCK_BIND))
    {
        return false;
    }
    return true;
...
bool SQRNetworkManager_Orbis::CreateVoiceRudpConnections(SceNpMatching2RoomId roomId, SceNpMatching2RoomMemberId peerMemberId, int playerMask)
{
    ...
    if (pConnection == nullptr)
    {
        ...
        if (ret < 0)
        {
            app.DebugPrintf("sceRudpBind %s failed : 0x%08x\n", getIPAddressString(sinp2pPeer.sin_addr).c_str(), ret);
            assert(0);
        }
        ...
        if (ret < 0)
        {
            app.DebugPrintf("sceRudpInitiate %s failed : 0x%08x\n", getIPAddressString(sinp2pPeer.sin_addr).c_str(), ret);
            assert(0);
        }
