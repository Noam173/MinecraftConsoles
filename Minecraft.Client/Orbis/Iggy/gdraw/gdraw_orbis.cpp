...
{
    if (gdraw->staging_stats.allocs_attempted == gdraw->staging_stats.allocs_succeeded + 1)
    { // warn the first time we run out of mem
...
static rrbool RADLINK gdraw_TextureDrawBufferBegin(gswf_recti *region, gdraw_texture_format format, U32 flags, void *owner, GDrawStats *stats)
{
    ...
    if (region)
    {
        ...
        if (w <= 0 || h <= 0)
        { // region doesn't intersect with current tile
...
static RADINLINE void fence_resources(void *r1, void *r2 = nullptr, void *r3 = nullptr, void *r4 = nullptr)
{
    ...
    if (r1)
    {
        ((GDrawHandle *)r1)->fence = fence;
    }
    if (r2)
    {
        ((GDrawHandle *)r2)->fence = fence;
    }
    if (r3)
    {
        ((GDrawHandle *)r3)->fence = fence;
    }
    if (r4)
    {
        ((GDrawHandle *)r4)->fence = fence;
    }
}
...
static void init_shader(ShaderCode *shader, const ShaderCode *src)
{
    *shader = *src;
    ...
    switch (shdr->m_type)
    {
    case Gnmx::kVertexShader:
        shader->vs->m_vsStageRegisters.m_spiShaderPgmHiVs = ~0u;
        shader->vs->patchShaderGpuAddress(gpu_ptr);
        break;
    case Gnmx::kPixelShader:
        shader->ps->m_psStageRegisters.m_spiShaderPgmHiPs = ~0u;
        shader->ps->patchShaderGpuAddress(gpu_ptr);
        break;
    case Gnmx::kComputeShader:
        shader->cs->m_csStageRegisters.m_computePgmHi = ~0u;
        shader->cs->patchShaderGpuAddress(gpu_ptr);
        break;
    default:
        assert(0);
    }
...
static void create_all_shaders_and_state()
{
    ...
    {
        init_shader(&gdraw->main_ps[0][i], pshader_basic_arr + i);
    }
    for (int i = 0; i < GDRAW_BLENDSPECIAL__count; i++)
    {
        init_shader(&gdraw->exceptional_blend[i], pshader_exceptional_blend_arr + i);
    }
    for (int i = 0; i < 32; i++)
    {
        init_shader(&gdraw->filter_ps[0][i], pshader_filter_arr + i);
    }
    for (int i = 0; i <= MAX_TAPS; i++)
    {
        init_shader(&gdraw->blur_ps[i], pshader_blur_arr + i);
    }
...
GDrawTexture *RADLINK gdraw_orbis_MakeTextureFromResource(U8 *file_in_memory, S32 len, IggyFileTexturePS4 *tex)
{
    ...
    case IFT_FORMAT_la_88:
        texture->setChannelOrder(Gnm::kTextureChannelX, Gnm::kTextureChannelX, Gnm::kTextureChannelX, Gnm::kTextureChannelY);
        break;
    case IFT_FORMAT_i_8:
        texture->setChannelOrder(Gnm::kTextureChannelX, Gnm::kTextureChannelX, Gnm::kTextureChannelX, Gnm::kTextureChannelX);
        break;
    case IFT_FORMAT_l_8:
        texture->setChannelOrder(Gnm::kTextureChannelX, Gnm::kTextureChannelX, Gnm::kTextureChannelX, Gnm::kTextureChannelConstant1);
        break;
    }
