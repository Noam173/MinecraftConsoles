...
void ItemRenderer::render(shared_ptr<Entity> _itemEntity, double x, double y, double z, float rot, float a)
{
    ...
    int count = 1;
    if (itemEntity->getItem()->count > 1)
    {
        count = 2;
    }
    if (itemEntity->getItem()->count > 5)
    {
        count = 3;
    }
    if (itemEntity->getItem()->count > 20)
    {
        count = 4;
    }
    if (itemEntity->getItem()->count > 40)
    {
        count = 5;
    }
...
void ItemRenderer::renderItemBillboard(shared_ptr<ItemEntity> entity, Icon *icon, int count, float a, float red, float green, float blue)
{
    ...
    if (icon == nullptr)
    {
        icon = entityRenderDispatcher->textures->getMissingIcon(entity->getItem()->getIconType());
    }
    float u0 = icon->getU0();
    float u1 = icon->getU1();
    float v0 = icon->getV0();
