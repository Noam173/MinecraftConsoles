...
shared_ptr<MultiplayerLocalPlayer> Minecraft::createExtraLocalPlayer(int idx, const wstring &name, int iPad, int iDimension, ClientConnection *clientConnection /*= nullptr*/, MultiPlayerLevel *levelpassedin)
{
    ...
    if (clientConnection == m_pendingLocalConnections[idx])
    {
        ...
        {
            level->addEntity(localplayers[idx]); // Don't add if we're passing the level in, we only do this from the client connection & we'll be handling adding it ourselves
        }
...
void Minecraft::applyFrameMouseLook()
{
    ...
    for (int i = 0; i < XUSER_MAX_COUNT; i++)
    {
        ...
        // Clamp pitch
        if (localplayers[i]->xRot < -90.0f)
        {
            localplayers[i]->xRot = -90.0f;
        }
        if (localplayers[i]->xRot > 90.0f)
        {
            localplayers[i]->xRot = 90.0f;
        }
        if (localplayers[i]->xRotO < -90.0f)
        {
            localplayers[i]->xRotO = -90.0f;
        }
        if (localplayers[i]->xRotO > 90.0f)
        {
            localplayers[i]->xRotO = 90.0f;
        ...
        // while (running)
        {
            ...
            for (int i = 0; i < XUSER_MAX_COUNT; i++)
            {
                    ...
                    if (localplayers[i])
                {
                    ...
                    {
                        localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_JUMP;
                    }
                    ...
                    if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_INVENTORY))
                    {
                    ...
                    {
                        localplayers[i]->ullButtonsPressed |= 1LL << ACTION_MENU_GTC_PAUSE;
                    }
#endif
                    ...
                    {
                        if (InputManager.ButtonDown(i, MINECRAFT_ACTION_SNEAK_TOGGLE))
                        {
                    ...
                    {
                        if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_SNEAK_TOGGLE))
                        {
                            localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_SNEAK_TOGGLE;
                        }
                    }
                    if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_RENDER_THIRD_PERSON))
                    {
                        localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_RENDER_THIRD_PERSON;
                    }
                    if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_GAME_INFO))
                    {
                    ...
                    if (app.DebugSettingsOn() && app.GetUseDPadForDebug())
                    {
                        ...
                        if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_DPAD_RIGHT))
                        {
                            localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_CHANGE_SKIN;
                        }
                        if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_DPAD_UP))
                        {
                            localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_FLY_TOGGLE;
                        }
                        if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_DPAD_DOWN))
                        {
                            localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_RENDER_DEBUG;
                        }
                        if (InputManager.ButtonPressed(i, MINECRAFT_ACTION_DPAD_LEFT))
                        {
                            localplayers[i]->ullButtonsPressed |= 1LL << MINECRAFT_ACTION_SPAWN_CREEPER;
                        }
                    }
                    else
#endif
...
#ifndef __PSVITA__
                        if (InputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_LEFT))
                        {
                            localplayers[i]->ullDpad_this |= 1LL << MINECRAFT_ACTION_DPAD_LEFT;
                            dirCount++;
                        }
                        if (InputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_RIGHT))
                        {
                            localplayers[i]->ullDpad_this |= 1LL << MINECRAFT_ACTION_DPAD_RIGHT;
                            dirCount++;
                        }
                        if (InputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_UP))
                        {
                            localplayers[i]->ullDpad_this |= 1LL << MINECRAFT_ACTION_DPAD_UP;
                            dirCount++;
                        }
                        if (InputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_DOWN))
                        {
                            localplayers[i]->ullDpad_this |= 1LL << MINECRAFT_ACTION_DPAD_DOWN;
                            dirCount++;
                        }
#endif
...
void Minecraft::tick(bool bFirst, bool bUpdateTextures)
{
        ...
        */
        ...
        else if (gameMode->isInputAllowed(MINECRAFT_ACTION_USE))
        {
            ...
            else
            {
                ...
                if (player->isRiding() || player->isSprinting() || player->isSleeping())
                {
                    autoRepeat = false;
                }
                if (useHeld)
                {
                    ...
                    {
                        player->lastClickTick[1] = ticks + (timer->ticksPerSecond * 2);
                    }
        ...
        if (selected || wheel != 0 || (player->ullButtonsPressed & (1LL << MINECRAFT_ACTION_DROP)))
        {
            ...
            if (!(player->ullButtonsPressed & (1LL << MINECRAFT_ACTION_DROP)) || (selectedItem != nullptr && selectedItem->GetCount() <= 1))
            {
    ...
    if (level != nullptr)
    {
        ...
        for (unsigned int i = 0; i < levels.length; ++i)
        {
            ...
            {
                continue; // Don't tick if the current player isn't in this level
            }
            ...
            {
                levels[i]->animateTick(Mth::floor(player->x), Mth::floor(player->y), Mth::floor(player->z));
            }

            if (levelsTickedFlags & (1 << i))
            {
                continue; // Don't tick further if we've already ticked this level this frame
            }
            ...
            if (levels[i] != nullptr)
            {
                if (!pause)
                {
                    if (levels[i]->skyFlashTime > 0)
                    {
                        levels[i]->skyFlashTime--;
                    }
...
void Minecraft::setLevel(MultiPlayerLevel *level, int message /*=-1*/, shared_ptr<Player> forceInsertPlayer /*=nullptr*/, bool doForceStatsSave /*=true*/, bool bPrimaryPlayerSignedOut /*=false*/)
{
    ...
    for (unsigned int i = 0; i < levels.length; ++i)
    {
        ...
        if (levels[i] != nullptr && level == nullptr)
        {
            ...
            {
                particleEngine->setLevel(nullptr);
            }
        }
    }
    ...
    if (level == nullptr)
    {
        if (levels[0] != nullptr)
        {
            ...
            if (levels[1] != nullptr)
            {
                levels[1]->savedDataStorage = nullptr;
            }
        }
        if (levels[1] != nullptr)
        {
...
void Minecraft::prepareLevel(int title)
{
    ...
    if (!gameMode->isCutScene())
    {
        if (progressRenderer != nullptr)
        {
            this->progressRenderer->progressStage(IDS_PROGRESS_SIMULATING_WORLD);
        }
        max = 2000;
    }
}
...
void Minecraft::respawnPlayer(int iPad, int dimension, int newEntityId)
{
    ...
    if (dynamic_cast<DeathScreen *>(screen) != nullptr)
    {
        setScreen(nullptr);
    }
