...
bool ChoiceTask::isCompleted()
{
    ...
    else
    {
        ...
        if (pMinecraft->localplayers[tutorial->getPad()]->isUnderLiquid(Material::water))
        {
            return false;
        }
#ifdef _WINDOWS64
        if (!m_bConfirmMappingComplete &&
            (InputManager.GetValue(xboxPad, m_iConfirmMapping) > 0 || g_KBMInput.IsKeyDown(VK_RETURN)))
#else
        if (!m_bConfirmMappingComplete &&
        ...
        if (!m_bCancelMappingComplete &&
            (InputManager.GetValue(xboxPad, m_iCancelMapping) > 0 || g_KBMInput.IsKeyDown('B')))
#else
        if (!m_bCancelMappingComplete &&
