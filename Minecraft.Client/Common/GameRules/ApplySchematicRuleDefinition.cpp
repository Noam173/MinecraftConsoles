...
void ApplySchematicRuleDefinition::addAttribute(const wstring &attributeName, const wstring &attributeValue)
{
    ...
    else if (attributeName.compare(L"x") == 0)
    {
        ...
        if (static_cast<int>(abs(m_location->x)) % 2 != 0)
        {
            m_location->x -= 1;
        }
    ...
    else if (attributeName.compare(L"y") == 0)
    {
        ...
        if (static_cast<int>(abs(m_location->y)) % 2 != 0)
        {
            m_location->y -= 1;
        }
        if (m_location->y < 0)
    ...
    else if (attributeName.compare(L"z") == 0)
    {
        ...
        if (static_cast<int>(abs(m_location->z)) % 2 != 0)
        {
            m_location->z -= 1;
        }
    ...
    else if (attributeName.compare(L"dim") == 0)
    {
        ...
        if (m_dimension > 1 || m_dimension < -1)
        {
            m_dimension = 0;
        }
...
void ApplySchematicRuleDefinition::processSchematic(AABB *chunkBox, LevelChunk *chunk)
{
    ...
    if (chunk->level->dimension->id != m_dimension)
    {
        return;
    }
    ...
    if (chunkBox->intersects(m_locationBox))
    {
        ...
        int targetBlocks = (m_locationBox->x1 - m_locationBox->x0) * (m_locationBox->y1 - m_locationBox->y0) * (m_locationBox->z1 - m_locationBox->z0);
