...
int SonyLeaderboardManager::scoreboardThreadEntry(LPVOID lpParam)
{
    ...
    } while ((self->m_running || self->m_eStatsState == eStatsState_Getting || needsWriting) && ShutdownManager::ShouldRun(ShutdownManager::eLeaderboardThread));
...
void SonyLeaderboardManager::scoreboardThreadInternal()
{
    ...
    if (m_titleContext == -1)
    {
        ...
        if (!ProfileManager.IsSignedInLive(primaryPad))
        {
            return;
        }
...
void SonyLeaderboardManager::CloseSession()
{
    ...
    if (m_openSessions == 0)
    {
        app.DebugPrintf("[SonyLeaderboardManager] CloseSession(): Quitting sceNpScore utility.\n");
    }
    else
    {
        app.DebugPrintf("[SonyLeaderboardManager] CloseSession(): %i sessions still open.\n", m_openSessions);
    }
}
...
void SonyLeaderboardManager::CancelOperation()
{
    ...
    if (m_requestId != 0)
    {
        ...
        if (ret < 0)
        {
            app.DebugPrintf("[SonyLeaderboardManager] CancelOperation(): Problem encountered aborting current operation, 0x%X.\n", ret);
        }
        else
        {
            app.DebugPrintf("[SonyLeaderboardManager] CancelOperation(): Operation aborted successfully.\n");
        }
    }
    else
    {
