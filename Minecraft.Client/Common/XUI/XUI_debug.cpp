...
HRESULT CScene_Debug::OnControlNavigate(XUIMessageControlNavigate *pControlNavigateData, BOOL &bHandled)
{
    if (m_bOnCheckboxes)
    {
        ...
        if (m_DebugCheckboxDataA[m_iCurrentCheckboxElement].hXuiObj != pControlNavigateData->hObjSource)
        {
            return S_OK;
        }
    ...
    {
        // If it's not from the current control, ignore it
        if (m_DebugButtonDataA[m_iCurrentButtonElement].hXuiObj != pControlNavigateData->hObjSource)
        {
            return S_OK;
        }
