    ...
    case VK_ESCAPE:
        {
            ...
            {
                pMinecraft->localgameModes[ProfileManager.GetPrimaryPad()]->getTutorial()->showTutorialPopup(true);
            }
...
HRESULT CScene_Win::OnNavReturn(HXUIOBJ hObj, BOOL &rfHandled)
{
    ...
    // This just allows it to be shown
    if (Minecraft::GetInstance()->localgameModes[ProfileManager.GetPrimaryPad()] != nullptr)
    {
        Minecraft::GetInstance()->localgameModes[ProfileManager.GetPrimaryPad()]->getTutorial()->showTutorialPopup(false);
    }
