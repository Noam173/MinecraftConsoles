...
//-----------------------------------------------------------------------------
CXuiCtrlMinecraftSlot::CXuiCtrlMinecraftSlot() : // m_hBrush(nullptr),
                                                 m_bDirty(FALSE),
    ...
    if (HRESULT_SUCCEEDED(hr))
    {
        ...
        if (Message.bHandled)
        {
            ...
            else
            {
                ...
                if (m_item != nullptr && (m_item->id != m_iID || m_item->getAuxValue() != m_iAuxVal || m_item->GetCount() != m_iCount))
                {
                    m_item = nullptr;
                }
            }
...
HRESULT CXuiCtrlMinecraftSlot::OnRender(XUIMessageRender *pRenderData, BOOL &bHandled)
{
    ...
    if (m_item != nullptr || (m_iID > 0 && m_iID < 32000))
    {
        ...
        if (m_item == nullptr)
        {
            m_item = std::make_shared<ItemInstance>(m_iID, m_iCount, m_iAuxVal);
        }
