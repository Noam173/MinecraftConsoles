            ...
            else
            {
                ...
                if (param->levelGen->isTutorial())
                {
                    ...
                    if (param->levelGen->requiresBaseSave() && !param->levelGen->getBaseSavePath().empty())
                    {
                        ...
                        if (!root.exists())
                        {
                            fileRoot = L"Windows64\\Tutorial\\" + param->levelGen->getBaseSavePath();
                        }
...
void CGameNetworkManager::GameInviteReceived(int userIndex, const INVITE_INFO *pInviteInfo)
{
    ...
    if (ProfileManager.IsFullVersion())
    {
        ...
        for (unsigned int index = 0; index < XUSER_MAX_COUNT; index++)
        {
            if (ProfileManager.IsSignedIn(index) && !ProfileManager.HasPlayStationPlus(userIndex))
            {
    ...
    else if (bContentRestricted)
    {
        ...
        if (joiningUsers > 1)
        {
            messageText = IDS_CONTENT_RESTRICTION_MULTIPLAYER;
        }
