        ...
        if (std::fread(magic, 1, 4, file) == 4 && memcmp(magic, "MCSV", 4) == 0)
        {
            ...
            if (version == 1)
            {
                for (uint32_t s = 0; s < count; s++)
                {
                    ...
                    if (std::fread(&ipLen, sizeof(uint16_t), 1, file) != 1)
                    {
                        break;
                    }
                    ...
                    char ipBuf[257] = {};
                    if (std::fread(ipBuf, 1, ipLen, file) != ipLen)
                    {
                        break;
                    }
                    if (std::fread(&port, sizeof(uint16_t), 1, file) != 1)
                    {
                        break;
                    }

                    if (std::fread(&nameLen, sizeof(uint16_t), 1, file) != 1)
                    {
                        break;
                    }
                    if (nameLen > 256)
                    ...
                    if (nameLen > 0)
                    {
                        if (std::fread(nameBuf, 1, nameLen, file) != nameLen)
                        {
                            break;
                        }
                    }
