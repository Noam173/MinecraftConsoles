...
void IUIScene_AbstractContainerMenu::onMouseTick()
{
    ...
    else
    {
        ...
        {
            m_bPointerDrivenByMouse = false;
            fInputDirX = (fInputX > 0.0f) ? 1.0f : (fInputX < 0.0f) ? -1.0f
                                                                    : 0.0f;
            fInputDirY = (fInputY > 0.0f) ? 1.0f : (fInputY < 0.0f) ? -1.0f
                                                                    : 0.0f;
    ...
    if (!IsVisible(eSectionUnderPointer))
    {
        eSectionUnderPointer = eSectionNone;
    }
    ...
    if (player != nullptr)
    {
        carriedItem = player->inventory->getCarried();
    }
    ...
    if (bPointerIsOverSlot)
    {
        ...
        if (bIsItemCarried)
        {
            ...
            if (bMayPlace && iSlotStackSizeRemaining > 0)
            {
                bCanPlaceOne = true;
            }
            if (bMayPlace && iSlotStackSizeRemaining > 1 && carriedItem->count > 1)
            {
                bCanPlaceAll = true;
            }
        }
            ...
            if (IsSectionSlotList(m_eCurrSection))
            {
                ...
                if (bSlotHasItem)
                {
                    ...
                    if (Minecraft::GetInstance()->localgameModes[iPad] != nullptr)
                    {
                        ...
                        {
                            message->m_titleString = Item::items[item->id]->getHoverName(item);
                        }
