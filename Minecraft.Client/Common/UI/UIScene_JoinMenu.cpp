...
void UIScene_JoinMenu::tick()
{
    ...
    if (m_friendInfoUpdatedOK)
    {
        ...
        {
            m_labelValues[eLabel_LevelType].init(app.GetString(IDS_LEVELTYPE_SUPERFLAT));
        }
        else
        {
...
void UIScene_JoinMenu::JoinGame(UIScene_JoinMenu *pClass)
{
    ...
    else
    {
        ...
        {
            if (!ProfileManager.AllowedToPlayMultiplayer(ProfileManager.GetPrimaryPad()))
            {
                noPrivileges = true;
            }
    ...
    case UPDATE_PLAYERS_TIMER_ID:
        {
            ...
            if (success)
            {
                ...
                for (unsigned int i = 0; i < MINECRAFT_NET_MAX_PLAYERS; ++i)
                {
                    if (m_selectedSession->data.players[i] != nullptr)
                    {
                        if (m_selectedSession->data.players[i] == selectedPlayerXUID)
                        {
                            selectedIndex = i;
                        }
