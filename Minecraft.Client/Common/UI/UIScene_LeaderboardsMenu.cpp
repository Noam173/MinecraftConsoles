...
#ifdef _XBOX
                            && !m_leaderboard.m_entries[selection - (m_leaderboard.m_entryStartIndex - 1)].m_bFriend && !m_leaderboard.m_entries[selection - (m_leaderboard.m_entryStartIndex - 1)].m_bRequestedFriend
#endif
                        )
                        {
...
void UIScene_LeaderboardsMenu::ReadStats(int startIndex)
{
    ...
    if (m_currentFilter == LeaderboardManager::eFM_MyScore || m_currentFilter == LeaderboardManager::eFM_TopRank)
    {
...
bool UIScene_LeaderboardsMenu::RetrieveStats()
{
    ...
    if (m_leaderboard.m_totalEntryCount == 0)
    {
        ...
        if (m_newSel < 0)
        {
            m_newTop = m_newSel = GetEntryStartIndex();
        }
    }
    // Additional read
    else
