...
UITTFFont *UIController::createFont(EFont fontLanguage)
{
    ...
    case eFont_Japanese:
        return new UITTFFont("Mojangles_TTF_jaJP", "Common/Media/font/JPN/DF-DotDotGothic16.ttf", 0x203B); // JPN
    // case eFont_SimpChinese:	Simplified Chinese is unsupported.
    case eFont_TradChinese:
        return new UITTFFont("Mojangles_TTF_cnTD", "Common/Media/font/CHT/DFTT_R5.TTC", 0x203B); // CHT
    case eFont_Korean:
        return new UITTFFont("Mojangles_TTF_koKR", "Common/Media/font/KOR/candadite2.ttf", 0x203B); // KOR
#else
    case eFont_Japanese:
        return new UITTFFont("Mojangles_TTF_jaJP", "Common/Media/font/JPN/DFGMaruGothic-Md.ttf", 0x2022); // JPN
#ifdef _DURANGO
    case eFont_SimpChinese:
        return new UITTFFont("Mojangled_TTF_cnCN", "Common/Media/font/CHS/MSYH.ttf", 0x2022); // CHS
#endif
    case eFont_TradChinese:
        return new UITTFFont("Mojangles_TTF_cnTD", "Common/Media/font/CHT/DFHeiMedium-B5.ttf", 0x2022); // CHT
    case eFont_Korean:
        return new UITTFFont("Mojangles_TTF_koKR", "Common/Media/font/KOR/BOKMSD.ttf", 0x2022); // KOR
#endif
...
void UIController::SetupFont()
{
    ...
    {
        app.DebugPrintf("[UIController] Font switch required for language transition to %i.\n", nextLanguage);
    }
    else
    {
...
void UIController::tickInput()
{
    ...
    if (!m_bSystemUIShowing)
    {
        ...
        {
            if (InputManager.ButtonPressed(ProfileManager.GetPrimaryPad(), ACTION_MENU_STICK_PRESS))
            {
                    ...
                    if (mouseMoved)
                    {
                        ...
                        if (controls)
                        {
                            ...
                            if (hitControlId >= 0 && pScene->getControlFocus() != hitControlId)
                            {
                                ...
                                if (hitCtrl && hitCtrl->getControlType() == UIControl::eTextInput && pScene->isDirectEditBlocking())
                                {
                    ...
                    if (leftDown && m_mouseDraggingSliderScene == pScene->getSceneType() && m_mouseDraggingSliderId >= 0)
                    {
                        ...
                        if (pSlider && pSlider->getVisible())
                        {
                            ...
                            if (sliderWidth > 0)
                            {
                                ...
                                if (fNewSliderPos < 0.0f)
                                {
                                    fNewSliderPos = 0.0f;
                                }
                                if (fNewSliderPos > 1.0f)
                                {
                                    fNewSliderPos = 1.0f;
                                }
...
void UIController::handleInput()
{
    ...
    for (unsigned int iPad = 0; iPad < XUSER_MAX_COUNT; ++iPad)
    {
        ...
        if (iPad != ProfileManager.GetPrimaryPad() && (!InputManager.IsPadConnected(iPad) || !InputManager.IsPadLocked(iPad)))
        {
            continue;
        }
#endif
    ...
    if (!app.GetGameStarted())
    {
        ...
        {
            if (InputManager.IsPadLocked(iPad) || !InputManager.IsPadConnected(iPad))
            {
                continue;
            }
...
void UIController::handleKeyPress(unsigned int iPad, unsigned int key)
{
    ...
    if (iPad == 0)
    {
        ...
        if (vk != 0)
        {
            ...
            if (g_KBMInput.IsKeyReleased(vk))
            {
                released = true;
                down = false;
            }
            if (!pressed && !released && g_KBMInput.IsKeyDown(vk))
            {
                down = true;
            }
        }
        ...
        if ((key == ACTION_MENU_OK || key == ACTION_MENU_A) && !g_KBMInput.IsMouseGrabbed())
        {
            if (m_mouseDraggingSliderId < 0 && !m_mouseClickConsumedByScene)
            {
                ...
                if (!pressed && !released && g_KBMInput.IsMouseButtonDown(KeyboardMouseInput::MOUSE_LEFT))
                {
                    down = true;
                }
            }
        ...
        if (key == ACTION_MENU_X && !g_KBMInput.IsMouseGrabbed())
        {
            ...
            if (!pressed && !released && g_KBMInput.IsMouseButtonDown(KeyboardMouseInput::MOUSE_RIGHT))
            {
                down = true;
            }
        }
    ...
    if (pressed)
    {
        app.DebugPrintf("Pressed %d\n", key);
    }
    if (released)
    {
        app.DebugPrintf("Released %d\n", key);
    }
    // Repeat handling
    if (pressed)
    {
...
bool UIController::NavigateBack(int iPad, bool forceUsePad, EUIScene eScene, EUILayer eLayer)
{
    ...
    if (app.GetGameStarted())
    {
        ...
        if (!navComplete && (iPad != 255) && (iPad >= 0))
        {
            ...
            if (!m_groups[static_cast<int>(group)]->GetMenuDisplayed())
            {
...
void UIController::CloseUIScenes(int iPad, bool forceIPad)
{
    ...
    {
        m_groups[static_cast<int>(group)]->getTutorialPopup()->SetTutorialDescription(&popupInfo);
    }

    if (group == eUIGroup_Fullscreen)
    {
        setFullscreenMenuDisplayed(false);
    }
...
void UIController::SetTooltipText(unsigned int iPad, unsigned int tooltip, int iTextID)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->SetTooltipText(tooltip, iTextID);
    }
}
...
void UIController::SetEnableTooltips(unsigned int iPad, BOOL bVal)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->SetEnableTooltips(bVal);
    }
}
...
void UIController::ShowTooltip(unsigned int iPad, unsigned int tooltip, bool show)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->ShowTooltip(tooltip, show);
    }
}
...
void UIController::SetTooltips(unsigned int iPad, int iA, int iB, int iX, int iY, int iLT, int iRT, int iLB, int iRB, int iLS, int iRS, int iBack, bool forceUpdate)
{
...
#ifndef _XBOX
    if (iX == IDS_TOOLTIPS_SELECTDEVICE)
    {
        iX = -1;
    }
    if (iX == IDS_TOOLTIPS_CHANGEDEVICE)
    {
        iX = -1;
    }

#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
    if (iY == IDS_TOOLTIPS_VIEW_GAMERCARD)
    {
        iY = -1;
    }
    if (iY == IDS_TOOLTIPS_VIEW_GAMERPROFILE)
    {
        iY = -1;
    }
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->SetTooltips(iA, iB, iX, iY, iLT, iRT, iLB, iRB, iLS, iRS, iBack, forceUpdate);
    }
}
...
void UIController::EnableTooltip(unsigned int iPad, unsigned int tooltip, bool enable)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->EnableTooltip(tooltip, enable);
    }
}
...
void UIController::AnimateKeyPress(int iPad, int iAction, bool bRepeat, bool bPressed, bool bReleased)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->handleInput(iPad, iAction, bRepeat, bPressed, bReleased, handled);
    }
}
...
void UIController::OverrideSFX(int iPad, int iAction, bool bVal)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTooltips())
    {
        m_groups[static_cast<int>(group)]->getTooltips()->overrideSFX(iPad, iAction, bVal);
    }
}
...
void UIController::DisplayGamertag(unsigned int iPad, bool show)
{
    ...
    EUIGroup group = static_cast<EUIGroup>(iPad + 1);
    if (m_groups[static_cast<int>(group)]->getHUD())
    {
        m_groups[static_cast<int>(group)]->getHUD()->ShowDisplayName(show);
    }
...
void UIController::SetSelectedItem(unsigned int iPad, const wstring &name)
{
    ...
    if (m_groups[static_cast<int>(group)]->getHUD())
    {
        m_groups[static_cast<int>(group)]->getHUD()->SetSelectedLabel(name);
    }
}
...
void UIController::HandleGameTick()
{
    ...
    for (unsigned int i = 0; i < eUIGroup_COUNT; ++i)
    {
        if (m_groups[i]->getHUD())
        {
            m_groups[i]->getHUD()->handleGameTick();
        }
    }
}
...
void UIController::SetTutorial(int iPad, Tutorial *tutorial)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTutorialPopup())
    {
        m_groups[static_cast<int>(group)]->getTutorialPopup()->SetTutorial(tutorial);
    }
}
...
void UIController::RemoveInteractSceneReference(int iPad, UIScene *scene)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTutorialPopup())
    {
        m_groups[static_cast<int>(group)]->getTutorialPopup()->RemoveInteractSceneReference(scene);
    }
}
#endif
...
void UIController::SetTutorialVisible(int iPad, bool visible)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTutorialPopup())
    {
        m_groups[static_cast<int>(group)]->getTutorialPopup()->SetVisible(visible);
    }
}
...
bool UIController::IsTutorialVisible(int iPad)
{
    ...
    if (m_groups[static_cast<int>(group)]->getTutorialPopup())
    {
        visible = m_groups[static_cast<int>(group)]->getTutorialPopup()->IsVisible();
    }
    return visible;
}
...
void UIController::ShowTrialTimer(bool show)
{
    ...
    {
        m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->showTrialTimer(show);
    }
}
...
void UIController::UpdateTrialTimer(unsigned int iPad)
{
...
#endif
    {
        ...
        if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
        {
            m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->setTrialTimer(wcTime);
        }
    }
    else
    {
        if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
        {
            m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->setTrialTimer(L"");
        }
    }
...
void UIController::UpdateAutosaveCountdownTimer(unsigned int uiSeconds)
{
    ...
    if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
    {
        m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->setTrialTimer(wcAutosaveCountdown);
    }
#endif
}
...
void UIController::ShowSavingMessage(unsigned int iPad, C4JStorage::ESavingMessage eVal)
{
    ...
    if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
    {
        m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->showSaveIcon(show);
    }
}
...
void UIController::ShowPlayerDisplayname(bool show)
{
    if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
    {
        m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->showPlayerDisplayName(show);
    }
}
...
void UIController::ShowPressStart(unsigned int iPad)
{
    ...
    if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
    {
        m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->showPressStart(iPad, true);
    }
}
...
void UIController::HidePressStart()
{
    ...
    if (m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay())
    {
        m_groups[static_cast<int>(eUIGroup_Fullscreen)]->getPressStartToPlay()->showPressStart(0, false);
    }
}
