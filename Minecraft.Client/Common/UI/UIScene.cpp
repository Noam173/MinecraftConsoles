...
{
    if (!force && (stealsFocus() && (getSceneType() != eUIScene_FullscreenProgress && !bHasFocus)))
    {
        return;
    }
        ...
        if (m_cacheSlotRenders)
        {
            if ((m_cachedSlotDraw.size() + 1) == m_expectedCachedSlotCount)
            {
                ...
                if (!useCommandBuffers || m_needsCacheRendered)
                {
                    ...
                    {
                        RenderManager.CBuffStart(list, true);
                    }
#endif
