...
void UIGroup::ReloadAll()
{
    ...
    {
        if (m_layers[highestRenderable]->hidesLowerScenes())
        {
            break;
        }
    }
    ...
    for (; highestRenderable >= 0; --highestRenderable)
    {
        ...
        {
            m_layers[highestRenderable]->ReloadAll(highestRenderable != static_cast<int>(eUILayer_Fullscreen));
        }
    }
}
...
void UIGroup::tick()
{
    ...
    if (m_iPad >= 0 && !ProfileManager.IsSignedIn(m_iPad))
    {
        return;
    }
    for (unsigned int i = 0; i < eUILayer_COUNT; ++i)
...
bool UIGroup::NavigateBack(int iPad, EUIScene eScene, EUILayer eLayer)
{
    ...
    for (unsigned int i = 0; i < eUILayer_COUNT; ++i)
    {
        if (eLayer < eUILayer_COUNT && eLayer != i)
        {
            continue;
        }
...
void UIGroup::closeAllScenes()
{
    ...
    for (unsigned int i = 0; i < eUILayer_COUNT; ++i)
    {
        ...
        if (i != static_cast<int>(eUILayer_Error))
        {
            m_layers[i]->closeAllScenes();
        }
    }
    updateStackStates();
}
