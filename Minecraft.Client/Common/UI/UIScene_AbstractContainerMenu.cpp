...
void UIScene_AbstractContainerMenu::handleDestroy()
{
    ...
    if (pMinecraft->localgameModes[m_iPad] != nullptr)
    {
        ...
        if (gameMode != nullptr)
        {
            gameMode->getTutorial()->changeTutorialState(m_previousTutorialState);
        }
    }
...
void UIScene_AbstractContainerMenu::PlatformInitialize(int iPad, int startIndex)
{
    ...
    if (g_savedInventoryCursorPos.hasSavedPos)
    {
        ...
        if (m_pointerPos.x < m_fPointerMinX)
        {
            m_pointerPos.x = m_fPointerMinX;
        }
        if (m_pointerPos.x > m_fPointerMaxX)
        {
            m_pointerPos.x = m_fPointerMaxX;
        }
        if (m_pointerPos.y < m_fPointerMinY)
        {
            m_pointerPos.y = m_fPointerMinY;
        }
        if (m_pointerPos.y > m_fPointerMaxY)
        {
            m_pointerPos.y = m_fPointerMaxY;
        }
    }
#endif
...
void UIScene_AbstractContainerMenu::customDraw(IggyCustomDrawCallbackRegion *region)
{
    ...
    if (item != nullptr)
    {
        customDrawSlotControl(region, m_iPad, item, m_menu->isValidIngredient(item, slotId) ? 1.0f : 0.5f, item->isFoil(), true);
    }
}
