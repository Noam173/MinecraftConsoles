...
void UIScene_SkinSelectMenu::customDraw(IggyCustomDrawCallbackRegion *region)
{
    ...
    else
    {
        ...
        {
            RenderManager.StateSetStencil(GL_EQUAL, region->stencil_func_ref, region->stencil_func_mask);
        }
#elif __PSVITA__
        // AP - make sure the skins are only drawn inside the smokey panel
        if (region->stencil_func_ref != 0)
        {
            RenderManager.StateSetStencil(SCE_GXM_STENCIL_FUNC_EQUAL, region->stencil_func_mask, region->stencil_write_mask);
        }
#else
        if (region->stencil_func_ref != 0)
        {
            RenderManager.StateSetStencil(GL_EQUAL, region->stencil_func_ref, region->stencil_func_mask, region->stencil_write_mask);
        }
#endif
...
int UIScene_SkinSelectMenu::UnlockSkinReturned(void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if ((result == C4JStorage::EMessage_ResultAccept) && ProfileManager.IsSignedIn(iPad))
    {
