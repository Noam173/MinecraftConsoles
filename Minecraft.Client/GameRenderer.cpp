...
void GameRenderer::tick(bool first) // 4J - add bFirst
{
    ...
    {
        return; // 4J added for split screen - only do rest of processing for once per frame
    }
...
void GameRenderer::pick(float a)
{
    ...
    if (mc->hitResult)
    {
        ...
        if (face == Facing::WEST && hitx < 0)
        {
            hitx -= 1;
        }
        if (face == Facing::EAST && hitx > 0)
        {
            hitx += 1;
        }
        if (face == Facing::NORTH && hitz < 0)
        {
            hitz -= 1;
        }
        if (face == Facing::SOUTH && hitz > 0)
        {
            hitz += 1;
        }
...
{
    if (!mc->cameraTargetPlayer->instanceof(eTYPE_LIVINGENTITY))
    {
        return;
    }
...
void GameRenderer::setupCamera(float a, int eye)
{
    ...
    if (mc->options->anaglyph3d)
    {
        glTranslatef((eye * 2 - 1) * 0.10f, 0, 0);
    }
    ...
    if (app.GetGameSettings(mc->player->GetXboxPad(), eGameSetting_ViewBob) && !mc->player->abilities.flying && !bNoLegAnim && !bNoBobbingAnim)
    {
        bobView(a);
    }
...
void GameRenderer::renderItemInHand(float a, int eye)
{
    ...
    if (localplayer != nullptr)
    {
        ...
        if (!(item && item->getItem()->id == Item::map_Id) && app.GetGameSettings(localplayer->GetXboxPad(), eGameSetting_DisplayHand) == 0)
        {
            renderHand = false;
        }
    }
    ...
    if (app.GetGameSettings(localplayer->GetXboxPad(), eGameSetting_ViewBob) && !localplayer->abilities.flying && !bNoLegAnim)
    {
        bobView(a);
    }
...
bool GameRenderer::ComputeViewportForPlayer(int j, D3D11_VIEWPORT &outViewport) const
{
    ...
    case C4JRender::VIEWPORT_TYPE_SPLIT_LEFT:
        ...
        outViewport.Height = h;
        break;
    case C4JRender::VIEWPORT_TYPE_SPLIT_RIGHT:
        outViewport.TopLeftX = halfW;
        outViewport.TopLeftY = 0;
        outViewport.Width = halfW;
        outViewport.Height = h;
        break;
