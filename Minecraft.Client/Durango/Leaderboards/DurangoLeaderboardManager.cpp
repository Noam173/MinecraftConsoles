    ...
    case eStatsState_ReceivedLeaderboardInfo:
        {
            ...
            auto task = concurrency::create_task(m_statsAsyncOp).then([this](CC::task<WFC::IVectorView<MXS::UserStatistics::UserStatisticsResult ^> ^> resultListTask) {
                ...
                catch (Platform::Exception ^ ex)
                {
                    ...
                    {
                        app.DebugPrintf("[LeaderboardManager] ERROR calling GetLeaderboardAsync: 404 Not Found - 0x%0.8x\n", ex->HResult);
                    }
                    else
                    {
                        app.DebugPrintf("[LeaderboardManager] ERROR calling GetLeaderboardAsync: 0x%0.8x\n", ex->HResult);
                    }
                }
                catch (...)
                {
...
void DurangoLeaderboardManager::CloseSession()
{
    ...
    {
        app.DebugPrintf("[LeaderboardManager] CloseSession(): %i sessions still open.\n", m_openSessions);
    }
}
...
bool DurangoLeaderboardManager::ReadStats_Friends(LeaderboardReadListener *listener, int difficulty, EStatsType type, PlayerUID myUID, unsigned int startIndex, unsigned int readCount)
{
    ...
    if (!LeaderboardManager::ReadStats_Friends(listener, difficulty, type, myUID, startIndex, readCount))
    {
        return false;
    }
...
bool DurangoLeaderboardManager::ReadStats_MyScore(LeaderboardReadListener *listener, int difficulty, EStatsType type, PlayerUID myUID, unsigned int readCount)
{
    ...
    if (!LeaderboardManager::ReadStats_MyScore(listener, difficulty, type, myUID, readCount))
    {
        return false;
    }
    setState(eStatsState_GettingLeaderboardInfo);
...
bool DurangoLeaderboardManager::ReadStats_TopRank(LeaderboardReadListener *listener, int difficulty, EStatsType type, unsigned int startIndex, unsigned int readCount)
{
    ...
    if (!LeaderboardManager::ReadStats_TopRank(listener, difficulty, type, startIndex, readCount))
    {
        return false;
    }
    setState(eStatsState_GettingLeaderboardInfo);
...
void DurangoLeaderboardManager::CancelOperation()
{
    ...
    if (m_leaderboardAsyncOp)
    {
        m_leaderboardAsyncOp->Cancel();
    }
    if (m_statsAsyncOp)
    {
        m_statsAsyncOp->Cancel();
    }
        ...
        .then([this, readCount, difficulty, type, filter](CC::task<MXSL::LeaderboardResult ^> resultTask) {
            try
            {
                ...
                if (filter == eFM_MyScore)
                {
                    ...
                    {
                        if (lastResult->Rows->GetAt(i)->XboxUserId == m_xboxLiveContext->User->XboxUserId)
                        {
                            userIncluded = true;
                        }
                    }
...
void DurangoLeaderboardManager::updateStatsInfo(int userIndex, int difficulty, EStatsType type, WFC::IVectorView<MXS::UserStatistics::Statistic ^> ^ statsResult)
{
    if (m_scores)
    {
        ...
        case eStatsType_Farming:
            userScore->m_statsSize = 6;
            break;
        case eStatsType_Mining:
            userScore->m_statsSize = 7;
            break;
        case eStatsType_Kills:
            userScore->m_statsSize = 7;
            break;
        case eStatsType_Travelling:
            userScore->m_statsSize = 4;
            break;
        }
...
wstring DurangoLeaderboardManager::stateToString(EStatsState eState)
{
    ...
    {
    case eStatsState_Idle:
        return L"eStatsState_Idle";
    case eStatsState_GettingLeaderboardInfo:
        return L"eStatsState_GettingLeaderboardInfo";
    case eStatsState_ReceivedLeaderboardInfo:
        return L"eStatsState_ReceivedLeaderboardInfo";
    case eStatsState_GettingStatsInfo:
        return L"eStatsState_GettingStatsInfo";
    case eStatsState_ReceivedStatsInfo:
        return L"eStatsState_ReceivedStatsInfo";
    case eStatsState_Failed:
        return L"eStatsState_Failed";
    case eStatsState_Ready:
        return L"eStatsState_Ready";
    case eStatsState_Canceled:
        return L"eStatsState_Canceled";
    case eStatsState_Max:
        return L"eStatsState_MAX";
    default:
        return L"UNKNOWN";
    }
}
