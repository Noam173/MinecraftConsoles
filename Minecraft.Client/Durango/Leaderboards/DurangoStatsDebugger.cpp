...
DurangoStatsDebugger *DurangoStatsDebugger::Initialize()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        sp->addArgs(i); // Difficulty Levels.
    }
    out->m_stats.push_back(sp);
...
void DurangoStatsDebugger::retrieveStats(int iPad)
{
    ...
    if (xboxLiveContext)
    {
        app.DebugPrintf("[DurangoStatsDebugger] 'XboxLiveContext' created successfully.\n");
    }
    else
    {
        app.DebugPrintf("[DurangoStatsDebugger] 'XboxLiveContext' not created.\n");
        return;
    }
