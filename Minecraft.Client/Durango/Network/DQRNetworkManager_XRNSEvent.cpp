...
void DQRNetworkManager::RTS_SendData(unsigned char *pucData, unsigned int dataSize, unsigned int sessionAddress, bool reliable, bool sequential, bool coalesce, bool broadcastMode, bool gameChannel)
{
    ...
    if (reliable)
    {
        message.m_flags |= eRTSFlags::RTS_MESSAGE_FLAG_RELIABLE;
    }
    if (sequential)
    {
        message.m_flags |= eRTSFlags::RTS_MESSAGE_FLAG_SEQUENTIAL;
    }
    if (coalesce)
    {
        message.m_flags |= eRTSFlags::RTS_MESSAGE_FLAG_COALESCE;
    }
    if (broadcastMode)
    {
        message.m_flags |= eRTSFlags::RTS_MESSAGE_FLAG_BROADCAST_MODE;
    }
    if (gameChannel)
    {
        message.m_flags |= eRTSFlags::RTS_MESSAGE_FLAG_GAME_CHANNEL;
    }
    m_RTSMessageQueueOutgoing.push(message);
    LeaveCriticalSection(&m_csRTSMessageQueueOutgoing);
}
