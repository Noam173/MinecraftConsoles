...
bool DQRNetworkManager::AddLocalPlayerByUserIndex(int userIndex)
{
    ...
    if (m_addLocalPlayerState != DNM_ADD_PLAYER_STATE_IDLE)
    {
        return false;
    }
...
bool DQRNetworkManager::RemoveLocalPlayerByUserIndex(int userIndex)
{
    ...
    if (m_removeLocalPlayerState != DNM_REMOVE_PLAYER_STATE_IDLE)
    {
        return false;
    }
...
{
    if (m_roomSyncData.playerCount == MAX_ONLINE_PLAYER_COUNT)
    {
        return false;
    }
    ...
    if (!m_isOfflineGame)
    {
        ...
        if (session != nullptr)
        {
            ...
            create_task(registerSessionAsync).then([this](task<void> t) {
                ...
                .wait();
            if (m_state == DNM_INT_STATE_HOSTING_FAILED)
            {
                return 0;
            }
