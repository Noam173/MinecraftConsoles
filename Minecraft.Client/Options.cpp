...
void Options::toggle(const Options::Option *option, int dir)
{
    ...
    if (option == Option::RENDER_CLOUDS)
    {
...
bool Options::getBooleanValue(const Options::Option *item)
{
    ...
    if (item == Option::ADVANCED_OPENGL)
    {
...
void Options::load()
{
    ...
    while ((line = br->readLine()) != L"") // 4J - was check against nullptr - do we need to distinguish between empty lines and a fail here?
    {
        ...
        if (cmds[0] == L"fov")
        {
            fov = readFloat(cmds[1]);
        }
        if (cmds[0] == L"gamma")
        {
            gamma = readFloat(cmds[1]);
        }
        ...
        {
            guiScale = _fromString<int>(cmds[1]);
        }
        if (cmds[0] == L"particles")
        {
            particles = _fromString<int>(cmds[1]);
        }
        if (cmds[0] == L"bobView")
        {
            bobView = cmds[1] == L"true";
        }
        if (cmds[0] == L"anaglyph3d")
        {
            anaglyph3d = cmds[1] == L"true";
        }
        ...
        {
            framerateLimit = _fromString<int>(cmds[1]);
        }
        if (cmds[0] == L"difficulty")
        {
            difficulty = _fromString<int>(cmds[1]);
        }
        ...
        if (cmds[0] == L"ao")
        {
            ambientOcclusion = cmds[1] == L"true";
        }
        if (cmds[0] == L"clouds")
        {
            renderClouds = cmds[1] == L"true";
        }
        if (cmds[0] == L"skin")
        {
            skin = cmds[1];
        }
        if (cmds[0] == L"lastServer")
        {
            lastMpIp = cmds[1];
        }
