...
bool ServerPlayer::hurt(DamageSource *dmgSource, float dmg)
{
    ...
    if (!server->isPvpAllowed() && invulnerableTime > 0 && dmgSource != DamageSource::outOfWorld)
    {
        return false;
    }
    ...
    if (returnVal)
    {
        ...
        else if (dmgSource == DamageSource::onFire || dmgSource == DamageSource::inFire)
        {
...
bool ServerPlayer::canHarmPlayer(wstring targetName)
{
    ...
    {
        if (this->name != targetName && (!isAllowedToAttackPlayers() || !server->isPvpAllowed()))
        {
            canHarm = false;
        }
    }
...
{
    if (!connection->hasClientTickedOnce())
    {
        return;
    }
