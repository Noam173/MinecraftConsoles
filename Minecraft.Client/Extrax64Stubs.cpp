...
HRESULT ATG::XMLParser::ParseXMLBuffer(CONST CHAR *strBuffer, UINT uBufferSize)
{
    return S_OK;
}
...
IQNetPlayer *IQNet::GetPlayerBySmallId(BYTE SmallId)
{
    for (DWORD i = 0; i < s_playerCount; i++)
    {
        if (m_player[i].m_smallId == SmallId && Win64_IsActivePlayer(&m_player[i], i))
        {
            return &m_player[i];
        }
    }
...
DWORD XUserAreUsersFriends(DWORD dwUserIndex, PPlayerUID pXuids, DWORD dwXuidCount, PBOOL pfResult, void *pOverlapped)
{
    return 0;
}
...
{
    return (iQuadrant == 0) || InputManager.IsPadConnected(iQuadrant);
}
...
UINT C_4JProfile::RequestSignInUI(bool bFromInvite, bool bLocalGame, bool bNoGuestsAllowed, bool bMultiplayerSignIn, bool bAddUser, int (*Func)(LPVOID, const bool, const int iPad), LPVOID lpParam, int iQuadrant)
{
    return 0;
}
...
int C_4JProfile::SetOldProfileVersionCallback(int (*Func)(LPVOID, unsigned char *, const unsigned short, const int), LPVOID lpParam)
{
    return 0;
}
