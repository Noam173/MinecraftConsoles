...
static void tag_resources(void *r1, void *r2 = NULL, void *r3 = NULL, void *r4 = NULL)
{
    ...
    if (r1)
    {
        ((GDrawHandle *)r1)->fence.value = now;
    }
    if (r2)
    {
        ((GDrawHandle *)r2)->fence.value = now;
    }
    if (r3)
    {
        ((GDrawHandle *)r3)->fence.value = now;
    }
    if (r4)
    {
        ((GDrawHandle *)r4)->fence.value = now;
    }
}
...
static void create_all_shaders(void)
{
    ...
    {
        create_pixel_shader(&gdraw->exceptional_blend[i], pshader_exceptional_blend_arr + i);
    }
    for (i = 0; i < 32; ++i)
    {
...
static void destroy_all_shaders()
{
    ...
    for (i = 0; i < 32; ++i)
    {
        destroy_shader(&gdraw->filter_prog[0][i]);
    }
    for (i = 0; i < MAX_TAPS + 1; ++i)
    {
        destroy_shader(&gdraw->blur_prog[i]);
    }
