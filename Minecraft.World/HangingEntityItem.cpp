...
bool HangingEntityItem::useOn(shared_ptr<ItemInstance> instance, shared_ptr<Player> player, Level *level, int xt, int yt, int zt, int face, float clickX, float clickY, float clickZ, bool bTestOnly)
{
    ...
    if (bTestOnly)
    {
        if (!player->mayUseItemAt(xt, yt, zt, face, instance))
        {
            return false;
        }
    ...
    if (entity != nullptr && entity->survives())
    {
        if (!level->isClientSide)
        {
            if (level->addEntity(entity) == TRUE)
            {
                ...
                {
                    player->awardStat(GenericStats::blocksPlaced(Item::painting_Id), GenericStats::param_blocksPlaced(Item::painting_Id, instance->getAuxValue(), 1));
                }
                else if (eType == eTYPE_ITEM_FRAME)
                {
                    player->awardStat(GenericStats::blocksPlaced(Item::itemFrame_Id), GenericStats::param_blocksPlaced(Item::itemFrame_Id, instance->getAuxValue(), 1));
                }
