...
void RedStoneOreTile::poofParticles(Level *level, int x, int y, int z)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (i == 0 && !level->isSolidRenderTile(x, y + 1, z))
        {
            yy = y + 1 + r;
        }
        if (i == 1 && !level->isSolidRenderTile(x, y - 1, z))
        {
            yy = y + 0 - r;
        }
        if (i == 2 && !level->isSolidRenderTile(x, y, z + 1))
        {
            zz = z + 1 + r;
        }
        if (i == 3 && !level->isSolidRenderTile(x, y, z - 1))
        {
            zz = z + 0 - r;
        }
        if (i == 4 && !level->isSolidRenderTile(x + 1, y, z))
        {
            xx = x + 1 + r;
        }
        if (i == 5 && !level->isSolidRenderTile(x - 1, y, z))
        {
            xx = x + 0 - r;
        }
