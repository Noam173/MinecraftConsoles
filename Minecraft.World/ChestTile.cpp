...
void ChestTile::recalcLockDir(Level *level, int x, int y, int z)
{
    ...
    if (n == id || s == id)
    {
        ...
        if ((Tile::solid[w] || Tile::solid[w2]) && !Tile::solid[e] && !Tile::solid[e2])
        {
            lockDir = 5;
        }
        if ((Tile::solid[e] || Tile::solid[e2]) && !Tile::solid[w] && !Tile::solid[w2])
        {
            lockDir = 4;
        }
    }
...
shared_ptr<Container> ChestTile::getContainer(Level *level, int x, int y, int z)
{
    ...
    if (isCatSittingOnChest(level, x, y, z))
    {
        return nullptr;
    }

    if (level->getTile(x - 1, y, z) == id && (level->isSolidBlockingTile(x - 1, y + 1, z) || isCatSittingOnChest(level, x - 1, y, z)))
    {
        return nullptr;
    }
    if (level->getTile(x + 1, y, z) == id && (level->isSolidBlockingTile(x + 1, y + 1, z) || isCatSittingOnChest(level, x + 1, y, z)))
    ...
    if (level->getTile(x, y, z - 1) == id && (level->isSolidBlockingTile(x, y + 1, z - 1) || isCatSittingOnChest(level, x, y, z - 1)))
    {
        return nullptr;
    }
    if (level->getTile(x, y, z + 1) == id && (level->isSolidBlockingTile(x, y + 1, z + 1) || isCatSittingOnChest(level, x, y, z + 1)))
    ...
    if (level->getTile(x - 1, y, z) == id)
    {
        container = std::make_shared<CompoundContainer>(IDS_CHEST_LARGE, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x - 1, y, z)), container);
    }
    if (level->getTile(x + 1, y, z) == id)
    {
        container = std::make_shared<CompoundContainer>(IDS_CHEST_LARGE, container, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x + 1, y, z)));
    }
    if (level->getTile(x, y, z - 1) == id)
    {
        container = std::make_shared<CompoundContainer>(IDS_CHEST_LARGE, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x, y, z - 1)), container);
    }
    if (level->getTile(x, y, z + 1) == id)
    {
        container = std::make_shared<CompoundContainer>(IDS_CHEST_LARGE, container, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x, y, z + 1)));
    }
