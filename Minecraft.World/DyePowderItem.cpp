...
bool DyePowderItem::useOn(shared_ptr<ItemInstance> itemInstance, shared_ptr<Player> player, Level *level, int x, int y, int z, int face, float clickX, float clickY, float clickZ, bool bTestUseOnOnly)
{
    ...
    if (itemInstance->getAuxValue() == WHITE)
    {
        ...
        if (growCrop(itemInstance, level, x, y, z, bTestUseOnOnly))
        {
            ...
            {
                level->levelEvent(LevelEvent::PARTICLES_PLANT_GROWTH, x, y, z, 0);
            }
            return true;
        }
    }
    ...
    if (tile == Tile::sapling_Id)
    {
        if (!bTestUseOnOnly)
        {
            if (!level->isClientSide)
            {
                ...
                {
                    static_cast<Sapling *>(Tile::sapling)->advanceTree(level, x, y, z, level->random);
                }
                itemInstance->count--;
            }
        }
    ...
    else if (tile == Tile::mushroom_brown_Id || tile == Tile::mushroom_red_Id)
    {
        if (!bTestUseOnOnly)
        {
            if (!level->isClientSide)
            {
                ...
                {
                    static_cast<Mushroom *>(Tile::tiles[tile])->growTree(level, x, y, z, level->random);
                }
                itemInstance->count--;
            }
        }
                ...
                for (int j = 0; j < 128; j++)
                {
                    ...
                    if (level->getTile(xx, yy, zz) == 0)
                    {
                        if (random->nextInt(10) != 0)
                        {
                            ...
                            {
                                level->setTileAndData(xx, yy, zz, Tile::tallgrass_Id, TallGrass::TALL_GRASS, Tile::UPDATE_ALL);
                            }
                        }
                        else if (random->nextInt(3) != 0)
                        {
                            ...
                            {
                                level->setTileAndUpdate(xx, yy, zz, Tile::flower_Id);
                            }
                        }
                        else
                        {
                            ...
                            {
                                level->setTileAndUpdate(xx, yy, zz, Tile::rose_Id);
                            }
                        }
                    }
