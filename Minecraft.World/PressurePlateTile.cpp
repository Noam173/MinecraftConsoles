...
int PressurePlateTile::getSignalStrength(Level *level, int x, int y, int z)
{
    ...
    {
        entities = level->getEntitiesOfClass(typeid(LivingEntity), getSensitiveAABB(x, y, z));
    }
    else if (sensitivity == players)
    {
        entities = level->getEntitiesOfClass(typeid(Player), getSensitiveAABB(x, y, z));
    }
    else
    {
        __debugbreak(); // 4J-JEV: We're going to delete something at a random location.
    }
