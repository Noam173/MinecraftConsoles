...
bool HouseFeature::place(Level *level, Random *random, int x, int y, int z)
{
    ...
    for (int xx = x0; xx < x0 + w; xx++)
    {
        for (int zz = z0; zz < z0 + d; zz++)
        {
            ...
            if (!m->blocksMotion() || m == Material::ice)
            {
                return false;
            }

            bool ok = false;
            if (doorSide == 0 && xx < x0 + 2)
            {
                ok = true;
            }
            if (doorSide == 1 && xx > x0 + w - 1 - 2)
            {
                ok = true;
            }
            if (doorSide == 2 && zz < z0 + 2)
            {
                ok = true;
            }
            if (doorSide == 3 && zz > z0 + d - 1 - 2)
            {
                ok = true;
            }
    ...
    for (int i = 0; i < (w * 2 + d * 2) * 3; i++)
    {
        ...
        {
            int count = 0;
            if (level->isSolidBlockingTile(xx - 1, y0 + 1, zz) && level->isSolidBlockingTile(xx + 1, y0 + 1, zz))
            {
                count++;
            }
            if (level->isSolidBlockingTile(xx, y0 + 1, zz - 1) && level->isSolidBlockingTile(xx, y0 + 1, zz + 1))
            {
                count++;
            }
            if (count == 1)
    ...
    for (int i = 0; i < (ww * 2 + dd * 2); i++)
    {
        ...
        {
            int count = 0;
            if (level->isSolidBlockingTile(xx - 1, yy + 2, zz))
            {
                count++;
            }
            if (level->isSolidBlockingTile(xx + 1, yy + 2, zz))
            ...
            if (level->isSolidBlockingTile(xx, yy + 2, zz - 1))
            {
                count++;
            }
            if (level->isSolidBlockingTile(xx, yy + 2, zz + 1))
