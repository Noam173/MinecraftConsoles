...
LevelChunk::LevelChunk(Level *level, byteArray blocks, int x, int z)
{
    ...
    if (Level::maxBuildHeight > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        ...
        {
            upperBlocks = new CompressedTileStorage(blocks, Level::COMPRESSED_CHUNK_SECTION_TILES);
        }
        else
        {
            upperBlocks = new CompressedTileStorage(true);
...
LevelChunk::~LevelChunk()
{
    ...
    if (upperSkyLight)
    {
        delete upperSkyLight;
    }
    if (upperBlockLight)
    {
        delete upperBlockLight;
    }
...
void LevelChunk::recalcHeight(int x, int yStart, int z)
{
    ...
    if (height < lowestHeightmap)
    {
        lowestHeightmap = height;
    }
    if (!level->dimension->hasCeiling)
    {
...
void LevelChunk::getNeighbourBrightnesses(int *brightnesses, LightLayer::variety layer, int x, int y, int z)
{
    ...
    if (layer == LightLayer::Sky)
    {
        light = y >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT ? upperSkyLight : lowerSkyLight;
    }
    else
    {
        light = y >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT ? upperBlockLight : lowerBlockLight;
    }
    ...
    if (layer == LightLayer::Sky)
    {
        light = (y - 1) >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT ? upperSkyLight : lowerSkyLight;
    }
    else
    {
        light = (y - 1) >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT ? upperBlockLight : lowerBlockLight;
    }
    if (light)
    {
        brightnesses[2] = light->get(x, (y - 1) % Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z);
    }
    ...
    if (light)
    {
        brightnesses[3] = light->get(x, (y + 1) % Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z);
    }
}
...
void LevelChunk::addEntity(shared_ptr<Entity> e)
{
    ...
    if (yc >= ENTITY_BLOCKS_LENGTH)
    {
        yc = ENTITY_BLOCKS_LENGTH - 1;
    }
    e->inChunk = true;
    e->xChunk = x;
    e->yChunk = yc;
...
shared_ptr<TileEntity> LevelChunk::getTileEntity(int x, int y, int z)
{
    ...
    if (it == tileEntities.end())
    {
        ...
        if (level->m_bDisableAddNewTileEntities)
        {
            return nullptr;
        }

        int t = getTile(x, y, z);
        if (t <= 0 || !Tile::tiles[t]->isEntityTile())
        {
            return nullptr;
        }
...
void LevelChunk::getEntitiesOfClass(const type_info &ec, AABB *bb, vector<shared_ptr<Entity>> &es, const EntitySelector *selector)
{
    ...
    for (int yc = yc0; yc <= yc1; yc++)
    {
        ...
        for (auto &e : *entities)
        {
            ...
            else if (e != nullptr && ec == typeid(*(e.get())))
            {
...
bool LevelChunk::shouldSave(bool force)
{
    ...
    {
        if (lastSaveHadEntities && level->getGameTime() >= lastSaveTime + 20 * 30)
        {
            return true;
        }
    }
...
int LevelChunk::getBlocksAndData(byteArray *data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting /* = true*/)
{
    ...
    {
        p += lowerBlocks->getDataRegion(*data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += upperBlocks->getDataRegion(*data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
    }

    // 4J - replaced data storage as now using SparseDataStorage
    if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += lowerData->getDataRegion(*data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += upperData->getDataRegion(*data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
    }
    ...
    if (includeLighting)
    {
        ...
        if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += lowerBlockLight->getDataRegion(*data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
        }
        if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += upperBlockLight->getDataRegion(*data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
        }

        if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += lowerSkyLight->getDataRegion(*data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
        }
        if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += upperSkyLight->getDataRegion(*data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
        }
    }
...
bool LevelChunk::testSetBlocksAndData(byteArray data, int x0, int y0, int z0, int x1, int y1, int z1, int p)
{
    ...
    if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        changed = lowerBlocks->testSetDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        changed = changed || upperBlocks->testSetDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
    }
...
int LevelChunk::setBlocksAndData(byteArray data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting /* = true*/)
{
    ...
    if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += lowerBlocks->setDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p, includeLighting ? nullptr : tileUpdatedCallback, this, 0);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += upperBlocks->setDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p, includeLighting ? nullptr : tileUpdatedCallback, this, Level::COMPRESSED_CHUNK_SECTION_HEIGHT);
    }
    /*
    for (int x = x0; x < x1; x++)
    for (int z = z0; z < z1; z++)
    ...
    if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += lowerData->setDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p, includeLighting ? nullptr : tileUpdatedCallback, this, 0);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += upperData->setDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p, includeLighting ? nullptr : tileUpdatedCallback, this, Level::COMPRESSED_CHUNK_SECTION_HEIGHT);
    }
    ...
    if (includeLighting)
    {
        ...
        if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += lowerBlockLight->setDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
        }
        if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += upperBlockLight->setDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
        }

        if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += lowerSkyLight->setDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p);
        }
        if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
        {
            p += upperSkyLight->setDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p);
        }
...
{
    if (hasGapsToCheck && !level->dimension->hasCeiling)
    {
        recheckGaps();
    }
}
...
{
    if (lowerData == nullptr)
    {
        lowerData = new SparseDataStorage();
    }
    if (upperData == nullptr)
    {
        upperData = new SparseDataStorage(true);
    }
...
void LevelChunk::getBlockLightData(byteArray data)
{
    ...
    if (data.length > Level::COMPRESSED_CHUNK_SECTION_TILES / 2)
    {
        upperBlockLight->getData(data, Level::COMPRESSED_CHUNK_SECTION_TILES / 2);
    }
}
...
void LevelChunk::setBlockLightData(byteArray data)
{
    ...
    if (data.length > Level::COMPRESSED_CHUNK_SECTION_TILES / 2)
    {
        upperBlockLight->setData(data, Level::COMPRESSED_CHUNK_SECTION_TILES / 2);
    }
}
...
void LevelChunk::compressBlocks()
{
    ...
    if (blocksToCompressLower)
    {
        blocksToCompressLower->compress();
    }
    if (blocksToCompressUpper)
    {
        blocksToCompressUpper->compress();
    }
#else
    blocks->compress();
#endif
...
void LevelChunk::compressData()
{
    ...
    // Attempt to do the actual compression
    if (dataToCompressLower)
    {
        dataToCompressLower->compress();
    }
    if (dataToCompressUpper)
    {
        dataToCompressUpper->compress();
    }
#else
    data->compress();
#endif
