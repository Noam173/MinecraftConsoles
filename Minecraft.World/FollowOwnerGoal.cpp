...
void FollowOwnerGoal::tick()
{
    ...
    if (navigation->moveTo(owner.lock(), speedModifier))
    {
        return;
    }
    if (tamable->isLeashed())
    {
        return;
    }
    if (tamable->distanceToSqr(owner.lock()) < TeleportDistance * TeleportDistance)
    {
        return;
    }
