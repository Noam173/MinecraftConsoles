...
void inline Level::setBrightnessCached(lightCache_t *cache, uint64_t *cacheUse, LightLayer::variety layer, int x, int y, int z, int brightness)
{
    ...
    if (y & 0xffffff00)
    {
        return; // Eliminate -ve ys and values > 255
    }
...
inline int Level::getBrightnessCached(lightCache_t *cache, LightLayer::variety layer, int x, int y, int z)
{
    ...
    if (y & 0xffffff00)
    {
        return getBrightness(layer, x, y, z); // Fall back on original method for out-of-bounds y
    }
    ...
    : seaLevel(constSeaLevel)
{
    ...
    {
        seaLevel = Level::genDepth / 2; // 4J added - sea level is one unit lower since 1.8.2, maintain older height for old levels
    }
...
{
    if (y1 < minBuildHeight || y0 >= maxBuildHeight)
    {
        return false;
    }
...
bool Level::isUnobstructed(AABB *aabb, shared_ptr<Entity> ignore)
{
    ...
    for (auto &e : *ents)
    {
        if (e && !e->removed && e->blocksBuilding && e != ignore)
        {
            return false;
        }
    }
        ...
        for (int x = x0; x < x1; x++)
        {
            for (int y = y0; y < y1; y++)
            {
                for (int z = z0; z < z1; z++)
                {
                    ...
                    if (t == Tile::fire_Id || t == Tile::lava_Id || t == Tile::calmLava_Id)
                    {
                        return true;
                    }
                }
...
bool Level::isTopSolidBlocking(Tile *tile, int data)
{
    ...
    if (tile->material->isSolidBlocking() && tile->isCubeShaped())
    {
        return true;
    }
    ...
    if (dynamic_cast<HopperTile *>(tile) != nullptr)
    {
        return true;
    }
...
bool Level::shouldFreeze(int x, int y, int z, bool checkNeighbors)
{
    ...
    if (y >= 0 && y < maxBuildHeight && getBrightness(LightLayer::Block, x, y, z) < 10)
    {
        ...
        if ((current == Tile::calmWater_Id || current == Tile::water_Id) && getData(x, y, z) == 0)
        {
            ...
            if (surroundedByWater && getMaterial(x - 1, y, z) != Material::water)
            {
            ...
            if (surroundedByWater && getMaterial(x, y, z - 1) != Material::water)
            {
                surroundedByWater = false;
            }
            if (surroundedByWater && getMaterial(x, y, z + 1) != Material::water)
...
{
    if (layer == LightLayer::Sky && canSeeSky(x, y, z))
    {
        return MAX_BRIGHTNESS;
    }
    int id = getTile(x, y, z);
    ...
    if (block >= MAX_BRIGHTNESS && Tile::lightEmission[id] > 0)
    {
        block = 1;
    }
    if (block < 1)
                        ...
                        if (expected > 0)
                        {
                            ...
                            if (xd + yd + zd < 17)
                            {
                                ...
                                for (int face = 0; face < 6; face++)
                                {
                                    ...
                                    if ((xx > maxXZ) || (xx < minXZ) || (zz > maxXZ) || (zz < minXZ))
                                    {
                                        continue;
                                    }
        ...
        if (expected != current)
        {
            ...
            if (expected > current)
            {
                ...
                if (xd + yd + zd < 17 && withinBounds)
                {
                    ...
                    if ((x - 1) >= minXZ)
                    {
                        if (getBrightnessCached(cache, layer, x - 1, y, z) < expected)
                        {
                            toCheck[toCheckCount++] = (((x - 1 - xc) + 32)) + (((y - yc) + 32) << 6) + (((z - zc) + 32) << 12);
                        }
                    }
                    if ((x + 1) <= maxXZ)
                    {
                    ...
                    if ((y - 1) >= 0)
                    {
                        if (getBrightnessCached(cache, layer, x, y - 1, z) < expected)
                        {
                            toCheck[toCheckCount++] = (((x - xc) + 32)) + (((y - 1 - yc) + 32) << 6) + (((z - zc) + 32) << 12);
                        }
                    }
                    ...
                    if ((z - 1) >= minXZ)
                    {
                        if (getBrightnessCached(cache, layer, x, y, z - 1) < expected)
                        {
                            toCheck[toCheckCount++] = (((x - xc) + 32)) + (((y - yc) + 32) << 6) + (((z - 1 - zc) + 32) << 12);
                        }
                    }
                    if ((z + 1) <= maxXZ)
                    {
...
bool Level::mayPlace(int tileId, int x, int y, int z, bool ignoreEntities, int face, shared_ptr<Entity> ignoreEntity, shared_ptr<ItemInstance> item)
{
    ...
    if (aabb != nullptr && !isUnobstructed(aabb, ignoreEntity))
    {
        return false;
    }
    ...
    if (targetTile != nullptr && targetTile->material == Material::decoration && tile == Tile::anvil)
    {
        return true;
    }
...
{
    if (getSignal(x, y - 1, z, 0) > 0)
    {
        return true;
    }
    if (getSignal(x, y + 1, z, 1) > 0)
    {
        return true;
    }
    if (getSignal(x, y, z - 1, 2) > 0)
    {
        return true;
    }
    if (getSignal(x, y, z + 1, 3) > 0)
    {
        return true;
    }
    if (getSignal(x - 1, y, z, 4) > 0)
    {
        return true;
    }
    if (getSignal(x + 1, y, z, 5) > 0)
    {
        return true;
    }
    return false;
...
shared_ptr<Player> Level::getNearestPlayer(double x, double y, double z, double maxDist, double maxYDist /*= -1*/)
{
    ...
    for (auto &p : players)
    {
        ...
        if (maxYDist > 0 && abs(p->y - y) > maxYDist)
        {
            continue;
        }
...
void Level::setBlocksAndData(int x, int y, int z, int xs, int ys, int zs, byteArray data, bool includeLighting /* = true*/)
{
    ...
    for (int xc = xc0; xc <= xc1; xc++)
    {
        ...
        for (int zc = zc0; zc <= zc1; zc++)
        {
            ...
            if (p < data.length)
            {
                p = lc->setBlocksAndData(data, x0, y0, z0, x1, y1, z1, p, includeLighting);
            }
...
bool Level::isChunkPostPostProcessed(int x, int z)
{
    if (!hasChunk(x, z))
    {
        return false; // This will occur for non-loaded chunks, not for edge chunks
    }

    LevelChunk *lc = getChunk(x, z);
    if (lc->isEmpty())
    {
        return true; // Since we've already eliminated non-loaded chunks, this should only occur for edge chunks. Consider those as fully processed
    }
...
bool Level::isChunkFinalised(int x, int z)
{
    for (int xo = -1; xo <= 1; xo++)
    {
        ...
        {
            if (!isChunkPostPostProcessed(x + xo, z + zo))
            {
                return false;
            }
        }
