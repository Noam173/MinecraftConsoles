...
bool SpruceFeature::place(Level *level, Random *random, int x, int y, int z)
{
    ...
    if ((belowTile != Tile::grass_Id && belowTile != Tile::dirt_Id) || y >= Level::maxBuildHeight - treeHeight - 1)
    {
        return false;
    }
    ...
    for (int heightPos = 0; heightPos <= topHeight; heightPos++)
    {
        ...
        for (int xx = x - currentRadius; xx <= x + currentRadius; xx++)
        {
            ...
            {
                int zo = zz - (z);
                if (abs(xo) == currentRadius && abs(zo) == currentRadius && currentRadius > 0)
                {
                    continue;
                }
                if (!Tile::solid[level->getTile(xx, yy, zz)])
                {
                    placeBlock(level, xx, yy, zz, Tile::leaves_Id, LeafTile::EVERGREEN_LEAF);
                }
            }
        }
    ...
    for (int hh = 0; hh < treeHeight - topOffset; hh++)
    {
        ...
        {
            placeBlock(level, x, y + hh, z, Tile::treeTrunk_Id, TreeTile::DARK_TRUNK);
        }
    }
    return true;
}
