...
void Entity::freeSmallId(int index)
{
    if (((size_t)TlsGetValue(tlsIdx)) == 0)
    {
        return; // Don't do anything with small ids if this isn't the server thread
    }
    if (index >= 2048)
    {
        return; // Don't do anything if this isn't a short id
    }
...
bool Entity::isExtraWanderingEnabled()
{
    if (((size_t)TlsGetValue(tlsIdx)) == 0)
    {
        return false; // Don't do anything with small ids if this isn't the server thread
    }
    if (entityId >= 2048)
    {
...
void Entity::resetPos()
{
    ...
    {
        setPos(x, y, z);
        if (level->getCubes(sharedThis, bb)->empty())
        {
            break;
        }
        y += 1;
...
void Entity::causeFallDamage(float distance)
{
    ...
    {
        rider.lock()->causeFallDamage(distance);
    }
}
...
void Entity::changeDimension(int i)
{
    ...
    // 4J: Restrictions on what can go through
    {
        ...
        if (newLevel->atEntityLimit(shared_from_this()))
        {
            return;
        }

        // 4J: Check level limit on living entities, minecarts and boats
        if (!instanceof(eTYPE_PLAYER) && !newLevel->canCreateMore(GetType(), Level::eSpawnType_Portal))
        {
            return;
        }
    }
