...
int TripWireSourceTile::getPlacedOnFaceDataValue(Level *level, int x, int y, int z, int face, float clickX, float clickY, float clickZ, int itemValue)
{
    ...
    if (face == Facing::NORTH && level->isSolidBlockingTileInLoadedChunk(x, y, z + 1, true))
    {
        dir = Direction::NORTH;
    }
    if (face == Facing::SOUTH && level->isSolidBlockingTileInLoadedChunk(x, y, z - 1, true))
    {
        dir = Direction::SOUTH;
    }
    if (face == Facing::WEST && level->isSolidBlockingTileInLoadedChunk(x + 1, y, z, true))
    {
        dir = Direction::WEST;
    }
    if (face == Facing::EAST && level->isSolidBlockingTileInLoadedChunk(x - 1, y, z, true))
    {
        dir = Direction::EAST;
    }
                                        ...
                                        /*4J-Jev, these parameters only used with 'updateSource' -->*/ int wireSource, int wireSourceData)
{
    ...
    if (id > 0) // ie. it isn't being removed.
    {
        ...
        if (canUpdate)
        {
            notifyNeighbors(level, x, y, z, dir);
        }
    }
