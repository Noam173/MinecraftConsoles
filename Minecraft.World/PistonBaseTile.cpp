...
{
    if (name.compare(EDGE_TEX) == 0)
    {
        return Tile::pistonBase->icon;
    }
    if (name.compare(PLATFORM_TEX) == 0)
    {
        return Tile::pistonBase->iconPlatform;
    }
    if (name.compare(PLATFORM_STICKY_TEX) == 0)
    {
        return Tile::pistonStickyBase->iconPlatform;
    }
    if (name.compare(INSIDE_TEX) == 0)
    {
        return Tile::pistonBase->iconInside;
    }
...
bool PistonBaseTile::getNeighborSignal(Level *level, int x, int y, int z, int facing)
{
    ...
    // check signals above
    if (level->hasSignal(x, y, z, 0))
    {
        return true;
    }
    if (level->hasSignal(x, y + 2, z, 1))
    {
        return true;
    }
    if (level->hasSignal(x, y + 1, z - 1, 2))
    {
        return true;
    }
    if (level->hasSignal(x, y + 1, z + 1, 3))
    {
        return true;
    }
    if (level->hasSignal(x - 1, y + 1, z, 4))
    {
        return true;
    }
    if (level->hasSignal(x + 1, y + 1, z, 5))
    {
        return true;
    }
...
bool PistonBaseTile::triggerEvent(Level *level, int x, int y, int z, int param1, int facing)
{
    ...
    else if (param1 == TRIGGER_CONTRACT)
    {
        ...
        if (isSticky)
        {
            ...
            if (!pistonPiece && block > 0 && (isPushable(block, level, twoX, twoY, twoZ, false)) && (Tile::tiles[block]->getPistonPushReaction() == Material::PUSH_NORMAL || block == Tile::pistonBase_Id || block == Tile::pistonStickyBase_Id))
            {
