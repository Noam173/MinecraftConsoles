...
bool NotGateTile::hasNeighborSignal(Level *level, int x, int y, int z)
{
    ...
    if (dir == 5 && level->hasSignal(x, y - 1, z, 0))
    {
        return true;
    }
    if (dir == 3 && level->hasSignal(x, y, z - 1, 2))
    {
        return true;
    }
    if (dir == 4 && level->hasSignal(x, y, z + 1, 3))
    {
        return true;
    }
    if (dir == 1 && level->hasSignal(x - 1, y, z, 4))
    {
        return true;
    }
    if (dir == 2 && level->hasSignal(x + 1, y, z, 5))
    {
        return true;
    }
    return false;
