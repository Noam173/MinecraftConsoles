...
void Player::aiStep()
{
    ...
    if (!level->isClientSide)
    {
        speed->setBaseValue(abilities.getWalkingSpeed());
    }
...
float Player::getDestroySpeed(Tile *tile, bool hasProperTool)
{
    ...
    if (isUnderLiquid(Material::water) && !EnchantmentHelper::hasWaterWorkerBonus(dynamic_pointer_cast<LivingEntity>(shared_from_this())))
    {
        speed /= 5;
    }
...
bool Player::hurt(DamageSource *source, float dmg)
{
    ...
    if (hasInvulnerablePrivilege() || (abilities.invulnerable && !source->isBypassInvul()))
    {
        return false;
    }
...
void Player::causeFoodExhaustion(float amount)
{
    ...
    if (abilities.invulnerable || hasInvulnerablePrivilege())
    {
        return;
    }

    // 4J Stu - Added 1.8.2 bug fix (TU6) - If players cannot eat, then their food bar should not decrease due to exhaustion
    if (app.GetGameHostOption(eGameHostOption_TrustPlayers) == 0 && getPlayerGamePrivilege(Player::ePlayerGamePrivilege_CannotBuild) != 0)
    {
        return;
    }
