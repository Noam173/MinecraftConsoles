        ...
        for (int zz = 0; zz < 16; zz++)
        {
            for (int yy = 0; yy < 8; yy++)
            {
                bool check = !grid[((xx) * 16 + (zz)) * 8 + (yy)] && ((xx < 15 && grid[((xx + 1) * 16 + (zz)) * 8 + (yy)]) //
                                                                      || (xx > 0 && grid[((xx - 1) * 16 + (zz)) * 8 + (yy)]) || (zz < 15 && grid[((xx) * 16 + (zz + 1)) * 8 + (yy)]) || (zz > 0 && grid[((xx) * 16 + (zz - 1)) * 8 + (yy)]) || (yy < 7 && grid[((xx) * 16 + (zz)) * 8 + (yy + 1)]) || (yy > 0 && grid[((xx) * 16 + (zz)) * 8 + (yy - 1)]));
                ...
                if (check)
                {
                    ...
                    if (yy < 4 && (!m->isSolid() && level->getTile(x + xx, y + yy, z + zz) != tile))
                    {
                        return false;
                    }
                }
            ...
            for (int yy = 4; yy < 8; yy++)
            {
                if (grid[((xx) * 16 + (zz)) * 8 + (yy)])
                {
                    if (level->getTile(x + xx, y + yy - 1, z + zz) == Tile::dirt_Id && level->getBrightness(LightLayer::Sky, x + xx, y + yy, z + zz) > 0)
                    {
                        ...
                        {
                            level->setTileAndData(x + xx, y + yy - 1, z + zz, Tile::mycel_Id, 0, Tile::UPDATE_CLIENTS);
                        }
                        else
                        {
                            level->setTileAndData(x + xx, y + yy - 1, z + zz, Tile::grass_Id, 0, Tile::UPDATE_CLIENTS);
                        }
                    }
                }
            }
    ...
    if (Tile::tiles[tile]->material == Material::lava)
    {
        for (int xx = 0; xx < 16; xx++)
        {
            for (int zz = 0; zz < 16; zz++)
            {
                for (int yy = 0; yy < 8; yy++)
                {
                    bool check = !grid[((xx) * 16 + (zz)) * 8 + (yy)] && ((xx < 15 && grid[(((xx + 1) * 16 + (zz)) * 8 + (yy))]) || (xx > 0 && grid[(((xx - 1) * 16 + (zz)) * 8 + (yy))]) || (zz < 15 && grid[(((xx) * 16 + (zz + 1)) * 8 + (yy))]) || (zz > 0 && grid[(((xx) * 16 + (zz - 1)) * 8 + (yy))]) || (yy < 7 && grid[(((xx) * 16 + (zz)) * 8 + (yy + 1))]) || (yy > 0 && grid[(((xx) * 16 + (zz)) * 8 + (yy - 1))]));
    ...
    if (Tile::tiles[tile]->material == Material::water)
    {
        for (int xx = 0; xx < 16; xx++)
        {
            for (int zz = 0; zz < 16; zz++)
            {
                ...
                {
                    level->setTileAndData(x + xx, y + yy, z + zz, Tile::ice_Id, 0, Tile::UPDATE_CLIENTS);
                }
            }
        }
    }
