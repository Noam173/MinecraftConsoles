...
void Creeper::readAdditionalSaveData(CompoundTag *tag)
{
    ...
    if (tag->contains(L"Fuse"))
    {
        maxSwell = tag->getShort(L"Fuse");
    }
    ...
    if (isAlive())
    {
        ...
        if (swell >= maxSwell)
        {
            ...
            if (!level->isClientSide)
            {
                ...
                if (isPowered())
                {
                    level->explode(shared_from_this(), x, y, z, explosionRadius * 2, destroyBlocks);
                }
                else
                {
                    level->explode(shared_from_this(), x, y, z, explosionRadius, destroyBlocks);
                }
                remove();
            }
        }
