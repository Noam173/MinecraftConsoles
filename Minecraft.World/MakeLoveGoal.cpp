...
void MakeLoveGoal::tick()
{
    ...
    {
        if (loveMakingTime == 0 && partner.lock()->isInLove())
        {
            breed();
        }
    }
    ...
    {
        level->broadcastEntityEvent(villager->shared_from_this(), EntityEvent::LOVE_HEARTS);
    }
}
