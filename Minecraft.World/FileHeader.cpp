...
void FileHeader::ReadHeader(LPVOID saveMem, ESavePlatform plat /*= SAVE_FILE_PLATFORM_LOCAL */)
{
    ...
    {
        System::ReverseULONG(&headerOffset);
    }
    ...
    if (isSaveEndianDifferent())
    {
        System::ReverseULONG(&headerSize);
    }
    ...
    if (isSaveEndianDifferent())
    {
        System::ReverseSHORT(&m_originalSaveVersion);
    }
    ...
    if (isSaveEndianDifferent())
    {
        System::ReverseSHORT(&m_saveVersion);
    }
...
vector<FileEntry *> *FileHeader::getDatFilesWithOnlineID(const PlayerUID &pUID)
{
    ...
    if (files)
    {
        sort(files->begin(), files->end(), FileEntry::newestFirst);
    }
    return files;
}
