            ...
            if (slot != nullptr)
            {
                ...
                else if (slot->mayPickup(player))
                {
                    ...
                    else
                    {
                        ...
                        if (clicked->id == carried->id && carried->getMaxStackSize() > 1 && (!clicked->isStackedByData() || clicked->getAuxValue() == carried->getAuxValue()) && ItemInstance::tagMatches(clicked, carried))
                        {
                            int c = clicked->count;
        ...
        if (carried != nullptr && (slot == nullptr || !slot->hasItem() || !slot->mayPickup(player)))
        {
            ...
            for (int pass = 0; pass < 2; pass++)
            {
                ...
                for (size_t i = start; i >= 0 && i < static_cast<int>(slots.size()) && carried->count < carried->getMaxStackSize(); i += step)
                {
                    ...
                    {
                        if (pass == 0 && target->getItem()->count == target->getItem()->getMaxStackSize())
                        {
                            continue;
                        }
...
void AbstractContainerMenu::setSynched(shared_ptr<Player> player, bool synched)
{
    if (synched)
    {
        ...
        if (it != unSynchedPlayers.end())
        {
            unSynchedPlayers.erase(it);
        }
    }
    else
    {
...
bool AbstractContainerMenu::moveItemStackTo(shared_ptr<ItemInstance> itemStack, int startSlot, int endSlot, bool backwards)
{
    ...
    if (itemStack->isStackable())
    {
        while (itemStack->count > 0 && ((!backwards && destSlot < endSlot) || (backwards && destSlot >= startSlot)))
        {
            ...
            if (target != nullptr && target->id == itemStack->id && (!itemStack->isStackedByData() || itemStack->getAuxValue() == target->getAuxValue()) && ItemInstance::tagMatches(itemStack, target))
            {
