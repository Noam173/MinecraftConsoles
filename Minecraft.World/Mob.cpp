...
Mob::~Mob()
{
    if (lookControl != nullptr)
    {
        delete lookControl;
    }
    if (moveControl != nullptr)
    {
        delete moveControl;
    }
...
void Mob::spawnAnim()
{
    for (int i = 0; i < 20; i++)
    {
        ...
        double dd = 10;
        level->addParticle(eParticleType_explode, x + random->nextFloat() * bbWidth * 2 - bbWidth - xa * dd, y + random->nextFloat() * bbHeight - ya * dd, z + random->nextFloat() * bbWidth * 2 - bbWidth - za * dd, xa, ya, za);
    }
}
...
void Mob::readAdditionalSaveData(CompoundTag *tag)
{
    ...
    if (tag->contains(L"CustomName") && tag->getString(L"CustomName").length() > 0)
    {
...
void Mob::aiStep()
{
    ...
    if (!level->isClientSide && canPickUpLoot() && !dead && level->getGameRules()->getBoolean(GameRules::RULE_MOBGRIEFING))
    {
        ...
        for (auto &it : *entities)
        {
            ...
            if (entity->removed || entity->getItem() == nullptr)
            {
                continue;
            }
...
void Mob::populateDefaultEquipmentSlots()
{
    if (random->nextFloat() < MAX_WEARING_ARMOR_CHANCE * level->getDifficulty(x, y, z))
    {
        ...
        for (int i = 3; i >= 0; i--)
        {
            ...
            if (i < 3 && random->nextFloat() < partialChance)
            {
                break;
            }
            if (item == nullptr)
            {
                ...
                if (equip != nullptr)
                {
                    setEquippedSlot(i + 1, std::make_shared<ItemInstance>(equip));
                }
            }
        }
    }
...
Item *Mob::getEquipmentForSlot(int slot, int type)
{
    ...
    case SLOT_HELM:
        if (type == 0)
        {
            return Item::helmet_leather;
        }
        if (type == 1)
        {
            return Item::helmet_gold;
        }
        if (type == 2)
        {
            return Item::helmet_chain;
        }
        if (type == 3)
        {
            return Item::helmet_iron;
        }
        if (type == 4)
        {
            return Item::helmet_diamond;
        }
    case SLOT_CHEST:
        if (type == 0)
        {
            return Item::chestplate_leather;
        }
        if (type == 1)
        {
            return Item::chestplate_gold;
        }
        if (type == 2)
        {
            return Item::chestplate_chain;
        }
        if (type == 3)
        {
            return Item::chestplate_iron;
        }
        if (type == 4)
        {
            return Item::chestplate_diamond;
        }
    case SLOT_LEGGINGS:
        if (type == 0)
        {
            return Item::leggings_leather;
        }
        if (type == 1)
        {
            return Item::leggings_gold;
        }
        if (type == 2)
        {
            return Item::leggings_chain;
        }
        if (type == 3)
        {
            return Item::leggings_iron;
        }
        if (type == 4)
        {
            return Item::leggings_diamond;
        }
    case SLOT_BOOTS:
        if (type == 0)
        {
            return Item::boots_leather;
        }
        if (type == 1)
        {
            return Item::boots_gold;
        }
        if (type == 2)
        {
            return Item::boots_chain;
        }
        if (type == 3)
        {
            return Item::boots_iron;
        }
        if (type == 4)
        {
            return Item::boots_diamond;
        }
    }
    ...
    if (itemstack != nullptr)
    {
        ...
        if (itemstack->id == Item::lead_Id)
        {
            if (canBeLeashed())
            {
                ...
                if (shared_from_this()->instanceof(eTYPE_TAMABLE_ANIMAL) && (tamableAnimal = dynamic_pointer_cast<TamableAnimal>(shared_from_this()))->isTame()) // 4J-JEV: excuse the assignment operator in here, don't want to dyn-cast if it's avoidable.
                {
