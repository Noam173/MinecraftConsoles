...
shared_ptr<ItemInstance> ItemInstance::remove(int count)
{
    ...
    if (tag != nullptr)
    {
        ii->tag = static_cast<CompoundTag *>(tag->copy());
    }
    this->count -= count;
...
CompoundTag *ItemInstance::save(CompoundTag *compoundTag)
{
    ...
    if (tag != nullptr)
    {
        compoundTag->put(L"tag", tag->copy());
    }
    return compoundTag;
}
...
{
    if (!getItem()->isEnchantable(shared_from_this()))
    {
        return false;
    }
    if (isEnchanted())
...
{
    if (tag != nullptr && tag->contains(L"ench"))
    {
        return true;
    }
    return false;
