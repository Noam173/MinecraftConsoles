                    ...
                    for (int x = 0; x < CHUNK_WIDTH; x++)
                    {
                        for (int z = 0; z < CHUNK_WIDTH; z++)
                        {
                            ...
                            if (emin == 0)
                            {
                                ...
                                if (yc * CHUNK_HEIGHT + y <= (level->getSeaLevel() - 10))
                                {
                                    tileId = Tile::stone_Id;
                                }
                                else if (yc * CHUNK_HEIGHT + y < level->getSeaLevel())
                                {
                    ...
                    else if (old == Tile::stone_Id)
                    {
                        if (run == -1)
                        {
                            ...
                            if (y < waterHeight && top == 0)
                            {
                                if (temp < 0.15f)
                                {
                                    top = static_cast<byte>(Tile::ice_Id);
                                }
                                else
                                {
        ...
        for (int z = 0; z < 16; z++)
        {
            ...
            if (h <= 0)
            {
                if (level->getHeightmap(xp - 1, zp) > 0 || level->getHeightmap(xp + 1, zp) > 0 || level->getHeightmap(xp, zp - 1) > 0 || level->getHeightmap(xp, zp + 1) > 0)
                {
                    ...
                    if (hadWater || (level->getTile(xp, y, zp - 1) == Tile::calmWater_Id && level->getData(xp, y, zp - 1) < 7))
                    {
                        hadWater = true;
                    }
                    if (hadWater || (level->getTile(xp, y, zp + 1) == Tile::calmWater_Id && level->getData(xp, y, zp + 1) < 7))
