...
int PathNavigation::getSurfaceY()
{
    ...
    {
        return static_cast<int>(mob->bb->y0 + 0.5);
    }
    ...
    while (tileId == Tile::water_Id || tileId == Tile::calmWater_Id)
    {
        ...
        if (++steps > 16)
        {
            return static_cast<int>(mob->bb->y0);
        }
    }
    return surface;
}
...
{
    if (level->canSeeSky(Mth::floor(mob->x), static_cast<int>(mob->bb->y0 + 0.5), Mth::floor(mob->z)))
    {
        return;
    }
...
bool PathNavigation::canMoveDirectly(Vec3 *startPos, Vec3 *stopPos, int sx, int sy, int sz)
{
    ...
    sx += 2;
    sz += 2;
    if (!canWalkOn(gridPosX, static_cast<int>(startPos->y), gridPosZ, sx, sy, sz, startPos, dirX, dirZ))
    {
        return false;
    }
    sx -= 2;
...
bool PathNavigation::canWalkOn(int x, int y, int z, int sx, int sy, int sz, Vec3 *startPos, double goalDirX, double goalDirZ)
{
    ...
    if (!canWalkAbove(startX, y, startZ, sx, sy, sz, startPos, goalDirX, goalDirZ))
    {
        return false;
    }
