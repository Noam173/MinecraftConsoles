...
void Villages::queryUpdateAround(int x, int y, int z)
{
    ...
    if (!hasQuery(x, y, z))
    {
        queries.push_back(new Pos(x, y, z));
    }
}
...
shared_ptr<Village> Villages::getClosestVillage(int x, int y, int z, int maxDist)
{
    ...
    for (auto &village : villages)
    {
        ...
        if (distSqr > requiredDist * requiredDist)
        {
            continue;
        }
...
void Villages::createDoorInfo(int x, int y, int z)
{
    ...
    if (dir == 0 || dir == 2)
    {
        ...
        if (canSeeX != 0)
        {
            unclustered.push_back(std::make_shared<DoorInfo>(x, y, z, canSeeX > 0 ? -2 : 2, 0, _tick));
        }
    }
    else
    {
        ...
        if (canSeeZ != 0)
        {
            unclustered.push_back(std::make_shared<DoorInfo>(x, y, z, 0, canSeeZ > 0 ? -2 : 2, _tick));
        }
    }
}
