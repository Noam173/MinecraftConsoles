...
Vec3 *LiquidTile::getFlow(LevelSource *level, int x, int y, int z)
{
    ...
    {
        bool ok = false;
        if (ok || isSolidFace(level, x, y, z - 1, 2))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x, y, z + 1, 3))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x - 1, y, z, 4))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x + 1, y, z, 5))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x, y + 1, z - 1, 2))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x, y + 1, z + 1, 3))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x - 1, y + 1, z, 4))
        {
            ok = true;
        }
        if (ok || isSolidFace(level, x + 1, y + 1, z, 5))
        {
            ok = true;
        }
        if (ok)
...
{
    if (name.compare(TEXTURE_WATER_STILL) == 0)
    {
        return Tile::water->icons[0];
    }
    if (name.compare(TEXTURE_WATER_FLOW) == 0)
    {
        return Tile::water->icons[1];
    }
    if (name.compare(TEXTURE_LAVA_STILL) == 0)
    {
        return Tile::lava->icons[0];
    }
    if (name.compare(TEXTURE_LAVA_FLOW) == 0)
    {
        return Tile::lava->icons[1];
    }
    return nullptr;
}
