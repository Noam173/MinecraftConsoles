...
LivingEntity::~LivingEntity()
{
    ...
    if (lastEquipment.data != nullptr)
    {
        delete[] lastEquipment.data;
    }
}
...
void LivingEntity::baseTick()
{
    ...
    if (isFireImmune() || level->isClientSide)
    {
        clearFire();
    }
...
bool LivingEntity::hurt(DamageSource *source, float dmg)
{
    ...
    if (level->isClientSide && dynamic_cast<EntityDamageSource *>(source) == nullptr)
    {
        return false;
    }
    noActionTime = 0;
    ...
    if (invulnerableTime > invulnerableDuration / 2.0f)
    {
        ...
        if (!level->isClientSide)
        {
            actuallyHurt(source, dmg - lastHurt);
        }
        lastHurt = dmg;
        sound = false;
    }
...
void LivingEntity::die(DamageSource *source)
{
    ...
    if (sourceEntity != nullptr)
    {
        sourceEntity->killed(dynamic_pointer_cast<LivingEntity>(shared_from_this()));
    }
...
shared_ptr<LivingEntity> LivingEntity::getKillCredit()
{
    ...
    if (lastHurtByPlayer != nullptr)
    {
        return lastHurtByPlayer;
    }
    if (lastHurtByMob != nullptr)
    {
        return lastHurtByMob;
    }
    return nullptr;
    ...
    if (!level->isClientSide)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (!ItemInstance::matches(current, previous))
            {
                ...
                if (current != nullptr)
                {
                    attributes->addItemModifiers(current);
                }
...
void LivingEntity::aiStep()
{
    ...
    if (abs(xd) < MIN_MOVEMENT_DISTANCE)
    {
        xd = 0;
    }
    if (abs(yd) < MIN_MOVEMENT_DISTANCE)
    {
        yd = 0;
    }
    if (abs(zd) < MIN_MOVEMENT_DISTANCE)
    {
        zd = 0;
    }
