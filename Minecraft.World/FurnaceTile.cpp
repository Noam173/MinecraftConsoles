...
void FurnaceTile::setPlacedBy(Level *level, int x, int y, int z, shared_ptr<LivingEntity> by, shared_ptr<ItemInstance> itemInstance)
{
    ...
    if (dir == 0)
    {
        level->setData(x, y, z, Facing::NORTH, Tile::UPDATE_CLIENTS);
    }
    if (dir == 1)
    {
        level->setData(x, y, z, Facing::EAST, Tile::UPDATE_CLIENTS);
    }
    if (dir == 2)
    {
        level->setData(x, y, z, Facing::SOUTH, Tile::UPDATE_CLIENTS);
    }
    if (dir == 3)
    {
        level->setData(x, y, z, Facing::WEST, Tile::UPDATE_CLIENTS);
    }
