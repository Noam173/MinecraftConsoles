...
int Inventory::getSlot(int tileId, int data)
{
    ...
    {
        if (items[i] != nullptr && items[i]->id == tileId && items[i]->getAuxValue() == data)
        {
            return i;
        }
    }
...
int Inventory::getSlotWithRemainingSpace(shared_ptr<ItemInstance> item)
{
    for (unsigned int i = 0; i < items.length; i++)
    {
        if (items[i] != nullptr && items[i]->id == item->id && items[i]->isStackable() && items[i]->count < items[i]->getMaxStackSize() && items[i]->count < getMaxStackSize() && (!items[i]->isStackedByData() || items[i]->getAuxValue() == item->getAuxValue()) && ItemInstance::tagMatches(items[i], item))
        {
            return i;
        }
    }
...
int Inventory::clearInventory(int id, int data)
{
    ...
    for (int i = 0; i < items.length; i++)
    {
        ...
        if (data > -1 && item->getAuxValue() != data)
        {
            continue;
        }
    ...
    if (carried != nullptr)
    {
        if (id > -1 && carried->id != id)
        {
            return count;
        }
        if (data > -1 && carried->getAuxValue() != data)
        {
            return count;
        }
...
bool Inventory::isSame(shared_ptr<Inventory> copy)
{
    for (unsigned int i = 0; i < items.length; i++)
    {
        if (!isSame(copy->items[i], items[i]))
        {
            return false;
        }
    }
    for (unsigned int i = 0; i < armor.length; i++)
    {
        if (!isSame(copy->armor[i], armor[i]))
        {
            return false;
        }
    }
