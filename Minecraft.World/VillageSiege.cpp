...
void VillageSiege::tick()
{
    ...
    else
    {
        ...
        if (siegeState == SIEGE_CAN_ACTIVATE)
        {
            ...
            if (timeOfDay < 0.50 || timeOfDay > 0.501)
            {
                return;
            }
...
bool VillageSiege::tryToSetupSiege()
{
    ...
    for (auto &player : *players)
    {
        ...
        if (_village->getDoorCount() < 10)
        {
            continue;
        }
        if (_village->getStableAge() < 20)
        {
            continue;
        }
...
Vec3 *VillageSiege::findRandomSpawnPos(int x, int y, int z)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        if (!_village->isInside(xx, yy, zz))
        {
            continue;
        }
        if (MobSpawner::isSpawnPositionOk(MobCategory::monster, level, xx, yy, zz))
        {
