...
bool TorchTile::isConnection(Level *level, int x, int y, int z)
{
    ...
    if (tile == Tile::fence_Id || tile == Tile::netherFence_Id || tile == Tile::glass_Id || tile == Tile::cobbleWall_Id)
    {
        return true;
    }
    return false;
...
bool TorchTile::checkDoPop(Level *level, int x, int y, int z, int type)
{
    if (checkCanSurvive(level, x, y, z))
    {
        ...
        if (!level->isSolidBlockingTileInLoadedChunk(x - 1, y, z, true) && dir == 1)
        {
            replace = true;
        }
        if (!level->isSolidBlockingTileInLoadedChunk(x + 1, y, z, true) && dir == 2)
        {
            replace = true;
        }
        if (!level->isSolidBlockingTileInLoadedChunk(x, y, z - 1, true) && dir == 3)
        {
            replace = true;
        }
        if (!level->isSolidBlockingTileInLoadedChunk(x, y, z + 1, true) && dir == 4)
        {
            replace = true;
        }
