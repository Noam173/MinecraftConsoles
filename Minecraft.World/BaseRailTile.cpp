...
bool BaseRailTile::Rail::hasRail(int x, int y, int z)
{
    ...
    if (isRail(level, x, y, z))
    {
        return true;
    }
    if (isRail(level, x, y + 1, z))
    {
        return true;
    }
    if (isRail(level, x, y - 1, z))
...
void BaseRailTile::neighborChanged(Level *level, int x, int y, int z, int type)
{
    ...
    if (dir == 2 && !level->isTopSolidBlocking(x + 1, y, z))
    {
        remove = true;
    }
    if (dir == 3 && !level->isTopSolidBlocking(x - 1, y, z))
    {
        remove = true;
    }
    if (dir == 4 && !level->isTopSolidBlocking(x, y, z - 1))
    {
        remove = true;
    }
    if (dir == 5 && !level->isTopSolidBlocking(x, y, z + 1))
    {
        remove = true;
    }
