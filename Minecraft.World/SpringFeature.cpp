...
bool SpringFeature::place(Level *level, Random *random, int x, int y, int z)
{
    ...
    if (level->getTile(x, y + 1, z) != Tile::stone_Id)
    {
        return false;
    }
    if (level->getTile(x, y - 1, z) != Tile::stone_Id)
    ...
    if (level->getTile(x, y, z) != 0 && level->getTile(x, y, z) != Tile::stone_Id)
    {
        return false;
    }

    int rockCount = 0;
    if (level->getTile(x - 1, y, z) == Tile::stone_Id)
    {
        rockCount++;
    }
    if (level->getTile(x + 1, y, z) == Tile::stone_Id)
    ...
    if (level->getTile(x, y, z - 1) == Tile::stone_Id)
    {
        rockCount++;
    }
    if (level->getTile(x, y, z + 1) == Tile::stone_Id)
