...
void DsItemEvent::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
    if (paramBlob.length == sizeof(Param))
    {
        ...
        if ((param->methodId == eAcquisitionMethod_Pickedup && onLeaderboard(eLeaderboardId_FARMING, eAcquisitionMethod_Pickedup, param)) || (param->methodId == eAcquisitionMethod_Mined && onLeaderboard(eLeaderboardId_FARMING, eAcquisitionMethod_Mined, param)))
        {
...
void DsMobKilled::handleParamBlob(shared_ptr<LocalPlayer> player, byteArray paramBlob)
{
    if (paramBlob.length == sizeof(Param))
    {
        ...
        {
        case eTYPE_MONSTER:
            if (param->mobType != SPIDER_JOCKEY_ID)
            {
                break;
            }
...
Stat *DurangoStats::get_achievement(eAward achievementId)
{
    ...
    if (binaryAchievement(achievementId) || enhancedAchievement(achievementId))
    {
