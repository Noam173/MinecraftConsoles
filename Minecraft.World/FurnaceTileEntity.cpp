        ...
        if (litTime == 0 && canBurn())
        {
            ...
            if (litTime > 0)
            {
                ...
                if (items[SLOT_FUEL] != nullptr)
                {
                    ...
                    if (items[SLOT_FUEL]->getItem()->id == Item::coal_Id && items[SLOT_FUEL]->getAuxValue() == CoalItem::CHAR_COAL)
                    {
                        m_charcoalUsed = true;
                    }
...
bool FurnaceTileEntity::canBurn()
{
    ...
    if (!items[SLOT_RESULT]->sameItem_not_shared(burnResult))
    {
        return false;
    }
    if (items[SLOT_RESULT]->count < getMaxStackSize() && items[SLOT_RESULT]->count < items[SLOT_RESULT]->getMaxStackSize())
    {
        return true;
    }
...
void FurnaceTileEntity::burn()
{
    ...
    if (items[SLOT_INPUT]->count <= 0)
    {
        items[SLOT_INPUT] = nullptr;
    }
}
...
bool FurnaceTileEntity::canTakeItemThroughFace(int slot, shared_ptr<ItemInstance> item, int face)
{
    ...
    {
        if (item->id != Item::bucket_empty_Id)
        {
            return false;
        }
    }
