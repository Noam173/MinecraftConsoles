        ...
        if (onGround)
        {
            ...
            if (level->getTile(xt, yt, zt) != Tile::pistonMovingPiece_Id)
            {
                    ...
                    if (tileData != nullptr && Tile::tiles[tile]->isEntityTile())
                    {
                        ...
                        if (tileEntity != nullptr)
                        {
                            ...
                            if (allTags)
                            {
                                for (Tag *tag : *allTags)
                                {
                                    if (tag->getName().compare(L"x") == 0 || tag->getName().compare(L"y") == 0 || tag->getName().compare(L"z") == 0)
                                    {
                                        continue;
                                    }
                ...
                else
                {
                    ...
                    {
                        spawnAtLocation(std::make_shared<ItemInstance>(tile, 1, Tile::tiles[tile]->getSpawnResourcesAuxValue(data)), 0);
                    }
                }
            }
        }
...
void FallingTile::addAdditonalSaveData(CompoundTag *tag)
{
    ...
    if (tileData != nullptr)
    {
        tag->putCompound(L"TileEntityData", tileData);
    }
}
