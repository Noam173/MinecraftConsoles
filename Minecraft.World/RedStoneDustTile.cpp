...
int RedStoneDustTile::getSignal(LevelSource *level, int x, int y, int z, int dir)
{
    ...
    bool w = shouldReceivePowerFrom(level, x - 1, y, z, Direction::WEST) || (!level->isSolidBlockingTile(x - 1, y, z) && shouldReceivePowerFrom(level, x - 1, y - 1, z, Direction::UNDEFINED));
    bool e = shouldReceivePowerFrom(level, x + 1, y, z, Direction::EAST) || (!level->isSolidBlockingTile(x + 1, y, z) && shouldReceivePowerFrom(level, x + 1, y - 1, z, Direction::UNDEFINED));
    bool n = shouldReceivePowerFrom(level, x, y, z - 1, Direction::NORTH) || (!level->isSolidBlockingTile(x, y, z - 1) && shouldReceivePowerFrom(level, x, y - 1, z - 1, Direction::UNDEFINED));
    bool s = shouldReceivePowerFrom(level, x, y, z + 1, Direction::SOUTH) || (!level->isSolidBlockingTile(x, y, z + 1) && shouldReceivePowerFrom(level, x, y - 1, z + 1, Direction::UNDEFINED));

    if (!level->isSolidBlockingTile(x, y + 1, z))
    {
        if (level->isSolidBlockingTile(x - 1, y, z) && shouldReceivePowerFrom(level, x - 1, y + 1, z, Direction::UNDEFINED))
        {
            w = true;
        }
        if (level->isSolidBlockingTile(x + 1, y, z) && shouldReceivePowerFrom(level, x + 1, y + 1, z, Direction::UNDEFINED))
        ...
        if (level->isSolidBlockingTile(x, y, z - 1) && shouldReceivePowerFrom(level, x, y + 1, z - 1, Direction::UNDEFINED))
        {
            n = true;
        }
        if (level->isSolidBlockingTile(x, y, z + 1) && shouldReceivePowerFrom(level, x, y + 1, z + 1, Direction::UNDEFINED))
...
bool RedStoneDustTile::shouldConnectTo(LevelSource *level, int x, int y, int z, int direction)
{
    ...
    else if (Tile::tiles[t]->isSignalSource() && direction != Direction::UNDEFINED)
    {
        return true;
    }
...
Icon *RedStoneDustTile::getTexture(const wstring &name)
{
    ...
    if (name.compare(TEXTURE_CROSS) == 0)
    {
        return Tile::redStoneDust->iconCross;
    }
    if (name.compare(TEXTURE_LINE) == 0)
    {
        return Tile::redStoneDust->iconLine;
    }
