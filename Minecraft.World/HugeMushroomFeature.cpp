...
bool HugeMushroomFeature::place(Level *level, Random *random, int x, int y, int z)
{
        ...
        for (int xx = x - offs; xx <= x + offs; xx++)
        {
            for (int zz = z - offs; zz <= z + offs; zz++)
            {
                ...
                if (type == 0 || yy < y + treeHeight)
                {
                    ...
                    if (xx == x - (offs - 1) && zz == z - offs)
                    {
                        data = 1;
                    }
                    if (xx == x - offs && zz == z - (offs - 1))
                    {
                        data = 1;
                    }
                ...
                if (data != 0 || y >= y + treeHeight - 1)
                {
                    ...
                    {
                        placeBlock(level, xx, yy, zz, Tile::hugeMushroom_brown_Id + type, data);
                    }
                }
            }
        }
    ...
    for (int hh = 0; hh < treeHeight; hh++)
    {
        ...
        if (!Tile::solid[t])
        {
            placeBlock(level, x, y + hh, z, Tile::hugeMushroom_brown_Id + type, 10);
        }
    }
    return true;
}
