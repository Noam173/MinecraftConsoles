...
Node *PathFinder::getNode(Entity *entity, int x, int y, int z, Node *size, int jumpSize)
{
    ...
    if (best != nullptr)
    {
        ...
        while (y > 0)
        {
            ...
            if (++drop >= 4)
            {
                return nullptr; // 4J - rolling this back to pre-java 1.6.4 version as we're suspicious of the performance implications of this
            }
        ...
        for (int yy = y; yy < y + size->y; yy++)
        {
            for (int zz = z; zz < z + size->z; zz++)
            {
                ...
                if (entity->level->getTileRenderShape(tileId) == Tile::SHAPE_RAIL)
                {
                    ...
                    if (entity->level->getTileRenderShape(xt, yt, zt) == Tile::SHAPE_RAIL || entity->level->getTileRenderShape(xt, yt - 1, zt) == Tile::SHAPE_RAIL)
                    {
                        continue;
                    }
                    else
