...
void HopperTileEntity::load(CompoundTag *base)
{
    ...
    if (base->contains(L"CustomName"))
    {
        name = base->getString(L"CustomName");
    }
...
void HopperTileEntity::save(CompoundTag *base)
{
    ...
    if (hasCustomName())
    {
        base->putString(L"CustomName", name);
    }
}
...
bool HopperTileEntity::suckInItems(Hopper *hopper)
{
    ...
    if (container != nullptr)
    {
        ...
        if ((worldly != nullptr) && (face > -1))
        {
            ...
            {
                if (tryTakeInItemFromSlot(hopper, container.get(), slots[i], face))
                {
                    return true;
                }
            }
...
bool HopperTileEntity::canPlaceItemInContainer(Container *container, shared_ptr<ItemInstance> item, int slot, int face)
{
    ...
    if (dynamic_cast<WorldlyContainer *>(container) != nullptr && !dynamic_cast<WorldlyContainer *>(container)->canPlaceItemThroughFace(slot, item, face))
    {
        return false;
    }
    return true;
...
{
    if (dynamic_cast<WorldlyContainer *>(container) != nullptr && !dynamic_cast<WorldlyContainer *>(container)->canTakeItemThroughFace(slot, item, face))
    {
        return false;
    }
    return true;
...
bool HopperTileEntity::canMergeItems(shared_ptr<ItemInstance> a, shared_ptr<ItemInstance> b)
{
    ...
    if (a->getAuxValue() != b->getAuxValue())
    {
        return false;
    }
    if (a->count > a->getMaxStackSize())
    {
        return false;
    }
    if (!ItemInstance::tagMatches(a, b))
    {
        return false;
    }
    return true;
