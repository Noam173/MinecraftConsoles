...
void AnvilMenu::createResult()
{
    ...
    else
    {
            ...
            else
            {
                ...
                if (additionalEnchantments)
                {
                    for (const auto &it : *additionalEnchantments)
                    {
                        ...
                        if (player->abilities.instabuild || input->id == EnchantedBookItem::enchantedBook_Id)
                        {
                            compatible = true;
                        }
        ...
        if (price <= 0)
        {
            if (DEBUG_COST)
            {
                app.DebugPrintf("No purchase, only tax; aborting");
            }
            result = nullptr;
        }
        if (namingCost == price && namingCost > 0 && cost >= 40)
        {
            if (DEBUG_COST)
            {
                app.DebugPrintf("Cost is too high; aborting");
            }
...
{
    if (level->getTile(x, y, z) != Tile::anvil_Id)
    {
        return false;
    }
