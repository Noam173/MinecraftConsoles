...
bool TargetGoal::canContinueToUse()
{
    ...
    if (mustSee)
    {
        ...
        {
            if (++unseenTicks > UnseenMemoryTicks)
            {
                return false;
            }
        }
...
bool TargetGoal::canAttack(shared_ptr<LivingEntity> target, bool allowInvulnerable)
{
    ...
    if (target == mob->shared_from_this())
    {
        return false;
    }
    if (!target->isAlive())
    ...
    {
        if (!allowInvulnerable && (dynamic_pointer_cast<Player>(target))->abilities.invulnerable)
        {
            return false;
        }
    }

    if (!mob->isWithinRestriction(Mth::floor(target->x), Mth::floor(target->y), Mth::floor(target->z)))
    {
        return false;
    }

    if (mustSee && !mob->getSensing()->canSee(target))
    {
        return false;
    }
