...
bool ItemEntity::merge(shared_ptr<ItemEntity> target)
{
    ...
    if (targetItem->getItem() != myItem->getItem())
    {
        return false;
    }
    if (targetItem->hasTag() ^ myItem->hasTag())
    {
        return false;
    }
    ...
    if (targetItem->getItem()->isStackedByData() && targetItem->getAuxValue() != myItem->getAuxValue())
    {
        return false;
    }
    if (targetItem->count < myItem->count)
    {
        return target->merge(dynamic_pointer_cast<ItemEntity>(shared_from_this()));
    }
...
bool ItemEntity::hurt(DamageSource *source, float damage)
{
    ...
    if (getItem() != nullptr && getItem()->id == Item::netherStar_Id && source->isExplosion())
    {
        return false;
    }
    markHurt();
