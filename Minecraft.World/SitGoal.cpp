...
bool SitGoal::canUse()
{
    ...
    if (owner == nullptr)
    {
        return true; // owner not on level
    }

    if (mob->distanceToSqr(owner) < FollowOwnerGoal::TeleportDistance * FollowOwnerGoal::TeleportDistance && owner->getLastHurtByMob() != nullptr)
    {
        return false;
    }
