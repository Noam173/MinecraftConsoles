...
vector<MobEffectInstance *> *PotionItem::getMobEffects(int auxValue)
{
    ...
    if (effects == nullptr)
    {
        ...
        if (effects != nullptr)
        {
            cachedMobEffects.insert(std::pair<int, vector<MobEffectInstance *> *>(auxValue, effects));
        }
    }
    return effects;
}
...
shared_ptr<ItemInstance> PotionItem::use(shared_ptr<ItemInstance> instance, Level *level, shared_ptr<Player> player)
{
    if (isThrowable(instance->getAuxValue()))
    {
        ...
        if (!level->isClientSide)
        {
            level->addEntity(std::make_shared<ThrownPotion>(level, player, instance->getAuxValue()));
        }
        return instance;
    }
...
{
    if (name.compare(DEFAULT_ICON) == 0)
    {
        return Item::potion->iconDrinkable;
    }
    if (name.compare(THROWABLE_ICON) == 0)
    {
        return Item::potion->iconThrowable;
    }
    if (name.compare(CONTENTS_ICON) == 0)
    {
        return Item::potion->iconOverlay;
    }
    return nullptr;
}
