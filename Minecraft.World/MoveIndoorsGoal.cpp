...
{
    if ((mob->level->isDay() && !mob->level->isRaining()) || mob->level->dimension->hasCeiling)
    {
        return false;
    }
...
void MoveIndoorsGoal::start()
{
    ...
    {
        mob->getNavigation()->moveTo(_doorInfo->getIndoorX() + 0.5, _doorInfo->getIndoorY(), _doorInfo->getIndoorZ() + 0.5, 1.0f);
    }
}
