...
bool Zombie::hurt(DamageSource *source, float dmg)
{
    if (Monster::hurt(source, dmg))
    {
        ...
        if ((target == nullptr) && getAttackTarget() != nullptr && getAttackTarget()->instanceof(eTYPE_LIVINGENTITY))
        {
            target = dynamic_pointer_cast<LivingEntity>(getAttackTarget());
        }
        if ((target == nullptr) && source->getEntity() != nullptr && source->getEntity()->instanceof(eTYPE_LIVINGENTITY))
        {
...
void Zombie::addAdditonalSaveData(CompoundTag *tag)
{
    ...
    if (isBaby())
    {
        tag->putBoolean(L"IsBaby", true);
    }
    if (isVillager())
    {
        tag->putBoolean(L"IsVillager", true);
    }
...
void Zombie::readAdditionalSaveData(CompoundTag *tag)
{
    ...
    if (tag->getBoolean(L"IsBaby"))
    {
        setBaby(true);
    }
    if (tag->getBoolean(L"IsVillager"))
    {
        setVillager(true);
    }
