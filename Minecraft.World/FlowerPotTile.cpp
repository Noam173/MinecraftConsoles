...
int FlowerPotTile::getTypeFromItem(shared_ptr<ItemInstance> item)
{
    ...
    if (id == Tile::rose_Id)
    {
        return TYPE_FLOWER_RED;
    }
    if (id == Tile::flower_Id)
    {
        return TYPE_FLOWER_YELLOW;
    }
    if (id == Tile::cactus_Id)
    {
        return TYPE_CACTUS;
    }
    if (id == Tile::mushroom_brown_Id)
    {
        return TYPE_MUSHROOM_BROWN;
    }
    if (id == Tile::mushroom_red_Id)
    {
        return TYPE_MUSHROOM_RED;
    }
    if (id == Tile::deadBush_Id)
    {
        return TYPE_DEAD_BUSH;
    }
