...
bool AvoidPlayerGoal::canUse()
{
    ...
    if (toAvoid.lock()->distanceToSqr(pos->x, pos->y, pos->z) < toAvoid.lock()->distanceToSqr(mob->shared_from_this()))
    {
        return false;
    }
    delete path;
